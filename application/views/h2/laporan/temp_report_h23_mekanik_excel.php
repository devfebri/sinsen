...
header("Content-Disposition: attachment; filename=Reporting H23 Per Mekanik Main Dealer_".$no." WIB.xls");
header("Pragma: no-cache");
header("Expires: 0");
?>
<h4><b>Reporting H23 Per Mekanik Main Dealer <?php echo $start_date." s/d ".$end_date?> - <?php echo date('H:i')?> WIB</b></h4>
	...
	if($mekanik->num_rows()>0){
		foreach ($mekanik->result() as $row) { 
 		...
 		echo "
 			<tr>
 				<td>$nom</td>
 				<td>$row->id_mekanik_flp - $row->nama_mekanik</td>
