...
header("Content-Disposition: attachment; filename=Reporting H23 Data Detail Revenue AHASS Main Dealer_".$no." WIB.xls");
header("Pragma: no-cache");
header("Expires: 0");
?>

<h4><b>Reporting H23 Data Detail Revenue AHASS Main Dealer <?php echo $start_date." s/d ".$end_date?> - <?php echo date('H:i')?> WIB<b></h4>
...
<table border="1">  
 	<tr> 		
        <td align="center"><b>No</b></td>
 		<td align="center"><b>Sumber Activity Promotion</b></td>
 		<td align="center"><b>Total Revenue</b></td>
 	</tr>
	...
	$sum_ass1=0;
 	$nom=1;	
	if($apRev->num_rows()>0){
		foreach ($apRev->result() as $row) { 
			
			$total_ass1_2 = $row->revenue_ap;

    ?>
 			<tr>
 				<td><?php echo $nom?></td>
 				<td><?php echo $row->name?></td>
                <td>Rp. <?php echo number_format($row->revenue_ap,0,',','.')?></td>
 			</tr>
	<?php 
    ...
    ?>
		<tr>
 				<td style='text-align:center' colspan='2'><b>Total Revenue</b></td>
 				<td><b>Rp. <?php echo number_format($sum_ass1,0,',','.')?></b></td>
 			</tr>
    <?php
	}else{
