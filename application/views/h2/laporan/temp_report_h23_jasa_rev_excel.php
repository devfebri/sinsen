...
header("Content-Disposition: attachment; filename=Reporting H23 Detail Revenue Jasa Main Dealer_".$no." WIB.xls");
header("Pragma: no-cache");
header("Expires: 0");
?>

<h4><b>Reporting H23 Detail Revenue Jasa Main Dealer <?php echo $start_date." s/d ".$end_date?> - <?php echo date('H:i')?> WIB</b></h4>
	...
	if($jasaRev->num_rows()>0){
        ...
        ?>
 			...
 			<tr>
                ...
                <td><?php echo number_format($row->ass,0,',','.')?></td>
                <td><?php echo number_format($row->ass1,0,',','.')?></td>
 				<td><?php echo number_format($row->ass2,0,',','.')?></td>
 				<td><?php echo number_format($row->ass3,0,',','.')?></td>
				<td><?php echo number_format($row->ass4,0,',','.')?></td>
				<td><?php echo number_format($row->claim,0,',','.')?></td>
                <td><?php echo number_format($row->cs,0,',','.')?></td>
                <td><?php echo number_format($row->ls,0,',','.')?></td>
                <td><?php echo number_format($row->lr,0,',','.')?></td>
				<td><?php echo number_format($row->hr,0,',','.')?></td>
 				<td><?php echo number_format($row->jr,0,',','.')?></td>
 				<td><?php echo number_format($row->or_plus,0,',','.')?></td>
 				<td><?php echo number_format($row->other,0,',','.')?></td>
				<td><?php echo number_format($row->pl,0,',','.')?></td>
                <td><?php echo number_format($row->pud,0,',','.')?></td>	
                <td><b><?php echo number_format($total_revenue_2,0,',','.')?></b></td>
 			</tr>
...
<?php 
    ...
    ?>
		...
		<tr>
				...
				<td><b><?php echo number_format($sum_all,0,',','.')?></b></td>
                ...
                <td><b><?php echo number_format($sum_jr,0,',','.')?></b></td>
