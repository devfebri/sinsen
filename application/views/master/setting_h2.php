          ...
          <div class="row">
            <div class="col-md-12">
              <form id="form_" class="form-horizontal">
                <div class="box-body">
                  <h4 style="padding-left: 15px"><b>Booking Grid</b></h4>
                  <br>
                  <div class="form-group">
                    <label class="col-sm-2 control-label">Jumlah Hari</label>
                    <div class="col-sm-4">
                      <input type="number" name="jumlah_hari" class="form-control" value="<?= $bg->jumlah_hari ?>" required>
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-sm-2 control-label">Waktu Mulai</label>
                    <div class="col-sm-4">
                      <input type="text" name="waktu_mulai" class="form-control" value="<?= $bg->waktu_mulai ?>" required>
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-sm-2 control-label">Waktu Selesai</label>
                    <div class="col-sm-4">
                      <input type="text" name="waktu_selesai" class="form-control" value="<?= $bg->waktu_selesai ?>" required>
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-sm-2 control-label">Interval</label>
                    <div class="col-sm-4">
                      <input type="text" value="<?= $bg->selisih_waktu ?>" name="selisih_waktu" class="form-control" required>
                    </div>
                  </div>
                </div>
          ...
          if ($('#form_').valid()) // check if form is valid
          {
            ...
            if (confirm("Apakah anda yakin ?") == true) {
              $.ajax({
                ...
                url: '<?= base_url('master/setting_h2/save') ?>',
                type: "POST",
                data: values,
                cache: false,
                dataType: 'JSON',
