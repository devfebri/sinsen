    ...
    ?>
      ...
      <div class="box box-default">
        ...
        <div class="box-body">
          ...
          <div class="row">
            <div class="col-md-12">
              <form class="form-horizontal" id="form_" action="master/event/<?= $form ?>" method="post" enctype="multipart/form-data">
                <?php if (isset($row)) : ?>
                ...
                <div class="box-body">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-4">
                      <input type="number" required class="form-control" name="unit_event_target" id="unit_event_target" value="<?= isset($row) ? $row->unit_event_target : '' ?>" autocomplete="off" <?= $disabled ?> onkeypress="return number_only(event)">
                    </div>
                  ...
                  <div class="form-group">
                    <label for="inputEmail3" class="col-sm-2 control-label">Event VE</label>
                    <div class="col-sm-4">
                      <select class='form-control' name="is_event_ve">
                        <option value='1' <?= isset($row) ? $row->is_event_ve == 1 ? 'selected' : '' : '' ?>>Ya</option>
                        <option value='0' <?= isset($row) ? $row->is_event_ve == 0 ? 'selected' : '' : 'selected' ?>>Tidak</option>
                      </select>
                    </div>
                  </div>
                  <!-- <div class="form-group">
                  ...
                  <table class="table table-bordered">
                    ...
                    <tbody>
                      <tr v-for="(bd, index) of budgets">
                        ...
                        <td>
                          <vue-numeric style="float: left;width: 100%;text-align: right;" class="form-control text-rata-kanan isi" v-model="bd.nominal" :disabled="mode=='detail'" v-bind:minus="false" :empty-value="0" separator="." />
                        </td>
                        <td align="center" v-if="mode=='insert'">
                    ...
                    <tfoot v-if="mode=='insert'">
                      <tr>
                        ...
                        <td>
                          <vue-numeric style="float: left;width: 100%;text-align: right;" class="form-control text-rata-kanan isi" v-model="budget.nominal" v-bind:minus="false" :empty-value="0" separator="." />
                        </td>
                        <td align="center">
                          ...
                          <select id="karyawan" class="form-control select2" onchange="form_.getJabatan()">
                            ...
                            if ($karyawan->num_rows() > 0) : ?>
                              ...
                              <?php foreach ($karyawan->result() as $tu) : ?>
                                <option value="<?= $tu->nama_lengkap ?>" data-id_karyawan_dealer="<?= $tu->id_karyawan_dealer ?>" data-nama_lengkap='<?= $tu->nama_lengkap ?>' data-jabatan="<?= $tu->jabatan ?>">
                        ...
                        <td>
                          <select style="width: 100%" id="job" class="form-control combo select2">
                            ...
                            <?php foreach ($jasa->result() as $rs) { ?>
                              <option value="<?= $rs->nama_jasa ?>" data-id_jasa_servis="<?= $rs->id_jasa_servis ?>">
                                <?= $rs->nama_jasa ?>
                              </option>
                            <?php }
                            ?>
      ...
      <script>
          ...
          methods: {
            ...
            getWarna: function() {
              ...
              $.each(warnas, function(i, value) {
                $('#id_warna').append($('<option>').text(warnas[i].id_warna + ' | ' + warnas[i].warna).attr({
                  'value': warnas[i].id_warna,
                  'warna': warnas[i].warna
                }));
        ...
        function submitBtn() {
          ...
          $.ajax({
            ...
            url: '<?= base_url('master/event/' . $form) ?>',
            type: "POST",
            data: values,
            cache: false,
            dataType: 'JSON',
          ...
          <table id="example2" class="table table-bordered table-hover">
            ...
            <tbody>
              <?php foreach ($event->result() as $rs) :
                $status = '';
                $button = '';
                $btn_edit = '<a data-toggle=\'tooltip\' title="Edit Data" class=\'btn btn-warning btn-xs btn-flat\' href=\'master/event/edit?id=' . $rs->id_event . '\'><i class=\'fa fa-edit\'></i></a>';
                $btn_approve = '<a data-toggle=\'tooltip\' title="Approval" class=\'btn btn-success btn-xs btn-flat\' href=\'master/event/approval_save?id=' . $rs->id_event . '\' onclick="return confirm(\'Are You Sure To Approve This Data ?\')" >Approved</a>';
              ...
              ?>
                <tr>
                  <td><a href="<?= base_url('master/event/detail?id=' . $rs->id_event) ?>"><?= $rs->kode_event ?></a></td>
                  <td><?= $rs->jenis_event ?></td>
                  <td><?= $rs->nama_event ?></td>
                  <td><?= $rs->start_date ?></td>
                  <td><?= $rs->end_date ?></td>
                  <td><?= $rs->desc_is_event_ve ?></td>
                  <td><?= $status ?></td>
                  <td align="center">
                    <?= $button ?>
                  </td>
      ...
      <script>
        function closePrompt(kode_event, id_event) {
          ...
          if (alasan_reject != null || alasan_reject == "") {
            ...
            window.location = '<?= base_url("master/event/reject_save?id=") ?>' + id_event + '&alasan_reject=' + alasan_reject;
