<table id="example2" class="table myTable1 table-bordered table-hover">
  ...
  <tbody>
	...
	$rw = $this->db->query("SELECT tr_faktur_stnk.status_faktur,  tr_faktur_stnk_detail.ktp, tr_faktur_stnk_detail.fisik, tr_faktur_stnk_detail.stnk, tr_faktur_stnk_detail.bpkb, tr_faktur_stnk_detail.kuasa, tr_faktur_stnk_detail.ckd, tr_faktur_stnk_detail.permohonan
		FROM tr_faktur_stnk JOIN tr_faktur_stnk_detail ON tr_faktur_stnk.no_bastd = tr_faktur_stnk_detail.no_bastd
       WHERE tr_faktur_stnk_detail.no_mesin = '$isi->no_mesin' and tr_faktur_stnk.status_faktur !='rejected'");
      ...
      if ($er->num_rows() > 0) {
        ...
        $alamat = ($ts->alamat_ktp_bpkb != '') ? $ts->alamat_ktp_bpkb : $ts->alamat ;
      } else {
        $nama_konsumen = "";
        $alamat = "";
      }
      ...
      $tr = $this->db->query("SELECT biaya_bbn, biaya_instansi FROM ms_bbn_dealer WHERE id_tipe_kendaraan = '$re->tipe_motor' and active = '1' order by created_at desc");
      if ($tr->num_rows() > 0) {
        $t = $tr->row();
    ...
    <?php
    $no_bbn_luar = $no;
    foreach ($dt_bbn_luar->result() as $isi) {
      $rw = $this->m_admin->getByID("tr_faktur_stnk_detail", "no_mesin", $isi->no_mesin);
      if ($rw->num_rows() > 0) {
        $row = $rw->row();
        if ($row->ktp == 'ya') $ktp = "checked";
      ...
      if ($ktp == '' or $fisik == '' or $stnk == '' or $bpkb == '' or $kuasa == '' or $ckd == '' or $pem == '') {
        ...
        echo "<td align='center'>
            <input type='hidden' value='$no' name='no'>
            <input type='hidden' value='$isi->id_bantuan_bbn_luar' name='id_sales_order_$no'>
            <input type='hidden' value='' name='no_spk_$no'>
            <input type='hidden' value='$biaya_bbn_md' name='biaya_bbn_$no'>
            <input type='hidden' value='$biaya_bbn_md' name='biaya_bbn_md_$no'>
            <input type='hidden' value='0' name='harga_unit_$no'>
            <input type='hidden' value='$isi->no_mesin' name='no_mesin_$no'>
            <input type='hidden' value='$isi->no_rangka' name='no_rangka_$no'>
            <input type='hidden' value='$alamat' name='alamat_$no'>
            <input type='hidden' value=\"".$nama_konsumen."\" name='nama_konsumen_$no'>
            <input type='checkbox' name='check_ktp_$no' $ktp>
          </td>      
    ...
    foreach ($dt_stnk_gc->result() as $isi) {
      ...
      $tr = $this->db->query("SELECT biaya_bbn, biaya_instansi FROM ms_bbn_dealer WHERE id_tipe_kendaraan = '$tipe_motor' and active = '1' order by created_at desc");
      if ($tr->num_rows() > 0) {
        $t = $tr->row();
