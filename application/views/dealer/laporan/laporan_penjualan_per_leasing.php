  ...
  <section class="content">
    ...
    <body>
      <?php if($tanggal2 != ''){ ?>
        ...
        <div style="text-align: center;font-size: 13pt"><b>Laporan Penjualan Per Leasing Per Bulan</b></div>        
        ...
        $sql_2 = $this->db->query("SELECT * FROM tr_sales_order INNER JOIN tr_spk ON tr_sales_order.no_spk = tr_spk.no_spk
              ...
              WHERE tr_sales_order.tgl_cetak_invoice2 BETWEEN '$tanggal2' AND '$tanggal3'
        ...
        ?>
          ...
          <table class='table table-bordered' style='font-size: 8pt' width='100%'>
            ...
            $sql = $this->db->query("SELECT *, tr_scan_barcode.no_mesin AS nosin,tr_spk.alamat AS alam FROM tr_sales_order INNER JOIN tr_spk ON tr_sales_order.no_spk = tr_spk.no_spk
              ...
              WHERE tr_sales_order.tgl_cetak_invoice2 BETWEEN '$tanggal2' AND '$tanggal3' AND tr_sales_order.id_dealer = '$id_dealer'
      ...
      <?php } 
        ...
        <p>Tanggal Periode Harus ditentukan dulu.</p>
      <?php } ?>                
    </body>
  </html>
  <?php } ?>
...
function getReport(){
  ...
  if (value.tanggal2 == '' && value.tanggal3 == '') {
    ...
    $('.loader').show();
    $('#btnShow').disabled;
    $("#showReport").attr("src",'<?php echo site_url("dealer/laporan_penjualan_per_leasing?") ?>cetak='+value.cetak+'&tanggal2='+value.tanggal2+'&tanggal3='+value.tanggal3);
