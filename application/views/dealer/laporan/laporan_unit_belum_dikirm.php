  ...
  <section class="content">
            ...
            <form class="form-horizontal" action="h1/ssu/create" id="frm" method="post" enctype="multipart/form-data">
              <div class="box-body">                                                              
                <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Tanggal Periode *</label>
                  <div class="col-sm-3">
                    <input type="text" id="tanggal2" name="spk_awal" class="form-control" placeholder="Tanggal Periode" autocomplete="off">
                  </div>
    ...
    <body>
      <?php if($tanggal2 != ''){ ?>
        ...
        <div style="text-align: center;font-size: 13pt"><b>Laporan Motor Belum Terkirim</b></div>        
        <!-- <div style="text-align: center; font-weight: bold;">Bulan : <?php echo $tgl ?></div> -->
        <hr>      
        <table border="0" width="100%">
          <tr>
            <td>Periode : <?php echo $tanggal2 ?></td>                     
          </tr>          
        </table>
        <br>
        <table class='table table-bordered' style='font-size: 10pt' width='100%'>
          <tr>                
            ...
            <td bgcolor='yellow' class='bold text-center'>No.SO</td>
            <td bgcolor='yellow' class='bold text-center'>Tgl.Inv</td>                
            <td bgcolor='yellow' class='bold text-center'>Customer</td>                
            <td bgcolor='yellow' class='bold text-center'>Model</td>                
            <td bgcolor='yellow' class='bold text-center'>Warna</td>                
            <td bgcolor='yellow' class='bold text-center'>Qty SO</td>                            
            <td bgcolor='yellow' class='bold text-center'>Qty Kirim</td>                            
            <td bgcolor='yellow' class='bold text-center'>Qty Sisa</td>                            
          </tr>
          <tr>
          <?php 
          $no=1;
          ...
          $sql = $this->db->query("SELECT * FROM tr_sales_order INNER JOIN tr_spk ON tr_sales_order.no_spk = tr_spk.no_spk
              ...
              WHERE tr_sales_order.ambil IS NULL AND tr_sales_order.id_dealer = '$id_dealer' AND LEFT(tr_sales_order.tgl_cetak_invoice2,10) = '$tanggal2'");
...
function getReport(){
  ...
  if (value.tanggal2 == '' && value.tanggal3 == '') {
    ...
    $('.loader').show();
    $('#btnShow').disabled;
    $("#showReport").attr("src",'<?php echo site_url("dealer/laporan_unit_belum_dikirm?") ?>cetak='+value.cetak+'&tanggal2='+value.tanggal2);
