...
<section class="content-header">
  ...
  <section class="content">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-4">
                    <select class="form-control select2" name="no_invoice" id="no_invoice">
                      ...
                      $sql = $this->db->query("SELECT no_invoice FROM tr_sales_order WHERE id_dealer = '$id_dealer'");
    ...
    <body>
        ...
        <div style="text-align: center;font-size: 13pt"><b>Laporan STNK Belum Diambil</b></div>        
        ...
        <table class='table table-bordered' style='font-size: 10pt' width='100%'>
          ...
          $sql = $this->db->query("SELECT *,tr_scan_barcode.no_mesin AS nosin 
              FROM tr_sales_order 
              ...
              WHERE tr_sales_order.id_dealer = '$id_dealer' $query");
          foreach ($sql->result() as $row) {
            ...
            $tgl_terima = $this->db->query("SELECT * FROM tr_tandaterima_stnk_konsumen INNER JOIN tr_tandaterima_stnk_konsumen_detail 
              WHERE tr_tandaterima_stnk_konsumen_detail.no_mesin = '$row->no_mesin'")->row()->tgl_cetak;
            ...
            echo "
              <tr>
                ...
                <td>$row->no_plat</td>                
                <td>$tgl_bastd</td>                
                <td>".tgl_indo(substr($tgl_terima,0,10),' ')."</td>                                                
              </tr>
            ";
            $no++;
          }
...
<script>
function getReport(){
  var value={no_invoice:document.getElementById("no_invoice").value,            
            cetak:'cetak',
  ...
  if (value.cetak == '') {
    ...
    $('.loader').show();
    $('#btnShow').disabled;
    $("#showReport").attr("src",'<?php echo site_url("dealer/laporan_stnk_belum_diambil?") ?>cetak='+value.cetak+'&no_invoice='+value.no_invoice);
