  ...
  <section class="content">
    ...
    <body>
      <?php if($tanggal2 != ''){ ?>
        ...
        <div style="text-align: center;font-size: 13pt"><b>Laporan Aging Schedule BPKB</b></div>        
        ...
        <table class='table table-bordered' style='font-size: 10pt' width='100%'>
          <tr>                
            ...
            <td bgcolor='yellow' class='bold text-center'>Tgl.Terima Faktur STNK</td>                            
            <td bgcolor='yellow' class='bold text-center'>Tgl.Pengiriman Ke BJ</td>                            
            <td bgcolor='yellow' class='bold text-center'>Tgl.STNK Selesai</td>                            
          ...
          $sql = $this->db->query("SELECT *,tr_scan_barcode.no_mesin AS nosin,ms_warna.warna AS war FROM tr_sales_order INNER JOIN tr_spk ON tr_sales_order.no_spk = tr_spk.no_spk
              ...
              WHERE tr_sales_order.id_dealer = '$id_dealer' AND LEFT(tr_tandaterima_stnk_konsumen.tgl_cetak,10) = '$tanggal2' AND tr_tandaterima_stnk_konsumen.jenis_cetak = 'bpkb'");              
          foreach ($sql->result() as $row) {
            $tgl_terima = $this->db->query("SELECT * FROM tr_pengajuan_bbn_detail INNER JOIN tr_kirim_biro ON tr_pengajuan_bbn_detail.id_generate = tr_kirim_biro.id_generate
                WHERE tr_pengajuan_bbn_detail.no_mesin = '$row->no_mesin'")->row()->tgl_terima;
            $tgl_stnk = $this->db->query("SELECT * FROM tr_terima_bj WHERE no_mesin = '$row->no_mesin'")->row()->tgl_stnk;
            ...
            echo "
              <tr>
                ...
                <td>$row->no_rangka</td>                
                <td>$row->no_plat</td>                
                <td>".tgl_indo(substr($row->tgl_cetak,0,10))."</td>                
                <td>".tgl_indo($tgl_terima,' ')."</td>                
                <td>".tgl_indo($tgl_stnk,' ')."</td>                
                <td>$us</td>                
              </tr>
            ";
            $no++;
...
function getReport(){
  ...
  if (value.tanggal2 == '' && value.tanggal3 == '') {
    ...
    $('.loader').show();
    $('#btnShow').disabled;
    $("#showReport").attr("src",'<?php echo site_url("dealer/laporan_aging_bpkb?") ?>cetak='+value.cetak+'&tanggal2='+value.tanggal2);
