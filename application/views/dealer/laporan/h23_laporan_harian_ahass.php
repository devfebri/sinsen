...
?>
  ...
  <body>
    ...
    //Details
    $datas = [
      'unit_entri' => 'Laporan Unit Entry AHASS Harian',
      'parts' => 'Laporan Penjualan Parts',
      'parts_oli' => 'Laporan Penjualan Oil',
      'parts_gmo' => 'Laporan Penjualan GMO',
      'jasa' => 'Laporan Pendapatan Jasa'
    ];
    foreach ($datas as $key => $dt) { ?>
      ...
      <table class="table table-bordered" border=1>
        ...
        <?php $no = 1;
        foreach ($details[$key] as $dts) { ?>
          <tr>
            <td><?= $no ?></td>
            <td><?= $dts['bulan'] ?></td>
            <?php $total = 0;
            foreach ($dts['data'] as $data) {
              $total += round($data['tot']); ?>
              <td align="right"><?= mata_uang_rp(round($data['tot'])) ?></td>
            <?php } //Data 
            ?>
