  ...
  <section class="content">
    ...
    <body>
      <table>
        ...
        <div style="text-align: center;font-size: 13pt"><b>Daily Distribution</b></div>        
        ...
        <table class='table table-bordered' style='font-size: 9pt' width='100%'>
          <tr>                
            <td bgcolor='yellow' class='bold text-center' width='25%' colspan="2" rowspan="2">Tipe Motor</td>
            <td bgcolor='yellow' class='bold text-center' colspan="31"><?php echo $tgl ?></td>
            <td bgcolor='yellow' class='bold text-center' width='5%' rowspan="2">Total</td>                
          </tr>
          <tr>            
            <?php 
            for ($i=1; $i <= 31; $i++) { 
          ...
          $sql_1 = $this->db->query("SELECT * FROM ms_tipe_kendaraan INNER JOIN tr_scan_barcode ON ms_tipe_kendaraan.id_tipe_kendaraan = tr_scan_barcode.tipe_motor
                  INNER JOIN tr_surat_jalan_detail ON tr_scan_barcode.no_mesin = tr_surat_jalan_detail.no_mesin
                  INNER JOIN tr_surat_jalan ON tr_surat_jalan.no_surat_jalan = tr_surat_jalan_detail.no_surat_jalan
                  WHERE LEFT(tr_surat_jalan.tgl_surat,4) = '$tahun' AND tr_surat_jalan.id_dealer = '$id_dealer'
          ...
          foreach ($sql_1->result() as $isi) {           
            echo "<tr>
                    ...
                    for ($i=1; $i <= 31; $i++) {  
                      ...
                      $cek_so2 = $this->db->query("SELECT COUNT(tr_surat_jalan_detail.no_mesin) AS jum FROM tr_surat_jalan_detail
...
<script>
    function getReport()
    {
      ...
      if (value.tipe == '') {
        ...
        $('.loader').show();
        $('#btnShow').disabled;
        $("#showReport").attr("src",'<?php echo site_url("dealer/daily_distribution?") ?>cetak='+value.cetak+'&tahun='+value.tahun+'&bulan='+value.bulan);
