...
?>
              ...
              <form class="form" id="frm" method="GET">
                ...
                <div class="box-body">
                  <div class="row">
                    ...
                    <div class="col-sm-3">
                      <label>Delivery Document ID</label>
                      <input type="text" class="form-control" id="id_generate" name="id_generate" readonly placeholder='Klik Untuk Memilih' onclick="setShowModalDelivery()">
                    </div>
                    <div class="col-sm-2">
                      <label>No. SPK</label>
                      <input type="text" class="form-control" id="no_spk" name="no_spk" readonly placeholder='Klik Untuk Memilih' onclick="setShowModalSPK()">
                    </div>
                    <div class="col-sm-3">
                      <label>No. SO</label>
                      <input type="text" class="form-control" id="id_sales_order" name="id_sales_order" readonly placeholder='Klik Untuk Memilih' onclick="setShowModalSalesOrder()">
                    </div>
                  </div>
                </div>
  ...
  <?php
  $data['data'] = ['modalDelivery', 'modalDeliveryFilterPeriodeBast', 'modalSPK', 'modalSPKFilterPeriodeBast', 'spk_so_generate_list_unit', 'modalSalesOrder', 'modalSalesOrderFilterPeriodeBast'];
  $this->load->view('dealer/dgi_api', $data); ?>
  <script src="assets/panel/plugins/datepicker/bootstrap-datepicker.js"></script>
  <script src="assets/moment/moment.min.js"></script>
  <script>
    function cekStartEnd() {
      start = $('#start').val();
      end = $('#end').val();
      if (start == '' && end == '') {
        alert('Tentukan Start Date dan End Date terlebih dahulu !');
        return false;
      }
      return true
    }

    function setShowModalDelivery() {
      if (cekStartEnd()) {
        showModalDelivery();
      }
    }

    function setShowModalSPK() {
      if (cekStartEnd()) {
        showModalSPK();
      }
    }

    function setShowModalSalesOrder() {
      if (cekStartEnd()) {
        showModalSalesOrder();
      }
    }
    ...
    function pilihDelivery(params) {
      $('#id_generate').val(params.id_generate);
    }
