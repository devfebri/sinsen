...
<body>
	...
	?>
		...
		<?php
		$dt_kel				= $this->db->query("SELECT ms_kelurahan.kelurahan, ms_kecamatan.id_kecamatan, ms_kecamatan.kecamatan, ms_kabupaten.id_kabupaten,ms_kabupaten.kabupaten, ms_provinsi.id_provinsi, ms_provinsi.provinsi
		...
		<table class="table" width="100%" border="0">
			...
			<?php
				$get_tipe 	= $this->db->query("
				SELECT no_spk_gc, ms_tipe_kendaraan.id_tipe_kendaraan, tr_sales_order_gc_nosin.no_mesin, ms_warna.id_warna, ms_tipe_kendaraan.tipe_ahm
				...
				foreach ($get_tipe->result() as $s) {
					$harga = $this->db->query("SELECT harga, biaya_bbn, qty, nilai_voucher, voucher_tambahan FROM tr_spk_gc_detail WHERE tr_spk_gc_detail.no_spk_gc = '$s->no_spk_gc' AND tr_spk_gc_detail.id_tipe_kendaraan = '$s->id_tipe_kendaraan' AND tr_spk_gc_detail.id_warna = '$s->id_warna' GROUP BY id_tipe_kendaraan")->row();
					$program = $this->db->query("SELECT id_program FROM tr_spk_gc WHERE tr_spk_gc.no_spk_gc = '$s->no_spk_gc'")->row();
					...
					$ta = $this->db->query("SELECT tahun_produksi FROM tr_fkb WHERE no_mesin_spasi = '$s->no_mesin'");
					...
					$no_spk = $s->no_spk_gc;
					$detail = $this->db->query("SELECT total_unit 
						FROM tr_spk_gc_kendaraan
						WHERE no_spk_gc='$no_spk' AND tr_spk_gc_kendaraan.id_tipe_kendaraan = '$s->id_tipe_kendaraan'")->row();
			...
			?>
				<table class="table table-bordered" style="border: 1px solid black">
					...
					<?php
					$get_nosin 	= $this->db->query("SELECT tr_sales_order_gc_nosin.no_mesin, tr_scan_barcode.no_rangka, nama_stnk 
						FROM tr_sales_order_gc_nosin INNER JOIN tr_scan_barcode ON tr_sales_order_gc_nosin.no_mesin = tr_scan_barcode.no_mesin
					...
					foreach ($get_nosin->result() as $r) {
						$cek_pik = $this->db->query("SELECT ms_tipe_kendaraan.tipe_ahm, ms_warna.warna 
							FROM tr_scan_barcode INNER JOIN ms_item 
		...
		<table class="table">
			...
			<?php } else {
				$finco				= $this->db->query("SELECT finance_company FROM ms_finance_company WHERE id_finance_company = '$dt_so->id_finance_company'");
				if ($finco->num_rows() > 0) {
					$t = $finco->row();
				...
				$get_info_kredit= $this->db->query("SELECT dp_stor,angsuran,tenor,nilai_voucher FROM tr_spk_gc_detail WHERE no_spk_gc = '$dt_so->no_spk_gc'");
				...
				$kerja				= $this->db->query("SELECT pekerjaan FROM ms_pekerjaan WHERE id_pekerjaan = '$dt_so->id_pekerjaan'");
				if ($kerja->num_rows() > 0) {
					$tr = $kerja->row();
		...
		<table class="table" border = "0">
			...
			<tr>
				<?php
				$norek_dealer = $this->db->query("SELECT id_norek_dealer FROM ms_norek_dealer WHERE id_dealer = '$dt_so->id_dealer' ")->row()->id_norek_dealer;
				$detail_norek_dealer = $this->db->query("SELECT id_bank, no_rek, nama_rek FROM ms_norek_dealer_detail WHERE id_norek_dealer = '$norek_dealer' LIMIT 0,2");
				foreach ($detail_norek_dealer->result() as $isi) {
					$bank = $this->db->query("SELECT bank FROM ms_bank WHERE id_bank = '$isi->id_bank'")->row();
					echo "
						<td align='right'>
							Atas Nama <br>
							Nama Bank <br>
