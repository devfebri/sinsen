              ...
              <div class="box-header with-border">
                <h3 class="box-title">
                  ...
                  if (in_array($this->m_admin->cari_dealer(), $myArrayDealer)) { 
                    ?>
                    <button class="btn btn-maroon btn-flat" onclick="showModalUploadToApi2()"><i class="fa fa-upload"></i> Upload Leads</button>
                <?php } ?>
                ...
                <div class="modal-footer">
                  <div class="row">
                    <div class="col-sm-12 col-md-12" align='center'>
                      ...
                      if (in_array($this->m_admin->cari_dealer(), $myArrayDealer)) { 
                        ?>
                        <a href="<?= base_url('./downloads/crm/template_upload_leads.xlsx') ?>" class="btn btn-success btn-flat">Template Upload</a>
                    <?php } ?>

                    <?php
                      $myArrayDealerLol = array(1,96,86,70,80);
                      if (in_array($this->m_admin->cari_dealer(), $myArrayDealerLol)) { 
                        ?>
                        <a href="<?= base_url('./downloads/crm/template_upload_leads_lol.xlsx') ?>" class="btn btn-success btn-flat">Template Upload</a>
                    <?php } ?>
                      ...
                      $.ajax({
                        ...
                        success: function(response) {
                          ...
                          if (response.status == 1) {
                            ...
                            html: getErrorTable(response.data.reject),
                          });

                          // Helper function to generate the HTML table from the "reject" array
                          function getErrorTable(errors) {
                          let tableHTML = `<table id="detail" border="1" class="table table-bordered table-responsive" border=1 responsive>
                                            <thead>
                                              <tr>
                                                <th>Line</th>
                                                <th>Error Details</th>
                          ...
                          errors.forEach((error) => {
                            ...
                            const errorMessages = Array.isArray(error['validasi-line']) ? error['validasi-line'] : [error['validasi-line']];
                            ...
                            if (errorMessages.length === 1) {
                              tableHTML += errorMessages[0];
                            } else {
                              tableHTML += '<ul>';
                              errorMessages.forEach((message) => {
                                tableHTML += `<li>${message}</li>`;
                              });
                              tableHTML += '</ul>';
                            }
