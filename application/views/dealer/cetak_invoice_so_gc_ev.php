...
<body>
	...
	if ($cek_no_invoice->num_rows() == 0 || $cek_no_invoice	 != '') {
		...
		$so = $this->db->query("SELECT tr_sales_order_gc.*,ms_dealer.nama_dealer,ms_dealer.alamat as alamat_dealer,ms_dealer.no_telp,ms_dealer.id_kelurahan as kelurahan_dealer, tr_spk_gc.*,ms_finance_company.finance_company,ms_dealer.admin_h1 FROM tr_sales_order_gc 						
...
<tr>
	<td  rowspan="4" width="60px">
		<img src="assets/panel/icon/honda.jpg" width="80px" height="100px" alt="" >
	</td>
...
?>
	<tr>
		...
		<td width="30%">: <?php echo $so->nama_npwp ?></td>

		<td width="20%">NPWP/NPPKP</td>
		<td width="30%">:</td>
	</tr>
	...
	<tr>
		...
		<td width="20%">:  <?php  echo $tgl_cetak_invoice = date('d-m-Y', strtotime($so->tgl_cetak_invoice)); ?></td>
	</tr>

	<tr>
		<td></td>
		<td><?php echo "$kecamatan \n"; ?></td>
		<td width="5%">NO SPK</td>
		<td width="20%">: <?php echo $so->no_spk_gc ?></td>
	</tr>

	<tr>
	<td>NIK</td>
		<td><?php echo "$kabupaten \n"; ?></td>
		<td width="5%">JENIS PEMBELIAN</td>
		<td width="20%">: <?php echo $so->jenis_beli ?></td>
	</tr>
	...
	<tr>
		...
		<td width="20%"><?php echo $so->id_finance_company ?></td>
		<?}else{ ?>
			<td></td>
			<td></td>
		<?}?>
		...
		<table border="0" width="100%">
			...
			<tr >
				<td align="left"  width="15%">No.</td>
				<td align="center"  width="35%">URAIAN</td>
				<td align="center"  width="15%">UNIT</td>
				<td align="center"  width="25%">HARGA SATUAN</td>
				<td align="center"  width="25%">HARGA JUAL</td>
			</tr>
			...
			<?php
				...
				foreach ($get_nosin->result() as $r) {
					...
					<tr>
						<td align='left'>$i</td>
						<td align='left'>$r->tipe_ahm - $r->warna  <br>
						No. Mesin  : $r->tipe_ahm - $r->warna  <br>
						No. Rangka : $r->tipe_ahm - $r->warna  <br>
						</td>
			...
			<tr>
				<td></td>
				<td></td>
				<td colspan="2">Jumlah Harga Jual</td>
				<?php
			...
			<tr>
				<td></td>
				<td></td>
				<td colspan="2">Dasar Pengenaan Pajak (DPP)</td>
				<?php
				$ppn = round($dpp * getPPN(0.1,$so->tgl_cetak_invoice));
				?>
				<td align='right'>Rp.<?php echo mata_uang($ppn) ?></td>
			...
			<tr>
				<td></td>
				<td></td>
				<td colspan="2">Pajak Pertambahan Nilai (PPN)</td>
				<?php
				$ppn = round($dpp * getPPN(0.1,$so->tgl_cetak_invoice));
				?>
				<td align='right'>Rp.<?php echo mata_uang($ppn) ?></td>
			...
			<tr>
				<td></td>
				<td></td>
				<td colspan="2">Biaya Balik Nama</td>
				<?php
				$nilai_bbn = $bbn;
				?>
			...
			<tr>
				...
				$total = '';
				?>
				<td align='right'>Rp.<?php echo mata_uang($total) ?></td>
			</tr>
			...
			<tr>
				<td></td>
				<td></td>
				<td colspan="2">Potongan Bantuan Pemerintah untuk KBLBB</td>
				<?php
				$subsidi_pemerintah = 7000000;
				?>
				<td align='right'>Rp.<?php echo mata_uang($subsidi_pemerintah) ?></td>
			</tr>
			...
			<tr>
				...
				<?php
				$total_bayar = $subsidi_pemerintah;
				?>
				<td align='right'>Rp.<?php echo mata_uang($total_bayar) ?></td>
			</tr>
			...
			<tr>
				<td colspan="4">* <?=$so->nama_dealer?> berada di bawah naungan PT. Sinar Sentosa Primatama selaku
								Main Dealer Sepeda Motor Honda <br>
				</td>
				<td></td>
			</tr>

			<tr>
				<td colspan="4">** Transaksi ini sepenuhnya menjadi tanggung jawab dealer yang bersangkutan</td>
				<td align="center">(<?=$so->admin_h1?>)</td>
			</tr>
		</table>
