...
header("Content-Disposition: attachment; filename=USTK_".$no.".xls");
...
<table border="1">  
  ...
  $sql = $this->db->query("SELECT tr_sales_order.no_mesin, ms_tipe_kendaraan.tipe_customer,ms_warna.warna,tr_prospek.jenis_kelamin,
    ...
    tr_cdb.menggunakan,ms_hobi.hobi,tr_prospek.id_flp_md,ms_karyawan_dealer.nama_lengkap,ms_dealer.kode_dealer_md,ms_dealer.nama_dealer,tr_sales_order.tgl_cetak_invoice,
    tr_spk.jenis_beli,tr_scan_barcode.no_rangka
    ...
    WHERE LEFT(tr_sales_order.created_at,7) BETWEEN '$tgl1' AND '$tgl2'");  
  foreach ($sql->result() as $isi) {        
    $cek = $this->m_admin->getByID("tr_ustk_detail","no_mesin",$isi->no_mesin);
    if($cek->num_rows() > 0){
      $jalan = 1;
    }else{
      $jalan = 0;
