...
<table width="100%">
 ...
 <table border='1'>  
 	<?php  	 	
 	$g_total=0;
 	/* $sql2 = $this->db->query("SELECT *,(SELECT no_faktur FROM tr_invoice_dealer WHERE no_do=tr_do_po.no_do) AS no_faktur
 	 FROM tr_do_po INNER JOIN ms_dealer ON tr_do_po.id_dealer = ms_dealer.id_dealer WHERE tgl_do = '$tgl1' AND tr_do_po.status = 'approved'");
	*/
 	$sql = $this->db->query("SELECT * FROM tr_invoice_dealer INNER JOIN tr_do_po ON tr_invoice_dealer.no_do = tr_do_po.no_do 
 	INNER JOIN ms_dealer ON tr_do_po.id_dealer = ms_dealer.id_dealer
	WHERE tr_invoice_dealer.tgl_faktur = '$tgl1' AND tr_do_po.status = 'approved' order by tr_invoice_dealer.no_faktur asc");
 	foreach ($sql->result() as $isi) {	 		 	
 		echo "
		 	...
		 	<tr> 		 		
		 		...
		 		<td align='center'>Harga Kosong Sebelum PPN</td>
		...
		$sql2 = $this->m_admin->get_detail_inv_dealer($isi->no_do,0);
 		$no=1;$t_total=0;
 		foreach ($sql2['detail'] as $row) {
		 	...
		 	<tr> 		 		
		 		...
		 		<td align='right'>".mata_uang3($row['harga_kosong_no_ppn'])."</td>		 		
