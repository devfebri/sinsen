      ...
      <div class="box-body">
        ...
        <div class="row">
		  ...
		  <!-- <button type="button" <?php echo $visible ?> name="save" value="Detail" data-toggle="collapse" data-target="#demo" class="btn btn-info btn-flat"><i class="fa fa-list"></i> Detail Piutang</button>                                     -->
                  <button type="button" <?php echo $visible ?> onclick="get_details('<?php echo $no_do; ?>',<?php echo $row->id_dealer ?>)" id="get_detail" name="save" value="Detail" data-toggle="collapse" data-target="#demo" class="btn btn-info btn-flat"><i class="fa fa-list"></i> Detail Piutang</button>                                    
                  <div id="demo" class="collapse">
                    ...
                    <table class="table table-bordered table-hover">
                      <tr align="center">
                        <th colspan="4">Daftar Piutang</th>
                      </tr>
                      <tr>
                        <th>No Invoice</th>
                    ...
                    ");
                        ...
                        <!-- <tr>
                          <td colspan="3"></td>
                          <td><?php echo mata_uang2($tot_piutang) ?></td>
                        </tr> -->
                        </tbody>
                    </table>
                  </div>
		  <!--<button type="button" <?php echo $visible ?> name="save_ajax" onclick="get_details('<?php echo $no_do; ?>')" value="Detail" data-toggle="collapse" data-target="#demo1" class="btn btn-info btn-flat"><i class="fa fa-list"></i> Abaikan<button> 
...
function get_details(no_do, id_dealer){
    $.ajax({
        type: "post",
        url: "<?=site_url('h1/invoice_dealer_unit/get_detail_piutang_dealer');?>",
        dataType: "html",
        data: {no_do:no_do, id_dealer: id_dealer},
        beforeSend: function() {
            $('#isi_data').append('<tr><td>Loading</td></tr');
        },
        success: function (response) {
            $('#isi_data').empty();
            $('#isi_data').append(response);
            $('#get_detail').attr("disabled",true);
        },
   ...
   var form_ = new Vue({
      ...
      computed: {
        ...
        dpp:function() {
          ...
          total = (((total_bayar-total_diskon)-(this.bunga_bank/360*parseInt(this.top_unit)))/(1+(( ((this.persen_ppn/100)+1)*this.bunga_bank/360)*parseInt(this.top_unit)))).toFixed(2);
          // return Math.round(total / 10) * 10;
           return Math.round(total).toFixed(0);
          // console.log('dpp:'+total);
        },
        ppn: function () {
           let ppn =  (this.dpp * (this.persen_ppn/100)).toFixed(2);
  ...
  if ($('#form_').valid()) // check if form is valid
  {
    ...
    <div class="box box-default">
      ...
      <div class="box-body">
        ...
        <div class="row">
          <div class="col-md-12">            
            <form class="form-horizontal" action="h1/invoice_dealer_unit/download_file_df_danamon" method="post" enctype="multipart/form-data">              
              <div class="box-body">       
                <div class="form-group">                  
                  <label for="inputEmail3" class="col-sm-2 control-label">Tanggal Faktur</label>
                  <div class="col-sm-4">
                    <input type="text" name="tgl_faktur" placeholder="Tanggal Faktur" class="form-control" id='tanggal2' autocomplete="off">                    
                  </div>     
                  <div class="col-md-1">
                    <button type="submit" class="btn btn-warning btn-flat"><i class="fa fa-download"></i>Download Danamon</button>
                  </div>                               
                </div>  
              </div><!-- /.box-footer -->  
            </form>
