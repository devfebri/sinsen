    ...
    <section class="content">
      <div class="box box-default">
        <div class="box-header with-border">
          <div class="row">
            <div class="col-md-12">
              <form class="form-horizontal" id="frm" method="GET" enctype="multipart/form-data">
                ...
                <div class="box-body">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-2">
                      <!-- <button type="button" onclick="getReport()" name="process" value="edit" class="btn bg-maroon btn-block btn-flat"><i class="fa fa-download"></i> Download .csv</button> -->
                      <button type="submit" class="btn bg-maroon btn-block btn-flat"><i class="fa fa-download"></i> Download .csv</button>
                    </div>
                  </div>
                </div><!-- /.box-body -->
    ...
    ?>
      ...
      <body>
        ...
        <table class="table table-bordered" border="1">
          ...
          $so = $this->db->query("SELECT tr_sales_order.*,nama_dealer,kode_dealer_md,nama_dealer,tr_spk.*,tipe_ahm,warna FROM tr_sales_order 
            ...
            WHERE (tgl_cetak_invoice IS NOT NULL OR tgl_cetak_invoice<>'') AND tgl_cetak_invoice BETWEEN '$start_date' AND '$end_date' $filter_dealer ORDER BY tgl_cetak_invoice ASC ");
          foreach ($so->result() as $so) {
            ...
            echo "
                ...
                <td>$so->tipe_ahm</td>
                <td>$so->warna</td>
                <td>" . mata_uang3($so->harga_off_road) . "</td>
                <td>" . $so->uang_muka . "</td>
                <td>" . $so->dp_stor . "</td>
                <td>Individu</td>
                <td>$jk</td>
          ...
          $so2 = $this->db->query("SELECT tr_sales_order_gc.*,tr_scan_barcode.tipe_motor AS id_tipe_kendaraan, tr_sales_order_gc_nosin.no_mesin,tr_scan_barcode.no_rangka, ms_dealer.nama_dealer,ms_dealer.kode_dealer_md,tr_spk_gc.*, ms_tipe_kendaraan.tipe_ahm,ms_warna.warna,ms_warna.id_warna FROM tr_sales_order_gc
            ...
            WHERE (tgl_cetak_invoice IS NOT NULL OR tgl_cetak_invoice<>'') AND tgl_cetak_invoice BETWEEN '$start_date' AND '$end_date' $filter_dealer2 ORDER BY tgl_cetak_invoice ASC ");
