...
<table class='table table-bordered' style='font-size: 9pt' width='100%'>
  <tr>                
    ...
    <td bgcolor='yellow' class='bold text-center' width='25%'>Nama Dealer</td>            
  ...
  */
  ...
  // $sql = $this->db->query("
  //   SELECT tr_sales_order.no_mesin, tr_sales_order.no_rangka, tr_scan_barcode.id_item, tr_spk.nama_konsumen, tr_faktur_stnk.no_bastd, tr_faktur_stnk.created_at
  //   FROM tr_sales_order 
  //   INNER JOIN tr_scan_barcode ON tr_sales_order.no_mesin = tr_scan_barcode.no_mesin 
  //   INNER JOIN tr_spk ON tr_sales_order.no_spk = tr_spk.no_spk
  //   LEFT JOIN tr_faktur_stnk_detail on tr_faktur_stnk_detail.no_mesin = tr_sales_order.no_mesin
  //   LEFT JOIN tr_faktur_stnk on tr_faktur_stnk.no_bastd = tr_faktur_stnk_detail.no_bastd AND tr_faktur_stnk.status_faktur not in ('rejected','approved')
  //   ");
  ...
  */
    ...
    $sql2 = $this->db->query("
      ...
      INNER JOIN tr_sales_order_gc ON tr_sales_order_gc.id_sales_order_gc = tr_sales_order_gc_nosin.id_sales_order_gc
      INNER JOIN tr_spk_gc ON tr_sales_order_gc.no_spk_gc = tr_spk_gc.no_spk_gc
      left join tr_faktur_stnk_detail on tr_sales_order_gc.id_sales_order_gc  = tr_faktur_stnk_detail.id_sales_order and tr_sales_order_gc_nosin.no_mesin = tr_faktur_stnk_detail.no_mesin 
      left join tr_faktur_stnk on tr_faktur_stnk.no_bastd = tr_faktur_stnk_detail.no_bastd 
