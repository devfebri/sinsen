  ...
  <section class="content">
            ...
            <form class="form-horizontal" id="frm" method="post" enctype="multipart/form-data">
              <div class="box-body">                                                              
                <div class="form-group">                  
                  ...
                  <div class="col-sm-2">                    
                    <input type="radio" name="tipe" id="tipe" value="per_dealer" class="minimal" checked> Rekap Per Dealer
                  </div>
                  <div class="col-sm-3">                    
                    <input type="radio" name="tipe" id="tipe" value="global" class="minimal"> Global Per Fincoy
                  </div>                    
                  <div class="col-sm-2">
    ...
    <body>      
      <?php if($tipe == 'global'){ $tgl = bln($bulan)." ".$tahun; ?>
        ...
        <div style="text-align: center;font-size: 13pt"><b>Laporan Penjualan via Finance Company</b></div>                
        ...
        <table class='table table-bordered' style='font-size: 9pt' width='100%'>
          <thead>
            <tr>
              ...
              <td bgcolor='yellow' class='bold text-center'>Nama Motor</td>                                        
              <?php 
              ...
              echo "<tr>
                      ...
                      foreach ($sql_1->result() as $isi) {                
                        $cek_so2 = $this->db->query("SELECT COUNT(tr_sales_order.no_mesin) AS jum FROM tr_sales_order 
                          ...
                          AND tr_spk.id_finance_company = '$isi->id_finance_company' AND tr_scan_barcode.tipe_motor = '$row->id_tipe_kendaraan'")->row()->jum;
                      ...
                      $cek_so3 = $this->db->query("SELECT COUNT(tr_sales_order.no_mesin) AS jum FROM tr_sales_order 
                        ...
                        AND tr_scan_barcode.tipe_motor = '$row->id_tipe_kendaraan'")->row()->jum;
          ...
          <tfoot>
            <tr>
              <td bgcolor='yellow' class='bold text-center' colspan="2">GRAND TOTAL</td>
              <?php 
              $sql_1 = $this->db->query("SELECT * FROM ms_finance_company WHERE active = 1");
              $bln = sprintf("%'.02d",$bulan);                            
              $tgl_surat_1 = $tahun."-".$bln;
              ...
              <td bgcolor='yellow' class='bold text-center'><?php echo mata_uang3($jumlah_jual2) ?></td>
              <td bgcolor='yellow' class='bold text-center'><?php echo mata_uang3($total) ?></td>
            </tr>
          </tfoot>
        </table>
        <?php
      }elseif($tipe=='per_dealer'){ $tgl = bln($bulan)." ".$tahun;         
        ?>
        <div style="text-align: center;font-size: 13pt"><b>Laporan Penjualan via Finance Company</b></div>                
        <div style="text-align: center;font-size: 10pt"><b>Bulan : <?php echo $tgl ?></b></div>                
        <br>
        ...
        ?>
          <table class='table table-bordered' style='font-size: 9pt' width='100%'>
            ...
            <tbody>
              ...
              $sql_2 = $this->db->query("SELECT * FROM ms_tipe_kendaraan INNER JOIN tr_scan_barcode ON ms_tipe_kendaraan.id_tipe_kendaraan = tr_scan_barcode.tipe_motor
                    INNER JOIN tr_sales_order ON tr_scan_barcode.no_mesin = tr_sales_order.no_mesin
                    WHERE LEFT(tr_sales_order.tgl_create_ssu,7) = '$tgl_surat_1' AND tr_sales_order.id_dealer = '$amb->id_dealer'
                    GROUP BY ms_tipe_kendaraan.id_tipe_kendaraan");
              $total=0;
              foreach ($sql_2->result() as $row) {                            
                echo "<tr>
                        ...
                        foreach ($sql_1->result() as $isi) {                
                          $cek_so2 = $this->db->query("SELECT COUNT(tr_sales_order.no_mesin) AS jum FROM tr_sales_order 
                            INNER JOIN tr_scan_barcode ON tr_sales_order.no_mesin = tr_scan_barcode.no_mesin                                                   
                            INNER JOIN tr_spk ON tr_sales_order.no_spk = tr_spk.no_spk
                            WHERE LEFT(tr_sales_order.tgl_create_ssu,7) = '$tgl_surat_1' AND tr_spk.jenis_beli = 'Kredit' AND tr_sales_order.id_dealer = '$amb->id_dealer'
                            AND tr_spk.id_finance_company = '$isi->id_finance_company' AND tr_scan_barcode.tipe_motor = '$row->id_tipe_kendaraan'")->row()->jum;
                          if(isset($cek_so2) AND $cek_so2 != 0) $jumlah_jual = $cek_so2;
                            else $jumlah_jual = 0;
                          $tot += $jumlah_jual;
                        ...
                        $cek_so3 = $this->db->query("SELECT COUNT(tr_sales_order.no_mesin) AS jum FROM tr_sales_order 
                          INNER JOIN tr_scan_barcode ON tr_sales_order.no_mesin = tr_scan_barcode.no_mesin                                                   
                          INNER JOIN tr_spk ON tr_sales_order.no_spk = tr_spk.no_spk
                          WHERE LEFT(tr_sales_order.tgl_create_ssu,7) = '$tgl_surat_1' AND tr_spk.jenis_beli = 'Cash' AND tr_sales_order.id_dealer = '$amb->id_dealer'
                          AND tr_scan_barcode.tipe_motor = '$row->id_tipe_kendaraan'")->row()->jum;
                        if(isset($cek_so3) AND $cek_so3 != 0) $jumlah_jual2 = $cek_so3;
                          else $jumlah_jual2 = 0;
                          $tot += $jumlah_jual2;
            ...
            <tfoot>
              <tr>
                ...
                $cek_so3 = $this->db->query("SELECT COUNT(tr_sales_order.no_mesin) AS jum FROM tr_sales_order 
                  INNER JOIN tr_scan_barcode ON tr_sales_order.no_mesin = tr_scan_barcode.no_mesin                                                   
                  INNER JOIN tr_spk ON tr_sales_order.no_spk = tr_spk.no_spk
                  WHERE LEFT(tr_sales_order.tgl_create_ssu,7) = '$tgl_surat_1' AND tr_spk.jenis_beli = 'Cash' AND tr_sales_order.id_dealer = '$amb->id_dealer'")->row()->jum;
                if(isset($cek_so3) AND $cek_so3 != 0) $jumlah_jual2 = $cek_so3;
                  else $jumlah_jual2 = 0;
                  $total += $jumlah_jual2;
                ?>
...
<script>
    function getReport()
    {
      ...
      if (value.tipe == '') {
        ...
        $('.loader').show();
        $('#btnShow').disabled;
        $("#showReport").attr("src",'<?php echo site_url("h1/laporan_penjualan_via?") ?>tipe='+value.tipe+'&cetak='+value.cetak+'&tahun='+value.tahun+'&bulan='+value.bulan);
