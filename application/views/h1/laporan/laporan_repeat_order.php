...
<section class="content-header">
  ...
  <section class="content">
            ...
            <form class="form-horizontal" id="frm" method="post" enctype="multipart/form-data">
              <div class="box-body">                                                              
                <div class="form-group">                  
                  <label for="inputEmail3" class="col-sm-2 control-label">Periode Laporan</label>                  
                  <div class="col-sm-2">
    ...
    <body>      
        ...
        <div style="text-align: center;font-size: 13pt"><b>Laporan Repeat Order <?php echo $tahun ?></b></div>                
        <br>
            ...
            foreach ($dealer->result() as $isi) {
              echo "<tr>
                      ...
                      for ($i=1; $i <= 12; $i++) {
                        ...
                        $cek_so2 = $this->db->query("SELECT tr_spk.no_ktp,tr_spk.nama_konsumen, COUNT(tr_spk.no_spk) AS jum FROM tr_spk INNER JOIN tr_sales_order ON tr_spk.no_spk = tr_sales_order.no_spk
                          INNER JOIN tr_scan_barcode ON tr_sales_order.no_mesin = tr_scan_barcode.no_mesin
                          WHERE LEFT(tr_sales_order.tgl_create_ssu,7) = '$tgl_surat_1' AND tr_sales_order.id_dealer = '$isi->id_dealer'
                          GROUP BY tr_spk.no_ktp HAVING COUNT(*) > 1")->num_rows();                                                   
...
<script>
    function getReport()
    {
      var value={tahun:document.getElementById("tahun").value,                
                cetak:'cetak',
                }
      ...
      if (value.tipe == '') {
        ...
        $('.loader').show();
        $('#btnShow').disabled;
        $("#showReport").attr("src",'<?php echo site_url("h1/laporan_repeat_order?") ?>cetak='+value.cetak+'&tahun='+value.tahun);
