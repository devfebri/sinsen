...
<table class="table table-bordered" width="100%" border="1" style="font-size: 13px;">
	 <tr>
		  <td align="center"><h2>SERAH TERIMA PLAT NOMOR KEPOLISIAN</h2></td>
	 </tr>
</table>
<br>
<table width="100%" border="0">
	 <tr>
		  ...
		  <td width="80%">: <?php echo $no_kirim_plat ?></td>        
	 </tr>    
	 <tr>
		  <td>Tanggal</td>
	 ...
	 <tr>
	 	<td colspan="2">
	 		<br>
	 		Dengan Hormat, <br>
	 		Dengan ini kami serah terimakan dokumen Plat Nomor Kepolisian sebagai berikut:
	 	</td>
	</tr>
</table> 	
<table class="table table-bordered" width="100%" border="1" style="font-size: 13px;">
	 ...
	 foreach ($query->result() as $isi) {
	 	$ds["no_kirim_plat"]	= $no_kirim_plat;
  	    $ds["no_plat"] 			= $isi->no_plat;
		$ds["no_mesin"] 		= $no_mesin	= $isi->no_mesin;	
		$ds["cetak"] = $dss["cetak"]	= "ya";	
		if(!isset($_GET['id'])){						
			$cek = $this->db->query("SELECT * FROM tr_kirim_plat_detail WHERE no_mesin = '$no_mesin'");
			if($cek->num_rows() > 0){
				$this->m_admin->update("tr_kirim_plat_detail",$dss,"no_mesin",$no_mesin);															
			}else{
				$this->m_admin->insert("tr_kirim_plat_detail",$ds);															
			}
			$dw["print_stnk"] 		= "ya";	
			$this->db->query("UPDATE tr_entry_stnk SET print_plat = 'ya' WHERE no_mesin = '$isi->no_mesin'");
		}
	 	echo "
	 	<tr>
...
<table border="0">
	<tr>
		<td colspan="5" align="center"><b>Plat Nomor Kepolisian telah diterima oleh Pihak PT.Sinar Sentosa Primatama</b></td>
	</tr>
	<tr>
