              ...
              <div class="box-body">       
                ...
                <table class="table table-bordered table-hover" id="exampleX">
                  ...
                  foreach($dt_map->result() as $row) { 
                    ...
                    $getdealer_1 = $this->db->query("SELECT id_dealer FROM tr_sales_order WHERE no_mesin ='$row->no_mesin'");
                    ...
                    $getdealer_3 = $this->db->query("SELECT id_dealer FROM tr_bantuan_bbn_luar WHERE no_mesin ='$row->no_mesin'");
                  ...
                  echo "          
                    <tr>
                      ...
                      <td>
                        ...
                        <input type='checkbox' class='data-check' name='konfirmasi_$no' value='$ro->id_generate'>
                      </td>";                                      
        ...
        <table id="example2" class="table table-bordered table-hover">
          ...
          foreach($dt_biro->result() as $row){       
            if($row->no_tanda_terima != ''){
              ...
              $cek1 = $this->db->query("SELECT * FROM tr_konfirmasi_map WHERE id_generate = '$row->id_generate'");
              if($cek1->num_rows() > 0){            
                $cek2 = $this->db->query("SELECT * FROM tr_konfirmasi_map_detail WHERE id_generate = '$row->id_generate' AND konfirmasi <> 'ya'");
                if($cek2->num_rows() > 0){            
...
function bulk_aktif(){
  ...
  if(list_id.length > 0){
    if(confirm('Are you sure process this '+list_id.length+' data?'))
      {
        $.ajax({
          type: "POST",
          data: {id:list_id},
          url: "<?php echo site_url('h1/konfirmasi_map/ajax_bulk')?>",
          dataType: "JSON",
