            ...
            <form class="form-horizontal" action="h1/po_dealer/update" method="post" enctype="multipart/form-data">
              ...
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-4">
                   <input type="text" class="form-control" value="<?= $row->tgl ?>">
                  </div>
                </div>                                                                                                                                                         
                <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Bulan</label>
      ...
      <div class="box-body">
        ...
        <table id="example2" class="table table-bordered table-hover">
          ...
          echo "          
            <tr>
              <td>$no</td>
              <td>$row->nama_dealer [$row->kode_dealer_md]</td>
              <td>
                <a href='h1/po_dealer/detail?id=$row->id_po'>
                  $row->id_po
                </a>
              </td>
              <td>$row->jenis_po</td>
              ...
              echo $tombol;
              ?>
                <!--a data-toggle='tooltip' title="Download PO" class="btn btn-warning btn-sm btn-flat" href="h1/po_h1/download?id=<?php echo $row->id_po ?>"><i class="fa fa-download"></i></a-->
                <!-- <a data-toggle='tooltip' title="Delete Data" onclick="return confirm('Are you sure to delete this data?')" class="btn btn-danger btn-sm btn-flat" href="h1/po_dealer/delete?id=<?php echo $row->id_po ?>"><i class="fa fa-trash-o"></i></a>
                <a data-toggle='tooltip' title="Edit Data" class='btn btn-primary btn-sm btn-flat' href='h1/po_dealer/edit?id=<?php echo $row->id_po ?>'><i class='fa fa-edit'></i></a> -->
              </td>
            </tr>
          <?php
          $no++;
      ...
      <div class="box-body">     
            ...
            <div class="box-header with-border">
              ...
              <div class="col-md-3">
                <h3 class="box-title" style="color: #3C8DBC;text-align: right;">
                  <?php 
                      $bulan = isset($row->po_period_m)?$row->po_period_m:date('m');
                      $tahun = isset($row->po_period_y)?$row->po_period_y:date('Y');
                   ?>
                  <b><?= strtoupper(bulan_pjg($bulan)) ?> <?= $tahun ?></b></h3>
              </div>
            </div>
          </div>
        </div>
        <div class="col-md-4">
          <div class="box box-primary">
            <div class="box-header with-border">
                <?php if ($mode!='detail'): ?>
                  <button type="button" @click.prevent="form_.saveForm('draft')" class="btn btn-primary btnSubmit"><i class="fa fa-save"></i> SAVE DRAFT</button><br><br>
                <?php endif ?>
                <?php 
                  $status ='draft';
                  $btn_tipe='warning';
                 ...
                 ?>
                <button type="button" class="btn btn-<?=$btn_tipe?>" style="width: 100%;text-align: left" disabled><b><?= strtoupper($status) ?></b></button>
            </div>
          </div>
        </div>
          ...
          <table v-if="po_type=='reg'" class="table table-bordered table-striped">
            ...
            <tbody>
              <tr v-for="(dtl, index) of details">
                ...
                <td>{{dtl.qty_indent}}</td>
                <td><vue-numeric style="float: left;width: 100%;text-align: right;"
                          class="form-control text-rata-kanan isi" v-model="dtl.total_harga" 
            ...
            <tfoot>
              <tr v-if="mode!='detail'">
                ...
                <td>{{detail.qty_indent}}</td>
                <td><vue-numeric style="float: left;width: 100%;text-align: right;"
                          class="form-control text-rata-kanan isi" v-model="totDetail" 
...
<script>
   var form_ = new Vue({
      ...
      methods: {
        cekPoFix: function (index=null) {
         ...
         cekPoT1: function (index=null) {
          if (index==null) {
            ...
            if (parseFloat(max_po_t1)>0) {
              if (parseInt(this.details[index].po_t1)>parseFloat(max_po_t1)) {
                alert('PO Fix melebihi batas maksimal!');
                this.details[index].po_t1='';
              }
              else if (parseInt(this.details[index].po_t1)<parseFloat(min_po_t1)) {
                alert('PO Fix kurang dari batas minimal !');
                this.details[index].po_t1='';
              }
            }
        ...
        getWarna: function() {
          ...
          $.each(warnas, function(i, value) {
            $('#id_warna').append($('<option>').text(warnas[i].warna).attr({'value':warnas[i].id_warna,'warna':warnas[i].warna}));
      ...
      computed: {
        totDetail:function() {
          po_fix     = this.detail.po_fix==''?0:this.detail.po_fix;
          qty_indent = this.detail.qty_indent==''?0:this.detail.qty_indent;
          total      = this.detail.harga * (parseInt(po_fix)+parseInt(qty_indent));
          this.detail.total_harga = total;
          return total;
        },
