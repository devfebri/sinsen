  ...
  <section class="content">
    ...
    if($set=="detail"){
      ...
      if($c_stnk->num_rows() > 0){
        $rr = $c_stnk->row();
        $cb_stnk = $this->m_admin->getByID("tr_kirim_stnk_detail","no_mesin",$row->no_mesin);
	        $no_stnk = ($cb_stnk->num_rows() > 0) ? $cb_stnk->row()->no_stnk : "" ;
	        $cb_bpkb = $this->m_admin->getByID("tr_kirim_bpkb_detail","no_mesin",$row->no_mesin);
	        $no_bpkb = ($cb_bpkb->num_rows() > 0) ? $cb_bpkb->row()->no_bpkb : "" ;
	        $cb_plat = $this->m_admin->getByID("tr_kirim_plat_detail","no_mesin",$row->no_mesin);
            ...
            <form class="form-horizontal" action="h1/penggantian_fkb/save" method="post" enctype="multipart/form-data">              
              <div class="box-body">       
                ...
                if($c_stnk->num_rows() > 0){
                  ...
                  $cek_bpkb = $this->db->query("SELECT tr_kirim_bpkb.no_kirim_bpkb FROM tr_kirim_bpkb INNER JOIN tr_kirim_bpkb_detail ON tr_kirim_bpkb.no_kirim_bpkb = tr_kirim_bpkb_detail.no_kirim_bpkb WHERE tr_kirim_bpkb_detail.no_mesin = '$row->no_mesin'");
                  $no_serah_terima_bpkb = ($cek_bpkb->num_rows() > 0) ? $cek_bpkb->row()->no_kirim_bpkb : "" ;
                  $cek_stnk = $this->db->query("SELECT tr_kirim_stnk.no_kirim_stnk FROM tr_kirim_stnk INNER JOIN tr_kirim_stnk_detail ON tr_kirim_stnk.no_kirim_stnk = tr_kirim_stnk_detail.no_kirim_stnk WHERE tr_kirim_stnk_detail.no_mesin = '$row->no_mesin'");
                  $no_serah_terima_stnk = ($cek_stnk->num_rows() > 0) ? $cek_stnk->row()->no_kirim_stnk : "" ;
                  $cek_plat = $this->db->query("SELECT tr_kirim_plat.no_kirim_plat FROM tr_kirim_plat INNER JOIN tr_kirim_plat_detail ON tr_kirim_plat.no_kirim_plat = tr_kirim_plat_detail.no_kirim_plat WHERE tr_kirim_plat_detail.no_mesin = '$row->no_mesin'");
                  $no_serah_terima_plat = ($cek_plat->num_rows() > 0) ? $cek_plat->row()->no_kirim_plat : "" ;
                  $tgl_bpkb = $rr->tgl_bpkb;
                  $tgl_stnk = $rr->tgl_stnk;
                  $tgl_plat = $rr->tgl_plat;
                  $tgl_terima_bpkb = $rr->tgl_terima_bpkb;
                  $tgl_terima_stnk = ($no_serah_terima_stnk=="") ? "" : $rr->tgl_terima_stnk;
                  $tgl_terima_plat = ($no_serah_terima_plat=="") ? "" : $rr->tgl_terima_plat;
                }else{                  
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({
        ...
        "columnDefs": [
        {
            "targets": [ 0,6,7,8,9,10,11,12,13,14 ], //first column / numbering column
