...
<table id="example2" class="table myTable1 table-bordered table-hover">
  ...
  foreach($dt_do_reg->result() as $row) {     
    ...
    $th = date("Y");
    $th1 = date("Y")+1;
    $cek_no = $this->db->query("SELECT COUNT(no_mesin) AS jum FROM tr_scan_barcode WHERE id_item = '$row->id_item' AND status = '1' AND tipe='RFS' AND (LEFT(fifo,4) <= '$th' OR LEFT(fifo,4) <= '$th1')");
    $cek_no2 = $this->db->query("SELECT COUNT(no_mesin) AS jum FROM tr_scan_barcode WHERE id_item = '$row->id_item' AND status = '1' AND tipe='NRFS' AND (LEFT(fifo,4) <= '$th' OR LEFT(fifo,4) <= '$th1')");
    ...
    if($jenis_do == 'po_reguler'){
      $qty_1 = $row->qty_po_fix - $row->qty_indent;
