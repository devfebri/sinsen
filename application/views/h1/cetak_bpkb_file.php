...
<table class="table table-bordered" width="100%" border="1" style="font-size: 13px;">
	 <tr>
		  <td align="center"><h2>SERAH TERIMA BPKB</h2></td>
	 </tr>
</table>
<br>
<table width="100%" border="0">
	 <tr>
		  ...
		  <td width="80%">: <?php echo $no_kirim_bpkb ?></td>        
	 </tr>    
	 <tr>
		  <td>Tanggal</td>
	 ...
	 <tr>
	 	<td colspan="2">
	 		<br>
	 		Dengan Hormat, <br>
	 		Dengan ini kami serah terimakan dokumen BPKB sebagai berikut:
	 	</td>
	</tr>
</table> 	


<table class="table table-bordered" border="1" style="font-size: 13px;">
	 <tr>
		  <td width="5%" align='center'>No</td>		 
		  <td width="30%" align='center'>Nama Dealer</td>		 
		  <td width="20%" align='center'>Nama Konsumen</td>		 
		  <td width="13%" align='center'>No Mesin</td>		 
		  <td width="13%" align='center'>No Polisi</td>		 
		  <td width="13%" align='center'>No BPKB</td>		 
	 </tr>
	 <?php 
	 $no=1;
	 foreach ($query->result() as $isi) {
	 	$ds["no_kirim_bpkb"]	= $no_kirim_bpkb;
  	$ds["no_bpkb"] 				= $isi->no_bpkb;
		$ds["no_mesin"] 		= $no_mesin	= $isi->no_mesin;	
		$ds["cetak"] = $dss["cetak"]	= "ya";	
		if(!isset($_GET['id'])){
			$cek = $this->db->query("SELECT * FROM tr_kirim_bpkb_detail WHERE no_mesin = '$no_mesin'");
			if($cek->num_rows() > 0){
				$this->m_admin->update("tr_kirim_bpkb_detail",$dss,"no_mesin",$no_mesin);															
			}else{
				$this->m_admin->insert("tr_kirim_bpkb_detail",$ds);															
			}
			$dw["print_stnk"] 		= "ya";	
			$this->db->query("UPDATE tr_entry_stnk SET print_bpkb = 'ya' WHERE no_mesin = '$isi->no_mesin'");
		}
	 	echo "
	 	<tr>
...
<table border="0">
	<tr>
		<td colspan="5" align="center"><b>Dokumen telah diterima oleh Pihak PT.Sinar Sentosa Primatama</b></td>
	</tr>
	<tr>
