  ...
  <?php 
    ...
    ?>
      <tr>
        ...
        <td><?= $dt['data']['no_rangka'] ?></td>
        <td><?= $dt['data']['id_tipe_kendaraan'].' | '.$dt['data']['id_warna'] ?></td>
        <td><?= $dt['data']['tipe_ahm'] .' | '.$dt['data']['warna'] ?></td>
        ...
        <td><?= $dt['data']['no_ktp'] ?></td>
        <td><?= $dt['data']['tgl_ajukan_claim'] ?></td>
                  ...
                  <div class="col-md-12">
                    ...
                    <table class="table table-bordered table-condensed table-hover">
                      ...
                      <thead>
                        ...
                        <th style="text-align: center;width: 36%">Syarat</th>

                        <th  style="text-align: center;width: 6%">Checklist</th>
                        <th  style="text-align: center;width: 19%">Lampiran</th>
                    ...
                    <tbody>
                        ...
                        <form id="frm_syarat" class="form-horizontal" name="frm_syarat" method="POST">
                          <?php /*
                        <div id="stack1" class="modal fade" tabindex="-3" data-focus-on="input:first" style="display: none; width:80%; margin:120px;">
                          <div class="modal-content">
                          <div class="modal-header">
                        ...
                        <!-- <button class="demo btn btn-primary btn-lg" type="button" data-toggle="modal" href="#stack1">View Demo</button> -->
                        */?>

                        <input type="hidden" id="id_program_md_modal_syarat" value="<?= $id_program_md ?>" >
                        <input type="hidden" id="id_dealer_syarat" value="<?= $id_dealer ?>" >

                        <?php
                        // $get_syarat = $_SESSION['generate_new'][$detail_key]['data']['syarat'];
                        $tot_syarat = count($get_syarat);
                        // print_r($get_syarat);die;
                      // if ($tot_syarat>0) {
                        $no=1;
                         ...
                         foreach ($get_syarat as $key => $rs) { ?>
                           <tr>
                              ...
                              <td style="text-align: center;">
                                <?php //&& $rs['file']!=''
                                 if($rs['filename'] !=''){?> 
                                  <a type="input" href="h1/claim_sales_program/download_image2?id=<?php echo $rs['id'] ?>&id_claim=<?php echo $rs['id_claim'] ?>" target="_blank" class="btn btn-success btn-xs"><?php echo $rs['filename']?></a>
                                <?php }else{ ?>
                                  <button type="button" disabled class="btn btn-success btn-xs">File Not Found</button>
                                <?php } ?>
                              </td>
                              <td>
                                ...
                                <select class="form-control select2" <?=$disabled?> style="width: 100%" id="alasan_reject_<?=$key?>" <?= $set_disabled ?>>
                                    <?php 
                                    $getReject=$this->db->query("SELECT id_alasan_reject, alasan_reject FROM ms_alasan_reject where active = '1' ORDER BY alasan_reject ASC");
                                    ?>
                       ...
                       // }
                        ?>
                        <tr>
                          <th colspan="2">Perlu Revisi? </th>
                          <td align="center">
                            <?php 
                                $checked_revisi='';
                                if ($perlu_revisi_dealer==1) {
                                  $checked_revisi='checked';
                                } 
                            ?>
                            <input type="checkbox" name="perlu_revisi_<?=$id_claim?>" id="perlu_revisi_<?=$id_claim?>" <?= $checked_revisi ?> <?= $mode=='detail'?'disabled':'' ?> >
                          </td>
                        </tr>
                    </tbody>
                  </table>          
  ...
  {<?php 
    ...
    if (isset($mode)) {
      ...
      $mode_='';
        ...
        var id_program_md_syarat = $('#id_program_md_modal_syarat').val();
        var id_dealer_syarat = $('#id_dealer_syarat').val();
        var value = {
        ...
        <?php for($i=0;$i<$tot_syarat;$i++){ ?>
                ...
                id_claim: id_claim,
                id_program_md_syarat: id_program_md_syarat,
                id_dealer_syarat: id_dealer_syarat,
                cek_perlu_revisi_dealer: $("#perlu_revisi_"+id_claim).is(":checked")?'1':null,
                mode:'<?= $mode ?>'
                <?=$mode_?>
