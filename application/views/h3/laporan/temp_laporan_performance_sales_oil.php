...
header("Content-Disposition: attachment; filename=Laporan Performance Salesman Parts by Oil_" . $no . " WIB.xls");
...
<table border="1">
	<?php if ($id_dealer != 'all') { ?>
		<caption><b>Laporan Performance Salesman Oil
				<br> <?php echo $laporan_performance_sales_parts->row()->nama_dealer ?> </b> <br><br></caption>
	<?php } else { ?>
		<caption><b>Laporan Performance Salesman Parts Oil
				<br> Periode <?php echo $start_date_2 . " s/d " . $end_date_2 ?>
			</b><br><br></caption>
	<?php } ?>

	<tr>
		<td colspan="9"><b>Performance Sales Out</b></td>
	</tr>
	<tr>
	...
	$sum_ach = 0; 
	$sum_growth = 0; 
	if ($laporan_performance_sales_oil->num_rows() > 0) {
		foreach ($laporan_performance_sales_oil->result() as $row) {
			...
			if($id_dealer == 'all'){
				...
				where DATE_FORMAT(ts.start_date,'%Y-%m')='$start_date_bulan' and  DATE_FORMAT(ts.end_date,'%Y-%m')='$end_date_bulan' AND ts.jenis_target_salesman='Oil' and ts.id_salesman = '$row->id_salesman'")->row();
			}else{
				$target_global = $this->db->query("SELECT ifnull(tso.total_amount,0) as target 
				FROM ms_h3_md_target_salesman ts
				JOIN ms_h3_md_target_salesman_oil tso on tso.id_target_salesman=ts.id 
				WHERE tso.id_dealer = $row->id_dealer and ts.id_salesman= '$row->id_salesman' and DATE_FORMAT(ts.start_date,'%Y-%m')='$start_date_bulan' and DATE_FORMAT(ts.end_date,'%Y-%m')='$end_date_bulan'")->row();
			}

			if($id_dealer == 'all'){
				$target_global_m1 = $this->db->query("SELECT ifnull(ts.target_salesman_global,0) as target
				from ms_h3_md_target_salesman ts 
				where DATE_FORMAT(ts.start_date,'%Y-%m')='$start_date_bulan_m1' and  DATE_FORMAT(ts.end_date,'%Y-%m')='$end_date_bulan_m1' AND ts.jenis_target_salesman='Oil' and ts.id_salesman = '$row->id_salesman'")->row();
			}else{
				$target_global_m1 = $this->db->query("SELECT ifnull(tso.total_amount,0) as target 
				FROM ms_h3_md_target_salesman ts
				JOIN ms_h3_md_target_salesman_oil tso on tso.id_target_salesman=ts.id 
				WHERE tso.id_dealer = $row->id_dealer and ts.id_salesman= '$row->id_salesman' and DATE_FORMAT(ts.start_date,'%Y-%m')='$start_date_bulan' and DATE_FORMAT(ts.end_date,'%Y-%m')='$end_date_bulan'")->row();
			}
