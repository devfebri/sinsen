...
<table id="myTable" class="table myt order-list" border="0">     
  ...
  <tbody>            
    ...
    foreach ($sql->result() as $isi) {
      ...
      $qty_order = $this->db->query("SELECT * FROM tr_so_oil_detail WHERE no_so_oil = '$isi->no_so_oil'")->row();            
      $sel = $this->db->query("SELECT SUM(qty_order) AS jum FROM tr_so_oil_detail INNER JOIN tr_so_oil ON tr_so_oil.no_so_oil = tr_so_oil_detail.no_so_oil 
        WHERE tr_so_oil_detail.id_part = '$isi->id_part' AND tr_so_oil.tgl_so BETWEEN '$tgl1' AND '$tgl2'")->row();
      if(is_null($sel->jum)){
        $sel_jum = 0;
      }else{
        $sel_jum = $sel->jum;
      ...
      $pb = $this->db->query("SELECT * FROM tr_pb_oli_reguler_detail INNER JOIN tr_pb_oli_reguler ON tr_pb_oli_reguler.no_pb_oli_reguler = tr_pb_oli_reguler_detail.no_pb_oli_reguler
        WHERE tr_pb_oli_reguler_detail.no_so_oil = '$isi->no_so_oil'")->row();
