<div class="box">
   <div class="box-header with-border">
      <div class="col-sm-6">
         <h3 class="box-title">
            <a href="h3/<?= $isi ?>/download_laporan_excel_cashback_tidak_langsung?id=<?= $sales_campaign['id'] ?>">
      ...
      <div class="col-sm-6 text-right">
        <?php if($sales_campaign['proses_ke_finance'] == 0): ?>
          <h3 class="box-title">
            <a href="h3/<?= $isi ?>/proses_ke_finance_cashback/<?= $sales_campaign['id'] ?>">
        ...
        <?php endif; ?>
         ...
         <h3 class="box-title">
            <div class="btn-group">
               ...
               <ul class="dropdown-menu">
                  <li><a href="h3/<?= $isi ?>/proses_insentif_cashback?id=<?= $sales_campaign['id'] ?>">Non-rekap</a></li>
                  <li><a href="h3/<?= $isi ?>/proses_insentif_cashback?id=<?= $sales_campaign['id'] ?>&rekap=true">Rekap</a></li>
               </ul>
            </div>
         </h3>
         <?php endif; ?>
         <?php if($sales_campaign['proses_ke_finance'] == 0): ?>
          <h3 class="box-title">
            <a href="h3/<?= $isi ?>/hitung_cashback_tidak_langsung_global?id=<?= $sales_campaign['id'] ?>">
            <button class="btn btn-info btn-flat">Hitung Perolehan Cashback</button>
            </a>
         </h3>
         <?php endif; ?>
      </div>
   ...
   <div class="box-body">
      ...
      <?php
         ...
         ?>
      <table id="perolehan_cashback_tidak_langsung" class="table table-bordered table-hover table-condensed">
         <thead>
            <tr>
               ...
               <?php foreach ($sales_campaign_global as $global) : ?>
               <td class='text-center'><?= $global['qty'] ?></td>
               <?php endforeach; ?>
            ...
            <tr>
               ...
               <?php endfor; ?>
               <?php foreach ($sales_campaign_global as $global) : ?>
               <td class='text-center'>Rp <?= number_format($global['cashback'], 0, ',', '.') ?></td>
               <?php endforeach; ?>
            </tr>
         </thead>
         <tbody></tbody>
      ...
      <script>
        editFinance = new Vue({
          ...
          methods: {
            ...
            update: function(){
              post = {};
              post.parts = _.chain(perolehan_cashback_tidak_langsung.rows().data())
                .map(function(data){
              ...
              this.loading = true;
              axios.post('h3/h3_md_ms_sales_campaign/update_pph21_cashback', Qs.stringify(post))
              .then(function(res){
          ...
          watch: {
            edit_mode: function(){
              perolehan_cashback_tidak_langsung.draw(false);
            }
          }
        })
      ...
      <script>
         $(document).ready(function() {
           perolehan_cashback_tidak_langsung = $('#perolehan_cashback_tidak_langsung').DataTable({
             processing: true,
             serverSide: true,
             scrollX: true,
             ordering: false,
             drawCallback: function(){
                $(".update_pph21").on("keyup", function(e){
                    $row = $(this).parents("tr");
                    row_data = perolehan_cashback_tidak_langsung.row($row).data();
             ...
             ajax: {
               url: "<?= base_url('api/md/h3/perolehan_cashback_tidak_langsung_global') ?>",
               dataSrc: "data",
               type: "POST",
               data: function(d) {
