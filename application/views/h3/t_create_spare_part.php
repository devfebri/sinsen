...
<table id="myTable" class="table myt order-list" border="0">     
  ...
  <tbody>            
    ...
    foreach ($sql->result() as $isi) {
      ...
      $qty_order = $this->db->query("SELECT * FROM tr_so_spare_detail WHERE no_so_spare = '$isi->no_so_spare'")->row();            
      $sel = $this->db->query("SELECT SUM(qty_order) AS jum FROM tr_so_spare_detail INNER JOIN tr_so_spare ON tr_so_spare.no_so_spare = tr_so_spare_detail.no_so_spare 
        WHERE tr_so_spare_detail.id_part = '$isi->id_part' AND tr_so_spare.tgl_so BETWEEN '$tgl1' AND '$tgl2'")->row();
      if(is_null($sel->jum)){
        $sel_jum = 0;
      }else{
        $sel_jum = $sel->jum;
      ...
      $qty=0;$qty_avs=0;
      $qty_cek = $this->db->query("SELECT * FROM tr_stok_part WHERE id_part = '$isi->id_part'");
      if($qty_cek->num_rows() > 0){
        $qty = $qty_cek->row()->qty;
        $qty_avs = $qty_cek->row()->qty_avs;
      }
      $pb = $this->db->query("SELECT * FROM tr_pb_spare_part_detail INNER JOIN tr_pb_spare_part ON tr_pb_spare_part.no_pb_spare_part = tr_pb_spare_part_detail.no_pb_spare_part
        WHERE tr_pb_spare_part_detail.no_so_spare = '$isi->no_so_spare'")->row();
