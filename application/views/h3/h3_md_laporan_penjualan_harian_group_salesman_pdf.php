...
<body>
    ...
    <?php $grandTotal = 0; ?>
    <?php foreach($data as $indexGroupSalesman => $groupSalesman): ?>
    <?php $marginPerTanggal = (count($data) != $indexGroupSalesman + 1) ? 'margin-bottom: 20px;' : null ?>
    <table class="table" style='<?= $marginPerTanggal ?>'>
        <tr>
            <td colspan='8' class='table-item text-bold'>Salesman : <?= $groupSalesman['nama_salesman'] ?></td>
        </tr>
        <tr>
        ...
        <?php foreach($groupSalesman['penjualan'] as $index => $penjualan): ?>
        ...
        <tr>
            ...
            <td class='table-item border-top border-bottom text-right text-bold' style='padding: 5px 0;'><?= number_format($groupSalesman['total_per_salesman'], 0, ',', '.') ?></td>
        </tr>
        <?php 
        $grandTotal += floatval($groupSalesman['total_per_salesman']);
        $menampilkanGrandTotal = count($data) == $indexGroupSalesman + 1;
