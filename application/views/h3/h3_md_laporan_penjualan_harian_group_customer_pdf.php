...
<body>
    ...
    <?php $grandTotal = 0; ?>
    <?php foreach($data as $indexGroupCustomer => $groupCustomer): ?>
    <?php $marginPerTanggal = (count($data) != $indexGroupCustomer + 1) ? 'margin-bottom: 20px;' : null ?>
    <table class="table" style='<?= $marginPerTanggal ?>'>
        <tr>
            <td colspan='8' class='table-item text-bold'>Customer : <?= $groupCustomer['nama_dealer'] ?></td>
        </tr>
        <tr>
            <td class='text-bold table-header text-center' width='15%'>No. Faktur</td>
            <td class='text-bold table-header text-center' width='20%'>Tgl Faktur</td>
        ...
        <?php foreach($groupCustomer['penjualan'] as $index => $penjualan): ?>
            <?php foreach($penjualan['parts'] as $indexPart => $part): ?>
            <tr>
                <?php if($indexPart == 0): ?>
                <td class='table-item'><?= $penjualan['no_faktur'] ?></td>
                <td class='table-item'><?= Mcarbon::parse($penjualan['tgl_faktur'])->format('d/m/Y') ?></td>
                <?php else: ?>
        ...
        <?php endforeach; ?>
        <tr>
            <td colspan='7' class='table-item border-top border-bottom text-bold' style='padding: 5px 0;'>Sub Total</td>
            <td class='table-item border-top border-bottom text-right text-bold' style='padding: 5px 0;'><?= number_format($groupCustomer['total_per_customer'], 0, ',', '.') ?></td>
        </tr>
        <?php 
        $grandTotal += floatval($groupCustomer['total_per_customer']);
        $menampilkanGrandTotal = count($data) == $indexGroupCustomer + 1;
