...
class Laporan_harian_ahass extends CI_Controller
{
  ...
  protected function template($data)
  {
    ...
    if ($name == "") {
      ...
      $data['dealer'] = $this->db->query("SELECT id_dealer,kode_dealer,nama_dealer from ms_dealer where id_dealer in('2',	'103',	'105',	'46',	'47',	'4',	'1',	'51',	'22',	'107',	'101',	'40',	'80',	'71',	'18',	'97',	'13',	'84',	'43',	'25',	'83',	'41',	'39',	'104',	'106',	'102',	'96',	'85',	'8',	'81',	'44',	'38',	'76',	'30',	'82',	'70',	'77',	'86',	'91',	'58',	'19',	'64',	'54',	'23',	'29',	'11',	'9',	'10',	'74',	'98',	'7',	'6',	'56',	'28',	'69',	'5',	'87',	'89',	'88',	'66',	'90',	'78',	'65')")->result();
      ...
      $filter = [
        ...
        $data['details'] = $this->db->query("select c.id_tipe_kendaraan as tipe_kendaraan,f.deskripsi, 
         ...
         $data['jasa']= $this->db->query("SELECT 
				...
				ON B.id_work_order=C.id_work_order WHERE C.id_dealer='{$filter['id_dealer']}' AND C.status='Closed'
		...
		$data['part_wo']=$this->db->query("select 
					...
					SUM(CASE WHEN A.kelompok_part in('BRAKE','PS') then B.harga_beli ELSE 0 END) AS brake,
					SUM(CASE WHEN A.kelompok_part in('ACCEC','FKT','TL','FED OIL','ACC','HELM','PA') then B.harga_beli ELSE 0 END) AS other
				 from tr_h23_nsc_parts B JOIN ms_part A ON A.id_part=B.id_part join tr_h23_nsc C ON C.no_nsc=B.no_nsc where C.id_dealer='{$filter['id_dealer']}' and C.referensi='work_order' 
				 and C.tgl_nsc BETWEEN '{$filter['start_date']}' and '{$filter['end_date']}'")->row();
