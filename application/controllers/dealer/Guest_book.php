...
class Guest_book extends CI_Controller {
	...
	public function getData()
	{
        ...
        $listData = $this->query_guest_book($search, $limit, $start, $order_field, $order_ascdesc);
        ...
        foreach($listData->result() as $row)
        {
            $data[]= array(
                ...
                $row->no_telp,
                $row->nama_lengkap,
                "<a href='dealer/guest_book/edit?id=".$row->id_guest_book."'>
                     <button ".$this->m_admin->set_tombol($id_menu,$group,"edit")."
                         class='btn btn-flat btn-xs btn-primary'><i class='fa fa-edit'></i>
                         Edit</button>
                 </a>"
            );     
        }
	...
	private function query_guest_book($search, $limit, $start, $order_field, $order_ascdesc)
	{
		$cari = '';
		if ($search != '') {
			$cari = " AND ( 
				a.nama_konsumen LIKE '%$search%'
			 OR a.alamat LIKE '%$search%' 
			 OR a.no_telp LIKE '%$search%' 
			 OR b.nama_lengkap LIKE '%$search%')";
		}
		$id_dealer = $this->m_admin->cari_dealer();
		$sql = "

		SELECT a.id_guest_book, a.nama_konsumen, a.alamat, a.no_telp, b.nama_lengkap FROM tr_guest_book_new AS a
		INNER JOIN ms_karyawan_dealer AS b ON a.id_karyawan_dealer=b.id_karyawan_dealer WHERE a.id_dealer = '$id_dealer' 
		$cari
	...
	private function count_filter($search)
	{
		...
		SELECT a.nama_konsumen, a.alamat, a.no_telp, b.nama_lengkap FROM tr_guest_book_new AS a
		INNER JOIN ms_karyawan_dealer AS b ON a.id_karyawan_dealer=b.id_karyawan_dealer WHERE a.id_dealer = '$id_dealer' 
		$cari
	...
	{
		$data['id_karyawan_dealer']				= $this->input->post('sales');
		$data['nama_konsumen']					= $this->input->post('nama_konsumen');
		$data['no_telp']						= $this->input->post('no_hp');
		$data['id_tipe_kendaraan'] 				= $this->input->post('id_tipe_kendaraan');	
		$data['id_warna'] 						= $this->input->post('id_warna');	
		...
		if ($_POST) {
			...
			$data['created_at']				= $waktu;		
			$data['created_by']				= $login_id;	
			$this->m_admin->insert('tr_guest_book_new',$data);
			$_SESSION['pesan'] 	= "Data has been saved successfully";
			$_SESSION['tipe'] 	= "success";
			redirect('dealer/guest_book/add','refresh');
		} else {
			redirect('dealer/guest_book/add','refresh');
		}
	...
	{		
		...
		$data['dt_status'] = $this->m_admin->getSortCond("ms_status","status","ASC");								
		$data['dt_permohonan'] = $this->m_admin->getSortCond("tr_prospek","id_list_appointment","ASC");								
		$data['dt_guest_book'] = $this->db->query("SELECT * FROM tr_guest_book_new 
						WHERE tr_guest_book_new.id_guest_book = '$id'  AND tr_guest_book_new.id_dealer='$id_dealer'");	
		$this->template($data);	
	}
	...
	public function update_new($id_guest_book)
	{
		$data['id_karyawan_dealer']				= $this->input->post('sales');
		$data['nama_konsumen']					= $this->input->post('nama_konsumen');
		$data['no_telp']						= $this->input->post('no_hp');
		...
		if ($_POST) {
			...
			$data['created_at']				= $waktu;		
			$data['created_by']				= $login_id;	
			$this->db->where('id_guest_book', $id_guest_book);
			$this->db->update('tr_guest_book_new', $data);
			$_SESSION['pesan'] 	= "Data has been edited successfully";
			$_SESSION['tipe'] 	= "success";
			redirect('dealer/guest_book','refresh');
		} else {
			redirect('dealer/guest_book','refresh');
		}
	}
