...
class H3_dealer_purchase_order extends Honda_Controller {
	...
	public function detail(){				
		...
		->select('rd.id_customer as customer')
		->select('ms_cust.nama_customer')
		->select('ms_cust.no_hp')
		->select('ms_cust.no_rangka')
		->select('ms_cust.no_mesin')
		->select('ms_cust.alamat')
		->select('ms_cust.tahun_produksi')
		->select('ms_cust.id_warna')
		->select('ms_cust.id_tipe_kendaraan')
		...
		->join('ms_customer_h23 as ms_cust', 'ms_cust.id_customer = rd.id_customer', 'left')
		->join('ms_kabupaten as kab', 'ms_cust.id_kabupaten = kab.id_kabupaten','left')
		->where('po.po_id', $this->input->get('id'))
		->get()->row();

		if($data['purchase_order']->po_revisi==1){
			$parts = $this->db
				->select('pop.*')
				...
				->join('tr_h3_dealer_request_document rd', 'rd.id_booking = po.id_booking')
	...
	public function revisi_po(){
		...
		->select('pop.*')
		->select('p.nama_part')
		->select('rdp.part_revisi_dari_md')
		->select('p.id_part_int')
		->select('p.current')
		->select('p.import_lokal')
		->select('p.hoo_flag')
		->select('p.hoo_max')
		->select('rdp.part_revisi_dari_md')
		->select('(CASE WHEN rdp.alasan_part_revisi_md = "discontinue" THEN "Discontinue" WHEN rdp.alasan_part_revisi_md = "part_set" THEN "Part Set" WHEN rdp.alasan_part_revisi_md = "supersede" THEN "Supersede" WHEN rdp.alasan_part_revisi_md = "lainnya" THEN "Lainnya" else "-" end) as alasan_part_revisi_md')
		->from('tr_h3_dealer_purchase_order as po')
		->join('tr_h3_dealer_purchase_order_parts as pop','pop.po_id_int=po.id')
		->join('ms_part as p', 'p.id_part = pop.id_part')
		->join('tr_h3_dealer_request_document_parts as rdp','rdp.id_booking=po.id_booking and rdp.id_part_int=pop.id_part_int','left')
		->where('pop.po_id', $this->input->get('id'))
		->get()->result_array();
	...
	public function save_revisi_po(){
		...
		$this->validate();
		$this->db->trans_start();
		$this->db->set('status','Revisi Dealer')
						 ->set('revised_dealer_at',date('Y-m-d H:i:s', time()))
						 ->set('total_amount',$this->input->post('total_amount'))
						 ->set('revised_dealer_by',$this->session->userdata('id_user'))
				         ->where('po_id',$this->input->post('po_id'))
		...
		$this->db->set('status','Revisi Dealer')
						 ->set('revised_po_dealer_at',date('Y-m-d H:i:s', time()))
						 ->set('revised_po_dealer_by',$this->session->userdata('id_user'))
				         ->where('id_booking',$this->input->post('id_booking'))
						 ->update('tr_h3_dealer_request_document');

		$request_document_parts = $this->getOnly([
			'id_part', 'harga_saat_dibeli', 'kuantitas', 'eta_terlama', 'eta_tercepat'
		], $this->input->post('parts'));

		//check apakah part tersebut revisi atau tidak dari MD
		foreach($request_document_parts as $part){
			$check_revisi_part = $this->db->select('part_revisi_dari_md')
										  ->select('part_sebelum_revisi')
										  ->select('id_part')
			...
			if($check_revisi_part['part_sebelum_revisi'] == 0 || $check_revisi_part['part_sebelum_revisi'] == ''||$check_revisi_part['part_sebelum_revisi'] == null){
				$check_int_id_booking = $this->db->select('id')
												 ->from('tr_h3_dealer_request_document')
												 ->where('id_booking',$this->input->post('id_booking'))
												 ->get()->row_array();

				$check_int_id_part = 	$this->db->select('id_part_int')
											 ->from('ms_part')
											 ->where('id_part',$part['id_part'])
											 ->get()->row_array();			
				$insert_data = array(
					'id_booking' => $this->input->post('id_booking'),
					'id_booking_int' => $check_int_id_booking['id'],
					'eta_tercepat' => $part['eta_tercepat'],
					'eta_terlama' => $part['eta_terlama'],
					'id_part' => $part['id_part'],
					'id_part_int' => $check_int_id_part['id_part_int'],
					'harga_saat_dibeli' => $part['harga_saat_dibeli'],
					'kuantitas' => $part['kuantitas'],
					'revisi_part_dealer' => 1,
				);
				$this->db->insert('tr_h3_dealer_request_document_parts', $insert_data);
			}
		}
		...
		if ($this->db->trans_status() AND $purchase_order != null) {
			send_json([
				'message' => 'Berhasil revisi purchase order',
				'payload' => $purchase_order,
				'redirect_url' => base_url('dealer/h3_dealer_purchase_order/detail?id=' . $purchase_order['po_id']) 
			]);
		}else{
			log_message('debug', sprintf('Purchase order dealer %s tidak berhasil di perbarui', $this->input->post('po_id')));
			send_json([
				'message' => 'Tidak berhasil revisi purchase order'
			], 422);
		}
	}
