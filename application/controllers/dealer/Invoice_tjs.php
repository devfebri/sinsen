...
class Invoice_tjs extends CI_Controller
{
	...
	var $title  = "ID Tanda Jadi Sementara";
	...
	public function save()
	{
		...
		// send_json($post);
		$id_invoice = $this->m_bayar->get_id_invoice_tjs();
		...
		// send_json($insert);
		$this->db->trans_begin();
		$this->db->insert('tr_invoice_tjs', $insert);
	...
	public function detail()
	{
		...
		$row = $this->m_bayar->getInvoiceTJS($filter);
		if ($row->num_rows() > 0) {
			$row = $data['row'] = $row->row();
			$filter_detail['no_spk'] = $row->id_spk;
			if ($row->jenis_spk == 'gc') {
