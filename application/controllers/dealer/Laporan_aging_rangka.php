...
class Laporan_aging_rangka extends CI_Controller {
	
	var $folder =   "dealer/laporan";
	var $page		=		"laporan_aging_rangka";
	var $title  =   "Laporan Aging Stock Sepeda Motor Per No Rangka";
	...
	{				
		if (isset($_GET['cetak'])) {
            if ($_GET['cetak'] == 'cetak') {
    			...
    			$html = $this->load->view('dealer/laporan/laporan_aging_rangka', $data, true);
    			...
    			$data['tanggal3']              	= $this->input->get('tanggal3');      
    			$data['id_dealer']              = $this->m_admin->cari_dealer();
    			$this->load->view('dealer/laporan/laporan_aging_rangka', $data);     
    		}
	    }else{
			$data['isi']    = $this->page;		
			$data['title']	= $this->title;															
