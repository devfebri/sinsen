...
class Invoice_pembelian extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['dt_invoice'] = $this->db->query("SELECT *,tr_invoice_dealer.no_do as do, (SELECT sum(harga) FROM tr_do_po_detail WHERE no_do = do) as amount FROM tr_invoice_dealer INNER JOIN tr_do_po ON tr_invoice_dealer.no_do = tr_do_po.no_do WHERE tr_invoice_dealer.status_invoice='printable' AND tr_do_po.id_dealer in (select id_dealer from ms_dealer a where a.kode_dealer_ahm ='$kode_dealer' or a.kode_dealer_md ='$kode_dealer') ORDER BY tr_invoice_dealer.tgl_faktur DESC");
		$this->template($data);
	...
	public function getData()
    {
        ...
        $id_dealer = $this->m_admin->cari_dealer();
        $dataInv = $this->get_invoice_pembelian($search, $limit, $start, $order_field, $order_ascdesc, $id_dealer);

        $data = array();
        foreach($dataInv->result() as $rows)
        {
            ...
            $data[]= array(
                ...
                $rows->no_faktur,
                $rows->tgl_faktur,
                mata_uang_rp($rows->total_bayar),
                "<a href='dealer/invoice_pembelian/cetak_invoice?id=$rows->id_invoice_dealer'>
                  <button $print class='btn btn-flat btn-xs btn-warning'><i class='fa fa-print'></i></button>
                </a>"
            );     
        }
        $total_data = $this->count_filter($search, $id_dealer);
        $output = array(
            "draw" => $_POST['draw'],
    ...
    public function get_invoice_pembelian($search, $limit, $start, $order_field, $order_ascdesc, $id_dealer)
    {

    	if ($search != '') {
			$query_cari = "AND tr_invoice_dealer.no_faktur LIKE '%$search%' or tr_invoice_dealer.tgl_faktur LIKE '%$search%' or tr_invoice_dealer.total_bayar LIKE '%$search%'";
		} else {
			$query_cari = "";
		}
    	...
    	$sql = "
	    	...
	    	SELECT
				tr_invoice_dealer.id_invoice_dealer,
				tr_invoice_dealer.no_faktur,
				tr_invoice_dealer.tgl_faktur,
				tr_invoice_dealer.total_bayar
			FROM
				tr_invoice_dealer
				INNER JOIN tr_do_po ON tr_invoice_dealer.no_do = tr_do_po.no_do 
			WHERE
				tr_invoice_dealer.status_invoice = 'printable' 
				AND tr_do_po.id_dealer in (select id_dealer from ms_dealer a where a.kode_dealer_ahm ='$kode_dealer' or a.kode_dealer_md ='$kode_dealer')
				$query_cari
			ORDER BY
				$order_field $order_ascdesc limit $start,$limit
    ...
    public function count_filter($search,$id_dealer)
	{
		if ($search != '') {
			$query_cari = "AND tr_invoice_dealer.no_faktur LIKE '%$search%' or tr_invoice_dealer.tgl_faktur LIKE '%$search%' or tr_invoice_dealer.total_bayar LIKE '%$search%'";
		} else {
	...
	public function cetak_invoice_old()
	{
		$waktu 			= gmdate("y-m-d h:i:s", time() + 60 * 60 * 7);
		$login_id		= $this->session->userdata('id_user');
		$tabel			= $this->tables;
		...
		//$pdf->Cell(25, 5, "", 0, 0);	  
		$pdf->Cell(30, 5, number_format($hs = $d * getPPN(0.1), 0, ',', '.'), 0, 1, 'R');
    ...
    $pdf->AddPage();
	  ...
	  $pdf->SetFont('times','',10);
	  $nosin = $this->m_admin->get_detail_inv_dealer($get_d->no_do, $bunga_bank);
	  $i=1;
