...
class Prospek extends CI_Controller
{
	...
	public function add()
	{
		...
		$data['set']		= "insert";
		$data['mode']		= "insert";
		$data['dt_prospek'] = $this->db->query("SELECT tr_prospek.*,ms_karyawan_dealer.id_karyawan_dealer FROM tr_prospek LEFT JOIN ms_karyawan_dealer ON tr_prospek.id_karyawan_dealer=ms_karyawan_dealer.id_karyawan_dealer
		...
		$data['dt_karyawan'] = $this->db->query("SELECT ms_karyawan_dealer.*,ms_dealer.nama_dealer FROM ms_karyawan_dealer 
			...
			WHERE ms_karyawan_dealer.id_dealer = '$id_dealer'
			 ...
			 AND ms_karyawan_dealer.id_jabatan IN('JBT-035','JBT-071','JBT-072','JBT-073','JBT-074','JBT-063','JBT-064','JBT-065','JBT-103','JBT-099')
	...
	public function add_gc()
	{
		...
		$data['dt_prospek'] = $this->db->query("SELECT tr_prospek.*,ms_karyawan_dealer.id_karyawan_dealer FROM tr_prospek LEFT JOIN ms_karyawan_dealer ON tr_prospek.id_karyawan_dealer=ms_karyawan_dealer.id_karyawan_dealer
																							ORDER BY ms_karyawan_dealer.nama_lengkap ASC");
		$id_dealer = $this->m_admin->cari_dealer();
		$data['dt_karyawan'] = $this->db->query("SELECT * FROM ms_karyawan_dealer WHERE id_dealer = '$id_dealer' AND id_flp_md <> ''  AND ms_karyawan_dealer.active='1' AND ms_karyawan_dealer.id_jabatan IN('JBT-035','JBT-071','JBT-072','JBT-073','JBT-074','JBT-063','JBT-064','JBT-065','JBT-103','JBT-099') ORDER BY nama_lengkap ASC");
		$data['dt_agama'] = $this->m_admin->getSortCond("ms_agama", "id_agama", "ASC");
		$data['dt_pendidikan'] = $this->m_admin->getSortCond("ms_pendidikan", "id_pendidikan", "ASC");
		$data['dt_pekerjaan'] = $this->m_admin->getSortCond("ms_pekerjaan", "id_pekerjaan", "ASC");
		$data['dt_pengeluaran'] = $this->m_admin->getSortCond("ms_pengeluaran_bulan", "pengeluaran", "ASC");
	...
	public function cari_id()
	{
		...
		$waktu 					= gmdate("Y-m-d H:i:s", time() + 60 * 60 * 7);
		$id_dealer 			= $this->m_admin->cari_dealer();
		$pr_num 				= $this->db->query("SELECT * FROM tr_prospek WHERE id_dealer = '$id_dealer' ORDER BY id_list_appointment DESC LIMIT 0,1");
		if ($pr_num->num_rows() > 0) {
			$row 	= $pr_num->row();
	...
	public function cari_id_gc()
	{
		...
		$pr_num 			= $this->db->query("SELECT * FROM tr_prospek_gc WHERE id_dealer = '$id_dealer' ORDER BY id_prospek_gc DESC LIMIT 0,1");
		if ($pr_num->num_rows() > 0) {
			$row 	= $pr_num->row();
	...
	public function save()
	{
		...
		if ($cek == 0) {
			...
			$data['program_umum'] 				= $this->input->post('program_umum') != '' ? $this->input->post('program_umum') : NULL;
