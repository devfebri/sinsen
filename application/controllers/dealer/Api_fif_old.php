...
class Api_fif extends CI_Controller {
	...
	var $baseUrl_inv = "https://restapi.fifgroup.co.id/fifinvoice/";
...
// 		$this->load->helper('fif_helper');	
	...
	public function getData()
    {
        ...
        foreach($dataSpk->result() as $rows)
        {
            $data[]= array(
                ...
                $rows->nama_konsumen,
                $rows->created_at,
                '<a href="dealer/api_fif/order_new?id_spk='.$rows->no_spk.'" class="btn btn-xs btn-info">Submit Order</a>'
            );     
        }
        	...
        	$btn_invoice = "";
	        	...
	        	// if ($id_dealer == '1') {
		        	...
		        	if ($rows->delivery == 'y' AND $rows->kirim_invoice == 'y' AND $rows->kirim_dokumen_invoice == 't' AND strtotime($rows->created_at) > strtotime('2021-10-31') AND $status_order_api == 'APPROVED') {
		        		$btn_invoice = '<a href="dealer/api_fif/upload_dokumen_invoice?id='.$rows->no_spk.'&inv_uuid='.$rows->inv_uuid.'&order_uuid='.$rows->order_uuid.'" class="btn btn-xs btn-success">Upload Dokuments</a>';
		        	}
	...
	public function order_new()
	{
		...
		$data = array(
			...
			"occupation_code"=> $m_fif->pekerjaan,
