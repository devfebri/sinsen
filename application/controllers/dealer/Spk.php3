...
class Spk extends CI_Controller
{
	...
	public function fetch_data_spk_datatables()
	{
        ...
        foreach($list as $row) {       
			...
			$btn_edit = "<a data-toggle='tooltip' $edit title='Edit' href='dealer/spk/edit?id=$row->no_spk'><button class='btn btn-flat btn-xs btn-primary'><i class='fa fa-edit'></i> Edit</button></a>";
			...
			$btn_cetak = "<a  data-toggle='tooltip' title='Cetak' target='_blank' href='dealer/spk/cetak?id=$row->no_spk'>
		...
		if ($cek == 0) {
			...
			if ($jenis_beli == 'Cash') {
				...
				if($req_unik_nik->num_rows()>0){
					...
					$subsidi_pemerintah = $req_unik_nik->row()->id_jenis_sales_program;

					if ($subsidi_pemerintah == 'SP-012'){


						$data['subsidi_pemerintah'] = 7000000;
					}
