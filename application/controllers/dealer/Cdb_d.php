...
class Cdb_d extends CI_Controller {
  ...
  var $title  =   "CDB (Customer Database) ";
	...
	{			
		...
		$data['dt_spk'] = $this->db->query("SELECT no_spk,tgl_spk, nama_konsumen, id_customer FROM tr_spk WHERE no_spk NOT IN (SELECT no_spk FROM tr_cdb WHERE no_spk is not null) AND tr_spk.status_spk<>'closed'  ORDER BY no_spk ASC limit 1");						
		$this->template($data);	
	}
	...
	public function getData()
	{
        ...
        $listData = $this->query_cdb_d($search, $limit, $start, $order_field, $order_ascdesc);
	...
	private function query_cdb_d($search, $limit, $start, $order_field, $order_ascdesc)
	{
		$cari = '';
		if ($search != '') {
			$cari = " AND ( 
		...
		SELECT tr_cdb.id_cdb, tr_cdb.no_spk, tr_spk.nama_konsumen, tr_spk.alamat, tr_spk.no_hp, tr_spk.no_ktp
	...
	public function getDataGc()
	{
        ...
        $listData = $this->query_cdb_gc($search, $limit, $start, $order_field, $order_ascdesc);
        ...
        foreach($listData->result() as $row)
        {
            $data[]= array(
                ...
                $row->no_npwp,
                $row->alamat,
                "<a  data-toggle='tooltip' ".$this->m_admin->set_tombol($id_menu,$group,"edit")." title=\"Edit Data\" class='btn btn-primary btn-sm btn-flat' href='dealer/cdb_d/edit_gc?id_c=".$row->id_cdb_gc."'><i class='fa fa-edit'></i></a>"
            );     
        }
        $total_data = $this->count_filter_gc($search);
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $total_data,
            "recordsFiltered" => $total_data,
	...
	private function query_cdb_gc($search, $limit, $start, $order_field, $order_ascdesc)
	{
		$cari = '';
		if ($search != '') {
			$cari = " AND ( 
				a.no_spk_gc LIKE '%$search%'
			 OR b.nama_npwp LIKE '%$search%' 
			 OR b.alamat LIKE '%$search%' 
			 OR b.no_npwp LIKE '%$search%' )";
		}
		$id_dealer = $this->m_admin->cari_dealer();
		$sql = "

		SELECT
			a.id_cdb_gc, a.no_spk_gc, b.no_npwp, b.nama_npwp, b.alamat
		FROM
			tr_cdb_gc a
			INNER JOIN tr_spk_gc b ON a.no_spk_gc = b.no_spk_gc 
		WHERE
	...
	private function count_filter_gc($search)
	{
		$cari = '';
		if ($search != '') {
			$cari = " AND ( 
	...
	{				
		...
		$data['dt_spk'] = $this->db->query("SELECT tr_spk.*,ms_kelurahan.id_kelurahan FROM tr_spk LEFT JOIN ms_kelurahan ON tr_spk.id_kelurahan=ms_kelurahan.id_kelurahan WHERE id_dealer='$id_dealer' AND no_spk NOT IN (SELECT no_spk FROM tr_cdb WHERE no_spk is not null)  AND tr_spk.status_spk<>'closed' and tr_spk.status_spk!='canceled' ORDER BY ms_kelurahan.kelurahan ASC");				
	...
	public function take_spk(){
		...
		if($cek->num_rows() > 0){
			...
			$jenis_wn_kk          = ($isi->jenis_wn =='WNI') ? '1' : '2' ;			
			$no_kk             = $isi->no_kk;			
			$npwp              = $isi->npwp;			
			...
			$no_ktp            = $isi->no_ktp;
			$jenis_kelamin_kk	   = ($prospek->jenis_kelamin == 'Pria') ? '1' : '2';
 			$agama             = $prospek->agama;
			$jenis_sebelumnya  = $prospek->jenis_sebelumnya;
			$merk_sebelumnya   = $prospek->merk_sebelumnya;
			$menggunakan       = $prospek->pemakai_motor;
			$sumber_prospek       = get_data('ms_sumber_prospek','id_dms',$prospek->sumber_prospek,'id_cdb');
			$sumber_prospek_value       = get_data('ms_sumber_prospek','id_dms',$prospek->sumber_prospek,'description');
  	...
  	$data_arr = ['no_spk'=>$no_spk,
  				...
  				'sumber_prospek_value'=>$sumber_prospek_value,
	...
	{		
		...
		if($this->input->post('menggunakan') == 'Saya sendiri'){
			$menggunakan = 'Saya Sendiri';
		}else{
			$menggunakan = $this->input->post('menggunakan');
		}
