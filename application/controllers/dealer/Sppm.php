...
class Sppm extends CI_Controller {
	...
	public function getData()
    {
		...
		if ($search != '') {
			$cari = " 
			and (tr_sppm.no_surat_sppm LIKE '%$search%' 
						OR tr_sppm.tgl_surat LIKE '%$search%' 
						OR tr_do_po.no_po LIKE '%$search%' 
						OR tr_do_po.no_do LIKE '%$search%'
						OR tr_do_po.tgl_do LIKE '%$search%'
						OR ms_plat_dealer.no_plat LIKE '%$search%'
						OR tr_sppm.driver LIKE '%$search%')
			";
		}

        $dataSppm = $this->db->query("

        	SELECT
				tr_sppm.id_sppm,
				tr_sppm.no_surat_sppm,
				tr_sppm.tgl_surat,
				tr_do_po.no_po,
				tr_do_po.no_do,
				tr_do_po.tgl_do,
				ms_plat_dealer.no_plat,
				tr_sppm.driver AS namadriver 
			FROM
				tr_sppm
				INNER JOIN tr_do_po ON tr_sppm.no_do = tr_do_po.no_do
				INNER JOIN ms_plat_dealer ON tr_sppm.no_pol = ms_plat_dealer.id_master_plat 
			WHERE
				tr_sppm.id_dealer = '$id_dealer' 
			$cari
        ...
        foreach($dataSppm->result() as $row)
        {
            ...
            $data[]= array(
                ...
                $row->no_plat,
                $row->namadriver,
                "<a href='dealer/sppm/cetak_sppm?id=$row->id_sppm' target='_blank' onclick='window.location.reload(true);'>
	...
	{				
		...
		*/
		...
		$data['dt_do'] 	= $this->db->query("SELECT tr_do_po.no_do ,no_po, sum(qty_do) as do , (SELECT count(no_mesin) as sj FROM tr_surat_jalan_detail 
					...
					WHERE tr_picking_list.no_do =  tr_do_po.no_do AND tr_surat_jalan_detail.ceklist = 'ya') as sj
					FROM tr_do_po 
					INNER JOIN tr_do_po_detail ON tr_do_po.no_do = tr_do_po_detail.no_do 
					INNER JOIN tr_picking_list ON tr_do_po.no_do=tr_picking_list.no_do
					WHERE tr_do_po.id_dealer = '$id_dealer' AND tr_do_po.status ='approved' AND 
					tr_do_po_detail.qty_do > 0 AND tr_picking_list.status='close' 
					and tr_do_po.tgl_do >= DATE_SUB(now(), INTERVAL 3 MONTH)
					group by tr_do_po.no_do ,no_po
					having do > sj
					ORDER BY tr_do_po.tgl_do DESC
		");											
	...
	{				
		...
		if($tot_ambil==0){
			$_SESSION['pesan'] 	= "Gagal, Tidak ada unit yang diambil.";
