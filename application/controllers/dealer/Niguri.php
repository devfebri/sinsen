...
class Niguri extends CI_Controller {
    ...
    var $tables =   "tr_niguri_dealer";	
		var $folder =   "dealer";
		var $page		=		"niguri";
    var $pk     =   "id_niguri_dealer";
    var $title  =   "Niguri";
	...
	{				
		...
		$data['dt_niguri'] = $this->db->query("SELECT * FROM tr_niguri_dealer WHERE id_dealer = '$id_dealer'");		
		...
		$id_dealer = $this->m_admin->cari_dealer();	
		$cek = $this->db->query("SELECT * FROM tr_niguri_dealer_detail");
		foreach ($cek->result() as $isi) {
			$cek2 = $this->db->query("SELECT * FROM tr_niguri_dealer WHERE id_niguri_dealer = '$isi->id_niguri_dealer'");
			if($cek2->num_rows() == 0){
				$this->db->query("DELETE FROM tr_niguri_dealer_detail WHERE id_niguri_dealer = '$isi->id_niguri_dealer'");			
			}			
		}	
		$this->template($data);	
	...
	public function t_niguri(){
		$id = $this->input->post('id_niguri_dealer');
		$dq = "SELECT tr_niguri_dealer_detail.*,ms_tipe_kendaraan.tipe_ahm,ms_tipe_kendaraan.deskripsi_ahm FROM tr_niguri_dealer_detail INNER JOIN ms_tipe_kendaraan
						ON tr_niguri_dealer_detail.id_tipe_kendaraan = ms_tipe_kendaraan.id_tipe_kendaraan
						WHERE tr_niguri_dealer_detail.id_niguri_dealer = '$id'";
		$data['dt_niguri'] = $this->db->query($dq);		
		$this->load->view('dealer/t_niguri',$data);
	}
	public function t_niguri_total(){
		...
		$data['sql'] 	= $this->db->query("SELECT SUM(a_m1) AS jum_m1,SUM(a_m) AS jum_m,SUM(a_fix) AS jum_fix,SUM(a_t1) AS jum_t1,SUM(a_t2) AS jum_t2,
						...
						SUM(d_m1) AS j_m1,SUM(d_m) AS j_m,SUM(d_fix) AS j_fix,SUM(d_t1) AS j_t1,SUM(d_t2) AS j_t2 FROM tr_niguri_dealer_detail
						WHERE tr_niguri_dealer_detail.id_niguri_dealer = '$id'");
		//$data['dt_niguri'] = $this->db->query($dq);				
		$this->load->view('dealer/t_niguri_total',$data);
	}
	public function t_niguri_v(){
		...
		$dq = "SELECT tr_niguri_dealer_detail.*,ms_tipe_kendaraan.tipe_ahm,ms_tipe_kendaraan.deskripsi_ahm FROM tr_niguri_dealer_detail INNER JOIN ms_tipe_kendaraan
						...
						WHERE tr_niguri_dealer_detail.id_niguri_dealer = '$id'";
		$data['dt_niguri'] = $this->db->query($dq);		
		$this->load->view('dealer/t_niguri_v',$data);
	}
	public function t_niguri_total_v(){
		...
		$data['sql'] 	= $this->db->query("SELECT SUM(a_m1) AS jum_m1,SUM(a_m) AS jum_m,SUM(a_fix) AS jum_fix,SUM(a_t1) AS jum_t1,SUM(a_t2) AS jum_t2,
						...
						WHERE tr_niguri_dealer_detail.id_niguri_dealer = '$id'");
		//$data['dt_niguri'] = $this->db->query($dq);				
		$this->load->view('dealer/t_niguri_total_v',$data);
	}
	...
	{				
		...
		$th 						= date("Y");
		$bln 						= date("m");		
		//$pr_num 				= $this->db->query("SELECT * FROM tr_niguri_dealer WHERE bulan = '$bln' AND tahun = '$th'")->num_rows();
		$pr_num 				= 0;
		if($pr_num > 0){
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."dealer/niguri'>";			
		}else{
			$this->template($data);	
		}								
	}
	...
	public function cari_id(){
		...
		$bln 						= date("m");		
		$id_dealer 			= $this->m_admin->cari_dealer();
		$d = $this->m_admin->getByID("ms_dealer","id_dealer",$id_dealer)->row();
		$kode_md = $d->kode_dealer_md;
		$pr_num 				= $this->db->query("SELECT * FROM tr_niguri_dealer WHERE id_dealer = '$id_dealer' ORDER BY id_niguri_dealer DESC LIMIT 0,1");						
		if($pr_num->num_rows()>0){
			$row 	= $pr_num->row();				
			$pan  = strlen($row->id_niguri_dealer)-5;
			$id 	= substr($row->id_niguri_dealer,$pan,5)+1;	
			if($id < 10){
					$kode1 = $th.$bln.$kode_md."0000".$id;          
      }elseif($id>9 && $id<=99){
					$kode1 = $th.$bln.$kode_md."000".$id;                    
      }elseif($id>99 && $id<=999){
					$kode1 = $th.$bln.$kode_md."00".$id;          					          
      }elseif($id>999){
					$kode1 = $th.$bln.$kode_md."0".$id;                    
      }
			$kode = $kode1;
		}else{
			$kode = $th.$bln.$kode_md."00001";
		} 	
		echo $kode;
	}
	...
	{		
		...
		$sql1 = $this->db->query("SELECT * FROM tr_niguri_dealer INNER JOIN tr_niguri_dealer_detail 
							ON tr_niguri.id_niguri_dealer=tr_niguri_dealer_detail.id_niguri_dealer INNER JOIN tr_niguri_desc
							ON tr_niguri_dealer_detail.id_niguri_dealer_detail=tr_niguri_desc.id_niguri_dealer_detail
		 					WHERE tr_niguri.bulan = '$a1' and tr_niguri.tahun = '$th'");
		$sql2 = $this->db->query("SELECT * FROM tr_niguri_dealer INNER JOIN tr_niguri_dealer_detail 
							ON tr_niguri.id_niguri_dealer=tr_niguri_dealer_detail.id_niguri_dealer INNER JOIN tr_niguri_desc
							ON tr_niguri_dealer_detail.id_niguri_dealer_detail=tr_niguri_desc.id_niguri_dealer_detail
		 					WHERE tr_niguri.bulan = '$a2' and tr_niguri.tahun = '$th'");
		$sql3 = $this->db->query("SELECT * FROM tr_niguri_dealer INNER JOIN tr_niguri_dealer_detail 
							ON tr_niguri.id_niguri_dealer=tr_niguri_dealer_detail.id_niguri_dealer INNER JOIN tr_niguri_desc
							ON tr_niguri_dealer_detail.id_niguri_dealer_detail=tr_niguri_desc.id_niguri_dealer_detail
		 					WHERE tr_niguri.bulan = '$a3' and tr_niguri.tahun = '$th'");
		$sql4 = $this->db->query("SELECT * FROM tr_niguri_dealer INNER JOIN tr_niguri_dealer_detail 
							ON tr_niguri.id_niguri_dealer=tr_niguri_dealer_detail.id_niguri_dealer INNER JOIN tr_niguri_desc
							ON tr_niguri_dealer_detail.id_niguri_dealer_detail=tr_niguri_desc.id_niguri_dealer_detail
		 					WHERE tr_niguri.bulan = '$a4' and tr_niguri.tahun = '$th'");
		$sql5 = $this->db->query("SELECT * FROM tr_niguri_dealer INNER JOIN tr_niguri_dealer_detail 
							ON tr_niguri.id_niguri_dealer=tr_niguri_dealer_detail.id_niguri_dealer INNER JOIN tr_niguri_desc
							ON tr_niguri_dealer_detail.id_niguri_dealer_detail=tr_niguri_desc.id_niguri_dealer_detail
		 					WHERE tr_niguri.bulan = '$a5' and tr_niguri.tahun = '$th'");
		$sql6 = $this->db->query("SELECT * FROM tr_niguri_dealer INNER JOIN tr_niguri_dealer_detail 
							ON tr_niguri.id_niguri_dealer=tr_niguri_dealer_detail.id_niguri_dealer INNER JOIN tr_niguri_desc
							ON tr_niguri_dealer_detail.id_niguri_dealer_detail=tr_niguri_desc.id_niguri_dealer_detail
		 					WHERE tr_niguri.bulan = '$a6' and tr_niguri.tahun = '$th'");
		$sql7 = $this->db->query("SELECT * FROM tr_niguri_dealer INNER JOIN tr_niguri_dealer_detail 
							ON tr_niguri.id_niguri_dealer=tr_niguri_dealer_detail.id_niguri_dealer INNER JOIN tr_niguri_desc
							ON tr_niguri_dealer_detail.id_niguri_dealer_detail=tr_niguri_desc.id_niguri_dealer_detail
		...
		if($sql5->num_rows() == 0){
	 	...
	 	$isi2 = $total_stock2 / (($retail_sales2 + $retail_sales3 + $retail_sales4) / 3) * 30;
	...
	public function cek_niguri_fix(){
		$id_niguri_dealer						= $this->input->post('id_niguri_dealer');			
		$id_tipe_kendaraan		= $this->input->post('id_tipe_kendaraan');		
		$th 	= $this->input->post('tahun');
		...
		//echo "ok|".$id_niguri_dealer."|".$id_tipe_kendaraan."|".$th."|".$bl."|".$bl1."|".$bl2."|".$bl_1."|".$bl_2;

		$cek_bulan_1 = $this->db->query("SELECT * FROM tr_niguri_dealer INNER JOIN tr_niguri_dealer_detail 
						ON tr_niguri_dealer_detail.id_niguri_dealer=tr_niguri.id_niguri_dealer WHERE tr_niguri.bulan = '$bl_1' AND tr_niguri.tahun = '$th'
						AND tr_niguri_dealer_detail.id_tipe_kendaraan = '$id_tipe_kendaraan'");

		$cek_bulan_n = $this->db->query("SELECT * FROM tr_niguri_dealer INNER JOIN tr_niguri_dealer_detail
						ON tr_niguri_dealer_detail.id_niguri_dealer=tr_niguri.id_niguri_dealer WHERE tr_niguri.bulan = '$bl' AND tr_niguri.tahun = '$th'
						AND tr_niguri_dealer_detail.id_tipe_kendaraan = '$id_tipe_kendaraan'");
	...
	public function save_niguri(){
		...
		//echo $id_niguri_dealer;
		$data['id_niguri_dealer']			= $this->input->post('id_niguri_dealer');			
		...
		//$cek2 = $this->m_admin->insert("tr_niguri_dealer_detail",$data);			

		$cek = $this->db->query("SELECT * FROM tr_niguri_dealer_detail WHERE id_niguri_dealer='$id_niguri_dealer' AND id_tipe_kendaraan = '$id_tipe_kendaraan'");
		if($cek->num_rows() > 0){
			$sq = $cek->row();
			$id = $sq->id_niguri_dealer_detail;
			$cek2 = $this->m_admin->update("tr_niguri_dealer_detail",$data,"id_niguri_dealer_detail",$id);						
		}else{
			$cek2 = $this->m_admin->insert("tr_niguri_dealer_detail",$data);						
		}
		echo "nihil";
	...
	public function delete_niguri(){
		...
		$id_niguri_dealer_detail 	= $this->input->post('id_niguri_dealer_detail');
		$this->db->query("DELETE FROM tr_niguri_dealer_detail WHERE id_niguri_dealer_detail = '$id_niguri_dealer_detail'");			
		echo "nihil";
	}
	public function cancel_niguri(){
		$id_niguri_dealer			= $this->input->post('id_niguri_dealer');			
		$this->m_admin->delete("tr_niguri_dealer","id_niguri_dealer",$id_niguri_dealer);
		$this->m_admin->delete("tr_niguri_dealer_detail","id_niguri_dealer",$id_niguri_dealer);
	}
    ...
    $sql = $this->db->query("SELECT * FROM ms_user INNER JOIN ms_karyawan_dealer ON ms_user.id_karyawan_dealer = ms_karyawan_dealer.id_karyawan_dealer 
      ...
      WHERE ms_user.id_user = '$id_user'")->row();
		...
		if($cek == 0){
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."dealer/niguri/add'>";
		}else{
	...
	{		
		...
		if($cek_approval == 'salah'){
			...
			if($this->db->trans_status() === FALSE){
				...
				$this->m_admin->delete("tr_niguri_dealer_detail","id_niguri_dealer",$id);
