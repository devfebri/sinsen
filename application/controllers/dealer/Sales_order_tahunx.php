...
*/
	...
	function check_approve_reject()
	{
		$id_dealer = $this->m_admin->cari_dealer();
		$spk = $this->input->post('id');

		$check= $this->db->query("SELECT no_spk ,count(1) as jum, status_approval  from tr_hasil_survey where status_approval ='approved' 
			and no_spk ='23/03/0/00207-00888'
			GROUP by no_spk 
			having jum > 1
	...
	public function save()
	{
		...
		if($unit_dealer != $id_dealer){
					...
					if($periode_akhir_program >= date('Y-m-d') && $periode_akhir_program!='' && $is_valid_program==1){
						...
						if($spk->program_gabungan !=''){
							...
							if($periode_akhir_program >= date('Y-m-d') && $periode_akhir_program!=''){
								...
								// pengecekan tahun motor 
								if(date('Y-m-d') >='2024-02-01' && $is_valid_program==1){
									$is_tahun_produksi = 1;
			...
			// cek ke valid tahun motor baru ke valid program (atau pakai && saja? tp msg nya gak bisa)
