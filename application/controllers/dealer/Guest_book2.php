	...
	{				
		...
		if (isset($_GET['mode'])) {
			...
			$data['dt_guest_book'] = $this->db->query("SELECT * FROM tr_guest_book_new 						
						left JOIN ms_tipe_kendaraan ON tr_guest_book_new.id_tipe_kendaraan = ms_tipe_kendaraan.id_tipe_kendaraan
						left JOIN ms_warna ON tr_guest_book_new.id_warna = ms_warna.id_warna
						WHERE tr_guest_book_new.id_dealer = '$id_dealer' 
						ORDER BY tr_guest_book_new.id_guest_book DESC ");	
		}		
		$this->template($data);	
