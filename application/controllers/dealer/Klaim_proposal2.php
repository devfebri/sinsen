...

{
...

	{
		...
		foreach ($fetch_data->result() as $rs) {
			...
			$this->db->where('id_program_md', $rs->id_program_md);

			$cek_expried_prog = $this->db->get('tr_sales_program')->row()->periode_akhir;

			$date_expried_prog = date('Y-m-d', strtotime('+5 days', strtotime($cek_expried_prog)));
			...
			if (strtotime($date_expried_prog) < strtotime(date('Y-m-d'))) {

				$status_proposal = "<label class='label label-danger'>Expired</label>";
...

	{
		...
		return $this->db->query("
   			SELECT * FROM (
   				SELECT 
   					program_umum AS id_program_md, tr_spk.no_spk,so.id_sales_order, so.no_mesin,so.no_rangka,harga_on_road,voucher_1, voucher_2,so.created_at,id_tipe_kendaraan,id_warna,jenis_beli,status_proposal,'umum' AS jenis_program,id_claim,tr_claim_dealer.alasan_reject
...

	{
		...
		if (!$this->input->post('submit')) {
			...
			//pengecekan pengajuan proposal

			$this->db->where('id_program_md', $id_program_md);
			...
			if (strtotime($date_expried_prog) < strtotime(date('Y-m-d'))) {
				...
				$_SESSION['pesan'] 	= "Pengajuan Klaim telah melewati batas waktu pengajuan klaim";
