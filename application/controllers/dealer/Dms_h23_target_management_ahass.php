...
class Dms_h23_target_management_ahass extends CI_Controller
{

  var $folder = "dealer";
  var $page   = "dms_h23_target_management_ahass";
  var $title  = "Target Management AHASS";
  ...
  public function make_query($recordsFiltered = null)
  {
    ...
    if ($recordsFiltered == true) {
      return $this->m_dms->getH23TargetManagementAHASS($filter)->num_rows();
    } else {
      return $this->m_dms->getH23TargetManagementAHASS($filter)->result();
    }
  }
  ...
  function deleted()
  {
    $get       = $this->input->get();
    $filter = ['id' => $get['id']];
    $cek = $this->m_dms->getH23TargetManagementAHASS($filter);
    if ($cek->num_rows() > 0) {
      $this->db->trans_begin();
      $deleted = ['deleted' => 1];
      $this->db->update('dms_h23_target_management_ahass', $deleted, ['id' => $get['id']]);
  ...
  function save()
  {
    ...
    // send_json($tes);
    $this->db->trans_begin();
    $this->db->insert('dms_h23_target_management_ahass', $insert);
    if ($this->db->trans_status() === FALSE) {
      $this->db->trans_rollback();
      $rsp = [
        'status' => 'error',
  ...
  function import_db()
  {
    ...
    if ($size > 0 and $name_r[count($name_r) - 1] == 'csv') {
      ...
      $this->db->trans_begin();
      if (isset($ins_batch)) {
        $this->db->insert_batch('dms_h23_target_management_ahass', $ins_batch);
      }
      if (isset($upd_batch)) {
        $this->db->update_batch('dms_h23_target_management_ahass', $upd_batch, 'id');
      }
  ...
  public function edit()
  {
    ...
    $filter['id'] = $id;
    $result = $this->m_dms->getH23TargetManagementAHASS($filter);
  ...
  function save_edit()
  {
    ...
    // send_json($tes);
    $this->db->trans_begin();
    $this->db->update('dms_h23_target_management_ahass', $update, ['id' => $id]);
    if ($this->db->trans_status() === FALSE) {
      $this->db->trans_rollback();
      $rsp = [
        'status' => 'error',
