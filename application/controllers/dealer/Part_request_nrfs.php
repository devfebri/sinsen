...
class Part_request_nrfs extends CI_Controller {

	var $tables = "tr_penerimaan_ksu_dealer";	
	var $folder = "dealer";
	var $page   = "part_request_nrfs";
	var $pk     = "id_penerimaan_ksu_dealer";
	var $title  = "Part Request NRFS";
   	...
   	var $order_column_dokumen = array('dokumen_nrfs_id','deskripsi_unit','deskripsi_warna','no_mesin','no_rangka',null); 
	...
	public function fetch_data()
   {
		...
		{  
			...
			$button  = '';
			$status  = '';
			$button3 = '<a href='.base_url('dealer/part_request_nrfs/edit?id='.$rs->request_id).' class="btn btn-warning btn-flat btn-xs"><i class="fa fa-pencil"></i> Edit</a>';
			$button1 = '<a href='.base_url('dealer/part_request_nrfs/detail?id='.$rs->request_id).' class="btn btn-primary btn-xs btn-flat mb-10"><i class="fa fa-eye"></i> View</a></br>';
			// $button2 = '<a onclick="return confirm(\'Are you sure to cancel this data ?\')" href='.base_url('dealer/part_request_nrfs/cancel_by_dealer?id='.$rs->request_id).' class="btn btn-danger btn-xs btn-flat mb-10"><i class="fa fa-close"></i> Cancelled by Dealer</a></br>';
			$button2 = '<button onclick="return cancelPrompt(\''.$rs->request_id.'\')" class="btn btn-danger btn-xs btn-flat mb-10"><i class="fa fa-close"></i> Cancelled by Dealer</button></br>';
			$button4 = '<a onclick="return confirm(\'Are you sure Send To MD ?\')" href='.base_url('dealer/part_request_nrfs/submit_md?id='.$rs->request_id).' class="btn btn-primary btn-xs btn-flat mb-10"><i class="fa fa-send"></i> Send To MD</a></br>';
			if ($rs->status_request=='draft') {
				$button = $button4.$button1.$button2.$button3;
				$status = '<label class="label label-warning">'.ucwords($rs->status_request).'</label>';
			}
			...
			$sub_array[] = '<a href='.base_url('dealer/part_request_nrfs/detail?id='.$rs->request_id).'>'.$rs->request_id.'</a>';
   ...
   function get_filtered_data(){  
	...
	{				
		...
		if ($cek_data->num_rows()>0) {
			...
			redirect('dealer/part_request_nrfs');
		}
	}
	...
	{				
		...
		$cek_data 		   = $this->db->query("SELECT tr_dokumen_nrfs.*,tr_part_request_nrfs.request_id,tr_part_request_nrfs.status_request FROM tr_part_request_nrfs
				JOIN tr_dokumen_nrfs ON tr_part_request_nrfs.dokumen_nrfs_id=tr_dokumen_nrfs.dokumen_nrfs_id
				WHERE request_id='$request_id' AND tr_part_request_nrfs.id_dealer=$id_dealer
				AND (tr_part_request_nrfs.status_request='draft' OR tr_part_request_nrfs.status_request='submitted')
			");
		if ($cek_data->num_rows()>0) {
			$row = $data['row']['dokumen']       = $cek_data->row();
			$data['row']['request_id']       = $cek_data->row()->request_id;
	...
	{				
		...
		$id_dealer  = $this->m_admin->cari_dealer();
		$cek_data   = $this->db->get_where('tr_part_request_nrfs',['request_id'=>$request_id,'id_dealer'=>$id_dealer]);
		if ($cek_data->num_rows()>0) {
			$data['status_request'] = 'submitted';

			$dealer    = $this->db->get_where('ms_dealer',['id_dealer'=>$id_dealer])->row();
			
			$ktg_notif = $this->db->get_where('ms_notifikasi_kategori',['id_notif_kat'=>3])->row();
			$notif = ['id_notif_kat'=> $ktg_notif->id_notif_kat,
				'id_referensi' => $request_id,
				'judul'        => "NRFS Parts Request dari Dealer $dealer->nama_dealer",
				'pesan'        => "Terdapat NRFS Parts Request dari Dealer $dealer->nama_dealer. Request ID = $request_id",
	...
	{				
		...
		$cek_data  = $this->db->get_where('tr_part_request_nrfs',['request_id'=>$request_id,'id_dealer'=>$id_dealer]);
		if ($cek_data->num_rows()>0) {
			$data['status_request'] = 'Cancelled by Dealer';
	...
	{		
		...
		$ktg_notif = $this->db->get_where('ms_notifikasi_kategori',['id_notif_kat'=>2])->row();
		$notif = ['id_notif_kat'=> $ktg_notif->id_notif_kat,
				'id_referensi' => $request_id,
				'judul'        => "NRFS Request Dari Warehouse",
				'pesan'        => "Terdapat NRFS request form dari warehouse.",
				'link'         => $ktg_notif->link.'/detail?id='.$request_id,
				'status'       =>'baru',
				'id_dealer'    => $id_dealer,
				'created_at'   => $waktu,
	...
	{		
		...
		$dokumen_nrfs_id = $data['dokumen_nrfs_id'] = $this->input->post('dokumen_nrfs_id');
		$data['id_dealer']       = $id_dealer;
		...
		foreach ($detail as $dtl) {
			$detail_dokumen[]=['dokumen_nrfs_id'=> $dokumen_nrfs_id,
						'id_part'=>$dtl['id_part'],
						'nama_part'=>$dtl['nama_part'],
						'qty_part'=>$dtl['qty_part'],
			];
		}

		$this->db->trans_begin();
			$this->db->update('tr_part_request_nrfs',$data,['request_id'=>$request_id]);
			$this->db->delete('tr_dokumen_nrfs_part',['dokumen_nrfs_id'=>$dokumen_nrfs_id]);
			$this->db->insert_batch('tr_dokumen_nrfs_part',$detail_dokumen);
		if ($this->db->trans_status() === FALSE)
      	{
			$this->db->trans_rollback();
			$response['status'] ='error';
   ...
   {  
     ...
     $search = $this->input->post('search')['value'];
	  if ($search!='') {
	      ...
	      $this->db->where("tr_dokumen_nrfs.status ='open'");
