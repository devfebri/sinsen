...
class Spk extends CI_Controller
{
	...
	public function update()
	{
		...
		if ($id == $id_) {
			...
			if($req_unik_nik->num_rows()>0){
				...
				$cek_kuota = $this->db->query("select no_spk, id_dealer from tr_spk tsp where no_spk!='$no_spk' and no_ktp ='$no_ktp' and ( program_umum  = '$program_1' or program_gabungan ='$program_2')");
				if($cek_kuota->num_rows()>0){
					// if($req_unik_nik->row()->kuota_program >= $cek_kuota->num_rows()){
						$is_tersedia=0;
					// }
	...
	public function getProgram()
	{
		...
		$cek_program = $this->db->query("SELECT *,(ahm_cash+md_cash+dealer_cash+other_cash+add_md_cash+add_dealer_cash) as tot_cash,(ahm_kredit+md_kredit+dealer_kredit+other_kredit+add_md_kredit+add_dealer_kredit) as tot_kredit,tr_sales_program.id_program_md as pmd, 
				(SELECT count(id_program_md)FROM tr_sales_program_gabungan WHERE tr_sales_program_gabungan.id_program_md_gabungan=pmd) as tot_gabungan 
		...
		if(count($cek_program) > 0){
			...
			if ($cek_program->kuota_program == 0 || $cek_program->kuota_program == NULL) {
				...
				if ($cek_dealer->num_rows() == 0) {
					$tersedia = 1; // tidak ada settingan kuota program maka diganti ke tersedia=0 ???
				} else {
	...
	public function getVoucherGabungan()
	{
		...
		if ($nilai_vouch_gab->num_rows() > 0) {
			...
			if ($cek_program->kuota_program == 0 || $cek_program->kuota_program == NULL) {
				$cek_dealer = $this->db->get_where('tr_sales_program_dealer', ['id_program_md' => $id_program_gabungan]);
				if ($cek_dealer->num_rows() == 0) {
					$tersedia = 1; // ??? apakah dibuat tidak tersedia karena sama" tidak ada setting kuota program dan kuota dealer
				} else {
					...
					if ($dealer->num_rows() > 0) {
						...
						// $tersedia = 0;
						$cek_terjual = $this->db->query("SELECT no_spk FROM tr_spk
							WHERE (tr_spk.program_umum='$id_program_gabungan' or tr_spk.program_gabungan='$id_program_gabungan') $where_edit 
							AND status_spk not in ('canceled') 
						")->num_rows();
						$tersedia = $cek_terjual <= $cek_program->kuota_program ? $cek_program->kuota_program - $cek_terjual : 0;
					}
