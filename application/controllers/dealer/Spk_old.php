...
var $title  =   "SPK (Surat Pesanan Kendaraan) ";
  function mata_uang($a){
	...
	{				
		...
		$data['dt_customer'] = $this->db->query("SELECT * FROM tr_prospek ORDER BY id_customer ASC");						
		$this->template($data);	
	...
	{				
		...
		$data['set']		= "history";
		$id_dealer = $this->m_admin->cari_dealer();
		$data['dt_spk'] = $this->db->query("SELECT * FROM tr_spk WHERE id_dealer = '$id_dealer' AND no_spk IN(SELECT no_spk FROM tr_sales_order) ORDER BY no_spk ASC");		
		$data['dt_customer'] = $this->db->query("SELECT * FROM tr_prospek ORDER BY id_customer ASC");						
		$data['dt_npwp'] = $this->db->query("SELECT * FROM tr_prospek_gc LEFT JOIN ms_kelurahan ON tr_prospek_gc.id_kelurahan=ms_kelurahan.id_kelurahan 
							WHERE tr_prospek_gc.id_dealer = '$id_dealer' AND tr_prospek_gc.status_prospek = 'Deal'
							ORDER BY tr_prospek_gc.nama_npwp ASC");																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																											
	...
	{						
		...
		$data['dt_customer'] = $this->db->query("SELECT tr_prospek.*,ms_karyawan_dealer.nama_lengkap FROM tr_prospek LEFT JOIN ms_dealer ON tr_prospek.id_dealer=ms_dealer.id_dealer
			...
			WHERE tr_prospek.active = '1' AND tr_prospek.id_dealer = '$id_dealer' 
	...
	{		
		...
		if($cek == 0){
			...
			$data['voucher_2'] 					= $this->input->post('voucher_2');			
			$data['voucher_tambahan_2']	= $this->input->post('voucher_tambahan_2');			
			...
			// 				JOIN tr_penerimaan_unit_dealer ON tr_penerimaan_unit_dealer_detail.id_penerimaan_unit_dealer=tr_penerimaan_unit_dealer.id_penerimaan_unit_dealer
			// 				WHERE tr_scan_barcode.status=4 
			// 				AND tipe_motor='$id_tipe_kendaraan' 
			// 				AND warna='$id_warna' 
   ...
   //              WHERE tr_scan_barcode.id_item = '$id_item' 
   //              AND tr_penerimaan_unit_dealer.id_dealer = '$id_dealer' 
   ...
   //              ");
			...
			if ($dt==false) {
				$indent = ['id_indent'=>$this->get_kode_indent(),
							'id_spk'            => $no_spk,
							'id_dealer'         => $id_dealer,
	...
	{		
		...
		if($cek == 0){
			...
			$data['no_telp'] 		= $dp['no_telp']				= $this->input->post('no_telp');	
	...
	{		
		...
		if($id == $id_){
			...
			$no_telp = $data['no_telp']=$da['no_telp']               = $this->input->post('no_telp');	
			...
			if ($cek_spk->id_tipe_kendaraan!=$id_tipe_kendaraan || $cek_spk->id_warna!=$id_warna) {
				...
				if ($dt==false) {
					...
					if ($cek_ind->num_rows()>0) {
						...
						$ins_indent = ['id_indent'=>$this->get_kode_indent(),
									'id_spk'            => $no_spk,
									'id_dealer'         => $id_dealer,
									'nama_konsumen'     => $nama_konsumen,
									'alamat'            => $alamat,
	...
	public function cetak(){
		...
		if ($spk->num_rows() > 0) {
			$row =$spk->row();
			$dt_kel				= $this->db->query("SELECT * FROM ms_kelurahan WHERE id_kelurahan = '$row->id_kelurahan'")->row();
			$kelurahan 		= $dt_kel->kelurahan;
			$id_kecamatan = $dt_kel->id_kecamatan;
		...
		if ($row->alamat_sama !='Ya') {
		  ...
		  */ $pdf->Ln(3);
		  ...
		  $pdf->SetFont('ARIAL','',10);
		  $pdf->Ln(2);
		  $pdf->Cell(33, 5, 'NAMA PEMESAN', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(60, 5, $row->nama_konsumen, 0, 0, 'L');$pdf->Cell(5, 5, '', 0, 0, 'L');
		  $pdf->Cell(32, 5, 'TEMPAT LAHIR', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(50, 5,  $row->tempat_lahir, 0, 1, 'L');
		  
		  $pdf->Cell(33, 5, 'NOMOR KTP', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(60, 5, $row->no_ktp, 0, 0, 'L');$pdf->Cell(5, 5, '', 0, 0, 'L');
		  $pdf->Cell(32, 5, 'TANGGAL LAHIR', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(50, 5, tgl_indo($row->tgl_lahir,' '), 0, 1, 'L');
		  $pdf->Ln(1);
		  $pdf->Cell(33, 5, 'ALAMAT DOMISILI', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(60, 5, $row->alamat, 0, 1, 'L');
		  
		  $pdf->Cell(33, 5, 'KELURAHAN', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(60, 5, $kelurahan, 0, 1, 'L');
		  
		  $pdf->Cell(33, 5, 'KECAMATAN', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(60, 5, $kecamatan, 0, 1, 'L');
		  
		  $pdf->Cell(33, 5, 'KOTA/KABUPATEN', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(60, 5, $kabupaten, 0, 1, 'L');
		  $pdf->Ln(1);
		  $pdf->Cell(33, 5, 'NOMOR TELP/HP', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(60, 5, $row->no_hp, 0, 1, 'L');
		  $pdf->Ln(1);
		  $pdf->Cell(33, 5, 'ALAMAT KTP', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(60, 5, $alamat2, 0, 1, 'L');
		   
		  $pdf->Cell(33, 5, 'KOTA/KABUPATEN', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(60, 5, $kabupaten2, 0, 1, 'L');
		  
		  $pdf->Cell(33, 5, 'KELURAHAN', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(60, 5, $kelurahan2, 0, 1, 'L');
		  
		  $pdf->Cell(33, 5, 'KECAMATAN', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(60, 5, $kecamatan2, 0, 0, 'L');
		  $pdf->SetFont('ARIAL','',9);
		...
		$qr_generate = "maps.google.com/local?q=$latitude,$longitude";
		  ...
		  $pdf->SetFont('ARIAL','',10);
		  $pdf->Cell(33, 5, 'NPWP', 0, 0, 'L');$pdf->Cell(5, 5, ':', 0,0, 'C');$pdf->Cell(60, 5, $row->npwp, 0, 1, 'L');
		  $pdf->Ln(1);
		$pdf->Image("https://chart.googleapis.com/chart?cht=qr&chl=$qr_generate&chs=77x77",155,46,40,0,'PNG');
		  ...
		  $pdf->Cell(33, 5, 'Harga', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, 'Rp. '.$this->mata_uang($row->harga), 0, 0, 'L');
	  	  $pdf->Cell(33, 5, 'Type', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, $row->tipe_ahm, 0, 1, 'L');
		  $pdf->Cell(33, 5, 'PPN', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, 'Rp. '.$this->mata_uang($row->ppn), 0, 0, 'L');
		  $pdf->Cell(33, 5, 'Warna', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, $row->warna, 0, 1, 'L');
		  $pdf->Cell(33, 5, 'Harga Off The Road', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, 'Rp. '.$this->mata_uang($row->harga_off_road), 0, 1, 'L');
		  // $pdf->Cell(33, 5, 'Tahun', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, $row->, 0, 1, 'L');
		  $pdf->Cell(33, 5, 'Biaya Surat', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, 'Rp. '.$this->mata_uang($row->biaya_bbn), 0, 1, 'L');
		  
		   $pdf->Cell(33, 5, 'Harga On The Road', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, 'Rp. '.$this->mata_uang($total_bayar), 0, 1, 'L');
		  $pdf->Ln(2);
		  $pdf->SetFont('ARIAL','B',10);
	  ...
	  if ($row->jenis_beli =='Cash') {
	  	$yy=191;
	  		...
	  		$pdf->SetFont('ARIAL','',10);
		  $pdf->Ln(1);
		  	$pdf->Cell(33, 5, 'Harga Tunai', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, 'Rp. '.$this->mata_uang($row->harga_on_road), 0, 1, 'L');
		  	$pdf->Cell(33, 5, 'Voucher', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, 'Rp. '.$this->mata_uang($row->voucher_1), 0, 0, 'L');
		  	$pdf->Cell(33, 5, 'Voucher Tambahan', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, 'Rp. '.$this->mata_uang($voucher_tambahan), 0, 1, 'L');
		  	$pdf->Cell(33, 5, 'Total Bayar', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, 'Rp. '.$this->mata_uang($total_bayar), 0, 1, 'L');
	  		...
	  		$pdf->SetFont('ARIAL','',10);
		  $pdf->Ln(1);
		  	$pdf->Cell(33, 5, 'Leasing / FINCO', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, $finance_company, 0, 1, 'L');
		  	...
		  	$pdf->Cell(33, 5, 'Uang Muka / DP', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, 'Rp. '.$this->mata_uang($row->dp_stor).' (UANG MUKA / DP YANG DISETOR KONSUMEN)', 0, 1, 'L');
		  	$pdf->Cell(33, 5, 'Voucher', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, 'Rp. '.$this->mata_uang($row->voucher_2).' ('.$program_umum.' '.$program_g.')', 0, 1, 'L');
		  	$voucher_tambahan = $row->voucher_tambahan_2+$row->diskon;
		  	$pdf->Cell(33, 5, 'Voucher Tambahan', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, 'Rp. '.$this->mata_uang($voucher_tambahan), 0, 1, 'L');
		  	$pdf->Cell(33, 5, 'Angsuran', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, 'Rp. '.$this->mata_uang($row->angsuran), 0, 0, 'L');
		  	$pdf->Cell(33, 5, 'Tenor', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, $row->tenor, 0, 1, 'L');
		  	$pdf->Cell(33, 5, 'Pekerjaan', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, $pekerjaan, 0, 1, 'L');
		  	$pdf->Cell(33, 5, 'Jabatan', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, $jabatan, 0, 0, 'L');
		  	$pdf->Cell(33, 5, 'Lama Kerja', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, $row->lama_kerja, 0, 1, 'L');
		  	...
		  	$pdf->Cell(33, 5, 'Status Rumah', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, $row->status_rumah, 0, 0, 'L');
		  	$pdf->Cell(33, 5, 'Total Penghasilan', 0, 0, 'L'); $pdf->Cell(2, 5, ':', 0, 0, 'L');$pdf->Cell(55, 5, "Rp. ".$penghasilan, 0, 1, 'L');
		  ...
		  $pdf->Ln(2);
		  $pdf->SetFont('ARIAL','',10);
		  $pdf->Cell(5, 5, '1. ', 0, 0, 'L'); $pdf->MultiCell(185, 5, 'HARGA yang tercantum dalam Surat Pesanan ini tidak mengikat dan Surat Pesanan ini bukan merupakan bukti pembayaran.', 0, 1);
		  $pdf->Cell(5, 5, '2. ', 0, 0, 'L'); $pdf->MultiCell(185, 5, 'Surat Pesanan ini dianggap SAH apabila telah ditandatangani oleh Pemesan, Sales Person, dan Kepala Cabang.', 0, 1);
		  $pdf->Cell(5, 5, '3. ', 0, 0, 'L'); $pdf->MultiCell(185, 5, 'Pembayaran dengan Cek/Bilyet Giro/Transfer dianggap sah apabila telah diterima di rekening :', 0, 1);
	  ...
	  // $nama_rek = implode(" atau ", $nama_rek);
		  $pdf->Cell(5, 5, '4. ', 0, 0, 'L'); $pdf->MultiCell(185, 5, 'Pembayaran Tunai dianggap sah apabila telah diterbitkan kwitansi oleh '.$r->nama_dealer.'.', 0, 1);
		  $pdf->Cell(5, 5, '5. ', 0, 0, 'L'); $pdf->MultiCell(185, 5, 'Pengiriman unit dan pengurusan surat-surat dilaksanakan setelah 100% harga kendaraan lunas.', 0, 1);
		  ...
		  $pdf->Ln(3);
		  $pdf->Cell(63, 5, 'PEMESAN,', 0, 0, 'C');$pdf->Cell(63, 5, 'SALES PERSON,', 0, 0, 'C');$pdf->Cell(63, 5, 'KEPALA CABANG,', 0, 1, 'C');
			$pdf->Ln(18);
		   ...
		   $pdf->Cell(63, 5, '( '.strtoupper($row->nama_konsumen).' )', 0, 0, 'C');$pdf->Cell(63, 5, '( '.$karyawan.' )', 0, 0, 'C');$pdf->Cell(63, 5,'( '.$kacab.' )', 0, 1, 'C');
			...
			if ($jenis_beli=='Cash') {
				...
				foreach ($cek_program->result() as $rs) {
							...
							if ($cek_program->num_rows() >0) {
								$ck = $cek_program->row();
								if ($ck->tot_cash >0) {
									echo "<option value='$rs->id_program_md'>$ck->id_program_md | $ck->judul_kegiatan</option>";
									$x++;
								}
							}
				}
