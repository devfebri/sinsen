...
class Spk extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('m_dealer_spk_datatables');
		//===== Load Library =====
	...
	{
		$id_dealer = $this->m_admin->cari_dealer();
		$list = $this->m_dealer_spk_datatables->get_datatables($id_dealer);
		$data = array();
		$no = $_POST['start'];
        ...
        foreach($list as $row) {       
			if (!empty($row->no_spk )) {
				$link_spk=" <a href='dealer/spk/detail?id=$row->no_spk'>
				$row->no_spk
				</a>";
			}else{
				$link_spk = "<span class='label label-danger'>Tidak Ditemukan</span>";
			  }
			...
			$approve_kacab  = $this->db->get_where('ms_karyawan_dealer',['id_karyawan_dealer'=>$_SESSION['id_karyawan_dealer']])->row()->id_jabatan;

			$btn_approve ='';
			$btn_reject  ='';

			// if ($approve_kacab =='JBT-053'){
			// 	$btn_approve .= "<a data-toggle='tooltip' $approval onclick=\"return confirm('Are you sure to approve this data?')\" title='Approve' class='btn btn-success btn-xs btn-flat' href='dealer/spk/approve?id=$row->no_spk'><i class='fa fa-check'></i> Approve</a>";
			// 	$btn_reject .= "<a data-toggle='tooltip' $approval onclick=\"return confirm('Are you sure to reject this data?')\" title='Reject' class='btn btn-danger btn-xs btn-flat' href='dealer/spk/reject?id=$row->no_spk'><i class='fa fa-close'></i> Reject</a>";
			// }

			$btn_approve .= "<a data-toggle='tooltip'  onclick=\"return confirm('Are you sure to approve this data?')\" title='Approve' class='btn btn-success btn-xs btn-flat' href='dealer/spk/approve?id=$row->no_spk'><i class='fa fa-check'></i> Approve</a>";
			
			
			// if($row->no_spk=='22/06/03/00010-00888'){
			// 	var_dump($row->status_spk);
			// 	die();
			// }
			
			$btn_reject  .= "<a data-toggle='tooltip' $approval onclick=\"return confirm('Are you sure to reject this data?')\" title='Reject' class='btn btn-danger btn-xs btn-flat' href='dealer/spk/reject?id=$row->no_spk'><i class='fa fa-close'></i> Reject</a>";
			$btn_edit = "<a data-toggle='tooltip' $edit title='Edit' href='dealer/spk/edit?id=$row->no_spk'><button class='btn btn-flat btn-xs btn-primary'><i class='fa fa-edit'></i> Edits</button></a>";
			$btn_cancel = "<a data-toggle='tooltip' title='Cancel' class='btn btn-danger btn-xs btn-flat' href='dealer/spk/cancel?id=$row->no_spk'><i class='fa fa-close'></i> Cancel</a>";
			$btn_cetak = "<a  data-toggle='tooltip' title='Cetak' target='_blank' href='dealer/spk/cetak?id=$row->no_spk'><button $print class='btn btn-flat btn-xs btn-warning'><i class='fa fa-print'></i> Cetak SPK</button></a> ";


			if ($row->status_spk == 'booking') {
			...
			if ($row->status_spk == 'input') {
			  ...
			  $tombol = "";
			  //sementara
			  // $tombol = $btn_approve . ' ' . $btn_cetak . ' ' . $btn_edit . ' ' . $btn_cancel;
			...
			// $cek_s = $this->db->query("SELECT status_approval FROM tr_hasil_survey WHERE no_spk = '$row->no_spk'");

			// if ($cek_s->num_rows() > 0) {
			//   $rr = $cek_s->row();
			//   $status_s = $rr->status_approval;
			// 	  if ($status_s == 'rejected' and $row->status_spk!='rejected') {
			// 	  $tombol = "<a data-toggle='tooltip' title='Edit' href='dealer/spk/edit?id=$row->no_spk'><button $edit class='btn btn-flat btn-xs btn-primary'><i class='fa fa-edit'></i> Edit</button></a>";
			// 	   $tombol .= ' '. $btn_cetak.' '. $btn_reject . ' ' . $btn_cancel ;
			// 	  }
			// }
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->m_dealer_spk_datatables->count_all($id_dealer),
			"recordsFiltered" => $this->m_dealer_spk_datatables->count_filtered($id_dealer),
			"data" => $data,
		);
	...
	public function get_data_history_spk($search, $limit, $start, $order_field, $order_ascdesc, $id_dealer)
	{
		$cari = '';
		if ($search != '') {
			$cari = " AND (tr_spk.id_tipe_kendaraan LIKE '%$search%' OR tr_spk.id_warna LIKE '%$search%' OR tr_spk.status_spk LIKE '%$search%' OR tr_spk.no_spk LIKE '%$search%' OR tr_spk.nama_konsumen LIKE '%$search%') ";
		} 
		...
		$sql = "
			...
			WHERE
				( so.id_sales_order IS NOT NULL OR tr_spk.status_spk in ('rejected','canceled') ) 
				AND tr_spk.id_dealer = '$id_dealer' $cari 
			ORDER BY
	...
	public function get_count_history_spk($search,$id_dealer)
	{
		$cari = '';
		if ($search != '') {
			$cari = " AND (tr_spk.id_tipe_kendaraan LIKE '%$search%' OR tr_spk.id_warna LIKE '%$search%' OR tr_spk.no_spk LIKE '%$search%' OR tr_spk.status_spk LIKE '%$search%' OR tr_spk.nama_konsumen LIKE '%$search%') ";
		} 
	...
	public function cetak_gc()
	{
		...
		$data['no_spk'] = $id;
		$sql = $this->db->query("SELECT tr_spk_gc.*, tr_spk_gc.tgl_spk_gc as tanggal_spk_gc, tr_prospek_gc.id_karyawan_dealer,
	...
	public function add()
	{
		...
		// $data['dt_finance'] = $this->db->query("select * from ms_finance_company where active ='1' order by finance_company asc");
		...
		$data['event'] = $this->m_prospek->getEvent();
		$id_dealer = $this->m_admin->cari_dealer();
		...
		$data['dt_customer'] = $this->db->query("SELECT tr_prospek.*,ms_karyawan_dealer.nama_lengkap 
		FROM tr_prospek 
		JOIN ms_dealer ON tr_prospek.id_dealer=ms_dealer.id_dealer
	...
	public function ajax_list_bpkb()
	{
		$list = $this->m_kelurahan->get_datatables();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $isi) {
			...
			$row[] = $kecamatan;
			$row[] = $kabupaten;
			$row[] = "<button title=\"Choose\" data-dismiss=\"modal\" onclick=\"chooseitemBpkb('$isi->id_kelurahan')\" class=\"btn btn-flat btn-success btn-sm\"><i class=\"fa fa-check\"></i></button>";
			$data[] = $row;
		}
		$output = array(
			"draw" => $_POST['draw'],
	...
	public function cari_gc()
	{
		...
		if( date('Y-m-d') >= '2023-01-01'){
			$thn   = date('Y');
			$id_dealer = $this->m_admin->cari_dealer();
			$pr_num  = $this->db->query("SELECT no_spk_gc FROM tr_spk_gc
				WHERE LEFT(created_at,4)='$thn' AND id_dealer = $id_dealer
				ORDER BY created_at DESC LIMIT 0,1");
		}else{
		...
		if ($pr_num->num_rows() > 0) {
			...
			$id 	= substr($row->no_spk_gc, 11, 5) + 1;
			if ($id < 10) {
				$kode1 = $th . "/" . $bln . "/" . $tgl . "/0000" . $id;
			} elseif ($id > 9 && $id <= 99) {
				$kode1 = $th . "/" . $bln . "/" . $tgl . "/000" . $id;
		...
		foreach ($hasil as $ambil) {
			...
			if ($cek_program->num_rows() > 0) {
				...
				if ($beli == 'cash') {
					...
					$md = $am->md_cash;
					$other = $am->other_cash;
					$add_dealer = $am->add_dealer_cash;
					$add_md = $am->add_md_cash;
				} else {
					$ahm = $am->ahm_kredit;
					$dealer = $am->dealer_kredit;
					$md = $am->md_kredit;
					$other = $am->other_kredit;
					$add_dealer = $am->add_dealer_kredit;
					$add_md = $am->add_md_kredit;
				}
				$h = $ahm + $md + $dealer + $other + $add_md + $add_dealer;
			} else {
				$h = 0;
			}
			$out = $out . "|" . $h;
	...
	public function cek_selected_event()
	{
		$id_prospek_gc	= $this->input->post('id_prospek_gc');
		$dt_event = $this->m_prospek->getEvent()->result();
		echo "<option value=''>--choose-</option>";
		foreach ($dt_event as $rs) {
			$id_event = $this->db->query("SELECT id_event FROM tr_prospek_gc WHERE id_prospek_gc = '$id_prospek_gc'")->row()->id_event;
			$selected = $rs->id_event == $id_event ? 'selected' : '';
			echo "<option value='$rs->id_event' $selected data-nama_event='$rs->nama_event'>$rs->id_event . ' | ' . $rs->nama_event</option>";
		}
	}
	...
	public function cek_bbn()
	{
		...
		*/
		$cek_harga = $this->db->query("select a.tgl_approve , a.start_date , b.id_item , b.harga_jual , a.status from ms_kelompok_md_harga a
			...
			where b.id_item ='$id_item' and a.start_date  <= '$date' and a.id_kelompok_harga ='A' and a.tgl_approve is not null
		...
		$harga 		= floor($harga_jual / getPPN(1.1,false));
		$ppn 		= floor(getPPN(0.1,false) * $harga);
	...
	public function save()
	{
		...
		if ($cek == 0) {
			...
			// tambahan kelurahan bpkb
			$data['id_kelurahan_bpkb'] 			= $this->input->post('id_kelurahan_bpkb');
			...
			$alamat = $data['alamat'] 						= htmlentities($this->input->post('alamat',false));
			...
			$data['alamat_ktp_bpkb']  = htmlentities($this->input->post('alamat_ktp_bpkb',false));			
			$data['bpkb_stnk_birth_place'] 			= $this->input->post('bpkb_stnk_birth_place');
			$data['bpkb_stnk_birth_date'] 			= $this->input->post('bpkb_stnk_birth_date');
			...
			$data['id_event']         = (int)$this->input->post('id_event');
			$data['no_kk']            = $this->input->post('no_kk');
			$data['alamat_kk']        = htmlentities($this->input->post('alamat_kk',false));
			...
			$data['alamat2'] 						= htmlentities($this->input->post('alamat2'));
			...
			if ($jenis_beli == 'Cash') {
				...
				$is_tersedia = 1;
				$program_1 = $this->input->post('program_umum');
				$program_2 = $this->input->post('program_gabungan');
				$req_unik_nik = $this->db->query("select id_program_md, kuota_program, id_program_ahm, id_jenis_sales_program from tr_sales_program tsp where unique_customer ='1' and (id_program_md  = '$program_1' or id_program_md ='$program_2')");

				$data_id = '';
				if($req_unik_nik->num_rows()>0){
					$data_id = $req_unik_nik->row()->id_program_md;

					$arr_id_program_md = "'";
					foreach ($req_unik_nik->result() as $rs) {
						$arr_id_program_md .= $rs->id_program_md ."','";
					}
					$arr_id_program_md = substr_replace($arr_id_program_md, '', -2);

					if($req_unik_nik->row()->id_program_ahm!=''){
						$cek_kuota = $this->db->query("select no_spk, id_dealer from tr_spk tsp where status_spk in ('close','booking','approved') and no_ktp ='$no_ktp' and (program_umum  in ($arr_id_program_md) or program_gabungan in($arr_id_program_md) )");
					}else{
						$cek_kuota = $this->db->query("select no_spk, id_dealer from tr_spk tsp where status_spk in ('close','booking','approved') and no_ktp ='$no_ktp' and (program_umum  = '$program_1' or program_gabungan ='$program_2')");
					}
					if($cek_kuota->num_rows()>0){
						// if($req_unik_nik->row()->kuota_program >= $cek_kuota->num_rows()){
							$is_tersedia=0;
						// }
					}
					
					$req_subsidi_ev = $this->db->query("SELECT 
						sum(spt.ahm_kredit +spt.md_kredit+ spt.dealer_kredit + spt.other_kredit  )as  potongan_kredit,
						sum(spt.ahm_cash   +spt.md_cash+spt.dealer_cash + spt.other_cash) as  potongan_cash,
					tsp.id_program_md, tsp.kuota_program, tsp.id_program_ahm, tsp.id_jenis_sales_program,
					spt.id_warna 
					from tr_sales_program tsp 
					left join tr_sales_program_tipe spt on spt.id_program_md = tsp.id_program_md 
					WHERE  tsp.unique_customer ='1' 
					and (tsp.id_program_md  = '$program_1' or tsp.id_program_md ='$program_2') ");
					$subsidi_pemerintah = $req_subsidi_ev->row();	
					
					$string = $subsidi_pemerintah->id_warna;

					if ($subsidi_pemerintah->id_jenis_sales_program == 'SP-012' && strpos($string, $id_warna) !== false ){
						if ($jenis_beli == 'Cash'){
							$data['subsidi_pemerintah'] = $subsidi_pemerintah->potongan_cash ;
						}

						if ($data['voucher_1'] 	=='7000000' ){
							$data['voucher_1'] = 0;
						}

						if ($data['voucher_2'] 	=='7000000' ){
							$data['voucher_2'] = 0;
						}
					}
				...
				if ($data['program_umum'] !== '' && $data['program_umum'] !== NULL && $data['voucher_1'] == 0 && $jenis_program_temp !=='SP-002' && $jenis_program_temp !=='SP-011') {
				...
				if ($format_kk == 'salah') {
					...
					$_SESSION['id_tipe'] 	= $this->input->post("id_tipe_kendaraan");
					echo "<script>history.go(-1)</script>";
				}elseif ($is_tersedia==0) {
					$_SESSION['pesan'] 	= "Kuota Program sudah mencapai limit!";
					$_SESSION['tipe'] 	= "danger";
					$_SESSION['id_warna'] 	= $this->input->post("id_warna");
					$_SESSION['id_tipe'] 	= $this->input->post("id_tipe_kendaraan");
					echo "<script>history.go(-1)</script>";
					...
					if ($this->db->trans_status() === FALSE) {
						...
						$cek_prospek = $this->db->query("SELECT leads_id FROM tr_prospek WHERE id_customer='{$data['id_customer']}' AND IFNULL(leads_id,'')!=''")->row();
						if ($cek_prospek != NULL) {
							$this->load->model('Api_crm_post_model', 'api_crm');
							$this->api_crm->api_4_stageId_10($no_spk);
						}
				...
				$is_tersedia = 1;
				$program_1 = $this->input->post('program_umum_k');
				$program_2 = $this->input->post('program_gabungan_k');
				$req_unik_nik = $this->db->query("select id_program_md, kuota_program, id_program_ahm , id_jenis_sales_program from tr_sales_program tsp where unique_customer ='1' and (id_program_md  = '$program_1' or id_program_md ='$program_2')");
					...
					if($cek_kuota->num_rows()>0){
						...
						// }
							...
							if ($subsidi_pemerintah->id_jenis_sales_program == 'SP-012' && strpos($string, $id_warna) !== false ){
								...
								if ($jenis_beli == 'Kredit'){
									$data['subsidi_pemerintah'] = $subsidi_pemerintah->potongan_kredit ;
								}
				...
				/*
				$req_unik_nik = $this->db->query("select id_program_md, kuota_program from tr_sales_program tsp where unique_customer ='1' and (id_program_md  = '$program_1' or id_program_md ='$program_2')");
				...
				if ($data['program_umum'] !== '' && $data['program_umum'] !== NULL && $data['voucher_2'] == 0 && $jenis_program_temp !=='SP-002'  && $jenis_program_temp !=='SP-011') {
					$_SESSION['pesan'] 	= "Sales program terpilih, tetapi voucher tidak terisi !";
					$_SESSION['tipe'] 	= "danger";
					$_SESSION['id_warna'] 	= $this->input->post("id_warna");
					$_SESSION['id_tipe'] 	= $this->input->post("id_tipe_kendaraan");
				...
				if ($format_kk == 'salah') {
					...
					if ($this->db->trans_status() === FALSE) {
						...
						$array_post = [
							'AppsOrderNumber'   => '',
							'DmsOrderNumber'    => '',
							'CustomerPhoneNumber' => $data['no_hp'],
							'CreditStatus' => 'Proses pengajuan ke leasing',
							'IndentStatus' => '',
							'DeliveryStatus' => '',
							'EstimatedDeliveryDate' => $data['tgl_pengiriman'],
							'EngineNumber' => '',
							'StnkStatus' => '',
							'BpkbStatus' => '',
							'VehicleNumber' => '',
						...
						$cek_prospek = $this->db->query("SELECT leads_id,id_prospek FROM tr_prospek WHERE id_customer='{$data['id_customer']}' AND input_from='sinsengo'")->row();
						if ($cek_prospek != NULL) {
							$this->db_crm = $this->load->database('db_crm', true);
							$get_leads = $this->db_crm->get_where("leads", ['idProspek' => $cek_prospek->id_prospek])->row();
						...
						$this->mokita_model->set_tracking($no_spk, $array_post);
						$this->db->trans_commit();
						$_SESSION['pesan'] 	= "Data has been saved successfully";
						$_SESSION['tipe'] 	= "success";
						echo "<meta http-equiv='refresh' content='0; url=" . base_url() . "dealer/spk/add'>";
	...
	public function edit()
	{
		...
		if ($rs_spk->num_rows() > 0) {
			...
			$data['dt_warna'] = $this->db->query("SELECT ms_item.id_warna,ms_warna.* from ms_item 
				...
				ORDER BY ms_warna.warna ASC");
			// $data['dt_finance'] = $this->m_admin->getSortCond("ms_finance_company", "finance_company", "ASC");
			$data['dt_finance'] = $this->db->query("Select id_finance_company,finance_company from ms_finance_company where active = 1 order by finance_company asc");
			// $data['dt_prospek'] = $this->m_admin->getSortCond("tr_prospek", "no_hp", "id_tipe_kendaraan", "alamat", "ASC");
			...
			")->row();
			$data['dt_spk']->kelurahan_kk = $dt_kk!=null?$dt_kk->kelurahan:'';
			$data['dt_spk']->kecamatan_kk = $dt_kk!=null?$dt_kk->kecamatan:'';
			$data['dt_spk']->kabupaten_kk = $dt_kk!=null?$dt_kk->kabupaten:'';
			$data['dt_spk']->provinsi_kk = $dt_kk!=null?$dt_kk->provinsi:'';
			$this->template($data);
		}
	}
	...
	public function update()
	{
		...
		$get_spk  = $this->m_admin->getByID($tabel, $pk, $id_);
		$cek      = $get_spk->num_rows();
		...
		//Cek Apakah SPK Input Dari SC
		$get_spk = $get_spk->row();
		
		// if($id =='23/04/08/00071-00888'){
			if($this->input->post('jenis_beli') == 'Kredit' && $get_spk->status_survey =='baru'){
				if($this->input->post('id_finance_company') != $get_spk->id_finance_company && $get_spk->id_finance_company!=''){
					$_SESSION['pesan'] 	= "Silahkan update terlebih dahulu Hasil Survey Leasing!";
					$_SESSION['tipe'] 	= "danger";
					// $_SESSION['id_warna'] 	= $this->input->post("id_warna");
					// $_SESSION['id_tipe'] 	= $this->input->post("id_tipe_kendaraan");
					redirect('dealer/spk/edit?id='.$id,'refresh');
					// echo "<script>history.go(-1)</script>";
					// echo "<meta http-equiv='refresh' content='0; url=" . base_url() . "dealer/spk/edit?id=$id'>";

				}
			}

			if($get_spk->status_spk =='close'){
				$_SESSION['pesan'] 	= "Status SPK sudah menjadi Sales Order!";
				$_SESSION['tipe'] 	= "danger";
				// $_SESSION['id_warna'] 	= $this->input->post("id_warna");
				// $_SESSION['id_tipe'] 	= $this->input->post("id_tipe_kendaraan");
				redirect('dealer/spk/add','refresh');
			}
			// echo "Silahkan hubungi IT MD.";die;
		// }

		if ($get_spk->input_from=='sc') {
			$ym = date('Y/m');
			$path = "./uploads/spk/" . $ym;
			if (!is_dir($path)) {
		...
		if ($id == $id_) {
			...
			if ($this->upload->do_upload('file_foto')) {
				...
				if ($get_spk->input_from=='sc') {
					$da['file_foto'] = $data['file_foto'] = 'uploads/spk/' . $ym . '/' . $this->upload->file_name;
				}
			...
			if ($this->upload->do_upload('file_kk')) {
				...
				if ($get_spk->input_from=='sc') {
					$da['file_kk'] = $data['file_kk'] = 'uploads/spk/' . $ym . '/' . $this->upload->file_name;
				}
				$one = $this->m_admin->getByID($tabel, $pk, $id)->row();
			} else {
				$file_kk = "besar";
			...
			$alamat = $data['alamat'] = $da['alamat'] 						= htmlentities($this->input->post('alamat',false));
			...
			$data['alamat2'] = $da['alamat2']               = htmlentities($this->input->post('alamat2',false));
			...
			$data['pendidikan'] = $da['pendidikan']     = $this->input->post('pendidikan');
			$data['status_pernikahan'] = $da['status_pernikahan']     = $this->input->post('status_pernikahan');
			...
			// if(($data['program_umum'] =='' && $data['program_gabungan'] =='') || ($data['program_umum'] == null && $data['program_gabungan'] == null) ){


			if($data['program_umum'] == '' || $data['program_umum'] == NULL){
				// perlu perbaikan pada return value dari function getvoucergabungan atau penggunaan tanda ##
				$data['voucher_1'] = 0;
				$da['voucher_1'] = 0;
			...
			$status_survey = $cek_spk->status_survey;

			$da['status_survey'] = 'baru';
			$ganti_leasing = 0;
			
			if($this->input->post('id_finance_company') != $cek_spk->id_finance_company ){
				$da['status_survey'] = $data['status_survey']				 = "baru";

				if($status_survey == 'approved'){
					$_SESSION['pesan'] 	= "Tidak bisa update Finco karena sudah diapprove leasing sebelumnya. Silahkan Hubungi SPV Area.";
					$_SESSION['tipe'] 	= "danger";
					redirect('dealer/spk/edit?id='.$id,'refresh');
				}else{
					$ganti_leasing = 1;
			...
			if ($cek_spk->id_tipe_kendaraan != $id_tipe_kendaraan || $cek_spk->id_warna != $id_warna) {
				...
				if ($cek_ind->num_rows() > 0) {
					$indt = $cek_ind->row();
					$this->db->where('id_spk', $no_spk);
					$this->db->update('tr_po_dealer_indent', array(
						'updated_at'        => $waktu,
						'updated_by'        => $login_id,
						'alasan_cancel_indent' => 'Ganti Tipe/Warna',
						'status' =>'canceled',
			...
			$req_unik_nik = $this->db->query("select id_program_md, kuota_program, id_program_ahm from tr_sales_program tsp where unique_customer ='1' and (id_program_md  = '$program_1' or id_program_md ='$program_2')");
			...
			if($req_unik_nik->num_rows()>0){
				...
				if($req_unik_nik->row()->id_program_ahm!=''){
					$cek_kuota = $this->db->query("select no_spk, id_dealer from tr_spk tsp where status_spk in ('close','booking','approved') and no_spk!='$no_spk' and no_ktp ='$no_ktp' and (program_umum  in ($arr_id_program_md) or program_gabungan in($arr_id_program_md) )");
				}else{
					$cek_kuota = $this->db->query("select no_spk, id_dealer from tr_spk tsp where status_spk in ('close','booking','approved') and no_spk!='$no_spk' and no_ktp ='$no_ktp' and (program_umum  = '$program_1' or program_gabungan ='$program_2')");
				}
				if($cek_kuota->num_rows()>0){
					// if($req_unik_nik->row()->kuota_program >= $cek_kuota->num_rows()){
						$is_tersedia=0;
			...
			if ($format_kk == 'salah') {
				...
				$_SESSION['id_tipe'] 	= $this->input->post("id_tipe_kendaraan");
				echo "<script>history.go(-1)</script>";
			}elseif ($jenis_beli == 'Cash' && ($data['program_umum'] == '' || $data['program_umum'] == NULL) && $data['voucher_1'] > 0) {
				$_SESSION['pesan'] 	= "Sales program tidak terpilih, tetapi voucher terisi !";
				$_SESSION['tipe'] 	= "danger";
				$_SESSION['id_warna'] 	= $this->input->post("id_warna");
				$_SESSION['id_tipe'] 	= $this->input->post("id_tipe_kendaraan");
				echo "<script>history.go(-1)</script>";
			}elseif ($jenis_beli == 'Kredit' && ($data['program_umum'] == '' || $data['program_umum'] == NULL) && $data['voucher_2'] > 0) {
				$_SESSION['pesan'] 	= "Sales program tidak terpilih, tetapi voucher terisi !";
				$_SESSION['tipe'] 	= "danger";
				$_SESSION['id_warna'] 	= $this->input->post("id_warna");
				$_SESSION['id_tipe'] 	= $this->input->post("id_tipe_kendaraan");
				...
				$this->db->trans_begin();
				// testing ernesto FIF
				// status survey : baru // status

				if ($jenis_beli == 'Kredit') {
					if($status_survey !='approved' && $status_survey!='rejected'){

						$update_old_survey = $this->db->query("SELECT no_order_survey, no_spk FROM tr_order_survey WHERE no_spk='$no_spk' AND status_survey in ('baru','') and order_uuid is null ");
	...
	public function approve()
	{
		...
		if ($cek_approval == 'salah') {
			...
			if ($spk->jenis_beli == 'Kredit') {
				...
				if($spk->status_survey !='approved' && $spk->status_survey!='rejected'){
					$update_old_survey = $this->db->query("SELECT no_order_survey, no_spk FROM tr_order_survey WHERE no_spk='$id' AND status_survey in ('baru','') ");
					if ($update_old_survey->num_rows() > 0) {
						foreach ($update_old_survey->result() as $dt) {
								$data_survey['status_survey'] = 'cancel';
								$this->m_admin->update('tr_order_survey', $data_survey, 'no_order_survey', $dt->no_order_survey);
						...
						// cek apabila ada salah status survey
				}
	...
	public function cetak()
	{
		...
		$spk = $this->m_admin->getByID("tr_spk", "no_spk", $id);
		if($spk->num_rows()> 0){
			$this->m_admin->update("tr_prospek", $data, "id_customer", $spk->row()->id_customer);
		}
	...
	public function getProgram()
	{
		...
		$cek_program = $this->db->query("SELECT tr_sales_program.id_program_md, tr_sales_program.judul_kegiatan,(ahm_cash+md_cash+dealer_cash+other_cash+add_md_cash+add_dealer_cash) as tot_cash, (ahm_kredit+md_kredit+dealer_kredit+other_kredit+add_md_kredit+add_dealer_kredit) as tot_kredit,tr_sales_program.id_program_md as pmd, 
				(SELECT count(id_program_md)FROM tr_sales_program_gabungan WHERE tr_sales_program_gabungan.id_program_md_gabungan=pmd) as tot_gabungan 
            ...
            AND
            (CASE 
            	WHEN tr_sales_program.kuota_program <>'*'
            	THEN 1=1 
            	ELSE 
            		CASE
	...
	public function getProgramTambahan()
	{
		...
		$set = $this->input->post('set'); // insert dan edit
		$no_spk = $this->input->post('no_spk'); // insert dan edit
		...
		if(count($cek_program) > 0){
			$judul_program = $cek_program->judul_kegiatan;
			$kode_dealer = '';
			//Semua Dealer
			$err_unique = 0;
			...
			$where_edit = '';
			if($set =='edit'){
				$temp_no_spk = $this->input->post('no_spk'); // insert dan edit
				$where_edit = "and tr_spk.no_spk !='$temp_no_spk'";
			}
			
			if($cek_program->unique_customer == 1){
				/*
				$cek_data = $this->db->query("
					SELECT tr_spk.no_spk , tr_spk.id_dealer , c.kode_dealer_md
					FROM tr_spk 
					join tr_sales_program a on a.id_program_md = tr_spk.program_umum
					left join tr_sales_program b on b.id_program_md = tr_spk.program_gabungan
					join ms_dealer c on c.id_dealer = tr_spk.id_dealer 
					WHERE (tr_spk.program_umum='$id_program_md' or tr_spk.program_gabungan ='$id_program_md') and tr_spk.no_ktp ='$ktp' 
					AND tr_spk.status_spk not in ('canceled') $where_edit
					order by tr_spk.tgl_spk desc
				");
				*/
				if($cek_program->id_program_ahm !==''){
					// looping utk menyimpan id program md supaya bisa di in kan ke array
					
					$arr_id_program_md = "'";
					$get_list_program_md = $this->db->get_where('tr_sales_program', ['id_program_ahm' => $cek_program->id_program_ahm])->result();
					foreach ($get_list_program_md as $rs) {
						$arr_id_program_md .= $rs->id_program_md ."','";
					}
					$arr_id_program_md = substr_replace($arr_id_program_md, '', -2);
					...
					$cek_data = $this->db->query("
						...
						left join tr_sales_program b on b.id_program_md = tr_spk.program_gabungan
						join ms_dealer c on c.id_dealer = tr_spk.id_dealer 
						WHERE (tr_spk.program_umum in ($arr_id_program_md) or tr_spk.program_gabungan in ($arr_id_program_md) ) and tr_spk.no_ktp ='$ktp' 
						AND tr_spk.status_spk not in ('canceled') $where_edit
						order by tr_spk.tgl_spk desc
					");
				}else{
				...
				if($cek_data->num_rows() > 0){
					$err_unique = 1;
					$kode_dealer = $cek_data->row()->kode_dealer_md;
				}
			}
	
			$tersedia = 0;
			if ($cek_program->kuota_program !='*') {
				// cek kuota program di header
				if($cek_program->kuota_program > 0){ // perlu cek utk unik customer, filter dealer dan potong/booking kuota
					$cek_terjual = $this->db->query("SELECT no_spk FROM tr_spk
								WHERE (tr_spk.program_umum='$id_program_md' or tr_spk.program_gabungan ='$id_program_md')
								$where_edit
								AND status_spk not in ('canceled') 
						")->num_rows();
					$tersedia = $cek_terjual <= $cek_program->kuota_program ? $cek_program->kuota_program - $cek_terjual : 0;

					// cek utk program sp-011
					if($tersedia==0 && $cek_program->id_jenis_sales_program=='SP-011'){
						$cek_terjual = $this->db->query("select count(a.no_mesin) as total from tr_sales_order a join tr_spk b on a.no_spk = b.no_spk where b.program_umum = '$id_program_md' or b.program_gabungan='$id_program_md' 
						")->row()->total;
						$tersedia = $cek_terjual <= $cek_program->kuota_program ? $cek_program->kuota_program - $cek_terjual : 0;
					}
			...
			if ($tersedia > 0) {
				...
				if ($nilai_voucher_program->num_rows() > 0) {
					...
					if ($program->id_jenis_sales_program == 'SP-002' || $program->id_jenis_sales_program == 'SP-011') {
				...
				$dt = date('Y-m-d');
				$cek_program = $this->db->query("
					SELECT id_program_md_gabungan as id_program_md, b.judul_kegiatan
					FROM tr_sales_program_gabungan a
					join tr_sales_program b on b.id_program_md = a.id_program_md_gabungan
					INNER JOIN ms_jenis_sales_program ON b.id_jenis_sales_program = ms_jenis_sales_program.id_jenis_sales_program 
					WHERE a.id_program_md=(
						SELECT id_program_md_gabungan FROM tr_sales_program_gabungan a
						join tr_sales_program_tipe b on a.id_program_md_gabungan = b.id_program_md  
						WHERE id_program_md_gabungan='$id_program_md' and b.id_tipe_kendaraan  = '$id_tipe_kendaraan' and b.id_warna like '%$id_warna%'
						group by id_program_md_gabungan 
					) AND '$dt' between b.periode_awal AND b.periode_akhir
					AND ms_jenis_sales_program.jenis_sales_program <> 'Group Customer'
					AND
					(CASE 
						WHEN b.kuota_program <>'*'
						THEN 1=1 
						ELSE 
							CASE
								WHEN (SELECT COUNT(id_dealer) FROM tr_sales_program_dealer WHERE id_program_md=b.id_program_md)>0
								THEN
									'$id_dealer' IN (SELECT id_dealer FROM tr_sales_program_dealer WHERE id_program_md=b.id_program_md) 
								ELSE
								1=0
							END
					END)
				...
				if($err_unique!=1){
					if ($jenis_beli == 'Cash') {
						...
						foreach ($cek_program->result() as $rs) {
							...
							$cek_program = $this->db->query("select id_program_md, (ahm_cash+md_cash+dealer_cash+other_cash+add_md_cash+add_dealer_cash) as tot_cash,(ahm_kredit+md_kredit+dealer_kredit+other_kredit+add_md_kredit+add_dealer_kredit) from tr_sales_program_tipe WHERE id_program_md='$rs->id_program_md' and id_tipe_kendaraan  = '$id_tipe_kendaraan' and id_warna like '%$id_warna%'");
							if ($cek_program->num_rows() > 0) {
								$ck = $cek_program->row();
								if ($ck->tot_cash > 0) {
									echo "<option value='$ck->id_program_md'>$ck->id_program_md | $rs->judul_kegiatan</option>";
									$x++;
								}
							}
						}
						...
						$xx = 0;
						foreach ($cek_program->result() as $rs) {
							// get nilai total kredit
							// $cek_program = $this->db->query("SELECT tr_sales_program.id_program_md, tr_sales_program.judul_kegiatan,(ahm_cash+md_cash+dealer_cash+other_cash+add_md_cash+add_dealer_cash) as tot_cash,(ahm_kredit+md_kredit+dealer_kredit+other_kredit+add_md_kredit+add_dealer_kredit) as tot_kredit,tr_sales_program.id_program_md as pmd, (SELECT count(id_program_md)FROM tr_sales_program_gabungan WHERE tr_sales_program_gabungan.id_program_md_gabungan=pmd) as tot_gabungan FROM tr_sales_program inner JOIN tr_sales_program_tipe on tr_sales_program.id_program_md=tr_sales_program_tipe.id_program_md WHERE tr_sales_program_tipe.id_tipe_kendaraan='$id_tipe_kendaraan' AND tr_sales_program.id_program_md='$rs->id_program_md_gabungan' AND id_warna LIKE '%$id_warna%' AND tr_sales_program_tipe.status<>'new' AND '$dt' between tr_sales_program.periode_awal AND tr_sales_program.periode_akhir");
							$cek_program = $this->db->query("select id_program_md, (ahm_cash+md_cash+dealer_cash+other_cash+add_md_cash+add_dealer_cash) as tot_cash,(ahm_kredit+md_kredit+dealer_kredit+other_kredit+add_md_kredit+add_dealer_kredit) as tot_kredit from tr_sales_program_tipe WHERE id_program_md='$rs->id_program_md' and id_tipe_kendaraan  = '$id_tipe_kendaraan' and id_warna like '%$id_warna%'");
							if ($cek_program->num_rows() > 0) {
								$ck = $cek_program->row();
								if ($ck->tot_kredit > 0) {
									echo "<option value='$ck->id_program_md'>$ck->id_program_md | $rs->judul_kegiatan</option>";
					...
					echo "Program '".$judul_program."' dengan No KTP ".$ktp." sudah digunakan di dealer ".$kode_dealer."##";
				}
			} else {
				if($err_unique == 1){
					echo "Program '".$judul_program."' dengan No KTP ".$ktp." sudah digunakan di dealer ".$kode_dealer."##";
		...
		}else if($id_program_md !=''){
			echo "Program sudah melewati batas / tidak ditemukan. Silahkan save ulang data.##";
		}else{
			echo "Terjadi Kesalahan / Program ID yang diikuti tidak sesuai dengan ketentuan.##";
		}
	}
	public function getVoucherGabungan()
	{
		...
		$nilai_voucher_program = $this->db->query("SELECT tr_sales_program.id_program_md, tr_sales_program.judul_kegiatan, tr_sales_program.id_jenis_sales_program,(ahm_cash+md_cash+dealer_cash+other_cash+add_md_cash+add_dealer_cash) as tot_cash,(ahm_kredit+md_kredit+dealer_kredit+other_kredit+add_md_kredit+add_dealer_kredit) as tot_kredit,tr_sales_program.id_program_md as pmd, (SELECT count(id_program_md)FROM tr_sales_program_gabungan WHERE tr_sales_program_gabungan.id_program_md_gabungan=pmd) as tot_gabungan FROM tr_sales_program inner JOIN tr_sales_program_tipe on tr_sales_program.id_program_md=tr_sales_program_tipe.id_program_md WHERE tr_sales_program_tipe.id_tipe_kendaraan='$id_tipe_kendaraan' AND id_warna LIKE '%$id_warna%' AND tr_sales_program_tipe.status<>'new' AND '$dt' between tr_sales_program.periode_awal AND tr_sales_program.periode_akhir  AND tr_sales_program.id_program_md='$id_program_md' ");
		if ($nilai_voucher_program->num_rows() > 0) {
			$program = $nilai_voucher_program->row();
			if ($jenis_beli == 'Cash') {
				$nilai = $nilai_voucher_program->row();
		...
		$nilai_vouch_gab = $this->db->query("SELECT tr_sales_program.id_program_md, tr_sales_program.judul_kegiatan,tr_sales_program.kuota_program,tr_sales_program.id_jenis_sales_program, tr_sales_program_tipe.jenis_barang,(ahm_cash+md_cash+dealer_cash+other_cash+add_md_cash+add_dealer_cash) as tot_cash,(ahm_kredit+md_kredit+dealer_kredit+other_kredit+add_md_kredit+add_dealer_kredit) as tot_kredit,tr_sales_program.id_program_md as pmd FROM tr_sales_program inner JOIN tr_sales_program_tipe on tr_sales_program.id_program_md=tr_sales_program_tipe.id_program_md WHERE tr_sales_program_tipe.id_tipe_kendaraan='$id_tipe_kendaraan' AND id_warna LIKE '%$id_warna%' AND tr_sales_program_tipe.status<>'new' AND '$dt' between tr_sales_program.periode_awal AND tr_sales_program.periode_akhir  AND tr_sales_program.id_program_md='$id_program_gabungan' ");
		if ($nilai_vouch_gab->num_rows() > 0) {
			...
			if ($cek_program->kuota_program !='*') {
				...
				if($cek_program->kuota_program > 0){ // perlu cek utk unik customer, filter dealer dan potong/booking kuota
					$cek_terjual = $this->db->query("SELECT no_spk FROM tr_spk
								WHERE (tr_spk.program_umum='$id_program_gabungan' or tr_spk.program_gabungan ='$id_program_gabungan')
								$where_edit
								AND status_spk not in ('canceled') 
						")->num_rows();
					$tersedia = $cek_terjual <= $cek_program->kuota_program ? $cek_program->kuota_program - $cek_terjual : 0;
					...
					// cek utk program sp-011
					if($tersedia==0 && $cek_program->id_jenis_sales_program=='SP-011'){
						$cek_terjual = $this->db->query("select count(a.no_mesin) as total from tr_sales_order a join tr_spk b on a.no_spk = b.no_spk where b.program_umum = '$id_program_gabungan' or b.program_gabungan='$id_program_gabungan' 
						")->row()->total;
						$tersedia = $cek_terjual <= $cek_program->kuota_program ? $cek_program->kuota_program - $cek_terjual : 0;
					}
				...
				$dealer = $this->db->get_where('tr_sales_program_dealer', ['id_program_md' => $id_program_gabungan, 'id_dealer' => $id_dealer]);
				if ($dealer->num_rows() > 0) {
					$dealer = $dealer->row();
					$cek_terjual = $this->db->query("SELECT no_spk FROM tr_spk
								WHERE (tr_spk.program_umum='$id_program_gabungan' or tr_spk.program_gabungan ='$id_program_gabungan')
	...
	public function cancel()
	{
		...
		$sql = "SELECT tr_spk.*,tipe_ahm,warna, (SELECT COUNT(id_spk) FROM tr_po_dealer_indent WHERE id_spk=no_spk) AS indent FROM tr_spk 
		JOIN ms_tipe_kendaraan ON ms_tipe_kendaraan.id_tipe_kendaraan=tr_spk.id_tipe_kendaraan
		JOIN ms_warna ON ms_warna.id_warna=tr_spk.id_warna
		WHERE no_spk = ? AND id_dealer = ?";

		$dt_spk = $this->db->query($sql, array(htmlentities($id),htmlentities($id_dealer)));
		...
		if ($dt_spk->num_rows() > 0) {
			...
			$data['event'] = $this->db->query("SELECT * FROM ms_event ORDER BY created_at DESC limit 1");
			// $id_dealer = $this->m_admin->cari_dealer();
			$data['dt_customer'] = $this->db->query("SELECT * FROM tr_prospek LEFT JOIN ms_tipe_kendaraan ON tr_prospek.id_tipe_kendaraan = ms_tipe_kendaraan.id_tipe_kendaraan 
							LEFT JOIN ms_kelurahan ON tr_prospek.id_kelurahan=ms_kelurahan.id_kelurahan 
							WHERE tr_prospek.id_dealer = '$id_dealer'
							ORDER BY tr_prospek.id_customer ASC limit 1");
			$data['dt_npwp'] = $this->db->query("SELECT * FROM tr_prospek_gc LEFT JOIN ms_kelurahan ON tr_prospek_gc.id_kelurahan=ms_kelurahan.id_kelurahan 
							WHERE tr_prospek_gc.id_dealer = '$id_dealer' AND tr_prospek_gc.status_prospek = 'Deal'
							ORDER BY tr_prospek_gc.nama_npwp ASC limit 1");
			$this->template($data);
