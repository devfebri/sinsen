...
*/
	...
	public function is_ev()
	{
		$tipe_kendaraan = $this->input->post('tipe_kendaraan');
		// $tipe_kendaraan = 'ME0';
		$id_dealer = $this->m_admin->cari_dealer();
		$is_ev     = $this->db->query("SELECT * FROM ms_setting_part_battery_ev where id_tipe_kendaraan='$tipe_kendaraan'")->row();
		$limit     = $is_ev->qty;
		...
		$sql = $this->db->query("SELECT stock_battery_int,serial_number,tipe,fifo FROM tr_stock_battery where no_sales_order is null and id_dealer ='$id_dealer' order by fifo asc");	
	...
	public function get_ev_serial_number()
	{
		$tipe_kendaraan = $this->input->post('tipe_kendaraan');
		$is_ev = $this->m_admin->checkIsEv($tipe_kendaraan);
		$status = 0;
		if($is_ev > 0){
			$status   = 1; 
		}
	...
	public function browse_serial_number_ev()
	{
		$no_spk = $this->input->post('no_spk');
		$id_dealer = $this->m_admin->cari_dealer();
		$is_ev_tipe_kendaraan = $this->db->select('tk.id_tipe_kendaraan')
		->from('tr_spk spk')
		->join('ms_tipe_kendaraan tk', 'tk.id_tipe_kendaraan = spk.id_tipe_kendaraan')
		->where('tk.id_kategori', 'ev')
		->where('spk.no_spk', $no_spk)
		->get();

		if ($is_ev_tipe_kendaraan->num_rows() > 0) {
			$this->db->select('*')
			->from('tr_penerimaan_battery_dealer_detail pbd')
			->join('tr_stock_battery sb', 'sb.serial_number = pbd.serial_number', 'left')
			->where('sb.no_sales_order IS NULL', NULL, FALSE)
			->where('sb.id_dealer', $id_dealer)
			->order_by('sb.fifo', 'ASC');
			$data['dt_serial_number'] = $this->db->get();
			$this->load->view('dealer/t_sales_order_browse_ev_acc',$data);
		}
	
	}

	public function cek_ev_acc()
	{
		$serial_number = $this->input->post('serial_number');
		$sql = $this->db->query("SELECT * from tr_stock_battery bs WHERE bs.serial_number ='$serial_number'")->row();
		if ($sql) {
			$response = array(
				'status' => 'success',
				'data'   => $sql
	...
	public function getDataHistory()
    {
        ...
        $dataSo = $this->db->query("
			...
			FROM
				...
				JOIN tr_scan_barcode ON tr_sales_order.no_mesin = tr_scan_barcode.no_mesin
        ...
        foreach($dataSo->result() as $row)
        {
        	$no_faktur = $this->db->query("SELECT nomor_faktur from tr_fkb where no_mesin_spasi='$row->no_mesin'");
	          ...
	          $tombol1 = "<a href='dealer/sales_order/cetak_so?id=$row->id_sales_order' target='_blank' >
                            ...
                            <a href='dealer/sales_order/cetak_barcode_v2?id=$row->id_sales_order' target='_blank' >
                              <button class='btn btn-flat btn-xs btn-danger'><i class='fa fa-print'></i> Cetak Barcode AHASS V2</button>
                            </a>";
	...
	public function add()
	{
		...
		$data['dt_spk'] = $this->db->query("SELECT DISTINCT(tr_spk.no_spk) FROM tr_spk 
				LEFT JOIN tr_hasil_survey ON tr_spk.no_spk = tr_hasil_survey.no_spk
				WHERE tr_spk.id_dealer = '$id_dealer' 
				-- AND tr_spk.no_spk = '22/06/25/00163-00888'
				AND tr_spk.no_spk IN (SELECT no_spk FROM tr_cdb) 
				AND tr_spk.no_spk NOT IN (SELECT no_spk FROM tr_sales_order WHERE no_spk IS NOT NULL) 
				AND tr_hasil_survey.status_approval = 'approved'
				AND tr_spk.jenis_beli = 'Kredit'
	...
	public function konsumen()
	{
		...
		$data['konsumen']	= $this->input->get('id');
		$data['mode'] = 'detail';
		$id_dealer = $this->m_admin->cari_dealer();
	...
	public function getNosin_gc()
	{
		...
		$sql = $this->db->query("SELECT tr_scan_barcode.*,ms_tipe_kendaraan.tipe_ahm,ms_warna.warna,ms_tipe_kendaraan.id_tipe_kendaraan FROM tr_scan_barcode
		...
		if ($sql->num_rows() > 0) {
			$dt_ve = $sql->row();
			echo "ok" . "|" . $dt_ve->no_mesin . "|" . $dt_ve->no_rangka . "|" . $dt_ve->tipe_ahm . " - " . $dt_ve->warna. "|" . $dt_ve->id_tipe_kendaraan;;
		} else {
			echo "There is no data found!";
		}
	}
	...
	public function addDetails()
	{
		...
		$id_sales_order_gc	 = '111111';
		$data['no_spk_gc']	= $no_spk_gc		=  '1111112';
		$data['no_mesin']	= $no_mesin		    =  '1111113';
		$data['created_by']				= $login_id;
		$data['created_at']				= $waktu;
		$id_tipe_kendaraan = 'ME0';
		$serial_number = '1111';

		if(isset($serial_number)) {
				$data = array(
					'serial_number' 	=> $serial_number,  
					'no_sales_order' 	=> $id_sales_order_gc,    
					'no_mesin'       	=> $no_mesin,    
					'created_at'    	=> $waktu,     
					'created_by'     	=> $login_id,    
					'no_spk'         	=> $no_spk_gc,    
					...
					'id_part'           => $this->m_admin->checkpartEv($serial_number),     
					'jenis_sales_order' => 'gc'
				);
				$this->m_admin->insert('tr_sales_order_acc_ev', $data);
			}
			...
			$data = array(
				...
				'created_at' => $waktu,
				'created_by'=> $login_id,
				'keterangan' =>'dari testing SO GC '.$serial_number
			);
	
			$this->m_admin->insert("tr_testing", $data);
	}
	...
	public function addDetail()
	{
		...
		if ($cek->num_rows() > 0) {
			...
			if ($cek_so < $cek_spk) {
				...
				$serial_number	= $this->input->post('serial_number_gc');
				$id_dealer = $this->m_admin->cari_dealer();
				if(isset($serial_number)) {
	...
	public function getDetail_nosin()
	{
		...
		if ($id == null or $id == '') {
			$data['detail'] = $this->db->query("SELECT tr_sales_order_gc_nosin.*,tr_scan_barcode.tipe_motor,ms_tipe_kendaraan.tipe_ahm,ms_warna.warna,tr_scan_barcode.no_rangka ,ms_tipe_kendaraan.id_tipe_kendaraan
		...
		$data['is_ev'] = $this->m_admin->spk_gc_check_is_ev($id)->result();
	...
	{
		// testing
		$data['konsumen']=$this->input->post('id_sales_order_set');
		...
		$data['ksu'] = $this->db->query("SELECT * FROM ms_koneksi_ksu 
								...
								WHERE ms_koneksi_ksu.id_tipe_kendaraan = '$id_tipe_kendaraan' and ms_koneksi_ksu.active = 1")->result();
	...
	public function take_kec()
	{
		...
		$data = $this->db->query("SELECT kelurahan, id_kecamatan FROM ms_kelurahan WHERE id_kelurahan = '$id_kelurahan'");
		if($data->num_rows()>0){
			$dt_kel			= $data->row();
			$kelurahan 		= $dt_kel->kelurahan;
			$id_kecamatan 	= $dt_kel->id_kecamatan;
			$dt_kec			= $this->db->query("SELECT kecamatan, id_kabupaten FROM ms_kecamatan WHERE id_kecamatan = '$id_kecamatan'")->row();
	...
	public function save()
	{
		...
		$unit_dealer = $this->db->query("
			select b.no_mesin , a.id_dealer 
			from tr_penerimaan_unit_dealer a join tr_penerimaan_unit_dealer_detail b on a.id_penerimaan_unit_dealer = b.id_penerimaan_unit_dealer 
			where a.status ='close' and b.no_mesin ='$no_mesin' and b.retur = 0
		")->row()->id_dealer;

		if($unit_dealer != $id_dealer){
			$_SESSION['pesan'] 	= "Login User Dealer Invalid. Silahkan login ulang kembali!";
			...
			// pengecekan periode program scp
			$is_valid_program = 1;
			if($spk->program_umum !='' ){
				$get_periode_program = $this->db->query("select periode_awal , periode_akhir, kuota_program, unique_customer from tr_sales_program WHERE id_program_md ='$spk->program_umum'");
				$periode_akhir_program = $get_periode_program->num_rows() > 0 ? $get_periode_program->row()->periode_akhir: '';
			
				// pengecekan kuota siapa cepat dia dapat tanpa melihat dealer mana
				$get_kuota = 0;
				$unik_cust = 0;
				if($get_periode_program->num_rows() > 0){
					$get_kuota = $get_periode_program->row()->kuota_program;
					$unik_cust = $get_periode_program->row()->unique_customer;;
				}

				if($get_kuota !='*' && $get_kuota!=0 && $unik_cust == 0){
					$get_penggunaan_kuota = $this->db->query("
						select count(a.no_mesin) as total from tr_sales_order a join tr_spk b on a.no_spk = b.no_spk where b.program_umum = '$spk->program_umum' or b.program_gabungan='$spk->program_umum'
					")->row()->total;

					if($get_penggunaan_kuota>=$get_kuota){
						$is_valid_program = 0;
					}
				}

				// pengecekan tahun motor 
				$is_tahun_produksi = 1;
				$get_tahun_program_umum = $this->db->query("select tahun_produksi from tr_sales_program_tipe WHERE id_program_md ='$spk->program_umum' and id_tipe_kendaraan = '$spk->id_tipe_kendaraan' and id_warna like '%$spk->id_warna%'");
				if ($get_tahun_program_umum->num_rows() > 0) {
					$tahun_produksi_program = $get_tahun_program_umum->row()->tahun_produksi;
					if($tahun_produksi_program == 'Semua Tahun' && $tahun_produksi != ''){
						$is_tahun_produksi = 1;
					}else{
						if($tahun_produksi  == ''){
							$is_tahun_produksi = 3;
							...
							$list_tahun = explode(", ",$tahun_produksi_program);
							if (in_array($tahun_produksi, $list_tahun)){
								$is_tahun_produksi = 1;
							}else{
								$is_tahun_produksi = 3;
								$is_valid_program = 0;
				...
				if($is_tahun_produksi == 3){
					$_SESSION['pesan'] 	= "Tahun Motor tidak sesuai dengan Program yang diikuti, Silahkan Cek Kembali";
					$_SESSION['tipe'] 	= "warning";
					echo "<meta http-equiv='refresh' content='0; url=" . base_url() . "dealer/sales_order/add'>";
				}else if($is_tahun_produksi == 2){
					$_SESSION['pesan'] 	= "Data tidak ditemukan, Silahkan Menghubungi Tim BC MD";
					$_SESSION['tipe'] 	= "warning";
					echo "<meta http-equiv='refresh' content='0; url=" . base_url() . "dealer/sales_order/add'>";
				}else{
					if($periode_akhir_program >= date('Y-m-d') && $periode_akhir_program!='' && $is_valid_program==1){
						// periode sesuai
						if($spk->program_gabungan !=''){
							$get_periode_program = $this->db->query("select periode_awal , periode_akhir, kuota_program, unique_customer from tr_sales_program WHERE id_program_md ='$spk->program_gabungan'");
							$periode_akhir_program   = $get_periode_program->num_rows() > 0 ? $get_periode_program->row()->periode_akhir: '';
							if($periode_akhir_program >= date('Y-m-d') && $periode_akhir_program!=''){
								// periode sesuai
								// pengecekan kuota siapa cepat dia dapat tanpa melihat dealer mana
								$get_kuota = 0;
								$unik_cust = 0;
								...
								if($get_kuota !='*' && $get_kuota!=0 && $unik_cust == 0){
									$get_penggunaan_kuota = $this->db->query("
										select count(a.no_mesin) as total from tr_sales_order a join tr_spk b on a.no_spk = b.no_spk where b.program_umum = '$spk->program_gabungan' or b.program_gabungan='$spk->program_gabungan'
									")->row()->total;
								...
								// pengecekan tahun motor 
								if($is_valid_program ==1){
									$is_tahun_produksi = 1;
									$get_tahun_program_umum = $this->db->query("select tahun_produksi from tr_sales_program_tipe WHERE id_program_md ='$spk->program_gabungan' and id_tipe_kendaraan = '$spk->id_tipe_kendaraan' and id_warna like '%$spk->id_warna%'");
									if ($get_tahun_program_umum->num_rows() > 0) {
										$tahun_produksi_program = $get_tahun_program_umum->row()->tahun_produksi;
										if($tahun_produksi_program == 'Semua Tahun' && $tahun_produksi != ''){
											$is_tahun_produksi = 1;
			...
			if($is_valid_program){ // cek periode scp
				if ($cek_nosin->num_rows() == 0) {
					...
					// $cek_indent = $this->db->get_where('tr_po_dealer_indent', ['id_spk' => $no_spk]);
					$cek_indent = $this->db->query("select id_spk, id_indent, id_tipe_kendaraan, id_warna, status from tr_po_dealer_indent where id_spk ='$no_spk' and status !='canceled'");
					if ($cek_indent->num_rows() > 0) {
						$ind       = $cek_indent->row();
						// cek apakah status nya cancel
						if ($ind->status == 'canceled') {
							# code...
						} else {
							// pembatalan po indent jika terdapat perbedaan warna dan tipe motor
							if($ind->id_tipe_kendaraan != $spk->id_tipe_kendaraan || $ind->id_warna != $spk->id_warna){
								$this->db->update('tr_po_dealer_indent', ['status'=>'canceled','id_reasons' => '12', 'alasan_cancel_indent' => 'Ganti Tipe/Varian', 'updated_at' => date('Y-m-d H:i:s'), 'updated_by' => $login_id], ['id_indent' => $ind->id_indent]);
							}else{
								$this->db->update('tr_po_dealer_indent', ['status' => 'completed','updated_at' => date('Y-m-d H:i:s'), 'updated_by' => $login_id], ['id_indent' => $ind->id_indent]);
							}
				...
				if ($save == 'ok') {
					...
					//Update Doc. SPK
					// $this->m_spk->cetakSPK($no_spk, 1); // 15 Januari 2022: ada kebutuhan apa utk menyimpan file spk di server?


					// unit EV set Serial Number 
					$serial_number = $this->input->post('serial_number');
					if(isset($serial_number)) {
						$no= 0;
						foreach ($serial_number as $key => $serial) {
							if (count($serial[$no])== 0 ){
								$data = array(
									...
									'id_part'           => $this->m_admin->checkpartEv($serial[$no]),     
									'id_dealer'			=>$id_dealer,
									'jenis_sales_order'	=>'regular',
								);
								...
								$this->m_admin->update('tr_stock_battery', $update_stock, 'serial_number', $serial[$no]);
							}
						}
					}
				...
				$_SESSION['pesan'] 	= "Program yang diikuti tidak sesuai/ sudah mencapai limit kuota, Silahkan Update SPK Kembali";
				$_SESSION['tipe'] 	= "danger";
				echo "<meta http-equiv='refresh' content='0; url=" . base_url() . "dealer/sales_order/add'>";
			}
		}
	...
	public function cetak_so()
	{
	  ...
	  */
		...
		$yy = 152;
		if ($dt_so->jenis_beli == 'Cash') {
			$is_ev 	= $this->m_admin->checkIsEv($dt_so->id_tipe_kendaraan);
			...
			if( count($dt_so->subsidi_pemerintah) > 0 ){
				$pdf->Cell(31, 5, 'Subsidi Pemerintah', 0, 0, 'L');
				$pdf->Cell(2, 5, ':', 0, 0, 'L');
				$pdf->Cell(62, 5, 'Rp. ' . $this->mata_uang($dt_so->subsidi_pemerintah), 0, 1, 'L');
			}
	...
	public function cetak_so_gc()
	{
		...
		// if($id == 'GC-00888/2023/12/06/0003') {
			// $html = $this->load->view('dealer/sales_order_cetak_gc_edit', $data, true);   
		// }else{
		...
		if ($get_data->num_rows() > 0) {
			...
			while ($i < 1) {
				...
				if ($cek > 0) {
					...
					$i++;
					// cek gc
					$cek_gc = $this->db->get_where('tr_sales_order_gc', ['no_invoice' => $new_kode])->num_rows();
					if ($cek_gc > 0) {
						$neww     = substr($new_kode, -5);
						$new_kode = $th . '/' . $id_sumber . '/INU/' . sprintf("%'.06d", $neww + 1);
						$i        = 0;
			...
			if ($print == 'ya') {
				...
				if ($cek_so->num_rows() > 0) {
					...
					if (is_null($cek_so->tgl_cetak_invoice)) {
						...
						$cek_prospek = $this->db->query("SELECT leads_id,input_from FROM tr_prospek WHERE id_customer='$cek_so->id_customer' AND IFNULL(leads_id,'')!=''")->row();
							if ($cek_prospek != NULL) {
								if ($cek_prospek->input_from=='sinsengo') {
									$date_path   = date("Y/m/d");
									$dealer      = $this->db->get_where('ms_dealer', ['id_dealer' => $cek_so->id_dealer])->row();
									$path        = "./uploads/invoice_so/$date_path/$dealer->kode_dealer_md/";
									if (!is_dir($path)) {
										mkdir($path, 0777, true);
									}
									$path .= str_replace('/', '-', $id) . ".pdf";
									$data2['path_invoice'] = str_replace('./', '', $path);
								}
								$this->load->model('api_crm_post_model', 'api_crm');
								$this->api_crm->api_4_stageId_12($cek_so->no_spk);
							}
					}
					if (isset($data2)) {
				...
				if ($cek_sp > 0) {
					...
					if ($cek_no_invoice->num_rows() == 0) {
						...
						$so = $this->db->query("SELECT tr_sales_order.*,md.nama_dealer,md.alamat as alamat_dealer,md.no_telp as no_telp_dealer,md.id_kelurahan as kelurahan_dealer, tr_scan_barcode.id_item, tr_spk.*, ms_tipe_kendaraan.tipe_ahm, ms_warna.warna,ms_finance_company.finance_company FROM tr_sales_order 
								...
								left join ms_warna on tr_scan_barcode.warna = ms_warna.id_warna
								left join ms_dealer on tr_sales_order.id_dealer = ms_dealer.id_dealer
								left join ms_dealer md on md.kode_dealer_ahm = ms_dealer.kode_dealer_ahm
						...
						if ($so->jenis_beli == 'Cash') {
							...
							$discount = round($discount_2 / getPPN(1.1,$so->tgl_cetak_invoice));
							...
							$ppn = round($nilai * getPPN(0.1,$so->tgl_cetak_invoice));
	...
	public function cetak_invoice_ev()
	{
			...
			if ($print == 'ya') {
				...
				if ($cek_sp > 0) {
					...
					if ($cek_no_invoice->num_rows() == 0) {
						...
						$so = $this->db->query("SELECT tr_sales_order.*,md.nama_dealer,md.alamat as alamat_dealer,md.no_telp as no_telp_dealer,md.id_kelurahan as kelurahan_dealer, 
						tr_scan_barcode.id_item, tr_spk.*, ms_tipe_kendaraan.tipe_ahm, ms_warna.warna,ms_finance_company.finance_company,
						tr_spk.voucher_1,
						tr_spk.voucher_2,
						tr_spk.voucher_tambahan_1,
						tr_spk.voucher_tambahan_2,
						tr_spk.subsidi_pemerintah,
						tr_shipping_list.no_frame
						FROM tr_sales_order 
								left join tr_scan_barcode on tr_scan_barcode.no_mesin = tr_sales_order.no_mesin
								left join tr_shipping_list on tr_shipping_list.no_mesin = tr_scan_barcode.no_mesin
								left join tr_spk on tr_spk.no_spk = tr_sales_order.no_spk
								left join ms_tipe_kendaraan on tr_scan_barcode.tipe_motor=ms_tipe_kendaraan.id_tipe_kendaraan
								left join ms_warna on tr_scan_barcode.warna = ms_warna.id_warna
								left join ms_dealer on tr_sales_order.id_dealer = ms_dealer.id_dealer
						...
						$pdf->Image('assets/panel/icon/honda.jpg', 10, 5, 20, 20, 'JPG');
						...
						$pdf->SetFont('ARIAL', '', 12);
						$pdf->Cell(200, 10, 'FAKTUR PENJUALAN', 0, 1, 'C');
						...
						if ($so->jenis_beli == 'Cash') {
							...
							$pdf->Cell(5, 5, ' : ', 0, 0, 'L');
							$pdf->Cell(100, 5, $so->nama_konsumen, 0, 0, 'L');

							$pdf->Cell(30, 5, 'NPWP/NPPKP', 0, 0, 'L');
							$pdf->Cell(5, 5, ' : ', 0, 0, 'L');
							$pdf->Cell(100, 5, $so->npwp, 0, 1, 'L');

							$pdf->Cell(20, 5, '', 0, 0, 'L');
							$pdf->Cell(5, 5, '', 0, 0, 'L');
							$pdf->Cell(100, 5, $so->alamat, 0, 0, 'L');

							$pdf->Cell(30, 5, 'TANGGAL', 0, 0, 'L');
							$pdf->Cell(5, 5, ' : ', 0, 0, 'L');
							$tgl_cetak_invoice = date('d-m-Y', strtotime($so->tgl_cetak_invoice2));
							$pdf->Cell(85, 5, $tgl_cetak_invoice, 0, 1, 'L');

							$pdf->Cell(20, 5, '', 0, 0, 'L');
							$pdf->Cell(5, 5, '', 0, 0, 'L');
							$pdf->Cell(100, 5, $kelurahan, 0, 0, 'L');
							
							$pdf->Cell(30, 5, 'NO FAKTUR', 0, 0, 'L');
							$pdf->Cell(5, 5, ' : ', 0, 0, 'L');
							$pdf->Cell(100, 5, $so->no_invoice, 0, 1, 'L');

							$pdf->Cell(20, 5, '', 0, 0, 'L');
							$pdf->Cell(5, 5, '', 0, 0, 'L');
							$pdf->Cell(100, 5, $kecamatan , 0, 0, 'L');
							
							$pdf->Cell(30, 5, 'NO SPK', 0, 0, 'L');
							$pdf->Cell(5, 5, ' : ', 0, 0, 'L');
							$pdf->Cell(100, 5, $so->no_spk, 0, 1, 'L');

							$pdf->Cell(20, 5, '', 0, 0, 'L');
							$pdf->Cell(5, 5, '', 0, 0, 'L');
							$pdf->Cell(100, 5, $kabupaten , 0, 0, 'L');
							
							$pdf->Cell(30, 5, 'NO SO', 0, 0, 'L');
							$pdf->Cell(5, 5, ' : ', 0, 0, 'L');
							$pdf->Cell(100, 5, $so->id_sales_order, 0, 1, 'L');
							
							$pdf->Cell(20, 5, 'NIK', 0, 0, 'L');
							$pdf->Cell(5, 5, ' : ', 0, 0, 'L');
							$pdf->Cell(100, 5, $so->no_ktp , 0, 0, 'L');

							$pdf->Cell(30, 5, 'JENIS PEMBELIAN', 0, 0, 'L');
							$pdf->Cell(5, 5, ' : ', 0, 0, 'L');
							$pdf->Cell(100, 5, $so->jenis_beli, 0, 1, 'L');

							$pdf->setXY(0, 0);
							$pdf->setXY(8, 73);
							$pdf->Line(8, 72, 208, 72);

							$pdf->Cell(6,  4, 'No', 0, 0, 'C');
							$pdf->Cell(20, 4, '', 0, 0, 'C');
							$pdf->Cell(74, 4, 'URAIAN', 0, 0, 'C');
							$pdf->Cell(35, 4, 'UNIT', 0, 0, 'C');
							$pdf->Cell(30, 4, 'HARGA SATUAN', 0, 0, 'C');
							$pdf->Cell(32, 4, 'HARGA JUAL', 0, 1, 'R');
							$pdf->Line(8, 77, 208, 77);
							$pdf->Ln(1);

							$pdf->Cell(10,  5, '1', 0, 0, 'L');
							$pdf->Cell(33, 4, $so->id_item, 0, 0, 'L');
							$pdf->Cell(72, 4, $so->tipe_ahm . ' - ' . $so->warna, 0, 0, 'L');
						
							$return  = $this->m_admin->detail_individu($so->no_spk);
							$harga_off_the_roads = $return['harga_off_road'];
							$harga_on_the_roads  = $return['harga_on_road'];
					
							$ppn_percent_11 = getPPN(1.1,$so->tgl_cetak_invoice);
							$ppn_percent_01 = getPPN(0.1,$so->tgl_cetak_invoice);

							// $sales_program = $this->db->query("SELECT * tr_sales_program");
							
							$voucher_1 			= $so->voucher_1;

							$voucher_2 			= $so->voucher_2;
							$voucher_tambahan_1 = $so->voucher_tambahan_1;
							$voucher_tambahan_2 = $so->voucher_tambahan_2;

							$program_sisapira   = $voucher_1 + $voucher_2 +$voucher_tambahan_1 + $voucher_tambahan_2;
							$sisapira 			= $program_sisapira / $ppn_percent_11;

							// $harga_units 	    = $harga_off_the_roads - $program_sisapira;
							$potongan_hargas    = $sisapira;
							// $dpps			    = $harga_units / $ppn_percent_11;
							// $harga_juals		= $potongan_hargas + $dpps;
							// $ppns               = $dpps * $ppn_percent_01;
							// $bbns 				= $return['bbn'];
							// $total 				= $dpps + $ppns + $bbns;
							// $subsidi_pemerintah = $so->subsidi_pemerintah;
							// $jumlah_yang_harus_dibayar = $total -$subsidi_pemerintah;
							...
							$pdf->Cell(24, 5, '1', 0, 0, 'L');
							$pdf->Cell(32, 4, 'Rp. ' . number_format($harga, 0, ',', '.'), 0, 0, 'L');
							$pdf->Cell(25, 4, 'Rp. ' . number_format($harga, 0, ',', '.'), 0, 1, 'R');

							$pdf->Cell(10, 4, '', 0, 0, 'L');
							$pdf->Cell(33, 4, '', 0, 0, 'L');
							$pdf->Cell(82, 4, 'No. Mesin    :   '. $so->no_mesin, 0, 0, 'L');
							$pdf->Cell(30, 4, '', 0, 0, 'C');
							$pdf->Cell(25, 5, '', 0, 0, 'C');
							$pdf->Cell(25, 5, '', 0, 1, 'R');
							...
							$pdf->Cell(10, 4,'', 0, 0, 'L');
							$pdf->Cell(33, 4,'', 0, 0, 'L');
							// $pdf->Cell(82, 4, 'No. Rangka : ' .$so->no_frame, 0, 0, 'L');
							$pdf->Cell(82, 4, 'No. Rangka :   ' .$so->no_frame. $so->no_rangka, 0, 0, 'L');
							$pdf->Cell(30, 4, '', 0, 0, 'C');
							$pdf->Cell(25, 5, '', 0, 0, 'C');
							$pdf->Cell(25, 5, '', 0, 1, 'R');
							$pdf->Line(8, 94, 208, 94);
							$pdf->Ln(4);
							$pdf->setX(50);
							
							$pdf->Cell(123, 5, 'Jumlah Harga Jual', 0, 0, 'L');
							$pdf->Cell(30, 5, 'Rp. ' . number_format($harga, 0, ',', '.'), 0, 1, 'R');
							$pdf->setX(50);

							$pdf->Cell(123, 5, 'Potongan Harga', 0, 0, 'L');
							$pdf->Cell(30, 5, 'Rp. ' . number_format($sisapira, 0, ',', '.'), "B", 1, 'R');
							$pdf->setX(50);

							$pdf->Cell(123, 5, 'Dasar Pengenaan Pajak (DPP)', 0, 0, 'L');
							$pdf->Cell(30, 5, 'Rp. ' . number_format($dpps, 0, ',', '.'), 0, 1, 'R');
							$pdf->setX(50);

							$pdf->Cell(123, 5, 'Pajak Pertambahan Nilai (PPN)', 0, 0, 'L');
							$pdf->Cell(30, 5, 'Rp. ' . number_format($ppns, 0, ',', '.'), 0, 1, 'R');
							$pdf->setX(50);
							
							$pdf->Cell(123, 5, 'Biaya Balik Nama', 0, 0, 'L');
							$pdf->Cell(30, 5, 'Rp. ' . number_format($bbns, 0, ',', '.'), "B", 1, 'R');
							$pdf->setX(50);

							$pdf->Cell(123, 5, 'Total', 0, 0, 'L');
				
							$pdf->Cell(30, 7, 'Rp. ' . number_format($total, 0, ',', '.'), 0, 1, 'R');
							$pdf->setX(50);

							$pdf->Cell(123, 5, 'Potongan Bantuan Pemerintah untuk KBLBB', 0, 0, 'L');
							$pdf->Cell(30, 5, 'Rp. ' . number_format($subsidi_pemerintah, 0, ',', '.'), 0, 1, 'R');
							$pdf->setX(50);
									
							$pdf->Cell(123, 5, 'Jumlah Yang Harus Dibayar', 0, 0, 'L');
							$pdf->Cell(30, 5, 'Rp. ' . number_format($jumlah_yang_harus_dibayar, 0, ',', '.'), 0, 1, 'R');
							$pdf->setX(50);
					
							$pdf->setY(144);
							$pdf->setX(170);
							$pdf->SetFont('ARIAL', '', 8);
							$pdf->Cell(300, 7, 'Administrasi', 0, 1, 'L');
							$pdf->SetFont('ARIAL', '', 8);
							$pdf->Cell(200, 14, '', 0, 1, 'R');

							$admin_h1 = $this->db->query("SELECT admin_h1 FROM ms_dealer WHERE id_dealer = '$id_dealer '")->row()->admin_h1;
							
							$pdf->Cell(140, 5,'', 0, 0, 'L');
							$pdf->Cell(60, 5, '( ' . $admin_h1 . ' )', 0, 1, 'C');
							// $pdf->Cell(200, 5, $so->nama_dealer, 0, 0, 'R');

							$pdf->setY(157);
							$pdf->SetFont('ARIAL', '', 6);
							$pdf->Cell(10, 3, '', 0, 1, 'L');
							$pdf->Cell(10, 3, '*  Dealer '.$so->nama_dealer.' berada di bawah naungan PT. Sinar Sentosa Primatama selaku', 0, 1, 'L');
							$pdf->Cell(10, 3, '   Main Dealer Sepeda Motor Honda ', 0, 1, 'L');
							$pdf->Cell(10, 3, '** Transaksi ini sepenuhnya menjadi tanggung jawab dealer yang bersangkutan', 0, 1, 'L');
							...
							$pdf->Cell(30, 5, 'LEASING', 0, 0, 'L');
							$pdf->Cell(5, 5, ' : ', 0, 0, 'L');
							$pdf->Cell(100, 5, $so->finance_company, 0, 1, 'L');

							$pdf->setXY(8, 73);
							$pdf->Line(6, 72, 208, 72);
							$pdf->Cell(6,  4, 'No.', 0, 0, 'C');
							$pdf->Cell(20, 4, '', 0, 0, 'C');
							$pdf->Cell(74, 4, 'URAIAN', 0, 0, 'C');
							$pdf->Cell(35, 4, 'UNIT', 0, 0, 'C');
							$pdf->Cell(30, 4, 'HARGA SATUAN', 0, 0, 'C');
							$pdf->Cell(29, 4, 'HARGA JUAL', 0, 1, 'R');
							$pdf->Line(8, 77, 208,77);
							$pdf->Ln(1);
							...
							$program_sisapira   = $so->voucher_1 + $so->voucher_2 + $so->voucher_tambahan_1 + $so->voucher_tambahan_2;
							$sisapira 			= $program_sisapira / $ppn_percent_11;
							$potongan_hargas        = $sisapira;
							...
							// $bbns 				= $return['bbn'];
							// $pdf->Cell(24, 5, '1', 0, 0, 'L');
							// $pdf->Cell(32, 4, 'Rp. ' . number_format($harga_units, 0, ',', '.'), 0, 0, 'L');
							// $pdf->Cell(30, 4, 'Rp. ' . number_format($harga_juals, 0, ',', '.'), 0, 1, 'L');
							...
							$pdf->Cell(24, 5, '1', 0, 0, 'L');
							$pdf->Cell(32, 4, 'Rp. ' . number_format($harga, 0, ',', '.'), 0, 0, 'C');
							$pdf->Cell(25, 4, 'Rp. ' . number_format($harga, 0, ',', '.'), 0, 1, 'R');
							...
							$pdf->Cell(25, 4, '', 0, 1, 'R');
							$pdf->Ln(2);
							$pdf->Line(8, 92, 208, 92);
							$pdf->setX(50);
							
							$pdf->Cell(123, 5, 'Jumlah Harga Jual', 0, 0, 'L');
							$pdf->Cell(30, 5, 'Rp. ' .number_format( $harga, 0, ',', '.'), 0, 1, 'R');
							// $pdf->Cell(30, 5, 'Rp. ' .number_format( $harga_juals, 0, ',', '.'), 0, 1, 'R');
							$pdf->setX(50);
						
							$pdf->Cell(123, 5, 'Potongan Harga', 0, 0, 'L');
							$pdf->Cell(30, 5, 'Rp. ' . number_format($potongan_hargas, 0, ',', '.'), "B", 1, 'R');
							$pdf->setX(50);
							...
							$pdf->setX(50);
							$pdf->Cell(123,5, 'Total', 0, 0, 'L');
							$pdf->Cell(30, 5, 'Rp. ' . number_format($total, 0, ',', '.'), 0, 1, 'R');
							$pdf->setX(50);
	...
	// public function cetak_invoice_gc_ev()
	// {
	// 	$tgl      = gmdate("y-m-d", time() + 60 * 60 * 7);
	// 	$waktu    = gmdate("Y-m-d H:i:s", time() + 60 * 60 * 7);
	// 	$jam      = gmdate("H:i:s", time() + 60 * 60 * 7);
	...
	// 	$data['no_invoice'] = $this->cek_no_invoice();
	// 	$data['waktu']    = gmdate("Y-m-d H:i:s", time() + 60 * 60 * 7);
	// 	$html = $this->load->view('dealer/cetak_invoice_so_gc_ev', $data, true);
	// 	$mpdf->WriteHTML($html);
	// 	$output = 'cetak_.pdf';
	// 	$mpdf->Output("$output", 'I');
	// }
	...
	public function cetak_cover_gc()
	{
		...
		$id = $this->input->get('id');
		$dt_so = $this->db->query("SELECT tr_sales_order_gc.*,tr_spk_gc.nama_bpkb as nama_bpkb1,
	...
	public function cetak_barcode_v2()
	{
		$tgl 				= gmdate("y-m-d", time() + 60 * 60 * 7);
		$waktu 			= gmdate("Y-m-d H:i:s", time() + 60 * 60 * 7);
		$login_id		= $this->session->userdata('id_user');
		...
		if ($so->num_rows() > 0) {
			...
			for ($i = 0; $i < 7; $i++) {
				$kotak=$pdf->Code128($x_b, $y_b, $so->no_mesin, 41, 9);
				...
				$deskripsi_ahm = strip_tags($so->deskripsi_ahm);
				$pdf->SetFont('Arial','B',8);
				// $pdf->cell(50, 27,"1", 1, 0);
				// $pdf->cell(49, 12, $kotak, "TRL", 2,"C");
				$pdf->cell(49, 12,"", "TRL", 2);
				$pdf->cell(49,3,"$so->no_mesin / $so->norangka", "LR", 2);
				$pdf->SetFont('Arial','B',8);
				$pdf->cell(49,3,"Tipe : $so->id_item ( $deskripsi_ahm ) \n", "LR", 2);
				$pdf->SetFont('Arial','',7);
				$pdf->multicell(49, 3, "Nama : $so->nama_bpkb  \nTgl. Pembelian : $tgl  ","LRB", 2);
	...
	public function cetak_barcode_gc_v2()
	{
		$tgl 			= gmdate("y-m-d", time() + 60 * 60 * 7);
		$waktu 			= gmdate("Y-m-d H:i:s", time() + 60 * 60 * 7);
		$login_id		= $this->session->userdata('id_user');
		...
		// head	  
		$pdf->SetFont('ARIAL', '', 7.5);
		// $so = $this->db->query("SELECT tr_sales_order_gc.*, tr_sales_order_gc_nosin.*,ms_dealer.nama_dealer,ms_dealer.alamat AS alamat_dealer,
		...
		$so = $this->db->query("
			select tr_scan_barcode.no_mesin, tr_scan_barcode.no_rangka as norangka, tr_sales_order_gc.tgl_cetak_invoice, tr_scan_barcode.id_item, 
			ms_tipe_kendaraan.deskripsi_ahm, tr_sales_order_gc_nosin.nama_stnk, tr_spk_gc.nama_bpkb
			FROM tr_sales_order_gc_nosin 
			LEFT JOIN tr_sales_order_gc ON tr_sales_order_gc_nosin.id_sales_order_gc = tr_sales_order_gc.id_sales_order_gc
			LEFT JOIN tr_scan_barcode ON tr_scan_barcode.no_mesin = tr_sales_order_gc_nosin.no_mesin
			LEFT JOIN tr_spk_gc ON tr_spk_gc.no_spk_gc = tr_sales_order_gc_nosin.no_spk_gc
		...
		$page = 0;
		if ($so->num_rows() > 0) {
			foreach ($so->result() as $so) { // jika qty unit > 1
				$per_page = 4; // ada 4 unit barcode utk 1 halaman
				if($page % $per_page == 0){
					$pdf->AddPage();
				}
				// $so = $so->row();
				$additional_kordinat = ($page % $per_page) *65;

				$x = 4;
				$y = 4 + $additional_kordinat;
				$batas = 3;
				$count_batas = 1;
				$x_b = 8;
				$y_b = 6 + $additional_kordinat;
				...
				for ($i = 0; $i < 7; $i++) {
					...
					if (isset($so->nama_bpkb)){
						$cek_nama_bpkb_barcode = $so->nama_bpkb;
					}else{
						$cek_nama_bpkb_barcode = $so->nama_stnk;
					}
					...
					$pdf->cell(49,3,"Tipe : $so->id_item ( $deskripsi_ahm ) \n", "LR", 2);
					$pdf->SetFont('Arial','',7);
					$pdf->multicell(49, 3, "Nama : $cek_nama_bpkb_barcode  \nTgl. Pembelian : $tgl  ","LRB", 2);
	...
	public function bastk()
	{
		...
		if ($cek_no_bastk->num_rows() == 0) {
			...
			$spk = $this->db->get("tr_spk spk")->row();
			if ((string)$spk->leads_id != '') {
				...
				$array_post = [
					...
					'CreditStatus' => '',
					'IndentStatus' => '',
					'DeliveryStatus' => 'Menunggu surat jalan',
					'EstimatedDeliveryDate' => $data['tgl_pengiriman'],
					'EngineNumber' => $cek_so->no_mesin,
					'StnkStatus' => '',
					'BpkbStatus' => '',
					'VehicleNumber' => '',
				];
			...
			$check_ev = $this->db->query("SELECT * from tr_sales_order_acc_ev so left join tr_shipping_list sl on sl.no_mesin = so.no_mesin  WHERE so.no_sales_order = '$id_sales_order' limit 1 ");
			if ($check_ev->num_rows() > 0) {
				$so_ev 		= $cek_no_bastk->row();
				$so_battery = $check_ev->row();
				$this->load->model('ev_model');
				$no_rangka = $so_battery->no_frame;
				$new_no_rangka = $no_rangka.$so_ev->no_rangka;

				$set_acc = array(
					'acc' => '5',
					'serial_number' => $so_battery->serial_number,
					'user' => $login_id,
					'bastNo'=>$so_ev->no_bastk,
					'bastDate'=>$so_ev->tgl_bastk,
					'frameNo'=>$new_no_rangka,
					'engineNo'=>$so_ev->no_mesin,
					'phoneNo'=>$so_ev->no_hp,
					'custName'=>$so_ev->nama_konsumen,
				);
				$this->ev_model->InsertAcc($set_acc);
				
				$battery= array(
					'acc_status'	 => '5',
					'no_rangka'   	 => $new_no_rangka,        
					'no_mesin'  	 => $so_ev->no_mesin,          
					'no_sales_order' => $filter,    
					'no_bast'        => $so_ev->no_bastk,   
					'tanggal_bast'   => $so_ev->tgl_bastk,    
					'nama_konsumen'  => $so_ev->nama_konsumen, 
					'no_hp'          => $so_ev->no_hp,     
					);

				$this->m_admin->update("tr_stock_battery",$battery,"serial_number",$so_battery->serial_number);
			}
	...
	public function save_bastk_gc()
	{
		...
		if ($this->db->trans_status() === FALSE) {
			...
			// ev proses insert 
			$no_spk = $post['id_sales_order_gc'];

			$this->db->select('acc.no_mesin, gcnosin.no_mesin, acc.no_spk, acc.serial_number, so.no_bastk, so.tgl_bastk as tgl_bastk, spkgc.nama_bpkb as nama_konsumen, spkgc.no_hp');
			$this->db->from('tr_sales_order_acc_ev acc');
			$this->db->join('tr_sales_order_gc_nosin gcnosin', 'gcnosin.no_mesin = acc.no_mesin', 'inner');
			$this->db->join('tr_sales_order_gc so', 'gcnosin.no_spk_gc = so.no_spk_gc', 'left');
			$this->db->join('tr_spk_gc spkgc', 'spkgc.no_spk_gc = so.no_spk_gc', 'inner');
			$this->db->where('acc.no_spk', $no_spk);
			$this->db->group_by('gcnosin.no_mesin');
			$cek_no_bastk = $this->db->get();
	
			if ($cek_no_bastk->num_rows() > 0) {
				$data=$cek_no_bastk->result();
				foreach($data as $so_ev){

					$no_bastk  = $so_ev->no_bastk;
					$no_rangka = $so_ev->no_rangka;
					$tgl_bastk = $so_ev->tgl_bastk;
					$no_hp     = $so_ev->no_hp;
					$nama_konsumen = $so_ev->nama_konsumen;
					$no_mesin       = $so_ev->no_mesin;
					$serial_number       = $so_ev->serial_number;
					...
					'no_sales_order' =>$filter,    
					'no_bast'        =>$tgl_bastk,   
					'tanggal_bast'   =>$tgl_bastk,    
					'nama_konsumen'  =>$nama_konsumen, 
					'no_hp'          =>$no_hp,     
					);
					$this->m_admin->update("tr_stock_battery",$battery,"serial_number",$serial_number);
				}
			}
        ...
        foreach($dataSo->result() as $row)
        {
        	...
        	$no_faktur = $this->db->query("SELECT nomor_faktur from tr_fkb where no_mesin_spasi='$row->no_mesin'");
	         ...
	         $cetak_so = "<a href='dealer/sales_order/cetak_so?id=$row->id_sales_order' target='_blank' >
						...
						<a href='dealer/sales_order/cetak_barcode_v2?id=$row->id_sales_order' target='_blank' >
                          <button $print class='btn btn-flat btn-xs btn-danger'><i class='fa fa-print'></i> Cetak Barcode AHASS V2</button>
                        </a>";
