	...
	function save_wo()
	{
		...
		foreach ($details as $keys => $val) {
			if ($val['masukkan_wo'] == 'true') {
				...
				if($val['id_type'] == 'LC' || $val['id_type'] == 'LT1' || $val['id_type'] == 'LT2' ||$val['id_type'] == 'LG'){
					if($val['id_type'] == 'LC'){
						if(isset($post['kesediaan_customer_lcr_id'])){
							if($post['kesediaan_customer_lcr_id'] != null || $post['kesediaan_customer_lcr_id'] != '' ){
								//Cek id_customer
								$id_customer = $this->db->select('sa.id_customer')
									->select('cust.no_mesin')
									->select('cust.no_rangka')
									->select('cust.id_tipe_kendaraan')
									->select('cust.id_warna')
									->select('cust.tahun_produksi')
									->from('tr_h2_sa_form sa')
									->join('ms_customer_h23 cust','cust.id_customer=sa.id_customer')
									->where('sa.id_sa_form',$id_sa_form)
									->get()
									->row_array();

								$ins_lcr_check_history = [
									'id_customer' => $id_customer['id_customer'],
									'no_mesin' => $id_customer['no_mesin'],
									'no_rangka' => $id_customer['no_rangka'],
									'id_tipe_kendaraan' => $id_customer['id_tipe_kendaraan'],
									'id_warna' => $id_customer['id_warna'],
									'tahun_produksi' => $id_customer['tahun_produksi'],
									'status_lcr' => 'LC',
									'kesediaan_customer_lcr_id' => $post['kesediaan_customer_lcr_id'],
									'record_reason_lcr_id'   => isset($post['record_reason_lcr_id']) && $post['record_reason_lcr_id'] != '' ? $post['record_reason_lcr_id'] : null,
									'hasil_pengecekan_lcr_id'   => isset($post['hasil_pengecekan_lcr_id']) && $post['hasil_pengecekan_lcr_id'] != '' ? $post['hasil_pengecekan_lcr_id'] : null,
									'id_work_order'   => $id_work_order,
									'tgl_servis' => $tgl,
									'status' => 'open',
									'id_jasa' => $val['id_jasa']
								];
								$this->db->insert('tr_h2_wo_lcr_history', $ins_lcr_check_history);
							}
						}
					}elseif($val['id_type']=='LT1'){
						if(isset($post['kesediaan_customer_lcr_treatment'])){
							//Cek apakah sudah ada di DB LCR History 
							if($post['kesediaan_customer_lcr_treatment'] != null || $post['kesediaan_customer_lcr_treatment'] != '' ){
								//Cek id_customer
								$id_customer = $this->db->select('sa.id_customer')
										->select('cust.no_mesin')
										->select('cust.no_rangka')
								...
								$ins_lcr_treatment_history = [
									...
									'tahun_produksi' => $id_customer['tahun_produksi'],
									'status_lcr' => 'LT1',
									'kesediaan_customer_lcr_id' => $post['kesediaan_customer_lcr_treatment'],
									'record_reason_lcr_id'   => isset($post['record_reason_lcr_id_treatment']) && $post['record_reason_lcr_id_treatment'] != '' ? $post['record_reason_lcr_id_treatment'] : null,
									'id_work_order'   => $id_work_order,
									'tgl_servis' => $tgl,
									'status' => 'open',
									'id_jasa' => $val['id_jasa']
								];
								$this->db->insert('tr_h2_wo_lcr_history', $ins_lcr_treatment_history);	
							}	
						}
					}elseif($val['id_type']=='LT2'){
						if(isset($post['kesediaan_customer_lcr_treatment'])){
							//Cek apakah sudah ada di DB LCR History 
							if($post['kesediaan_customer_lcr_treatment'] != null || $post['kesediaan_customer_lcr_treatment'] != '' ){
							//Cek id_customer
					...
					}elseif($val['id_type']=='LG'){
						if(isset($post['kesediaan_customer_lcr_ganti'])){
							if($post['kesediaan_customer_lcr_ganti'] != null || $post['kesediaan_customer_lcr_ganti'] != '' ){
								//Cek id_customer
								$id_customer = $this->db->select('sa.id_customer')
									->select('cust.no_mesin')
									->select('cust.no_rangka')
								...
								$ins_lcr_ganti_history = [
									...
									'tahun_produksi' => $id_customer['tahun_produksi'],
									'status_lcr' => 'LG',
									'kesediaan_customer_lcr_id' => $post['kesediaan_customer_lcr_ganti'],
									'record_reason_lcr_id'   => isset($post['record_reason_lcr_id_ganti']) && $post['record_reason_lcr_id_ganti'] != '' ? $post['record_reason_lcr_id_ganti'] : null,
									'id_work_order'   => $id_work_order,
									'tgl_servis' => $tgl,
									'status' => 'open',
									'id_jasa' => $val['id_jasa']
								];
								$this->db->insert('tr_h2_wo_lcr_history', $ins_lcr_ganti_history);
							}
						}
					}
				}
		...
		WHERE sa.id_sa_form='$id_sa_form' AND IFNULL(customer_apps_booking_number,'')!=''
		")->row();
if ($book != null) {
	$response_ce_apps_message = $this->updateServiceCeApps(true, $book, $id_work_order, isset($ins_details) ? $ins_details : [], isset($ins_parts) ? $ins_parts : []);
}
		...
		if ($this->db->trans_status() === FALSE) {
			...
			$pesan = "Sukses membuat work order.";
	...
	function save_update_wo()
	{
		...
		foreach ($details as $keys => $val) {
			if (isset($val['update'])) {
				...
				if($val['id_type'] == 'LC'){
					if(isset($post['kesediaan_customer_lcr_id'])){
						$lcr_check_history = $this->db->select('id_work_order')
						...
						if(!empty($lcr_check_history)){
							$ins_lcr_check_history = array(
								'kesediaan_customer_lcr_id' => $post['kesediaan_customer_lcr_id'],
								'record_reason_lcr_id' => isset($post['record_reason_lcr_id']) && $post['record_reason_lcr_id'] != '' ? $post['record_reason_lcr_id'] : null,
								'hasil_pengecekan_lcr_id' => isset($post['hasil_pengecekan_lcr_id']) && $post['hasil_pengecekan_lcr_id'] != '' ? $post['hasil_pengecekan_lcr_id'] : null,
								'id_jasa' => $val['jasa']
							...
							$this->db->update('tr_h2_wo_lcr_history', $ins_lcr_check_history);
						}else{
							if($post['kesediaan_customer_lcr_id'] != null || $post['kesediaan_customer_lcr_id'] != '' ){
								//Cek id_customer
								$id_customer = $this->db->select('sa.id_customer')
									...
									->from('tr_h2_sa_form sa')
									->join('ms_customer_h23 cust','cust.id_customer=sa.id_customer')
									->where('sa.id_sa_form',$wo->id_sa_form)
									->get()
									->row_array();
					...
					if(isset($post['kesediaan_customer_lcr_treatment'])){
						//Cek apakah sudah ada di DB LCR History 
						$lcr_treatment_history = $this->db->select('id_work_order')
													->from('tr_h2_wo_lcr_history')
													->where('status_lcr','LT1')
													->where('id_work_order',$id_work_order)
													->get()
													->row_array();
	
						if(!empty($lcr_treatment_history)){
							$ins_lcr_treatment_history = array(
								'kesediaan_customer_lcr_id' => $post['kesediaan_customer_lcr_treatment'],
								'record_reason_lcr_id' => isset($post['record_reason_lcr_id_treatment']) && $post['record_reason_lcr_id_treatment'] != '' ? $post['record_reason_lcr_id_treatment'] : null,
								'id_jasa' => $val['jasa']
							);
							...
							$this->db->where('id_work_order', $id_work_order);
							$this->db->where('status_lcr', 'LT1');
							$this->db->update('tr_h2_wo_lcr_history', $ins_lcr_treatment_history);
						}else{
							if($post['kesediaan_customer_lcr_treatment'] != null || $post['kesediaan_customer_lcr_treatment'] != '' ){
								//Cek id_customer
								$id_customer = $this->db->select('sa.id_customer')
					...
					if(isset($post['kesediaan_customer_lcr_ganti'])){
						$lcr_ganti_history = $this->db->select('id_work_order')
							->from('tr_h2_wo_lcr_history')
							->where('status_lcr','LG')
							->where('id_work_order',$id_work_order)
							->get()
						...
						if(!empty($lcr_ganti_history)){
							$ins_lcr_ganti_history = array(
								'kesediaan_customer_lcr_id' => $post['kesediaan_customer_lcr_ganti'],
								'record_reason_lcr_id' => isset($post['record_reason_lcr_id_ganti']) && $post['record_reason_lcr_id_ganti'] != '' ? $post['record_reason_lcr_id_ganti'] : null,
								'id_jasa' => $val['jasa']
							);
							...
							$this->db->where('id_work_order', $id_work_order);
							$this->db->where('status_lcr', 'LG');
							$this->db->update('tr_h2_wo_lcr_history', $ins_lcr_ganti_history);
						}else{
							if($post['kesediaan_customer_lcr_ganti'] != null || $post['kesediaan_customer_lcr_ganti'] != '' ){
								//Cek id_customer
								$id_customer = $this->db->select('sa.id_customer')
				...
				if($val['id_type'] == 'LC'){
					...
					if(!empty($lcr_check_history)){
						...
						$this->db->where('id_work_order', $id_work_order);
						$this->db->where('status_lcr', 'LC');
						$this->db->where('id_jasa', $val['id_jasa']);
						$this->db->update('tr_h2_wo_lcr_history', $ins_lcr_check_history);
					}		
				}elseif($val['id_type']=='LT1'){
					//Cek apakah sudah ada di DB LCR History 
		...
		$book = $this->db->query("SELECT book.id_booking,book.customer_apps_booking_number
				...
				")->row();
		if ($book != null) {
			$response_ce_apps_message = $this->updateServiceCeApps(false, $book, $id_work_order, isset($ins_details) ? $ins_details : [], isset($ins_parts) ? $ins_parts : []);
		}
		...
		if(isset($post['kesediaan_customer_lcr_id']) || isset($post['kesediaan_customer_lcr_treatment']) || isset($post['kesediaan_customer_lcr_ganti'])){
				if(isset($post['kesediaan_customer_lcr_id'])){
						//Cek apakah sudah ada di DB LCR History 
						$lcr_check_history = $this->db->select('id_work_order')
									->select('id_jasa')
						...
						$check_jasa_lcr_check = $this->db->select('wop.id_jasa')
														 ->from('tr_h2_wo_dealer wo')	
														 ->join('tr_h2_wo_dealer_pekerjaan wop','wo.id_work_order=wop.id_work_order')	
														 ->join('ms_h2_jasa jasa','jasa.id_jasa = wop.id_jasa')
														 ->where_in('wo.status',array('open', 'pause', 'pending', 'closed'))
														 ->where('wop.pekerjaan_batal',0)
														 ->where('jasa.id_type','LC')
														 ->where('wo.id_work_order',$id_work_order)
														 ->get()
														 ->row_array();	

						$jasa_lcr = null;								 
						if(!empty($check_jasa_lcr_check)){
							$jasa_lcr = $check_jasa_lcr_check['id_jasa'];
						}else{
							$jasa_lcr = null;
						}

						if(!empty($lcr_check_history)){			
							$ins_lcr_history = array(
								'kesediaan_customer_lcr_id' => $post['kesediaan_customer_lcr_id'],
								'record_reason_lcr_id' => isset($post['record_reason_lcr_id']) && $post['record_reason_lcr_id'] != '' ? $post['record_reason_lcr_id'] : null,
								'hasil_pengecekan_lcr_id' => isset($post['hasil_pengecekan_lcr_id']) && $post['hasil_pengecekan_lcr_id'] != '' ? $post['hasil_pengecekan_lcr_id'] : null,
								'id_jasa' => $jasa_lcr
							...
							$this->db->where('id_work_order', $id_work_order);
							$this->db->where('status_lcr', 'LC');
							$this->db->update('tr_h2_wo_lcr_history', $ins_lcr_history);
						}else{
							if($post['kesediaan_customer_lcr_id'] != null || $post['kesediaan_customer_lcr_id'] != '' ){
								//Cek id_customer
								$id_customer = $this->db->select('sa.id_customer')
				...
				if(isset($post['kesediaan_customer_lcr_treatment'])){
					...
					$check_jasa_lcr_treatment = $this->db->select('wop.id_jasa')
												->from('tr_h2_wo_dealer wo')	
												->join('tr_h2_wo_dealer_pekerjaan wop','wo.id_work_order=wop.id_work_order')	
												->join('ms_h2_jasa jasa','jasa.id_jasa = wop.id_jasa')
												->where_in('wo.status',array('open', 'pause', 'pending', 'closed'))
												->where('wop.pekerjaan_batal',0)
												->where('jasa.id_type','LT1')
												->where('wo.id_work_order',$id_work_order)
												->get()
												->row_array();	
					$jasa_lcr = null;
					if(!empty($check_jasa_lcr_treatment)){
						$jasa_lcr = $check_jasa_lcr_treatment['id_jasa'];
					}else{
						$jasa_lcr = null;
					}
				...
				if(isset($post['kesediaan_customer_lcr_ganti'])){
					...
					$check_jasa_lcr_ganti = $this->db->select('wop.id_jasa')
						->from('tr_h2_wo_dealer wo')	
						->join('tr_h2_wo_dealer_pekerjaan wop','wo.id_work_order=wop.id_work_order')	
						->join('ms_h2_jasa jasa','jasa.id_jasa = wop.id_jasa')
						->where_in('wo.status',array('open', 'pause', 'pending', 'closed'))
						->where('wop.pekerjaan_batal',0)
						->where('jasa.id_type','LG')
						->where('wo.id_work_order',$id_work_order)
						->get()
						->row_array();	
						
					$jasa_lcr = null;
					if(!empty($check_jasa_lcr_ganti)){
						$jasa_lcr = $check_jasa_lcr_ganti['id_jasa'];
					}else{
						$jasa_lcr = null;
					}
...
<?php
	...
	function reason_cancel()
	{
		...
		if(!empty($lcr_history)){
			$update_lcr_history = array(
				'status' => 'cancel_wo'
			);

			$this->db->where('id_work_order', $id_work_order);
			$this->db->update('tr_h2_wo_lcr_history', $update_lcr_history);
		}
