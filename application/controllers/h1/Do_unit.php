...
class Do_unit extends CI_Controller {
	...
	{				
		...
		// $data['dt_do'] = $this->db->query("SELECT * FROM tr_do_po INNER JOIN ms_dealer ON tr_do_po.id_dealer = ms_dealer.id_dealer ORDER BY tr_do_po.id DESC limit 100");	
		...
		// $data['dt_po'] = $this->db->query("SELECT * FROM tr_po_dealer INNER JOIN ms_dealer ON tr_po_dealer.id_dealer = ms_dealer.id_dealer WHERE tr_po_dealer.status = 'input' order by tr_po_dealer.created_at desc limit 100");	
		$this->template($data);	
		$_SESSION['no_do']  = "";
	...
	{				
		...
		$data['dt_item'] = $this->db->query("
			SELECT ms_item.*,ms_tipe_kendaraan.tipe_ahm,ms_warna.warna 
			FROM ms_item INNER JOIN ms_tipe_kendaraan
			ON ms_item.id_tipe_kendaraan=ms_tipe_kendaraan.id_tipe_kendaraan INNER JOIN ms_warna
			ON ms_item.id_warna=ms_warna.id_warna
			WHERE ms_item.active = 1
		");
		$data['dt_po'] = $this->db->query("SELECT id_dealer FROM tr_po_dealer INNER JOIN ms_dealer ON tr_po_dealer.id_dealer = ms_dealer.id_dealer WHERE tr_po_dealer.status = 'input' limit 1");	
		$name = $this->session->userdata('nama');
		if($name=="")
		{
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."panel'>";
	...
	public function getData()
    {
        ...
        foreach($do->result() as $row)
        {
			...
			$tampil = '';
			if ($_SESSION['id_karyawan_dealer'] =='A1006418'){
...
///---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	public function set_unit_nrfs(){
		$id_item = $this->input->post('id_item');
		$no_do = $this->input->post('id_do');
		$query = $this->db->query("Update tr_do_po_detail set qty_do = 0 WHERE no_do = '$no_do' and id_item = '$id_item'");
		echo 'Qty DO Berhasil diupdate!';
	}		
}