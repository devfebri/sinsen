...
class Invoice_dealer_unit extends CI_Controller {
	...
	public function set_diskon_tambahan()
	{				
		$data['isi']    = $this->isi;		
		$data['page']   = $this->page;		
		$id 						= $this->input->get('id');			
		$data['title']	= "Diskon Tambahan ".$this->title;															
		$data['set']		= "diskon_tambahan";			
		$no_invoice 					= $this->input->get('id');							

		$data['dt_invoice']	= $this->db->query("SELECT * FROM tr_invoice_dealer INNER JOIN tr_do_po ON tr_invoice_dealer.no_do = tr_do_po.no_do 
				INNER JOIN ms_dealer ON tr_do_po.id_dealer = ms_dealer.id_dealer
				INNER JOIN ms_gudang ON tr_do_po.id_gudang = ms_gudang.id_gudang
		 		WHERE tr_invoice_dealer.id_invoice_dealer = '$no_invoice'");
		$this->template($data);			
	}	

	public function save_set_diskon(){	
			$waktu              = gmdate("y-m-d H:i:s", time()+60*60*7);
			$login_id           = $this->session->userdata('id_user');
			$tabel              = $this->tables;
			$pk                 = $this->pk;			
    ...
    $pdf->AddPage();
	  ...
	  {
	  	$pdf->Cell(10, 5, $i, 0, 0);
	    $pdf->Cell(25, 5, $r->id_item, 0, 0);
	    $pdf->Cell(80, 5, strip_tags($r->deskripsi_ahm)." / ".$r->warna, 0, 0);
      ...
      if($cek2->num_rows() > 0){
        ...
        $po = 0;
	  	...
	  	$potongan = $potongan + (($r->disc + $po) * $r->qty_do);
	  	//$p = $p + $potongan;
	  }
	...
	public function cari_data(){
		...
		echo $row->no_faktur."|".$row->nama_dealer."|".$row->no_do;
	}

	public function cetak_penjualan_unit()
	{
