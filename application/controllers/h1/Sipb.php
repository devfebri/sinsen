...
class Sipb extends CI_Controller {
	...
	public function show_dt(){
		
		$list = $this->H1_sipb_rec->get_datatables();
        $data = array();
        $no = $this->input->post('start');
        foreach ($list as $field) {
			
			$bulan = substr($field->tgl_sipb, 2,2);
			$tahun = substr($field->tgl_sipb, 4,4);
			$tgl = substr($field->tgl_sipb, 0,2);
			$tanggal = $tgl."-".$bulan."-".$tahun;
  
			$bulan1 = substr($field->tgl_spes, 2,2);
			$tahun1 = substr($field->tgl_spes, 4,4);
			$tgl1 = substr($field->tgl_spes, 0,2);
			$tanggal2 = $tgl1."-".$bulan1."-".$tahun1;

			// if(!is_null($field->warna)){
			// 	$warna = $field->warna;
			// }else{
			// 	$warna = $field->id_warna;
			// }
	
			// if(!is_null($field->tipe_ahm)){
			// 	$tipe = $field->tipe_ahm;
			// }else{
			// 	$tipe = $field->id_tipe_kendaraan;
			// }
        ...
        $output = array(
            ...
            "recordsTotal" => $this->H1_sipb_rec->count_all(),
            "recordsFiltered" => $this->H1_sipb_rec->count_filtered(),
            "data" => $data,
        );
