...
class Cdb extends CI_Controller {
	...
	public function update_p(){
		...
		$data = $this->db->query("
			select * from ms_h3_dealer_stock where id_part_int is null and id_gudang_int is null and id_rak_int is NULL
			-- and id_part ='02510000220'
			group by id_part
			order by id_part asc
		");

		if($data->num_rows()>0){
			$i=0;
			foreach($data->result() as $row){
				$prt = $this->db->get_where('ms_part', ['id_part' => $row->id_part]);
				if($prt->num_rows()>0){
					$i++;
					// echo $prt->row()->id_part.'-'.$prt->row()->id_part_int.'<br>';
					$id_part_int = $prt->row()->id_part_int;
					// $this->db->query("update ms_h3_dealer_stock set id_part_int = $id_part_int where id_part = '$row->id_part'");					
					echo $i.' '.$prt->row()->id_part.' -> '.$prt->row()->id_part_int.': OK<br>';
				}else{
				}
			}
		}
	...
	function get_data_generate($start_date,$end_date,$no_mesin)
	{
		...
		$so_in = $this->db->query("SELECT tr_sales_order.no_mesin,tr_sales_order.id_sales_order,tr_scan_barcode.no_rangka,tr_spk.nama_konsumen,tr_spk.no_ktp,tr_spk.alamat,tr_spk.no_spk,tr_sales_order.id_dealer,tgl_cetak_invoice,id_kelurahan2, tr_spk.tempat_lahir, tr_prospek.alamat_kantor, tr_prospek.no_telp_kantor, tr_prospek.id_kelurahan_kantor, tr_prospek.nama_tempat_usaha, tr_prospek.sub_pekerjaan, tr_prospek.pekerjaan_lain, ms_sub_pekerjaan.id_pekerjaan, ms_sub_pekerjaan.required_instansi, tr_spk.pekerjaan
			...
			WHERE tgl_cetak_invoice BETWEEN '$start_date' AND '$end_date' $filter
		...
		$so_gc = $this->db->query("SELECT tr_sales_order_gc_nosin.id_sales_order_gc,tr_sales_order_gc_nosin.no_mesin,tr_scan_barcode.no_rangka,nama_npwp AS nama_konsumen,tr_spk_gc.no_ktp,tr_spk_gc.alamat,tr_spk_gc.no_spk_gc,tr_sales_order_gc.id_dealer,tgl_cetak_invoice,tr_spk_gc.id_kelurahan,tr_spk_gc.kodepos,jenis_beli,tr_spk_gc.no_npwp,tr_spk_gc.id_prospek_gc,tgl_berdiri,tr_spk_gc.no_hp,tr_spk_gc.no_telp,tr_spk_gc.email,tr_spk_gc.nama_penanggung_jawab,id_kelurahan2,nama_npwp,status_nohp 
				...
				WHERE tgl_cetak_invoice BETWEEN '$start_date' AND '$end_date' $filter_gc
	...
	public function unduh(){		
		...
		if($cdb!='' && $ustk !='' && $kk !=''){
			...
			if($cdb == 1 && $ustk == 2 && $kk =='' ){
				$index =1; $n = 2;
			}else if($cdb =='' && $ustk == 2 && $kk ==3){
				$index =2; $n = 3;
			}else if($cdb ==1 && $ustk == '' && $kk ==3){
				$index = 1; $n = 3;
			}else if($cdb ==1){
				$index = 1; $n = 1;
			}else if($ustk ==2){
		...
		for ($i=$index; $i <= $n; $i++) { 
			echo "<iframe style='display:none;' src='".base_url()."h1/cdb/unduh".$i."?start_date=".$start_date."&end_date=".$end_date."&m=".$m."&no_mesin=".$no_mesin."'></iframe>";
		}
	...
	public function unduh1(){		
		...
		$data_generate = $this->get_data_generate($start_date,$end_date,$no_mesin);
		
		/* off kan update generate ulang
