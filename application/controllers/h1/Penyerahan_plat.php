...
class Penyerahan_plat extends CI_Controller {
	...
	{		
		...
		$this->load->model('m_penyerahan_plat_datatables');	
		//===== Load Library =====
	...
	{				
		...
		/*$data['dt_plat']	= $this->db->query("SELECT * FROM tr_penyerahan_plat INNER JOIN ms_dealer ON tr_penyerahan_plat.id_dealer = ms_dealer.id_dealer 
				where tr_penyerahan_plat.created_at > '2022-01-01' ORDER BY tr_penyerahan_plat.no_serah_plat DESC");*/
		$this->template($data);			
	}	
	public function fetch_data_penyerahan_plat_datatables()
	{
		$list = $this->m_penyerahan_plat_datatables->get_datatables();
        ...
        foreach($list as $row) {       
			...
			$print = $this->m_admin->set_tombol($id_menu,$group,'print');   
			  ...
			  if (!empty($row->no_serah_plat)) {
				$tombol_id_serah_plat ="<a $print href='h1/penyerahan_plat/cetak?id=$row->no_serah_plat' class='btn btn-primary btn-flat btn-xs' target='_blank'>Cetak Tanda Terima</a> ";            
				$link_id_serah_plat =" <a href='h1/penyerahan_plat/detail?id=$row->no_serah_plat'>
				$row->no_serah_plat
			  </a>";
			}else{
				$tombol_id_serah_plat = "<span class='label label-danger'>Tidak Ditemukan</span>";
			  }

			$no++;
			$rows = array();
			$rows[] = $no;
			$rows[] = $link_id_serah_plat;
			$rows[] = $row->tgl_serah_terima;
			$rows[] = $row->nama_dealer;
			$rows[] = $row->alamat;
			$rows[] = $tombol_id_serah_plat;
			$data[] = $rows;
		}

		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->m_penyerahan_plat_datatables->count_all(),
			"recordsFiltered" => $this->m_penyerahan_plat_datatables->count_filtered(),
			"data" => $data,
		);
...
// 	}
	...
	$data['dt_plat'] = $this->db->query("SELECT tr_entry_stnk.*,tr_pengajuan_bbn_detail.id_tipe_kendaraan,a.status_bj,a.serah_plat,
			...
			WHERE tr_pengajuan_bbn.id_dealer = '$id_dealer' AND a.status_bj = 'input'  AND a.serah_plat IS NULL and tgl_terima_plat is not null ");		 		
	$this->load->view('h1/t_penyerahan_plat',$data);
}
	...
	public function cari_id(){		
		...
		$pr_num = $this->db->query("SELECT * FROM tr_penyerahan_plat where left(created_at,4) ='$th' ORDER BY created_at DESC LIMIT 0,1");							
		if($pr_num->num_rows()>0){
			$row 	= $pr_num->row();				
			$pan  = strlen($row->no_serah_plat)-9;
			$id 	= substr($row->no_serah_plat,$pan,6)+1;	
			if($id < 10){
	...
	{				
		...
		$da['tgl_serah_terima'] =  $this->input->post("tgl_serah_terima");	
	...
	public function cetak()
	{
		...
		$detail = $this->db->query("SELECT tr_penyerahan_plat_detail.no_serah_plat, tr_pengajuan_bbn_detail.no_mesin , tr_pengajuan_bbn_detail.nama_konsumen , tr_entry_stnk.no_plat
 		FROM tr_penyerahan_plat_detail INNER JOIN tr_pengajuan_bbn_detail ON tr_penyerahan_plat_detail.no_mesin = tr_pengajuan_bbn_detail.no_mesin 
		join tr_entry_stnk on tr_entry_stnk.no_mesin = tr_penyerahan_plat_detail.no_mesin 
        	WHERE tr_penyerahan_plat_detail.no_serah_plat='$id' ORDER BY tr_entry_stnk.no_plat ASC");		
		$jum = $detail->num_rows();
