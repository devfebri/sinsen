...
class Bantuan_bbn extends CI_Controller {
	...
	{		
		...
		$data['id_kelurahan'] = $id_kelurahan 			= $this->input->post('id_kelurahan');
		...
		if($id_kelurahan == ''){
			...
			$_SESSION['pesan'] 		= "Data Kelurahan tidak boleh kosong";
		...
		if ($dt_bantuan->num_rows()>0) {
	  	...
	  	if($utc->num_rows() > 0) $utc_hasil = $utc->row()->cc_motor;
			...
			$pdf = new FPDF('p','mm',array(209.6,296));
			  ...
			  // $pdf->Image(base_url().'/assets/panel/images/STNK_BARU.jpg', 0, 0, 209.6);
	  		 // $pdf->Image(base_url().'/assets/panel/images/STNK_LAMA.jpg', 0, 0, 209.6);
			  ...
			  */
			  ...
			  //Lama
			  $pdf->SetXY(46,73); //+2
			  $pdf->Cell(103, 5, 'Honda', 0, 0, 'L');
			  $pdf->Cell(80, 5, $r->warna_samsat, 0, 1, 'L');
			  $pdf->SetX(46);
			  $pdf->Cell(103, 5, $r->deskripsi_samsat, 0, 0, 'L');
			  $pdf->Cell(80, 5, 'Bensin', 0, 1, 'L');
			  $pdf->SetX(46);
			  $pdf->Cell(103, 5, 'Sepeda Motor', 0, 0, 'L');
			  $pdf->Cell(80, 5, '', 0, 1, 'L');
			  $pdf->SetX(46);
			  $pdf->Cell(103, 5, 'Solo', 0, 0, 'L');
			  $pdf->Cell(80, 5, '', 0, 1, 'L');
			  $pdf->SetX(46);
			  $pdf->Cell(103, 5, $r->tahun_produksi, 0, 0, 'L');
			  $pdf->Cell(80, 5, '', 0, 1, 'L');
			  $pdf->SetX(46);
			  $pdf->Cell(103, 5, $utc_hasil, 0, 1, 'L');
			  $pdf->SetX(46);
			  $pdf->Cell(103, 5, 'MH1'.$r->no_rangka, 0, 1, 'L');
			  $pdf->SetX(46);
			  $pdf->Cell(103, 5, $r->no_mesin, 0, 1, 'L');
			  $pdf->SetXY(46,162);
