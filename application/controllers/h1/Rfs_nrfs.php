...
class Rfs_nrfs extends CI_Controller {
	...
	public function cek_lokasi($rfs_text){
			//$rfs_text = $this->input->get('no');
			...
			$cek1 = $this->db->query("SELECT * FROM ms_lokasi_unit INNER JOIN ms_gudang ON ms_lokasi_unit.id_gudang = ms_gudang.id_gudang 
							...
							AND ms_lokasi_unit.isi < ms_lokasi_unit.qty AND ms_gudang.gudang = '$gudang' ORDER BY ms_lokasi_unit.id_lokasi_unit ASC LIMIT 0,1");
			...
			$cek2 = $this->db->query("SELECT DISTINCT(tr_scan_barcode.lokasi),ms_lokasi_unit.id_lokasi_unit,ms_lokasi_unit.isi FROM tr_scan_barcode INNER JOIN ms_lokasi_unit ON
							...
							AND tr_scan_barcode.tipe_motor = '$row->tipe_motor' AND tr_scan_barcode.warna = '$row->warna' AND ms_gudang.gudang = '$gudang' 
			...
			$cek3 = $this->db->query("SELECT * FROM ms_lokasi_unit INNER JOIN ms_gudang ON ms_lokasi_unit.id_gudang = ms_gudang.id_gudang
						  ...
						  AND ms_gudang.gudang = '$gudang' ORDER BY ms_lokasi_unit.id_lokasi_unit ASC LIMIT 0,1");
				...
				if($isi_scan < $cek_maks->qty){					
					for($i=1; $i <= $cek_maks->qty; $i++) { 							
						...
						if($cek_slot2->num_rows() == 0){
							$cek_rfs = $this->db->query("SELECT * FROM tr_scan_ubah_detail WHERE lokasi_tujuan = '$isi_lokasi' AND slot_tujuan = '$sl' AND status = 'baru'");
		...
		return $isi_lokasi."|".$slot_baru."|".$row->lokasi."|".$row->slot."|".$jum."|".$cek1->num_rows()."|".$cek2->num_rows()."|".$cek3->num_rows()."|".$cek4->num_rows()."|".$cek5->num_rows();
	}
	public function save_rfs(){
		...
		if($cek->tipe == 'RFS'){
			...
			$rt = $this->cek_lokasi($rfs_text);
			$ambil = explode("|", $rt);
			$data['no_mesin']							= $rfs_text;
			$data['id_scan_ubah']					= $id_ubah;		
			$data['lokasi_tujuan']				= $ambil[0];			
			$data['slot_tujuan']					= $ambil[1];			
			$data['lokasi_awal']					= $ambil[2];
			$data['slot_awal']						= $ambil[3];
			$data['status']								= "baru";
			$data['jum']									= $ambil[4];
			...
			if($c->num_rows() > 0){			
				...
				if($ambil[5] > 0 OR $ambil[6] > 0 OR $ambil[7] > 0 OR $ambil[8] > 0 OR $ambil[9] > 0){
	...
	{		
		...
		if($cek == 0){			
			$id_scan_ubah = $data['id_scan_ubah'] 		= $this->input->post('id_scan_ubah');
			...
			$cek = $this->m_admin->getByID("tr_scan_ubah_detail","id_scan_ubah",$id_scan_ubah);
			if($cek->num_rows() > 0){
				...
				$_SESSION['pesan'] 	= "Tidak ada no mesin yang ditambahkan";
	...
	{		
		...
		$tabel			= $this->tables;
		$pk					= $this->pk;
		$id  = $id_scan_ubah	= $this->input->post($pk);
		$cek 				= $this->m_admin->getByID($tabel,$pk,$id)->num_rows();
		...
		if($save == 'update'){	
			...
			foreach ($cek->result() as $isi) {				
				...
				$id_ubah		= $id_scan_ubah;					
				$rt = $this->cek_lokasi($no_mesin);
				$ambil = explode("|", $rt);
				$data2['no_mesin']						= $no_mesin;
				$data2['id_scan_ubah']				= $id_ubah;		
				$data2['lokasi_tujuan']				= $ambil[0];			
				$data2['slot_tujuan']					= $ambil[1];			
				$data2['lokasi_awal']					= $ambil[2];
				$data2['slot_awal']						= $ambil[3];
				$data2['status']							= "baru";
				$data2['jum']									= $ambil[4];
																					
				$cek2 = $this->m_admin->update("tr_scan_ubah_detail",$data2,"no_mesin",$no_mesin);				

				$get = $this->m_admin->getByID("tr_scan_barcode","no_mesin",$no_mesin)->row();				
				$da['lokasi'] = $lokasi_tujuan					= $ambil[0];
				$da['slot'] = $slot_tujuan							= $ambil[1];

				$this->m_admin->update_stock($get->tipe_motor,$get->warna,"NRFS",'+','1');
				$this->m_admin->update_stock($get->tipe_motor,$get->warna,"RFS",'-','1');
				$this->m_admin->update("tr_scan_barcode",$da,"no_mesin",$no_mesin);
				$this->m_admin->set_log($no_mesin,"NRFS",$lokasi_tujuan."-".$slot_tujuan);

				$this->db->query("UPDATE ms_lokasi_unit SET isi = '$isi->jum' WHERE id_lokasi_unit = '$lokasi_tujuan'");				
				$this->db->query("UPDATE tr_scan_barcode SET tipe = 'NRFS' WHERE no_mesin = '$no_mesin'");										
				$this->db->query("UPDATE tr_scan_ubah_detail SET status = 'close' WHERE no_mesin = '$no_mesin'");										
			}
		}elseif($save == 'reject'){
