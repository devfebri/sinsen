...
class Monitor_orderin extends CI_Controller {
	...
	public function	download(){				
		...
		if ($selectedOption==='order_in'){
			...
			$select ="b.no_order_survey ,
			a.no_spk , n.kode_dealer_md, n.nama_dealer, a.created_at as tgl_spk , 
			...
			k.id_flp_md , l.jabatan ,  a.status_spk as status, m.status_approval as status_credit,
			...
			a.id_tipe_kendaraan, h.tipe_ahm , g.warna ,
		...
		$where .="and (m.status_approval IN ('rejected','approved') or a.jenis_beli ='cash' or a.status_spk in ('canceled','booking','approved','rejected','close')) ";

		}else if($selectedOption==='all_spk'){
			$select ="
			a.no_spk,a.created_at as tgl_spk ,a.no_ktp , a.nama_konsumen ,a.id_tipe_kendaraan,a.id_warna,a.harga_on_road ,  a.angsuran, a.alamat, a.tenor,a.id_tipe_kendaraan, a.id_warna, a.status_spk as status ,
			a.harga_on_road , a.jenis_beli , (case when i.finance_company is null then '-' else i.finance_company end) as finance_company , a.uang_muka , ifnull((a.voucher_1 + a.voucher_2),0) as voucer , a.voucher_tambahan_1 ,
			a.jenis_beli,(a.uang_muka - ifnull((a.voucher_1 + a.voucher_2),0)) as dp_stor,
			c.kelurahan,
			d.kecamatan,
			e.kabupaten,
			f.pekerjaan,
			...
			n.nama_dealer
			";
			$join .= "left join ms_finance_company i on i.id_finance_company = a.id_finance_company";
		}
		...
		$sql = $this->db->query("SELECT 
			...
			$join
			WHERE DATE(a.created_at) >= '$tgl1' AND DATE(a.created_at) <= '$tgl2'
			-- where date(a.created_at) = '$tgl1' 
			$where
			...
			if ($selectedOption==='order_in'){
				...
				$data['sql'] = $sql;	
				if($_POST['process']=='excel'){
					$this->load->view("h1/report/template/temp_monitor_orderin_all_spk",$data);
				}else if($_POST['process']=='csv'){
