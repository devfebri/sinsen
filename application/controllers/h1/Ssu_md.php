...
class Ssu_md extends CI_Controller {
    var $tables =   "tr_ssu_konsumen";	
		var $folder =   "h1";
		var $page		=		"ssu_md";
	...
	{				
		...
		$data['set']	= "view";
		$data['dt_ssu_konsumen'] = $this->db->query("SELECT * FROM tr_ssu_konsumen INNER JOIN ms_agama ON tr_ssu_konsumen.id_agama=ms_agama.id_agama");		
	...
	public function t_kendaraan(){
		...
		$data['dt_kendaraan'] = $this->db->query("SELECT * FROM tr_ssu_kendaraan WHERE id_list_appointment = '$id'");		
		$this->load->view('h1/t_ssu_kendaraan',$data);
	}
	public function t_keluarga(){
		$id = $this->input->post('id_list_appointment');
		$data['dt_keluarga'] = $this->db->query("SELECT * FROM tr_ssu_keluarga WHERE id_list_appointment = '$id'");		
		...
		$this->load->view('h1/t_ssu_keluarga',$data);
	}
	...
	public function cari_id(){
		...
		$pr_num 				= $this->db->query("SELECT * FROM tr_ssu_konsumen ORDER BY id_list_appointment DESC LIMIT 0,1");						
	...
	public function save_kendaraan(){
		...
		$c = $this->db->query("SELECT * FROM tr_ssu_kendaraan WHERE id_list_appointment = '$id_list_appointment' AND no_mesin = '$no_mesin'");
		if($c->num_rows() > 0){
			echo "no";
		}else{
			$cek2 = $this->m_admin->insert("tr_ssu_kendaraan",$data);						
			echo "ok";
		}							
	}	
	public function delete_kendaraan(){
		$id = $this->input->post('id_ssu_kendaraan');		
		$this->db->query("DELETE FROM tr_ssu_kendaraan WHERE id_ssu_kendaraan = '$id'");			
		echo "nihil";
	}
	public function cek_kecamatan(){
		...
		echo $id_kecamatan."|".$kecamatan."|".$id_kabupaten."|".$kabupaten."|".$id_provinsi."|".$provinsi;
	}
	public function save_keluarga(){
		...
		$cek = $this->db->get_where("tr_ssu_keluarga",array("id_list_appointment"=>$id_list_appointment,"nik"=>$nik));
		if($cek->num_rows() > 0){
			$sq = $cek->row();
			$id = $sq->id_ssu_keluarga;
			$this->m_admin->update("tr_ssu_keluarga",$data,"id_ssu_keluarga",$id);			
		}else{
			$this->m_admin->insert("tr_ssu_keluarga",$data);			
		}
		echo "nihil";
	}
	public function delete_keluarga(){
		$id = $this->input->post('id_ssu_keluarga');		
		$this->db->query("DELETE FROM tr_ssu_keluarga WHERE id_ssu_keluarga = '$id'");			
		echo "nihil";
	}
	...
	{		
		...
		if($cek == 0){
			...
			$data['status_ssu']				 				= $this->input->post('status_ssu');
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."h1/ssu_md/add'>";
		}else{
	...
	public function edit(){
		...
		$data['set']		= "edit";
		$data['dt_ssu'] = $this->m_admin->getByID("tr_ssu_konsumen","id_list_appointment",$id);
		$data['dt_item'] = $this->db->query("SELECT * FROM tr_scan_barcode WHERE status = '1' ORDER BY id_scan_barcode ASC");						
		$data['dt_jenis'] = $this->m_admin->getSortCond("ms_jenis_pembelian","jenis_pembelian","ASC");							
		$data['dt_pekerjaan'] = $this->m_admin->getSortCond("ms_pekerjaan","pekerjaan","ASC");							
		$data['dt_pendidikan'] = $this->m_admin->getSortCond("ms_pendidikan","pendidikan","ASC");							
	...
	{		
		...
		if($cek == 0 or $id == $id_){		
			...
			$_SESSION['tipe'] 	= "success";
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."h1/ssu_md'>";
		}else{
			$_SESSION['pesan'] 	= "Duplicate entry for primary key";
			$_SESSION['tipe'] 	= "danger";
			echo "<script>history.go(-1)</script>";
