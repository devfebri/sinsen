...
class Voucher_pengeluaran_banks extends CI_Controller
{
	var $tables = "tr_voucher_bank";
	var $folder = "h1";
	var $page   = "voucher_pengeluaran_banks";
	var $isi    = "bank_kas";
	...
	public function __construct()
	{
		...
		$this->load->model('m_vocer_pengeluaran_bank');
	...
	{
		$list = $this->m_vocer_pengeluaran_bank->get_datatables();
		$data = array();
		$no = $_POST['start'];
		...
		foreach ($list as $row) {
			...
			$dt = $this->db->query("SELECT vbd.*, coa.tipe_transaksi
			FROM tr_voucher_bank_detail vbd left join ms_coa coa on coa.kode_coa = vbd.kode_coa  
			WHERE vbd.id_voucher_bank = '$row->id_voucher_bank' group by vbd.id_voucher_bank_detail  ORDER BY id_voucher_bank_detail asc");
			...
			foreach ($dt->result() as $key) {
				...
				if(in_array($key->kode_coa, $coa_pph)){
					$totNominal-= $key->nominal;
				}else if(in_array($key->kode_coa, $coa_potongan)){
					$totNominal-= $key->nominal;
				}else if(in_array($key->kode_coa, $coa_ppn)){
					$totNominal+= $key->nominal;
				}else{
					$totNominal+= $key->nominal;
				}
					
				// if($key->tipe_transaksi == 'Kredit'){    
				// 	$totNominal-= $key->nominal;
					
				// }else if ($key->tipe_transaksi == 'Debit'){
				// 	$totNominal+= $key->nominal;
				// }else
				// {
				// 	$totNominal+= $key->nominal;
			...
			$changeValue = $totNominal;
			$totNominalsetJumlah = ltrim($changeValue,'-');
			// total = debit - kredit - diskon + ppn - pph
			$totNominal = $totNominalsetJumlah ;
			...
			<a href='h1/voucher_pengeluaran_bank/batal?id=$row->id_voucher_bank' class='btn btn-flat btn-xs btn-danger'>Batal</a>
			";

			$rows = array();
			$rows[] = $no;
			$rows[] = $row->id_voucher_bank;
			$rows[] = $row->account;
			$rows[] = $row->tgl_entry;
			$rows[] = $customer;
			$rows[] = $row->jenis_bayar;
			$rows[] = $row->pph;
			$rows[] = number_format($totNominal);
			$rows[] = $status;
			$rows[] = $tmbl;
			$data[] = $rows;
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->m_vocer_pengeluaran_bank->count_all(),
			"recordsFiltered" => $this->m_vocer_pengeluaran_bank->count_filtered(),
			"data" => $data,
		);
	...
	public function history()
	{
		...
		$data['page']   = $this->page;
		$data['set']		= "history";
		$data['dt_voucher']	= $this->m_admin->getByID("tr_voucher_bank","status","batal");
		$this->template($data);
	}
	public function add()
	{
	...
	public function cetak()
	{
		...
		// $sql2 = $this->db->query("SELECT vbd.*, coa.tipe_transaksi, case when pph.pasal_pph is not null then 1  else 0 end as pph
		// FROM tr_voucher_bank_detail vbd left join ms_coa coa on coa.kode_coa = vbd.kode_coa  
		// left join ms_pph pph on pph.kode_coa = vbd.kode_coa 
		// WHERE vbd.id_voucher_bank = '$id' group by vbd.id_voucher_bank_detail  ORDER BY id_voucher_bank_detail asc");

		$sql2 = $this->db->query("SELECT vbd.*, coa.tipe_transaksi  
		FROM tr_voucher_bank_detail vbd 
		left join ms_coa coa on coa.kode_coa = vbd.kode_coa  
		WHERE vbd.id_voucher_bank = '$id' group by vbd.id_voucher_bank_detail 
		ORDER BY id_voucher_bank_detail asc");
		...
		foreach ($sql2->result() as $isi) {		
		  ...
		  $coa_potongan = array(
			...
			// if($isi->tipe_transaksi == 'Kredit'){    
				
			if(in_array($isi->kode_coa, $coa_pph)){
				$totNominal-= $isi->nominal;
			}else if(in_array($isi->kode_coa, $coa_potongan)){
				$totNominal-= $isi->nominal;
			}else if(in_array($isi->kode_coa, $coa_ppn)){
				$totNominal+= $isi->nominal;
			}else{
				$totNominal+= $isi->nominal;
			}

			// }else if ($isi->tipe_transaksi == 'Debit'){
			// 	$totNominal+= $isi->nominal;
			// }else
			// {
			// 	$totNominal+= $isi->nominal;
	...
	if ($row->tipe_customer == 'Dealer') {
      ...
      $cek_bank = $this->db->query("SELECT * FROM ms_norek_dealer 
      	...
      	WHERE ms_norek_dealer.id_dealer = '$id_dealer' 
		and ms_norek_dealer_detail.id_norek_dealer_detail = '$row->rekening_tujuan'
		ORDER BY ms_norek_dealer_detail.id_norek_dealer_detail DESC LIMIT 0,1");
    ...
    $sambung = ($customer2!='') ? " - " : "" ;
		$pdf->SetWidths(array(140,45));
		$pdf->SetAligns(array('L','R'));				
    $pdf->Row(array($customer2.$sambung.str_replace('&nbsp;','',$row->deskripsi),"Rp. ".$this->mata_uang2($totNominal)));    								    
		
		$pdf->SetFont('times', '', 10);	
		$pdf->SetWidths(array(115,10,30));		

		$sql2 = $this->db->query("	SELECT vbd.*, coa.tipe_transaksi , coa.kode_coa FROM tr_voucher_bank_detail vbd left join ms_coa coa on coa.kode_coa = vbd.kode_coa  WHERE vbd.id_voucher_bank = '$id'  group by vbd.id_voucher_bank_detail  ORDER BY id_voucher_bank_detail asc ");
		foreach ($sql2->result() as $isi) {	

			if($isi->tipe_transaksi == 'Kredit'){

				if($isi->kode_coa =='2.01.21065.00' || $isi->kode_coa == '2.01.21064.00' || $isi->kode_coa =='6.01.6013.01' || $isi->kode_coa =='2.01.21063.00' || $isi->kode_coa =='2.01.21062.00'){
    	...
    	$sambung2 = ($customer2!='') ? "  " : "" ; // hilangkan buka kurung
    	$sambung3 = ($customer2!='') ? "  " : "" ; // hilangkan tutup kurung
	...
	public function cari_ref()
	{
		...
		$total = $this->sisaRef($referensi,$tipe);
	...
	public function t_bg()
	{
		...
		$this->load->view('h1/t_voucher_bgs', $data);
	}
	...
	public function save()
	{
		...
		$sql = $this->db->query("SELECT id_voucher_bank  from  tr_voucher_bank  order by id_voucher_bank DESC limit 1")->row()->id_voucher_bank;

		$query = $this->db->select('id_voucher_bank')
                  ->from('tr_voucher_bank')
                  ->order_by('id_voucher_bank', 'DESC')
                  ->limit(1)
                  ->get();

		$latest_voucher = $query->row()->id_voucher_bank;
		$get_id_voucher = $latest_voucher + 1;
		$cek = $this->m_admin->getByID($tabel, $pk, $get_id_voucher)->num_rows();

		$data['account']         			= $this->input->post('account');
		$data['id_voucher_bank'] 			= $get_id_voucher;
		...
		$kode_coa_temp 		= $_POST['temp']['kode_coa'];
		$sisa_hutang_temp 	= $_POST['temp']['sisa_hutang'];
		$nominal_temp 		= $_POST['temp']['nominal'];
		$keterangan_temp 	= $_POST['temp']['keterangan'];

		$initial = 0;
		$reset = count($kode_coa_temp) - 1;

		for ($i = 0; $i <= $reset ; $i++) {
			$check_voucher = $kode_coa_temp[$initial];

			if ($check_voucher !== ''  ){
				$voucher_bank_detail[] = array(
					'id_voucher_bank' => $get_id_voucher,
					'kode_coa' =>$kode_coa_temp[$initial],
					'coa' => NULL,
					'referensi' => NULL,
					'nominal' =>$nominal_temp[$initial],
					'sisa_hutang' => $sisa_hutang_temp[$initial],
					'keterangan' => $keterangan_temp[$initial]
				);
				$initial ++;
			}
		}
		...
		if ($via_bayar == 'BG'){
			...
			$query = $this->db->select('id_voucher_bank_bg')
                  ->from('tr_voucher_bank_bg')
                  ->order_by('id_voucher_bank_bg', 'DESC')
                  ->limit(1)
                  ->get();

			$latest_voucher = $query->row()->id_voucher_bank_bg;
			$voucher_bg = $latest_voucher + 1;

			$no_bg				= $_POST['bg']['no_bg'];
			$tgl_jatuh_tempo	= $_POST['bg']['tgl_jatuh_tempo'];
			$nominal		    = $_POST['bg']['nominal'];
			$initial_bg = 0;

			$reset = count($tgl_jatuh_tempo) - 1;
			for ($i = 0; $i <= $reset ; $i++) {
					if ($tgl_jatuh_tempo[$initial] !== NULL){
					$voucher_bank_bg[] = array(
						'id_voucher_bank_bg' =>$voucher_bg,
						'id_voucher_bank'    =>$get_id_voucher,
						'no_bg' 			 =>$no_bg[$initial_bg],
						'tgl_bg' 			 =>$tgl_jatuh_tempo[$initial_bg],
						'nominal_bg' 		 =>$nominal[$initial_bg],
					);
					$initial_bg ++;
					$voucher_bg ++;
				}
		...
		$query = $this->db->select('id_voucher_bank_transfer')
                  ->from('tr_voucher_bank_transfer')
                  ->order_by('id_voucher_bank_transfer', 'DESC')
                  ->limit(1)
                  ->get();

				  
		$latest_voucher = $query->row()->id_voucher_bank_transfer;
		$voucher_transfer = $latest_voucher + 1;

		$tgl_transfer             =  $_POST['voucher']['tgl_transfer'];
		$nominal_transfer		  =  $_POST['voucher']['nominal_transfer'];
		$initial_transfer = 0;

		$reset =  count($tgl_transfer) - 1 ;

		for ($i = 0; $i <= $reset; $i++) {
			if ($tgl_transfer[$initial_transfer] !== ''  ){
				$voucher_bank_transfer[] = array(
					'id_voucher_bank_transfer' => $voucher_transfer,
					'id_voucher_bank'    => $get_id_voucher,
					'tgl_transfer'       => $tgl_transfer[$initial_transfer],
					'nominal_transfer'   => $nominal_transfer[$initial_transfer],
				);
				$initial_transfer ++;
				$voucher_transfer ++;
			}
	...
	$cek = $this->m_admin->getByID($tabel, $pk, $get_id_voucher)->num_rows();
		...
		if ($cek == 0 ){
			$this->db->insert('tr_voucher_bank', $data);
			$this->db->insert_batch('tr_voucher_bank_detail', $voucher_bank_detail);
			if ($via_bayar == 'BG'){
				$this->db->insert_batch('tr_voucher_bank_bg', $voucher_bank_bg);
			}else{
				$this->db->insert_batch('tr_voucher_bank_transfer', $voucher_bank_transfer);
			}
	...
	public function sisaRef($referensi,$kategori = null)
	{
		$total = 0;
		// perlu tambahkan kolom status utk update status pelunasan pembayaran
		// cekvoucher bank dibuat jd per kategori utk melakukan pengecekan pembayaran apakah sudah lunas atau blm

		if($kategori=='retur'){
			$cek12 = $this->db->query("select a.no_mesin,b.id_dealer from tr_retur_dealer_detail a join tr_retur_dealer b on a.no_retur_dealer  = b.no_retur_dealer where a.no_retur_dealer = '$referensi'");
			if ($cek12->num_rows() > 0) {
				$np = "";
				$total_semua = 0;
				$harga = 0;
				...
				foreach ($cek12->result() as $isi) {
					$cari_no_do = $this->db->query("SELECT tr_picking_list_view.id_item, tr_picking_list.no_do FROM tr_picking_list_view JOIN tr_picking_list ON tr_picking_list_view.no_picking_list = tr_picking_list.no_picking_list join tr_do_po on tr_do_po.no_do = tr_picking_list.no_do
						WHERE tr_picking_list_view.no_mesin = '$isi->no_mesin' and tr_do_po.id_dealer = '$isi->id_dealer' ORDER BY tr_picking_list.no_do ASC LIMIT 0,1");
					...
					$get_tgl_faktur = $this->db->query("SELECT tgl_faktur FROM tr_invoice_dealer WHERE no_do = '$no_do'");
					$tgl_faktur = ($get_tgl_faktur->num_rows() > 0) ? $get_tgl_faktur->row()->tgl_faktur :false;
					$harga += $harga_asli;
					$ppn += $harga_asli * getPPN(0.1,$tgl_faktur);
		...
		if($kategori=='AHM'){
			...
			if ($cek3->num_rows() > 0) {
				...
				$total = $ppn + $pph + $amount;
				$pembayaran  = $this->m_admin->cekOnlyVoucherPengeluaranBank($referensi, $total);

				if($pembayaran == $total){
					$this->m_admin->update('tr_invoice',array('status_pelunasan' => 1),'no_faktur',$referensi);
					return 0;
				}else{
		...
		if($kategori=='adm_bbn'){
			...
			if ($cek4->num_rows() > 0) {
				...
				if($pembayaran == $total){
					$this->m_admin->update('tr_adm_bbn',array('status_pelunasan' => 1),'id_adm_bbn',$referensi);
					return 0;
				}else{
					return $total - $pembayaran;
				}
		...
		if($kategori=='bbn_bpkb'){
			...
			if ($cek5->num_rows() > 0) {
				...
				if($pembayaran == $total){
					$this->m_admin->update('tr_adm_bpkb',array('status_pelunasan' => 1),'id_adm_bpkb',$referensi);
					return 0;
				}else{
					return $total - $pembayaran;
				}
		...
		if($kategori=='bbn_stnk'){
			...
			if ($cek6->num_rows() > 0) {
				...
				if($pembayaran == $total){
					$this->m_admin->update('tr_adm_stnk',array('status_pelunasan' => 1),'id_adm_stnk',$referensi);
					return 0;
				}else{
					return $total - $pembayaran;
				}
		...
		if($kategori=='adm_jual'){
			...
			if ($cek11->num_rows() > 0) {
				...
				if($pembayaran == $total){
					$this->m_admin->update('tr_adm_jual',array('status_pelunasan' => 1),'id_adm_jual',$referensi);
					return 0;
				}else{
					return $total - $pembayaran;
				}
		...
		if($kategori=='rekap_penerimaan'){
			...
			*/
			...
			if ($cek10->num_rows() > 0) {
				...
				if($pembayaran == $total){
					$this->m_admin->update('tr_rekap_tagihan',array('status_pelunasan' => 1),'id_rekap_tagihan',$referensi);
					return 0;
				}else{
					return $total - $pembayaran;
				}			
		...
		// return $total = $this->m_admin->cekOnlyVoucherPengeluaranBank($referensi, $total);
	...
	public function getRef()
	{
		...
		if ($tipe_customer == 'Dealer') {
			$item9 = $this->db->query("SELECT * FROM tr_claim_sales_program
							...
							where tr_claim_sales_program.id_dealer= $id_dealer
			...
			foreach ($item9->result() as $isi9) {
				...
				if ($cek->num_rows() > 0) {
					$cek = $cek->row();
					if ($cek->sum == 0) {
						if ($this->sisaRef($isi9->id_claim_sp,'SCP') > 0) {
							echo "<option category='SCP' value='$isi9->id_claim_sp'>$isi9->id_program_md - $isi9->id_claim_sp</option>";
						}
					}
				}
			}
			...
			foreach ($item10->result() as $isi10) {
				if ($this->sisaRef($isi10->no_retur_dealer,'retur') > 0) {
					echo "<option category='retur' value='$isi10->no_retur_dealer'>$isi10->no_retur_dealer</option>";
				}
			}
		}
		if ($tipe_customer == 'Lain-lain') {
			...
			foreach ($item8->result() as $isi8) {
				if ($this->sisaRef($isi8->id_tagihan_lain,'tagihan_lain') > 0) {
					echo "<option category='tagihan_lain' value='$isi8->id_tagihan_lain'>$isi8->id_tagihan_lain</option>";
				}
			}
		}
		if ($tipe_customer == 'Vendor') {
			...
			//      }
			if ($id_vendor == 'AHM') {
				$item3 = $this->db->query("SELECT * FROM tr_invoice where status_pelunasan = 0 GROUP BY no_faktur ORDER BY no_faktur ASC");
				echo 'Line 3';
				foreach ($item3->result() as $isi3) {
					if ($this->sisaRef($isi3->no_faktur,'AHM') > 0) {
						echo "<option category='AHM' value='$isi3->no_faktur'>$isi3->no_faktur</option>";
					}
				}
			} else {
				if ($jenis_bayar == 'Unit') {
					...
					foreach ($item7->result() as $isi7) {
						if ($this->sisaRef($isi7->id_rekap_asuransi,'rekap_asuransi') > 0) {
							echo "<option category='rekap_asuransi' value='$isi7->id_rekap_asuransi'>$isi7->id_rekap_asuransi</option>";
						}
					}
					$item4 = $this->db->query("SELECT * FROM tr_adm_bbn WHERE status_pelunasan = 0 and nama_biro_jasa='$id_vendor' ORDER BY id_adm_bbn ASC");
					echo 'Line 5';
					//echo $this->sisaRef('201911/ABB/00003');
					foreach ($item4->result() as $isi4) {
						if ($this->sisaRef($isi4->id_adm_bbn,'adm_bbn') > 0) {
							echo "<option category='adm_bbn' value='$isi4->id_adm_bbn'>$isi4->id_adm_bbn</option>";
						}
					}
					$item5 = $this->db->query("SELECT * FROM tr_adm_bpkb WHERE status_pelunasan = 0 and nama_biro_jasa='$id_vendor' ORDER BY id_adm_bpkb ASC");
					echo 'Line 6';
					foreach ($item5->result() as $isi5) {
						if ($this->sisaRef($isi5->id_adm_bpkb,'bbn_bpkb') > 0) {
							echo "<option category='bbn_bpkb' value='$isi5->id_adm_bpkb'>$isi5->id_adm_bpkb</option>";
						}
					}
					$item6 = $this->db->query("SELECT * FROM tr_adm_stnk WHERE status_pelunasan = 0 and nama_biro_jasa='$id_vendor' ORDER BY id_adm_stnk ASC");
					echo 'Line 7';
					foreach ($item6->result() as $isi6) {
						if ($this->sisaRef($isi6->id_adm_stnk,'bbn_stnk') > 0) {
							echo "<option category='bbn_stnk' value='$isi6->id_adm_stnk'>$isi6->id_adm_stnk</option>";
						}
					}
					$item8 = $this->db->query("SELECT * FROM tr_adm_jual WHERE status_pelunasan = 0 and nama_biro_jasa='$id_vendor' ORDER BY id_adm_jual ASC");
					echo 'Line 9';
					foreach ($item8->result() as $isi8) {
						if ($this->sisaRef($isi8->id_adm_jual,'adm_jual') > 0) {
							echo "<option category='adm_jual' value='$isi8->id_adm_jual'>$isi8->id_adm_jual</option>";
						}
					}
				} else {
					echo 'Line 8';
					$item7 = $this->db->query("SELECT * FROM tr_rekap_tagihan WHERE status_pelunasan = 0 and id_vendor='$id_vendor' ORDER BY id_rekap_tagihan ASC");
					foreach ($item7->result() as $isi7) {
						if ($this->sisaRef($isi7->id_rekap_tagihan,'rekap_penerimaan') > 0) {
							echo "<option category='rekap_penerimaan' value='$isi7->id_rekap_tagihan'>$isi7->id_rekap_tagihan</option>";
						}
					}
				} // Else Unit
			} // Else cek id vendor
	...
	public function sisaRef_bak($referensi)
	{
	...
	public function edit()
	{
		...
		if ($cek->num_rows() > 0) {
			...
			$data['details'] = $this->db->query("SELECT vbd.*,coa.tipe_transaksi
			from tr_voucher_bank_detail vbd left join ms_coa coa on coa.kode_coa =vbd.kode_coa 
			WHERE vbd.id_voucher_bank='$id_voucher_bank' 	
			group by vbd.id_voucher_bank_detail ORDER BY vbd.id_voucher_bank_detail asc ")->result();
			
			
			// $data['details'] = $this->db->query("SELECT * FROM tr_voucher_bank_detail WHERE id_voucher_bank='$id_voucher_bank' ORDER BY id_voucher_bank_detail asc")->result();
