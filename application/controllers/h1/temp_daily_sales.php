...
<table border="1">  
	...
	$sql = $this->db->query("SELECT ms_tipe_kendaraan.id_tipe_kendaraan,ms_tipe_kendaraan.tipe_ahm FROM ms_tipe_kendaraan 
		WHERE active = 1 ORDER BY ms_tipe_kendaraan.id_tipe_kendaraan ASC LIMIT 20,10");
 	foreach ($sql->result() as $row) { 		 		
		...
		if($bulan_s!=$bulan_sekarang){
			// $cek_stok_ssu = $this->db->query("SELECT jum FROM tr_stok_ssu WHERE id_tipe_kendaraan = '$row->id_tipe_kendaraan'
			// 	AND LEFT(tanggal) = '$tgl_bln' $where_ssu");
