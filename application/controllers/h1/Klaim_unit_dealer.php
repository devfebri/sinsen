...
class Klaim_unit_dealer extends CI_Controller {

    var $tables =   "tr_sales_program";	
	var $folder =   "h1";
	var $page	=	"klaim_unit_dealer";
    var $pk     =   "id_sales_program";
    var $title  =   "Klaim Unit Dealer";
	...
	{		
		...
		$this->load->model('M_klaim_unit_dealer_datatables');	
		//===== Load Library =====
	...
	{
		$list = $this->M_klaim_unit_dealer_datatables->get_datatables();
		$data = array();
		$no = $_POST['start'];
        ...
        foreach($list as $row) {       
			  ...
			  if (!empty($row->id_program_md)) {
				$button_id_program =" <a href='h1/klaim_unit_dealer/detail?id=$row->id_program_md'>$row->id_program_md </a>";
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->M_klaim_unit_dealer_datatables->count_all(),
			"recordsFiltered" => $this->M_klaim_unit_dealer_datatables->count_filtered(),
			"data" => $data,
		);
	...
	{		
		...
		$this->load->view("h1/Klaim_unit_dealer/detail?id=.$sales_program_md.");
	}

	public function approve_status_testing()
	{		
		$generate= $this->input->get('id_claim');
		$sales_program_md= $this->input->get('sales_program');
		// $tabel	= 'tr_claim_sales_program';	
		...
		$update['status'] = 'close';
		$update['closed_at'] = date('Y-m-d h:i:s');
   		 echo "<meta http-equiv='refresh' content='0; url=".base_url()."h1/Klaim_unit_dealer/detail?id=".$sales_program_md."'>";
	}
	...
	{		
		...
		$filter['id_claim'] =       $generate        = $this->input->post('id_claim');
		$filter['sales_program'] =	$sales_program_md= $this->input->post('sales_program');
		$filter['id_dealer'] =	$id_dealer       = $this->input->post('id_dealer');

		$button_check =$this->input->post('process');
				
		$claim_approve_dealer = $this->db->query("SELECT  tcd.id_claim,tcd.send_dealer  from tr_claim_dealer tcd 
		WHERE tcd.id_program_md ='$sales_program_md' and tcd.id_dealer ='$id_dealer' AND send_dealer is not NULL 
		group by tcd.id_claim ")->result();

	

		if(count($claim_approve_dealer)!==0){
			$_SESSION['pesan'] 	= "Data Gagal Dikirim ke Dealer";
			$_SESSION['tipe'] 	= "error";
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."h1/Klaim_unit_dealer/detail?id=".$sales_program_md."'>";
		}else{
			if ($button_check == 'approve'){		
				$this->close_program_dealer($filter);
			}else if ($button_check == 'send_dealer'){
				$this->generate_claim_to_dealer($filter);
			}
		}
	...
	public function close_program_dealer($filter)
	{

		$login_id		= $this->session->userdata('id_user');
		$waktu 			  = gmdate("y-m-d h:i:s", time()+60*60*7);
		$id_claim	      = $filter['id_claim'];
		$id_dealer		  = $filter['id_dealer'];
		$sales_program_md = $filter['sales_program'];

		$tabel 		= "tr_claim_sales_program";

		$claim_approve_dealer = $this->db->query("SELECT * from tr_claim_sales_program WHERE id_dealer='$id_dealer' and id_program_md ='$sales_program_md'")->result();

		foreach($claim_approve_dealer AS $val){
			$result[] = array(
				'id_claim_sp' 	=> $val->id_claim_sp,
				'id_program_md' =>  1,
				'closed_at'     => $waktu,
				'closed_by'     => $login_id
				);
		...
		// $db=$this->db->update_batch($tabel, $result, 'id_claim_sp');
		// $this->m_admin->update($tabel,$update,$pk,$generate);
	...
	public function generate_claim_to_dealer()
	{	
			$id_dealer         = $this->input->post('id_dealer');
			$sales_program_md  = $this->input->post('sales_program');	
			...
			$db=$this->db->update_batch($tabel2, $result, 'id_claim');
