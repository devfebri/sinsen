...
class Invoice_dealer_unit extends CI_Controller {
	...
	{				
		...
		$data['dt_invoice']	= $this->db->query("SELECT * FROM tr_invoice_dealer INNER JOIN tr_do_po ON tr_invoice_dealer.no_do = tr_do_po.no_do 
			WHERE tr_invoice_dealer.status_invoice <> 'printable' AND tr_invoice_dealer.status_invoice <> 'reject finance' ORDER BY tr_invoice_dealer.id_invoice_dealer DESC");
		$this->template($data);			
	}
	public function normalisasi($no_do){				
	...
	public function ajax_list()
	{
      ...
      $rt = $this->m_admin->getByID("ms_dealer","id_dealer",$row->id_dealer)->row(); 
			...
			if($isi->status_invoice == 'printable'){				   
				...
				$row[] = $isi->tgl_cair;				
				$row[] = $status;				
				$row[] = $tampil." ".$tampil2." ".$tampil3." ".$tampil4;								
				$data[] = $row;
			}										
		}
		$output = array(
	...
	{				
        ...
        foreach($dt_do_reg->result() as $isi){
			...
			")->row();
	        $th = date("Y");
	        $cek_rfs = $this->db->query("SELECT COUNT(no_mesin) AS jum FROM tr_scan_barcode WHERE id_item = '$isi->id_item' AND status = '1' AND tipe='RFS' AND LEFT(fifo,4) <= '$th'");
	...
	$header = array('Country', 'Capital');
    	...
    	$pdf->SetTextColor(255);
	$pdf->Cell(100,5,'s','LR',0,'L',1);
	$a=$pdf->GetY();
	$pdf->setX(110);
        $pdf->Cell(100,15,'a','LR',1,'L',1);
	$pdf->SetY($a+5);
        $pdf->Cell(100,5,'d','LR',1,'L',1);
	...
	*/
	  ...
	  // $pdf->Cell(30, 5, 'Kpd. Nama ', 0, 0);
	  // $pdf->MultiCell(70, 5, ': '.$nama_dealer.'', 0, "L");	


	  $pdf->Cell(30, 5, 'NPWP', 0, 0);	  
	  $pdf->Cell(70, 5, ': 03.074.830.5-331.000', 0, 0);
	  // $pdf->MultiCell(100, 5, $nama_dealer, 0, "L");	 	  
	  ...
	  $pdf->Cell(70, 5, ': No. PKP 03.074.830.5-331.000', 0, 0);
	  ...
	  $pdf->Cell(70, 5, ': 30 Nopember 2010', 0, 0);
	  ...
	  $nosin = $this->m_admin->get_detail_inv_dealer($get_d->no_do);
	  $i=1;
	...
	{
		echo json_encode($this->m_admin->get_detail_inv_dealer('RG-20190800001','GB2-BB'));
	}
