...
class Niguri extends CI_Controller {
    ...
    $total = $cek_ready->jum + $cek_booking->jum + $cek_pl->jum + $cek_nrfs->jum  + $cek_pinjaman->jum + $r2 + $rr;
	...
	public function edit_niguri(){
	  ...
	  $isi_bln 		= $bln;			  
		...
		$id_item					= $this->input->post('id_item');			
		$data['id_item']	= $this->input->post('id_item');			
		$data['a_m1']			= $this->input->post('am1');			
		$data['a_m']			= $this->input->post('am');			
	...
	{		
		...
		if($cek == 0){						
		  $isi_bln 								= $bln;			  
			...
			for ($i=1; $i <= $jumlah ; $i++) { 				
				$da['id_niguri']	= $this->input->post("id_niguri");			
				$da['id_item']		= $this->input->post("id_item_".$i."");			
				$da['a_m1']			= $this->input->post("a_m1_".$i."");			
				$da['a_m']			= $this->input->post("a_m_".$i."");			
				$da['a_fix']		= $this->input->post("a_fix_".$i."");			
				$da['a_t1']			= $this->input->post("a_t1_".$i."");					
				$da['a_t2']			= $this->input->post("a_t2_".$i."");					
				$da['b_m1']			= $this->input->post("b_m1_".$i."");			
				$da['b_m']			= $this->input->post("b_m_".$i."");			
				$da['b_fix']		= $this->input->post("b_fix_".$i."");					
				$da['b_t1']			= $this->input->post("b_t1_".$i."");					
				$da['b_t2']			= $this->input->post("b_t2_".$i."");					
				$da['c_m1']			= $this->input->post("c_m1_".$i."");			
				$da['c_m']			= $this->input->post("c_m_".$i."");			
				$da['c_fix']		= $this->input->post("c_fix_".$i."");					
				$da['c_t1']			= $this->input->post("c_t1_".$i."");					
				$da['c_t2']			= $this->input->post("c_t2_".$i."");					
				$da['d_m1']			= $this->input->post("d_m1_".$i."");			
				$da['d_m']			= $this->input->post("d_m_".$i."");			
				$da['d_fix']		= $this->input->post("d_fix_".$i."");					
				$da['d_t1']			= $this->input->post("d_t1_".$i."");					
				$da['d_t2']			= $this->input->post("d_t2_".$i."");	
				...
				//if($a_fix <= $batas_atas_t1 AND $a_fix >= $batas_bawah_t1 AND $a_t1 <= $batas_atas_t2 AND $a_t1 >= $batas_bawah_t2){			
					...
					if($hasil_tgl == 'ok'){
						...
						if($cek->num_rows() > 0){
							$sq = $cek->row();
							$id = $sq->id_niguri_detail;
							$cek2 = $this->m_admin->update("tr_niguri_detail",$da,"id_niguri_detail",$id);												
						}else{
							$cek2 = $this->m_admin->insert("tr_niguri_detail",$da);						
						}
						//$_SESSION['pesan2'] 	= "Data niguri aman";
					}else{						
						$_SESSION['pesan2'] 	= "Data niguri ".$bul."-".$tahun." sudah dibuat";
					}
				/*}else{
					$_SESSION['pesan2'] 	= "Nilai Fix & T1 tidak sesuai dengan batas yg telah ditentukan";					
				}*/
			}			
