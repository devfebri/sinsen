...
class Pengajuan_bbn_md extends CI_Controller
{
	...
	public function get_data_pengajuan_bbm_md($page='')
	{
        ...
        foreach($dataBbnMd->result() as $row)
        {
            ...
            if($cek->num_rows() > 0){
              $id_    = $cek->row();
              if ($id_->status_pengajuan=='checked' && $st=='approved') {
                $status = "<span class='label label-primary'>approved</span>";
              }else{
                $status = "<span class='label label-danger'>$st</span>";
              }
            }else{
              $id2    = $cek2->row();              
	...
	public function edit()
	{
		...
		$query = $this->db->query("SELECT tr_sales_order.*,ms_pekerjaan.pekerjaan AS kerja,tr_scan_barcode.no_rangka,ms_item.id_item,ms_tipe_kendaraan.id_tipe_kendaraan,ms_tipe_kendaraan.tipe_ahm,
			ms_warna.warna,tr_spk.*, tr_spk.nama_bpkb as nama_konsumen, tr_spk.alamat_ktp_bpkb as alamat, tr_spk.no_ktp_bpkb as no_ktp, (CASE WHEN id_kelurahan_bpkb IS NULL THEN id_kelurahan ELSE id_kelurahan_bpkb END) AS id_kelurahan
			FROM tr_sales_order 
		...
		// data ketika sudah disimpan
		$cek  = $this->db->query("SELECT no_mesin FROM tr_pengajuan_bbn_detail WHERE no_bastd = '$no_bastd' AND no_mesin = '$no_mesin'");
	...
	public function take_kec()
	{
		...
		if($data->num_rows()>0){
			...
			$dt_kab				= $this->db->query("SELECT kabupaten, id_provinsi FROM ms_kabupaten WHERE id_kabupaten = '$id_kabupaten'")->row();
	...
	public function download()
	{
		...
		if($query->num_rows()>0){
			...
			foreach ($query->result() as $isi) {
				$get_leads = $this->mokita_model->cek_sales_order_from_sinsengo(['no_mesin' => $isi->no_mesin],'h1/pengajuan_bbn_md/download');
				if ($get_leads) {
				...
				if ($cek_biaya_bbn_md_bj->num_rows() > 0) {
					...
					$cek_customer = $this->db->query("SELECT tr_prospek_gc.jenis FROM tr_sales_order_gc_nosin
						INNER JOIN tr_sales_order_gc ON tr_sales_order_gc_nosin.id_sales_order_gc = tr_sales_order_gc.id_sales_order_gc 
						INNER JOIN tr_spk_gc ON tr_sales_order_gc.no_spk_gc = tr_spk_gc.no_spk_gc
					...
					$jenis = ($cek_customer->num_rows() > 0) ? $cek_customer->row()->jenis : "" ; // salah disini?
			...
			if (isset($kirim_ce_stnk)) {
				foreach ($kirim_ce_stnk as $key => $spk) {
					...
					$array_post = [
						...
						'StnkStatus' => 'Proses pembuatan STNK',
						'BpkbStatus' => 'Proses pembuatan BPKB',
						'VehicleNumber' => '',
					];
			...
			$_SESSION['pesan'] 	= "Data tidak ada/ sudah pernah digenerate, Silahkan cek kembali!";
