...
class Voucher_pengeluaran_bank extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['page']   = $this->page;
		$data['set']		= "view";
		// $data['dt_voucher']	= $this->db->query("SELECT * FROM tr_voucher_bank WHERE status <> 'batal' and created_at >='2021-08-31' order by id_voucher_bank desc");
		$this->template($data);
	}
	...
	public function sisaRef($referensi,$kategori = null)
	{
		...
		// RETUR
		if($kategori=='retur'){
			$cek12 = $this->db->query("select a.no_mesin,b.id_dealer, b.tgl_retur from tr_retur_dealer_detail a join tr_retur_dealer b on a.no_retur_dealer  = b.no_retur_dealer where a.no_retur_dealer = '$referensi'");
			if ($cek12->num_rows() > 0) {
				$np = "";
				$total_semua = 0;
				$harga = 0;
				...
				foreach ($cek12->result() as $isi) {
					$cari_no_do = $this->db->query("SELECT tr_picking_list_view.id_item, tr_picking_list.no_do FROM tr_picking_list_view JOIN tr_picking_list ON tr_picking_list_view.no_picking_list = tr_picking_list.no_picking_list join tr_do_po on tr_do_po.no_do = tr_picking_list.no_do
						WHERE tr_picking_list_view.no_mesin = '$isi->no_mesin' and tr_do_po.id_dealer = '$isi->id_dealer' and tr_picking_list.tgl_pl < '$isi->tgl_retur' ORDER BY tr_picking_list.no_do desc LIMIT 0,1");
