...
class Voucher_pengeluaran_bank extends CI_Controller {
	...
	public function cari_ref(){				
		...
		$cek3	= $this->db->query("SELECT total_bayar as total FROM tr_monitor_tempo WHERE no_faktur = '$referensi'");						       		
		...
		$cek10 = $this->db->query("SELECT IFNULL(SUM(total),0) AS jum FROM tr_rekap_tagihan_detail 
	 ...
	 //    $total 	=	$row2->total;
	  ...
	  if($cek3->num_rows()>0){
	  	// echo 'xx';
			...
			$dt_invoice = $this->db->query("SELECT * FROM tr_invoice WHERE tr_invoice.no_faktur = '$referensi'");     
      $ppn=0;$pph=0;
      foreach ($dt_invoice->result() as $isi) {
        $ppn = $ppn + $isi->ppn;            
        $pph = $pph + $isi->pph;            
      }
      $get_amount = $this->db->query("SELECT qty, harga FROM tr_invoice WHERE no_faktur ='$referensi'");
      $amount = 0;
	  ...
	  }elseif ($cek9->row()->jum>0){
	  	// echo 6;
		$total = $cek9->row()->jum;
	  }elseif ($cek10->row()->jum>0){
	 	// echo 7;
		$total = $cek10->row()->jum;
	  }else{
	  		$total = $this->m_admin->cekOnlyVoucherBank($referensi,$total);}
	...
	public function getRef()
	{
      ...
      //     }
	     ...
	     //      }
	       ...
	       if ($id_vendor=='AHM') {
	       	 $item3 = $this->db->query("SELECT * FROM tr_monitor_tempo ORDER BY no_rekap ASC");
	        ...
	        $item5 = $this->db->query("SELECT * FROM tr_adm_bpkb WHERE nama_biro_jasa='$id_vendor' ORDER BY id_adm_bpkb ASC");
	          ...
	          foreach ($item6->result() as $isi6) {
	            if ($this->sisaRef($isi6->id_adm_stnk)>0) {
		      	 ...
		      	 echo '<option value="">--choose--</option>';
	          foreach ($item7->result() as $isi7) {
	          	// if ($this->sisaRef($isi7->id_rekap_tagihan)>0) {
	...
	public function getRekap()
	{
		...
		$dt_rekap = $this->db->query("SELECT * FROM tr_rekap_ekspedisi 
          ...
          foreach($dt_rekap->result() as $row) {                                         
            ...
            if ($cek>0) {
              $rekap[] = ['no_rekap' => $row->id_rekap_ekspedisi,
              			  'nominal' => $cek
          	  ];
            }
	...
	{		
		...
		if ($via_bayar=='Transfer') {
			...
			foreach ($transfers as $val) {
				$ins_transfer[] = ['id_voucher_bank'=>$id_voucher_bank,
		...
		if ($via_bayar=='BG') {
			...
			foreach ($bg_ as $val) {
				$ins_bg[] = ['id_voucher_bank'=>$id_voucher_bank,
								 'no_bg'=>$val['no_bg'],
								 'tgl_bg'=>$val['tgl_bg'],
		...
		foreach ($details as $key => $val) {
			$ins_detail[] = ['id_voucher_bank'=> $id_voucher_bank,
