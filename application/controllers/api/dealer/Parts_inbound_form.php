...
class Parts_inbound_form extends CI_Controller
{
    ...
    public function index()
    {
        ...
        foreach ($records as $each) {
            $sub_arr = (array) $each;
            $sub_arr['action'] = $this->load->view('additional/action_parts_inbound_form', [
    ...
    public function make_query()
    {
        ...
        ->select('p.id_part')
        ->select('p.nama_part')
        ->select('fp.kuantitas as qty')
        ->select('0 as qty_order')
        ->select('"" as id_gudang')
        ->select('"" as id_rak')
        ->select('"" as id_reason')
        ->select('"" as reason')
    ...
    public function make_datatables()
    {
        ...
        if ($search != '') {
            ...
            $this->db->or_like('fp.id_rak', $search);
            $this->db->or_like('fp.id_rak', $search);
        }
