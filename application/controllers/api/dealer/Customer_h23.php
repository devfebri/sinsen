<?php

defined('BASEPATH') or exit('No direct script access allowed');

class Customer_h23 extends CI_Controller
{
    public function __construct(){
        parent::__construct();
        $this->load->model('m_admin');
    }

    public function index()
    {
        $records = $this->make_datatables();
        $data = array();
        foreach ($records as $each) {
            $sub_arr = (array) $each;
            $sub_arr['action'] = $this->load->view('additional/action_index_customer_h23', [
                'id' => $each->id_customer
            ], true);
            $data[] = $sub_arr;
        }

        send_json([
            'draw' => intval($this->input->post('draw')),
            'recordsFiltered' => $this->recordsFiltered(),
            'recordsTotal' => $this->recordsTotal(),
            'data' => $data
        ]);
    }

    public function make_query()
    {
        $this->db
        ->select('c.id_customer')
        ->select('concat(kabupaten.kabupaten, " - ", provinsi.provinsi) as domisili', false)
        ->select('c.no_identitas')
        ->select('c.nama_customer')
        ->select('c.no_polisi')
        ->select('c.email')
        ->select('c.no_hp')
        ->from('ms_customer_h23 as c')
        ->join('ms_kabupaten as kabupaten', 'kabupaten.id_kabupaten = c.id_kabupaten','left')
        ->join('ms_provinsi as provinsi', 'provinsi.id_provinsi = c.id_provinsi','left')
        ->where('c.id_dealer', $this->m_admin->cari_dealer())
        ;

        if($this->input->post('id_kabupaten') != null){
            $this->db->where('c.id_kabupaten', $this->input->post('id_kabupaten'));
        }
    }



    public function make_datatables()
    {
        $this->make_query();

        $search = trim($this->input->post('search')['value']);

        if ($search != '') {
            $this->db->group_start();
            $this->db->like('c.id_customer', $search);
            $this->db->or_like('kabupaten.kabupaten', $search);
            $this->db->or_like('provinsi.provinsi', $search);
            $this->db->or_like('c.no_identitas', $search);
            $this->db->or_like('c.nama_customer', $search);
            $this->db->or_like('c.no_polisi', $search);
            $this->db->or_like('c.email', $search);
            $this->db->or_like('c.no_hp', $search);
            $this->db->group_end();
        }

        if (isset($_POST["order"])) {
            $indexColumn = $_POST['order']['0']['column'];
            $this->db->order_by($_POST['columns'][$indexColumn]['data'], $_POST['order']['0']['dir']);
        } else {
            $this->db->order_by('c.id_customer', 'asc');
        }

        if ($_POST["length"] != - 1) {
            $this->db->limit($_POST['length'], $_POST['start']);
        }

        return $this->db->get()->result();
    }

    public function recordsFiltered()
    {
        $this->make_query();
        return $this->db->get()->num_rows();
    }

    public function recordsTotal(){
        $this->make_query();
        return $this->db->count_all_results();
    }
}
