...
class Master_plafon_finance extends CI_Controller

{
    public function index()
    {
        ...
        foreach ($this->db->get()->result_array() as $row) {
            $row['checkbox'] = $this->load->view('additional/md/h3/action_check_plafon_md', [
                'id' => $row['id'],
                'plafon_id' => $this->session->userdata('plafon_id_finance') != null ? $this->session->userdata('plafon_id_finance') : []
            ], true);

            $row['action'] = $this->load->view('additional/md/h3/action_index_h3_md_master_plafon_finance', [
                'id' => $row['id']
            ], true);
    ...
    public function make_query()
    {
        ...
        if ($this->input->post('history') != null and $this->input->post('history') == 1) {
            ...
            $this->db->group_start();
            $this->db->group_start();
            $this->db->where('plafon.status', 'Approved by Admin');
            $this->db->or_where('plafon.status', 'Approved by Finance');
            $this->db->group_end();
    ...
    public function set_plafon_id(){
        ...
        $plafon_id_marketing = $this->session->userdata('plafon_id_finance') != null ? $this->session->userdata('plafon_id_finance') : [];
        if(!in_array($id, $plafon_id_marketing)){
            $plafon_id_marketing[] = $id;
            $this->session->set_userdata('plafon_id_finance', $plafon_id_marketing);
        }

        send_json($this->session->userdata('plafon_id_finance'));
    }
