...
class Delivery_order_filter_monitoring_kerja_picker extends CI_Controller {

    public function index() {
        $this->make_datatables(); $this->limit();
        $data = [];
        foreach ($this->db->get()->result_array() as $record) {
            $record['action'] = $this->load->view('additional/md/h3/action_do_filter_monitoring_kerja_picker_index', [
                'data' => json_encode($record),
                'id_do_sales_order' => $record['id_do_sales_order']
            ], true);
            $data[] = $record;
        }
