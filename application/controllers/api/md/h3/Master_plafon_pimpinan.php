...
class Master_plafon_pimpinan extends CI_Controller
{
    public function index()
    {
        ...
        foreach ($this->db->get()->result_array() as $row) {
            $row['checkbox'] = $this->load->view('additional/md/h3/action_check_plafon_md', [
                'id' => $row['id'],
                'plafon_id' => $this->session->userdata('plafon_id_pimpinan') != null ? $this->session->userdata('plafon_id_pimpinan') : []
            ], true);

            $row['action'] = $this->load->view('additional/md/h3/action_index_h3_md_master_plafon_pimpinan', [
                'id' => $row['id']
            ], true);
    ...
    public function make_query()
    {
        ...
        ->select('IF(plafon.keterangan_pimpinan IS NULL, plafon.keterangan_finance, plafon.keterangan_pimpinan) as keterangan', false)
        ...
        if ($this->input->post('history') != null and $this->input->post('history') == 1) {
            ...
            $this->db->group_start();
            $this->db->where('plafon.status', 'Approved by Finance');
    ...
    public function set_plafon_id(){
        ...
        $plafon_id_marketing = $this->session->userdata('plafon_id_pimpinan') != null ? $this->session->userdata('plafon_id_pimpinan') : [];
        if(!in_array($id, $plafon_id_marketing)){
            $plafon_id_marketing[] = $id;
            $this->session->set_userdata('plafon_id_pimpinan', $plafon_id_marketing);
        }

        send_json($this->session->userdata('plafon_id_pimpinan'));
    }
