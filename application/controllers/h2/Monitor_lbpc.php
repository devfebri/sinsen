...
class Monitor_lbpc extends CI_Controller
{
	...
	var $pk_det     =   "id_";
	var $folder =   "h2";
	var $page		=		"monitor_lbpc";
	var $title  =   "Monitor LBPC (Lembaran Biaya Penggantian Claim)";
	...
	public function index()
	{
		...
		$data['set']	= "view";
		$data['dt_result'] = $this->db->query("SELECT * FROM tr_lbpc 
			WHERE print_ke>0 AND no_lbpc IN (SELECT no_lbpc FROM tr_rekap_claim_waranty WHERE id_ptca IS NULL)
	...
	public function save()
	{
		...
		if ($cek == 0) {
			...
			echo "<meta http-equiv='refresh' content='0; url=" . base_url() . "h2/maintain_lokasi/add'>";
		} else {
	...
	public function cetak_s()
	{
		...
		$this->load->view('h2/cetak_stiker', $data);
	}
}
