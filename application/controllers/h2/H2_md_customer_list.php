...
class H2_md_customer_list extends CI_Controller
{

    var $folder =   "h2";
    var $page   =   "h2_md_customer_list";
    ...
    public function index()
    {
        ...
        $data['listFU']    = $this->H2_md_customer_list_model->getDataHasilFU();
        $data['kendaraan'] = $this->H2_md_customer_list_model->getDataKendaraan();
        $data['dt_dealer'] = $this->H2_md_customer_list_model->getDataDealer();
        $data['jasaType']  = $jasaType = $this->H2_md_customer_list_model->getDataJasaType();
        $this->template($data);
    }
    ...
    public function getDataTable()
    {
        ...
        foreach ($this->db->get()->result_array() as $row) {
            ...
            $row['action'] = '<a id="' . $row['id_customer'] . '" href="h2/h2_md_customer_list/detail_data?id_customer=' . $row['id_customer'] . '" type="button" class="btn btn-primary btn-xs" target="_blank">Detail</a>';
    ...
    {
        $data['id_dealer'] = $id_dealer = $this->input->post('pilih_ahass');
        $data['mc_type'] = $mc_type = $this->input->post('filters_id_tipe_kendaraan');
        $data['start_date'] = $this->input->post('last_fu_start');
        $data['end_date'] = $this->input->post('last_fu_end');

        if ($_POST['process'] == 'export_excel') {
            $data['downloadExcel'] = $this->H2_md_customer_list_model->downloadExcel();
            $this->load->view("h2/laporan/temp_h2_md_customer_list_excel", $data);
        }
    }
    ...
    {
        $this->db->trans_begin();
        $data = $this->H2_md_customer_list_model->generateData();
        
        $datatotal = count($data);

        $data_insert = [
            'tgl_generate' => date('Y-m-d H:i:s'),
            'assign_dealer' => $this->input->post('pilih_ahass'),
    ...
    {
        $this->db->trans_begin();
        $data = $this->H2_md_customer_list_model->actual_service_data();
    ...
    public function detail_data($id_customer)
    {
        $data['isi']            = $this->page;
        $data['title']          = $this->title;
        $data['set']              = "detail";
        $data['id_customer']    = $id_customer = $this->input->get('id_customer');
        $data['getDetailData']  = $this->H2_md_customer_list_model->getDetailData($id_customer);
        $this->template($data);
    }
    ...
    // 	$data = array(
    // 		'tujuan_penggunaan_motor' => $tujuan_penggunaan_motor,
    // 		'tgl_lahir' => $tgl_lahir
    // 	);
    ...
    // 	$this->H2_dealer_customer_list_model->update_data($where,$data,'ms_customer_h23');
    //     $_SESSION['pesan'] 	= "Data ".$id_customer. " Berhasil di Update";
