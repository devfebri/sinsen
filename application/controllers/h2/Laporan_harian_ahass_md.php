    ...
    if (isset($_GET['cetak'])) {
      ...
      if($filter['id_dealer']=="all"){
            ...
            $data['details'] = $this->db->query("select c.id_tipe_kendaraan as tipe_kendaraan,f.deskripsi, 
				...
				on a.id_work_order=d.id_work_order left join ms_h2_jasa e on e.id_jasa=d.id_jasa left join ms_ptm f on c.id_tipe_kendaraan=f.tipe_marketing where a.status='Closed' and left(a.created_at,10) between '{$filter['start_date']}' and '{$filter['end_date']}' GROUP BY c.id_tipe_kendaraan")->result();
         $data['jasa']= $this->db->query("SELECT 
				...
				ON B.id_work_order=C.id_work_order WHERE C.status='Closed' AND B.pekerjaan_batal='0'
	    ...
	    $data['part']=$this->db->query("select 
				 ...
				 from tr_h23_nsc_parts B JOIN ms_part A ON A.id_part=B.id_part join tr_h23_nsc C ON C.no_nsc=B.no_nsc where C.referensi='sales' 
		...
		$data['part_wo']=$this->db->query("select 
					...
					SUM(CASE WHEN A.kelompok_part in('BRAKE','PS') then B.harga_beli*B.qty ELSE 0 END) AS brake,
					SUM(CASE WHEN A.kelompok_part in('ACCEC','FKT','TL','FED OIL','ACC','HELM','PA','PACC') then B.harga_beli*B.qty ELSE 0 END) AS other
				 from tr_h23_nsc_parts B JOIN ms_part A ON A.id_part=B.id_part join tr_h23_nsc C ON C.no_nsc=B.no_nsc join tr_h2_wo_dealer wo on C.id_referensi=wo.id_work_order join tr_h2_sa_form sa on wo.id_sa_form=sa.id_sa_form where C.referensi='work_order' 
				 and C.tgl_nsc BETWEEN '{$filter['start_date']}' and '{$filter['end_date']}'")->row();
