...
class Laporan_harian_ahass_md extends CI_Controller
{

  var $folder =   "h2/laporan";
  var $page    =    "laporan_harian_ahass_md";
  ...
  protected function template($data)
  {
    ...
    if ($name == "") {
      ...
      $data['dealer'] = $this->db->query("SELECT id_dealer,kode_dealer_ahm,nama_dealer from ms_dealer where id_dealer in('2',	'103',	'105',	'46',	'47',	'4',	'1',	'51',	'22',	'107',	'101',	'40',	'80',	'71',	'18',	'97',	'13',	'84',	'43',	'25',	'83',	'41',	'39',	'104',	'106',	'102',	'96',	'85',	'8',	'81',	'44',	'38',	'76',	'30',	'82',	'70',	'77',	'86',	'91',	'58',	'19',	'64',	'54',	'23',	'29',	'11',	'9',	'10',	'74',	'98',	'7',	'6',	'56',	'28',	'69',	'5',	'87',	'89',	'88',	'66',	'90',	'78',	'65')")->result();
    ...
    if (isset($_GET['cetak'])) {
      ...
      $filter = [
        ...
        $data['details'] = $this->db->query("select c.id_tipe_kendaraan as tipe_kendaraan,f.deskripsi, 
					...
					SUM(CASE WHEN c.id_tipe_kendaraan=c.id_tipe_kendaraan and e.id_type='HR' then 1 ELSE 0 end )as total_hr,
					...
					SUM(CASE WHEN c.id_tipe_kendaraan=c.id_tipe_kendaraan THEN 1 else 0 end) as total_job,
					SUM(CASE WHEN a.status='Closed' then 1 else 0 end) as total_entri
	    ...
	    $data['part']=$this->db->query("select 
					...
					'TAS','TB1','TBVL') then B.harga_beli ELSE 0 END) AS spart,
					SUM(CASE WHEN A.kelompok_part in('GMO','ACB','ACG','FLUID','OIL') then B.harga_beli ELSE 0 END) AS oil,
					SUM(CASE WHEN A.kelompok_part in('SPLUR','SP','SPLUG','SP2') then B.harga_beli ELSE 0 END) AS busi,
					SUM(CASE WHEN A.kelompok_part in('TIRE','TR','TIRE1') then B.harga_beli ELSE 0 END) AS tire,
					SUM(CASE WHEN A.kelompok_part in('BLDRV') then B.harga_beli ELSE 0 END) AS belt,
					SUM(CASE WHEN A.kelompok_part in('BATT','BT') then B.harga_beli ELSE 0 END) AS battery,
					SUM(CASE WHEN A.kelompok_part in('BRAKE','PS') then B.harga_beli ELSE 0 END) AS brake,
					SUM(CASE WHEN A.kelompok_part in('ACCEC','FKT','TL','FED OIL','ACC','HELM','PA') then B.harga_beli ELSE 0 END) AS other
		...
		$data['part_wo']=$this->db->query("select 
					...
					'EPHVL','HAH','HPLAS','HRW','HSD','OISTC','OTHR','PAINT','PT','RPHVL','RW','RW2','RW3','RWHVL','SA','SAOIL','SHOCK',
					'TAS','TB1','TBVL') then B.harga_beli ELSE 0 END) AS spart,
					SUM(CASE WHEN A.kelompok_part in('GMO','ACB','ACG','FLUID','OIL') and sa.id_type !='ASS1' then B.harga_beli ELSE 0 END) AS oil,
					SUM(CASE WHEN A.kelompok_part in('GMO','ACB','ACG','FLUID','OIL') and sa.id_type ='ASS1' then B.harga_beli ELSE 0 END) AS oil_kpb,
					SUM(CASE WHEN A.kelompok_part in('SPLUR','SP','SPLUG','SP2') then B.harga_beli ELSE 0 END) AS busi,
					SUM(CASE WHEN A.kelompok_part in('TIRE','TR','TIRE1') then B.harga_beli ELSE 0 END) AS tire,
					SUM(CASE WHEN A.kelompok_part in('BLDRV') then B.harga_beli ELSE 0 END) AS belt,
					SUM(CASE WHEN A.kelompok_part in('BATT','BT') then B.harga_beli ELSE 0 END) AS battery,
