...
// 		force_download($name_file, $content);
	...
	public function ssu()
	{
		...
		foreach ($sql->result() as $isi) {
			...
			$sj = $this->db->query("SELECT tr_surat_jalan_detail.no_surat_jalan, tr_surat_jalan_detail.no_mesin FROM tr_surat_jalan_detail INNER JOIN tr_surat_jalan ON tr_surat_jalan_detail.no_surat_jalan = tr_surat_jalan.no_surat_jalan
...
*/
	...
	public function udh(){
		...
		if(date('Y') == '2021'){ 
			...
			// $tgl_awal = date('Y-m-d', strtotime('-1 month', strtotime($tgl)));
			$tgl_awal = date('Y-01-01');
		}
		
		// tambahkan logic utk awal tahun

		$tgl_akhir = date('Y-m-01');		
		// $tgl_akhir = '2021-07-01';

		$content = '';
		$sql = $this->m_admin->generate_udh($tgl_awal, $tgl_akhir); // perlu diimprove
