...
$name = $this->session->userdata('nama');
	...
	{				
		...
		$data['set']	= "view";
		$data['dt_hobi'] = $this->m_admin->getAll($this->tables);							
		$this->template($data);	
	}
	public function add()
	{				
	...
	{		
		...
		if($cek == 0){
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."master/hobi/add'>";
		}else{
	...
	{		
		...
		if($cek_approval == 'salah'){
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."master/hobi'>";
		}
	}
	...
	{		
		...
		$id 			= $this->input->get('id');
		$d 				= array($pk=>$id);		
		$data['dt_hobi'] = $this->m_admin->kondisi($tabel,$d);
		$data['isi']    = $this->page;		
		$data['title']	= $this->title;		
		$data['set']	= "edit";									
		$this->template($data);	
