...
class Series extends CI_Controller {
    ...
    var $tables =   "ms_series";	
		var $folder =   "master";
		var $page		=		"series";
    var $pk     =   "id_series";
    var $title  =   "Master Data Series";
...
$name = $this->session->userdata('nama');
	...
	{				
		...
		$data['set']		= "view";
		$data['dt_series'] = $this->db->query("SELECT * FROM ms_series INNER JOIN ms_segment ON ms_series.id_segment=ms_segment.id_segment");
		$this->template($data);	
	}
	public function add()
	{				
	...
	{		
		...
		if($cek == 0){
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."master/series/add'>";
		}else{
	...
	{		
		...
		if($cek_approval == 'salah'){
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."master/series'>";
		}
	}
	...
	{		
		...
		$id 				= $this->input->get('id');
		$d 					= array($pk=>$id);		
		$data['dt_series'] = $this->m_admin->kondisi($tabel,$d);
