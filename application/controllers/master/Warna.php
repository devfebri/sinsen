...
$name = $this->session->userdata('nama');
	...
	{				
		...
		$data['set']		= "view";
		$data['dt_warna'] = $this->m_admin->getAll($this->tables);
		$this->template($data);	
	}
	public function add()
	{				
	...
	{		
		...
		if($cek == 0){
			...
			$data['warna_samsat'] 		= $this->input->post('warna_samsat');
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."master/warna/add'>";
		}else{
	...
	{		
		...
		if($cek_approval == 'salah'){
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."master/warna'>";
		}
	}
	...
	{		
		...
		$id 				= $this->input->get('id');
		$d 					= array($pk=>$id);		
		$data['dt_warna'] = $this->m_admin->kondisi($tabel,$d);
