...
class Dealer extends CI_Controller
{
	...
	public function dealer_urut()
	{
		$data['isi']    = $this->page;
		$data['title']	= $this->title;
		$data['set']		= "dealer_urut";
		$data['dt_dealer'] = $this->db->query("SELECT ms_dealer.*,ms_kelompok_harga.kelompok_harga,ms_kelurahan.kelurahan FROM ms_dealer LEFT JOIN ms_kelurahan ON
																					ms_dealer.id_kelurahan=ms_kelurahan.id_kelurahan LEFT JOIN ms_kelompok_harga ON
																					ms_dealer.id_kelompok_harga=ms_kelompok_harga.id_kelompok_harga");
		$this->template($data);
	...
	public function save()
	{
		...
		if ($this->upload->do_upload('logo')) { // Lakukan upload dan Cek jika proses upload berhasil
			...
			$logo = $return['file']['file_name'];
			// return $return;
		} else {
			// Jika gagal :
			$return = array('result' => 'failed', 'file' => '', 'error' => $this->upload->display_errors());

			$_SESSION['pesan'] 	= $return['error'];
		...
		$this->upload->initialize($config); // Load konfigurasi uploadnya
		if ($this->upload->do_upload('favicon')) { // Lakukan upload dan Cek jika proses upload berhasil
			// Jika berhasil :
			$return = array('result' => 'success', 'file' => $this->upload->data(), 'error' => '');
			$favicon = $return['file']['file_name'];
			// return $return;
		} else {
			// Jika gagal :
			$return = array('result' => 'failed', 'file' => '', 'error' => $this->upload->display_errors());
		...
		$data['tampil_ppn_h23']			= $this->input->post('tampil_ppn_h23');
		$data['logo']									= $logo;
	...
	public function update()
	{
		...
		if ($cek == 0 or $id == $id_) {
			...
			if ($_FILES['logo']['name'] == '' or $_FILES['favicon']['name'] == '') {
				$logo = $this->input->post('logo_old');
				$favicon = $this->input->post('favicon_old');
			} else {
				$return = array();
