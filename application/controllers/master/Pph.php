...
$name = $this->session->userdata('nama');
	...
	{				
		...
		$data['set']	= "view";
		$data['dt_pph'] = $this->m_admin->getAll($this->tables);							
		$this->template($data);	
	}
	public function add()
	{				
	...
	{		
		...
		if($cek == 0){
			$data['pasal_pph'] 			= $this->input->post('pasal_pph');
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."master/pph/add'>";			
		}else{
	...
	{		
		...
		if($cek_approval == 'salah'){
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."master/pph'>";
		}
	}
	...
	{		
		...
		$id 			= $this->input->get('id');
		$d 				= array($pk=>$id);		
		$data['dt_pph'] = $this->m_admin->kondisi($tabel,$d);
		$data['isi']    = $this->page;		
		$data['title']	= $this->title;		
		$data['set']	= "edit";									
		$this->template($data);	
