...
class Segment extends CI_Controller {
    ...
    var $tables =   "ms_segment";	
		var $folder =   "master";
		var $page		=		"segment";
    var $pk     =   "id_segment";
    var $title  =   "Master Data Segment";
...
$name = $this->session->userdata('nama');
	...
	{				
		...
		$data['set']		= "view";
		$data['dt_segment'] = $this->db->query("SELECT * FROM ms_segment INNER JOIN ms_kategori ON ms_segment.id_kategori=ms_kategori.id_kategori");
		$this->template($data);	
	}
	public function add()
	{				
	...
	{		
		...
		if($cek == 0){
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."master/segment/add'>";
		}else{
	...
	{		
		...
		if($cek_approval == 'salah'){
			...
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."master/segment'>";
		}
	}
	...
	{		
		...
		$id 				= $this->input->get('id');
		$d 					= array($pk=>$id);		
		$data['dt_segment'] = $this->m_admin->kondisi($tabel,$d);
