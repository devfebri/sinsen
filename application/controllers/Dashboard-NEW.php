...
class Dashboard extends CI_Controller
{
	...
	public function showDetailStok()
	{
		...
		$sql = "
		select id_tipe_kendaraan, tipe_ahm, IFNULL(unfill_md,0) as unfill_md, IFNULL(intransit_md,0) as intransit_md, IFNULL(stok_md,0) as stok_md, IFNULL(unfill,0) as unfill_dealer, IFNULL(intransit,0) as intransit_dealer, IFNULL(stok_dealer,0) as stok_dealer, IFNULL(sales,0) as sales
		from (
		select ms_tipe_kendaraan.id_tipe_kendaraan,ms_tipe_kendaraan.tipe_ahm,
		((select sum(jumlah) from tr_sipb where RIGHT(tgl_sipb,4) >= '2020' and ms_tipe_kendaraan.id_tipe_kendaraan = id_tipe_kendaraan group by id_tipe_kendaraan) - (select count(1) from tr_shipping_list where no_mesin not in ( select no_mesin from tr_shipping_list where cabang = '' ) and ms_tipe_kendaraan.id_tipe_kendaraan = id_modell group by id_modell)) as unfill_md,
		...
		(
		select COUNT(tr_picking_list_view.no_mesin) AS jum 
		FROM tr_do_po 
		...
		AND ms_item.id_tipe_kendaraan = ms_tipe_kendaraan.id_tipe_kendaraan AND tr_picking_list_view.retur = 0
		) as unfill,
		(select count(1) from tr_scan_barcode a join tr_surat_jalan_detail b on a.no_mesin = b.no_mesin where status =3 and a.tipe_motor = ms_tipe_kendaraan.id_tipe_kendaraan and b.retur = 0 and b.ceklist = 'ya') as intransit,
		(SELECT COUNT(DISTINCT(tr_scan_barcode.no_mesin)) AS jum FROM tr_penerimaan_unit_dealer 
		...
		AND tr_scan_barcode.tipe_motor = ms_tipe_kendaraan.id_tipe_kendaraan AND tr_scan_barcode.status = 4) as stok_dealer,
		(
		select count(a.no_mesin) from tr_scan_barcode a join (
		select b.no_mesin from tr_sales_order_gc a join tr_sales_order_gc_nosin b on a.id_sales_order_gc = b.id_sales_order_gc where tgl_cetak_invoice >'$tgl_bln'
		UNION
		select no_mesin from tr_sales_order where tgl_cetak_invoice >'$tgl_bln'
		)b on a.no_mesin = b.no_mesin where a.tipe_motor = ms_tipe_kendaraan.id_tipe_kendaraan and a.status = 5
		)as sales
		from ms_tipe_kendaraan 
		-- where ms_tipe_kendaraan.id_tipe_kendaraan in ('HWD','LKA','GB2','LH0','ES4')
		)x
		";
	...
	public function history_spk()
	{
		$id_dealer = $this->m_admin->cari_dealer();
		//$id_dealer = '65';
		$sql = "SELECT id_tipe_kendaraan,id_warna,id_customer,so.no_spk,alamat,no_ktp 
		FROM tr_spk 
		LEFT JOIN tr_sales_order so ON so.no_spk=tr_spk.no_spk
		WHERE (so.id_sales_order IS NOT NULL OR tr_spk.status_spk='rejected')
		";
		// $sql .= " WHERE (id_dealer = '$id_dealer' AND expired = 1) OR (id_dealer = '$id_dealer' AND no_spk IN(SELECT no_spk FROM tr_sales_order))";
		$sql .= " AND so.id_dealer='$id_dealer'";
	...
	public function history_indent()
	{
		...
		$sql .= " WHERE (tr_po_dealer_indent.status = 'completed' OR tr_po_dealer_indent.status = 'canceled' OR OR tr_po_dealer_indent.status = 'closed')";
	...
	function chartByFinco()
	{
		...
		$get_finco = $this->getFinco($bulan);
		foreach ($get_finco->result() as $rs) {
			$bulan_ini = $this->m_admin->get_penjualan_inv('range_tanggal', $tanggal_arr, null, null, null, null, $rs->id_finance_company);
			$bulan_kemarin = $this->m_admin->get_penjualan_inv('range_tanggal', $tanggal_arr_min1, null, null, null, null, $rs->id_finance_company);
	...
	function get_dp_finco($periode, $waktu, $id_finance_company, $range = null)
	{
		...
		$get_unit = $this->db->query("SELECT COUNT(no_mesin) AS jum FROM(
			SELECT *,FLOOR((dp_stor/total_bayar)*100) AS persen_dp FROM (
	...
	function getSalesFincoByDP()
	{
		...
		<td style="text-align:center;"><b>10-20%</b></td>
		...
		$dp_10   = $this->get_penjualan_dp_finco('range_tanggal', $tanggal_arr, null, [0, 9]);
		$dp_1020 = $this->get_penjualan_dp_finco('range_tanggal', $tanggal_arr, null, [10, 20]);
		$dp_20   = $this->get_penjualan_dp_finco('range_tanggal', $tanggal_arr, null, [21, 100]);
		echo '</tbody>';
		echo '<tfoot>';
		echo '<tr>';
		echo '<td style="text-align:center"><b>Total</td>';
	...
	function getSalesFincoByDPDealer()
	{
		...
		$tot = ['10' => 0, '1020' => 0, '20' => 0];
		foreach ($get_finco->result() as $fnc) {
			$dp_10   = $this->get_penjualan_dp_finco_dealer('range_tanggal', $tanggal_arr, $fnc->id_finance_company, [0, 9]);
			$dp_1020 = $this->get_penjualan_dp_finco_dealer('range_tanggal', $tanggal_arr, $fnc->id_finance_company, [10, 20]);
			$dp_20   = $this->get_penjualan_dp_finco_dealer('range_tanggal', $tanggal_arr, $fnc->id_finance_company, [21, 100]);
			if ($dp_10 > 0 or $dp_1020 > 0 or $dp_20 > 0) {
				$tot_dp = $dp_10 + $dp_1020 + $dp_20;
				$tot['10']   += $dp_10;
				$tot['1020'] += $dp_1020;
