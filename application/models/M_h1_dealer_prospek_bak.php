...
class M_h1_dealer_prospek extends CI_Model
{
  ...
  function getProspek($filter = NULL)
  {
    ...
    prov_kantor.provinsi AS prov_kantor,tr_prospek.id_event,($event) AS nama_event,tr_prospek.no_hp";
    if (isset($filter['select'])) {
      if ($filter['select'] == 'count') {
        ...
        test_ride_preference AS preferensi_uji_perjalanan,
    ...
    FROM tr_prospek 
      ...
      LEFT JOIN ms_sumber_prospek ps ON ps.id=tr_prospek.sumber_prospek
			$where
      $order
      $limit
			");
  ...
  function getProspekActivity($id_karyawan_dealer, $user, $honda_id = NULL)
  {
    ...
    return [
      ...
      'target'   => (int)$this->m_dms->getH1TargetManagement($filter_target_prospek)->row()->sum_prospek,
