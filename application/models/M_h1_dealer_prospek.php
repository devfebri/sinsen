...
class M_h1_dealer_prospek extends CI_Model
{
  ...
  function getProspek_leads_count($filter = NULL)
  {
    ...
    if (isset($filter['id_karyawan_dealer'])) {
      if ($filter['id_karyawan_dealer'] != '') {
        ...
        $where .= " AND tr_prospek.id_flp_md is null";
    }
    ...
    $count_leads = $this->db->query("SELECT 
    COUNT(1) as jumlah_leads,
    CASE when tr_prospek_fol_up.id_prospek is not null then 1 else NULL end as jumlah
    from tr_prospek 
    left join tr_prospek_fol_up  on tr_prospek.id_prospek = tr_prospek_fol_up.id_prospek 
    WHERE $where AND tr_prospek.input_from ='ldd'  AND tgl_prospek ='$currentDate' 
    having jumlah is null
  ...
  function getProspek($filter = NULL)
  {
    ...
    $id_status_fu   = 'SELECT id_status_fu   FROM tr_prospek_fol_up WHERE id_prospek=tr_prospek.id_prospek ORDER BY id DESC LIMIT 1';
    $last_kodeHasilStatusFollowUp   = 'SELECT kodeHasilStatusFollowUp   FROM tr_prospek_fol_up WHERE id_prospek=tr_prospek.id_prospek ORDER BY id DESC LIMIT 1';
    ...
    if (isset($filter['ontimeSLA2_desc'])) {
      if ($filter['ontimeSLA2_desc'] != '') {
        $where .= " AND ($ontimeSLA2_desc)='{$filter['ontimeSLA2_desc']}'";
      }
    }
    if (isset($filter['ontimeSLA2_multi'])) {
      if ($filter['ontimeSLA2_multi'] != '') {
        $ontimeSLA2_multi = arr_in_sql($filter['ontimeSLA2_multi']);
        $where .= " AND ($ontimeSLA2_desc) IN($ontimeSLA2_multi)";
      }
    }
    ...
    if (isset($filter['id_customer_in_spk'])) {
      ...
      if (isset($filter['belum_assign_sales_people'])) {
        if ($filter['belum_assign_sales_people'] != '') {
          $assign_sales =  $filter['belum_assign_sales_people'];
            if ($assign_sales == 1) {
              $where .= " AND tr_prospek.id_karyawan_dealer is NULL";
            }else{
              $assign_sales_in = arr_in_sql($filter['belum_assign_sales_people']);
              $where .= " AND tr_prospek.id_karyawan_dealer in  ($assign_sales_in)";
            }
        }
      }
    ...
    if (isset($filter['id_customer_in_spk_or_no_deal'])) {
      $where .="AND (id_customer IN (SELECT id_customer FROM tr_spk 
      JOIN tr_sales_order ON tr_sales_order.no_spk=tr_spk.no_spk
      WHERE tr_sales_order.status_delivery IS NOT NULL AND id_customer=tr_prospek.id_customer
       ) OR IFNULL((SELECT kodeHasilStatusFollowUp FROM tr_prospek_fol_up WHERE id_prospek=tr_prospek.id_prospek ORDER BY id DESC LIMIT 1),'')=4)";
    }


    if (isset($filter['belum_fu'])) {
      if ($filter['belum_fu'] != '') {
        $where .= " AND (SELECT COUNT(id) FROM tr_prospek_fol_up WHERE id_prospek=tr_prospek.id_prospek AND check_date IS NULL)>0";
      }
    }
    ...
    if (isset($filter['kodeHasilStatusFollowUpNot'])) {
      if ($filter['kodeHasilStatusFollowUpNot'] != '') {
        $where .= " AND IFNULL((SELECT kodeHasilStatusFollowUp FROM tr_prospek_fol_up WHERE id_prospek=tr_prospek.id_prospek ORDER BY id DESC LIMIT 1),'') != {$filter['kodeHasilStatusFollowUpNot']}";
      }
    }
    ...
    if (isset($filter['idTipeKendaraanIn'])) {
      $id_tipe_kendaraan_in = arr_in_sql($filter['idTipeKendaraanIn']);
      $where .= " AND tr_prospek.id_tipe_kendaraan IN ($id_tipe_kendaraan_in) ";
    }

    if (isset($filter['no_hp'])) {
      if ($filter['no_hp'] != '') {
        $where .= " AND tr_prospek.no_hp='{$filter['no_hp']}'";
      }
    }
    if (isset($filter['jumlah_fu'])) {
      if ($filter['jumlah_fu'] != '') {
        $where .= " AND ($jml_fu)='{$filter['jumlah_fu']}'";
      }
    }
    
    if (isset($filter['leads_idIn'])) {
      if ($filter['leads_idIn'] != '') {
        $leads_id = arr_in_sql($filter['leads_idIn']);
        $where .= " AND tr_prospek.leads_id IN ($leads_id)";
      }
    }
    ...
    //     $where .= " AND ('{$prd[0]}' BETWEEN event.start_date AND event.end_date OR '{$prd[1]}' BETWEEN event.start_date AND event.end_date)";
    //   }
    // }
    ...
    if (isset($filter['periode_event'])) {
      if ($filter['periode_event'] != '') {
        $prd = $filter['periode_event'];
        $where .= " AND (left(tr_prospek.created_at,12) BETWEEN '$prd[0]' AND '$prd[1]' )";
      }
    }
    
    if (isset($filter['deskripsiEventIn'])) {
      if ($filter['deskripsiEventIn'] != '') {
        $id_event = arr_in_sql($filter['deskripsiEventIn']);
        $where .= " AND tr_prospek.id_event IN ($id_event)";
      }
    }
    
    if (isset($filter['periode_next_fu'])) {
      if ($filter['periode_next_fu'] != '') {
        $prd    = $filter['periode_next_fu'];
        $where .= " AND ($tgl_next_fol_up) BETWEEN '{$prd[0]}' AND '{$prd[1]}'";
      }
    }

    if (isset($filter['show_hasil_fu_not_deal'])) {
      $fs = $filter['show_hasil_fu_not_deal'];
      $where .= " AND 
                      CASE WHEN $fs=0 THEN
                        CASE WHEN IFNULL(($last_kodeHasilStatusFollowUp),0)=4 THEN 0 ELSE 1 END 
                      ELSE 1
                      END = 1  
                      ";
    }
    ...
    if (isset($filter['kodeHasilStatusFollowUpIn'])) {
      if ($filter['kodeHasilStatusFollowUpIn'] != '') {
        $kode_hasil = arr_in_sql($filter['kodeHasilStatusFollowUpIn']);
        $where .= " AND ($last_kodeHasilStatusFollowUp) IN ($kode_hasil)";
      }
    }

    if (isset($filter['sourceLeadsIn'])) {
      if ($filter['sourceLeadsIn'] != '') {
        $kode_hasil = arr_in_sql($filter['sourceLeadsIn']);
        $where .= " AND tr_prospek.sumber_prospek IN ($kode_hasil)";
      }
    }
    if (isset($filter['platformDataIn'])) {
      if ($filter['platformDataIn'] != '') {
        $platform = arr_in_sql($filter['platformDataIn']);
        $where .= " AND tr_prospek.platformData IN ($platform)";
      }
    }

    if (isset($filter['id_status_fu_in'])) {
      if ($filter['id_status_fu_in'] != '') {
        $fu = arr_in_sql($filter['id_status_fu_in']);
        $where .= " AND ($id_status_fu) IN ($fu)";
      }
    }
    if (isset($filter['search'])) {
      ...
      if ($search != '') {
        $where .= " AND (tr_prospek.nama_konsumen LIKE '%$search%'
                            ...
                            OR tr_prospek.leads_id like '%$search%'
                            ) 
            ";
      }
    }
    ...
    if (isset($filter['order'])) {
      ...
      if ($order != '') {
        if (isset($order['field'])) {
          ...
          $order_column = [null,'tr_prospek.id_prospek','id_list_appointment','tr_prospek.leads_id','nama_konsumen','nama_lengkap','tr_prospek.created_at','plf.platform_data','CASE WHEN ps.id IS NULL THEN tr_prospek.sumber_prospek ELSE ps.description END','nama_event','event.start_date',"($id_status_fu)","CASE WHEN ($pernahTerhubung) = 4 THEN 'Ya' ELSE 'Tidak' END","($hasil_fu)","($jml_fu)","($tgl_next_fol_up)","tr_prospek.updated_at","($ontimeSLA2_desc)"];
    ...
    LEFT(tr_prospek.created_at,10) tgl_assign,RIGHT(tr_prospek.created_at,8) jam_assign,($ontimeSLA2_desc)  ontimeSLA2_desc, CASE WHEN ($pernahTerhubung) = 4 THEN 'Ya' ELSE 'Tidak' END pernahTerhubung,($hasil_fu) hasil_fu,($jml_fu) jml_fu,($tgl_next_fol_up) tgl_next_fol_up,($id_status_fu) id_status_fu
    ";
    if (isset($filter['select'])) {
      ...
      } else if($filter['select'] == 'select_view'){
        $select = "tr_prospek.id_prospek,tr_prospek.id_list_appointment,tr_prospek.leads_id, tr_prospek.nama_konsumen, tr_prospek.input_from,
          ms_karyawan_dealer.nama_lengkap, tr_prospek.created_at, tr_prospek.updated_at,
          CASE WHEN tr_prospek.customerType='R' THEN 'Reguler' ELSE 'VIP' END AS customerTypeDesc,
          kel.kelurahan AS kel,
          kec.kecamatan AS kec,
          kab.kabupaten AS kab,
  ...
  function getIDProspek($sumber_prospek, $id_dealer = NULL, $is_mobile = false)
  {
    ...
    $get_data  = $this->db->query("SELECT id_prospek FROM tr_prospek
			...
			AND created_at between '$tanggal_awal' and '$tanggal_akhir 23:59:59'
  ...
  function getProspekGCDetail($filter)
  {
    ...
    // perlu ditambahkan table ms_kelompok_md_harga utk ambil tgl approve
    ...
    WHERE 
    ms_kelompok_md.id_item = itm.id_item and ms_kelompok_md.start_date <= '$hari_ini'
  ...
  function getProspekFollowUpCrm($id_prospek)
  {
    ...
    foreach ($data as $dt) {
      ...
      $dt->deskripsiHasilStatusFollowUp = '';
      if ($hasil!=null) {
          $dt->deskripsiHasilStatusFollowUp = isset($hasil->deskripsiHasilStatusFollowUp)?$hasil->deskripsiHasilStatusFollowUp:'';
      }
  ...
  function getProspekHistoryAssigned($id_prospek)
  {
    return $this->db->query("SELECT phr.*,als.name alasan,
        CONCAT(kry_baru.id_flp_md,' - ',kry_baru.nama_lengkap) nama_sales_baru,
        CONCAT(kry_lama.id_flp_md,' - ',kry_lama.nama_lengkap) nama_sales_lama
        FROM tr_prospek_history_reassign phr
        JOIN sc_ms_alasan_reassign als ON als.id=phr.alasan_reassign_id
        JOIN ms_karyawan_dealer kry_baru ON kry_baru.id_karyawan_dealer=phr.id_karyawan_dealer_baru
        JOIN ms_karyawan_dealer kry_lama ON kry_lama.id_karyawan_dealer=phr.id_karyawan_dealer_lama
        WHERE id_prospek='$id_prospek'")->result();
  }
}
