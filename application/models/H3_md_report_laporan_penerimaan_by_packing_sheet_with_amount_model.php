...
class H3_md_report_laporan_penerimaan_by_packing_sheet_with_amount_model extends Honda_Model {
    ...
    public function generate($periode_awal = null, $periode_akhir = null, $no_penerimaan_barang = null){
        ...
        foreach($data as $row){
            ...
            foreach($row['items'] as $item){
                ...
                $this->excel->setActiveSheetIndex(0)->setCellValue(sprintf('L%s', $start_row), $item['qty_diterima']);
                $this->excel->setActiveSheetIndex(0)->setCellValue(sprintf('M%s', $start_row), $item['het']);
                $this->excel->setActiveSheetIndex(0)->setCellValue(sprintf('N%s', $start_row), $item['hpp']);
                $this->excel->setActiveSheetIndex(0)->setCellValue(sprintf('O%s', $start_row), $item['jumlah_harga']);
                $this->excel->setActiveSheetIndex(0)->setCellValue(sprintf('P%s', $start_row), $item['kelompok_part']);
    ...
    public function download(){
        ...
        ob_end_clean();
		$filename = 'Report Laporan Penerimaan Barang by Packing Sheet & Amount';
    ...
    public function generatePdf($periode_awal = null, $periode_akhir = null, $no_penerimaan_barang = null){
        ...
        $html = $this->load->view('h3/h3_md_laporan_penerimaan_by_packing_sheet_dan_amount', [
			'data' => $data,
    ...
    private function get_data($periode_awal = null, $periode_akhir = null, $no_penerimaan_barang = null){
        ...
        $data = array_map(function($row){
            $nomor_invoice = $this->db
            ->select('fdo_ps.invoice_number')
            ->from('tr_h3_md_fdo_ps as fdo_ps')
            ->where('fdo_ps.packing_sheet_number = pbi.packing_sheet_number', null, false)
            ->get_compiled_select();
            ...
            ->join('tr_h3_md_fdo_parts as fdo_parts', "(fdo_parts.id_part = pbi.id_part AND fdo_parts.nomor_packing_sheet = pbi.packing_sheet_number)", 'left')
            ...
            // ->where("fdo_parts.invoice_number IN ({$nomor_invoice})", null, false)
