<?php
	...
	class H2_md_report_h23_model extends CI_Model{
		...
		// public function getDataDealerWhere($id_dealer){
		// 	$filter_dealer = '';
        //   		if ($id_dealer=='all') {
        //    			$filter_dealer = " AND b.id_dealer='103'";
        //  		}elseif($id_dealer!='all'){
		...
		public function downloadReportAhass($id_dealer,$start_date,$end_date){
			...
			$all_ahass = $this->db->query("SELECT md.nama_dealer, wo.id_dealer, SUM(CASE WHEN jasa.id_type='ASS1' then 1 ELSE 0 end )as total_ass1,
				...
				where wo.status='Closed' and wo.created_njb_at>='$start_date 00:00:00' and wo.created_njb_at<='$end_date 23:59:59' and wop.pekerjaan_batal=0 $filter_dealer
		...
		public function hariKerja($id_dealer, $start_date,$end_date){
			$filter_dealer = '';
          		if ($id_dealer!='all') {
           			$filter_dealer = " AND a.id_dealer='$id_dealer'";
         		}

			$hariKerja = $this->db->query("SELECT id_dealer , count(tanggal) as tgl
			 FROM tr_h2_absen_mekanik a 
			 where tanggal >='$start_date' and tanggal<='$end_date' $filter_dealer
			 group by id_dealer");
		...
		public function jumlahUE($id_dealer,$start_date,$end_date){
			...
			where b.status='closed' and left(b.created_njb_at,10)>='$start_date' and left(b.created_njb_at,10)<='$end_date'  $filter_dealer
		...
		public function downloadReportTransaksional_include_fed_oil($id_dealer,$start_date,$end_date){

			$filter_dealer = '';
          		if ($id_dealer!='all') {
           			$filter_dealer = " AND b.id_dealer='$id_dealer'";
         		}
			$transaksi = $this->db->query("SELECT a.kode_dealer_ahm, a.kode_dealer_md, b.id_work_order, b.created_at, b.start_at, h.harga as harga_jasa, c.harga as harga_part, (ifnull(h.harga,0) + ifnull(c.harga,0)) as grand_total, e.nama_part, c.id_part, d.deskripsi, upper(replace((case when i.no_mesin is not null then i.no_mesin else g.no_mesin end),' ','')) as no_mesin, b.no_njb, b.created_njb_at, g.no_polisi, g.id_tipe_kendaraan, f.no_claim_c2, b.tipe_pembayaran, f.alasan_ke_ahass, j.name , k.keterangan, 
			upper((case when i.no_rangka is not null then i.no_rangka else g.no_rangka end)) as no_rangka,
			(case when b.start_at is not null and b.status = 'open' then 'start' else b.status end) as status, h.diskon_value as diskon_jasa, c.diskon_value as diskon_part, l.id_flp_md as id_mekanik, l.nama_lengkap as nama_mekanik, m.id_flp_md as id_sa, m.nama_lengkap as nama_sa, left(timediff(right(b.closed_at ,8),right(b.start_at ,8)),8) as durasi_pekerjaan 
			-- (case when pos.id_dealer is null then '-' else pos.nama_pos end) as pos		 	
							...
							LEFT JOIN ms_part AS e ON e.id_part  = c.id_part 
							...
							WHERE left(b.created_njb_at,10)>='$start_date' and left(b.created_njb_at,10)<='$end_date' and b.status in ('closed','cancel','canceled') $filter_dealer order by a.nama_dealer asc, b.created_njb_at asc, d.deskripsi asc");
		...
		public function downloadReportTransaksional($id_dealer,$start_date,$end_date){
			...
			-- (case when pos.id_dealer is null then '-' else pos.nama_pos end) as pos		 	
							...
							WHERE b.created_njb_at>='$start_date 00:00:00' and b.created_njb_at<='$end_date 23:59:59' and b.status in ('closed','cancel','canceled') and h.pekerjaan_batal=0 $filter_dealer order by a.nama_dealer asc, b.closed_at asc, d.deskripsi asc");
		...
		public function pendapatanJasa($id_dealer,$start_date,$end_date){
		   ...
		   $jasa = $this->db->query("SELECT 
									...
									WHERE C.status='closed'AND left(C.created_njb_at,10)>='$start_date' and left(C.created_njb_at,10)<='$end_date' $filter_dealer")->row();
		   return $jasa;
		}
		...
		public function activityPromotion($id_dealer,$start_date,$end_date){
		   ...
		   $activityPromotion = $this->db->query("SELECT c.activity_promotion_id, j.name as activity_promotion,
						...
						WHERE b.status='closed' and b.created_njb_at>='$start_date 00:00:00' and b.created_njb_at<='$end_date 23:59:59' $filter_dealer
						GROUP BY j.name
						ORDER BY j.name asc");
		...
		public function dataTOJ11($id_dealer, $start_date,$end_date){
			...
			$toj11 = $this->db->query("SELECT md.nama_dealer, wo.id_dealer, SUM(CASE WHEN sa.id_type='ASS1' then 1 ELSE 0 end )as total_ass1,
					...
					JOIN ms_dealer md on md.id_dealer = wo.id_dealer 
					where wo.status='Closed' and wo.created_njb_at>='$start_date 00:00:00' and wo.created_njb_at<='$end_date 23:59:59' $filter_dealer
					group by wo.id_dealer
					ORDER BY md.nama_dealer asc");
		...
		public function JasaRevenue($id_dealer,$start_date,$end_date){
		   ...
		   $jasaRev = $this->db->query("SELECT D.nama_dealer,
									...
									JOIN ms_dealer D ON D.id_dealer = C.id_dealer
	   								...
	   								AND C.created_njb_at>='$start_date 00:00:00' and C.created_njb_at<='$end_date 23:59:59' 
									AND B.pekerjaan_batal=0 $filter_dealer
		...
		public function partRevenue($id_dealer,$start_date,$end_date){
			...
			// $partRevSales = $this->db->query("SELECT d.nama_dealer, SUM(CASE WHEN c.kelompok_part in('ACB','ACC','ACG','AH','AHM','BB','BBIST','BM1','BR','BRNG',
			...
			// 'SPOKE','STR','TAS','TDI','TR','VALVE','VV') then a.tot_nsc ELSE 0 END) AS spart,
			...
			//    SUM(CASE WHEN c.kelompok_part ='PACC' then a.tot_nsc ELSE 0 END) AS accessories,
			//    SUM(CASE WHEN c.kelompok_part in('TB','TB1','TBHGP','TBVL','TIRE','TIRE1') then a.tot_nsc ELSE 0 END) AS ban,
			//    SUM(CASE WHEN c.kelompok_part ='BATT' then a.tot_nsc ELSE 0 END) AS aki,
			//    SUM(CASE WHEN c.kelompok_part in('SPLUR','SP','SPLUG') then a.tot_nsc ELSE 0 END) AS busi,
			//    SUM(CASE WHEN c.kelompok_part ='BLDRV' then a.tot_nsc ELSE 0 END) AS belt,
			//    SUM(CASE WHEN c.kelompok_part ='BRAKE' then a.tot_nsc ELSE 0 END) AS brake,
			//    SUM(CASE WHEN c.kelompok_part ='PLAST' then a.tot_nsc ELSE 0 END) AS plastic_part,
			//    SUM(CASE WHEN c.kelompok_part ='EC' then a.tot_nsc ELSE 0 END) AS element_cleaner,
			//    SUM(CASE WHEN c.kelompok_part in('ACCEC','TL','FED OIL','HELM','CTHV1','OTHERS') then a.tot_nsc ELSE 0 END) AS other
			//   from tr_h23_nsc a
			...
			//   where left(a.created_at ,10)>='$start_date' and left(a.created_at,10)<='$end_date' $filter_dealer
			//   group by a.id_dealer  
			//   order by d.nama_dealer asc ");
			$partRevSales = $this->db->query(" SELECT d.nama_dealer,
			...
			WHEN b.tipe_diskon='Value' then ((b.harga_beli*b.qty)-b.diskon_value) ELSE b.harga_beli*b.qty END) ELSE 0 END) AS fed
			  ...
			  where a.created_at>='$start_date 00:00:00' and a.created_at<='$end_date 23:59:59' $filter_dealer
			  group by a.id_dealer  
			  order by d.nama_dealer asc ");
			return $partRevSales;
		}
		...
		public function salesIn($id_dealer,$start_date,$end_date){
		   ...
		   WHERE ps.tgl_faktur >='$start_date 00:00:00' and ps.tgl_faktur <='$end_date 23:59:59' $filter_dealer 
		...
		public function mekanikPosServiceRev($id_dealer,$start_date,$end_date,$type){
		   ...
		   $mekanikposService = $this->db->query("SELECT a.nama_dealer, h.nama_lengkap as nama_mekanik, SUM(b.grand_total) as revenue_mekanik
							...
							JOIN tr_h2_wo_dealer b on a.id_dealer = b.id_dealer
							JOIN ms_karyawan_dealer h on b.id_karyawan_dealer =h.id_karyawan_dealer		
							WHERE left(b.created_njb_at,10) between '$start_date' and '$end_date' $filter_pos $filter_dealer
							GROUP BY b.id_karyawan_dealer
							ORDER BY b.id_karyawan_dealer asc");
		...
		public function saPosServiceRev($id_dealer,$start_date,$end_date,$type){
		   ...
		   $saposService = $this->db->query("SELECT a.nama_dealer, SUM(b.grand_total) as revenue_sa, h.nama_lengkap as nama_sa
						FROM ms_dealer a 
						JOIN tr_h2_wo_dealer b on a.id_dealer = b.id_dealer
						JOIN ms_user c on c.id_user = b.created_by 
		...
		public function activityPromotionRev($id_dealer,$start_date,$end_date){
		   ...
		   $apRev = $this->db->query("SELECT sum(b.grand_total) as revenue_ap,j.name 
						...
						JOIN dms_ms_activity_promotion j on j.id = c.activity_promotion_id
						where left(b.created_njb_at,10) between '$start_date' and '$end_date' $filter_dealer
		...
		public function activityPromotionRev2($id_dealer,$start_date,$end_date){
			...
			SUM(CASE WHEN a.activity_promotion_id ='15' THEN b.grand_total ELSE 0 END) AS home_care
				...
				where left(b.created_njb_at,10)>='$start_date' and left(b.created_njb_at,10)<='$end_date' and b.status='closed' $filter_dealer
				group by b.id_dealer 
				order by d.nama_dealer asc ");
		   return $apRev2;
		}
		...
		public function dataMekanik($id_dealer, $start_date, $end_date){
		   ...
		   $dataMekanik=$this->db->query("SELECT DISTINCT b.id_karyawan_dealer, c.id_flp_md,b.id_absen, c.nama_lengkap, d.nama_dealer, sum(case when b.aktif=1 then 1 else 0 end) as kehadiran, sum(case when b.aktif=0 then 1 else 0 end) as tidak_hadir, a.id_dealer, ('16:30:00'-DATE_FORMAT(a.created_at,'%h:%m:%s')) as waktu
						...
						join ms_dealer d on d.id_dealer = a.id_dealer 
						where a.tanggal >= '$start_date' and a.tanggal <= '$end_date' $filter_dealer
						group by c.id_karyawan_dealer");
						return $dataMekanik;
		}

		// public function distinctAHASS($id_dealer,$start_date,$end_date){
		// 	$filter_dealer = '';
		// 	if ($id_dealer!='all') {
		// 		 $filter_dealer = " AND b.id_dealer='$id_dealer'";
		//    }

		// 	$dataAHASS=$this->db->query("SELECT a.nama_dealer, count(DISTINCT c.id_karyawan_dealer) as jmlh_mekanik FROM ms_dealer a 
		// 	left join tr_h2_absen_mekanik b on a.id_dealer=b.id_dealer 
		// 	left join tr_h2_absen_mekanik_detail c on b.id_absen = c.id_absen
		// 	where b.tanggal >= '$start_date' and b.tanggal <= '$end_date' $filter_dealer
		// 	group by b.id_karyawan_dealer");
		// 	return $dataAHASS;
		// }
		...
		public function dataSegmentMotor($id_dealer,$start_date,$end_date){
			$filter_dealer = '';
			   ...
			   $ue_segment=$this->db->query("SELECT a.nama_dealer,
						...
						where left(b.created_njb_at,10)>= '$start_date' and left(b.created_njb_at,10)<= '$end_date' $filter_dealer
						group by b.id_dealer
		...
		public function dataSegmentMotor3($id_dealer,$start_date,$end_date){
			$filter_dealer = '';
			   ...
			   $ue_segment3=$this->db->query("SELECT DISTINCT h.id_series, b.id_dealer
						...
						LEFT JOIN ms_customer_h23 d on c.id_customer = d.id_customer
						JOIN ms_tipe_kendaraan h on d.id_tipe_kendaraan=h.id_tipe_kendaraan
						where left(b.created_njb_at,10) between '$start_date' and '$end_date' and h.id_series!='' $filter_dealer
		...
		public function dataUEperJam($id_dealer,$start_date,$end_date){
			...
			$ue_jam=$this->db->query("SELECT a.nama_dealer,
						...
						JOIN tr_h2_wo_dealer b on a.id_dealer = b.id_dealer
						JOIN tr_h2_sa_form c on c.id_sa_form=b.id_sa_form
						where b.status='closed' and b.created_njb_at >= '$start_date 00:00:00' and b.created_njb_at <='$end_date 23:59:59' $filter_dealer")->row();
   			return $ue_jam;
		}

		public function get_peformance_fd_sa($id_dealer,$start_date,$end_date){
			$get_data = $this->db->query("
				select a.id_work_order , a.no_njb , d.nama_lengkap as nama_sa ,  c.no_nsc , a.total_jasa , a.total_part, IFNULL(c.tot_nsc_part,0) as total_nsc_part, IFNULL(c.tot_nsc_oli,0) as total_nsc_oli, a.created_at as create_wo , a.closed_at as close_wo, 
				...
				left join ms_user g on g.id_user =  (case when a.cetak_njb_by is null then a.cetak_gab_by else a.cetak_njb_by end)
				left join ms_karyawan_dealer h on h.id_karyawan_dealer = g.id_karyawan_dealer 
				where date(a.closed_at) between '$start_date' and '$end_date' and a.id_dealer = $id_dealer
			");
		...
		public function report_picking_slip($id_dealer,$start_date,$end_date){
			$filter_dealer = '';
				if ($id_dealer!='all') {
				 	$filter_dealer = " AND ps.id_dealer='$id_dealer'";
		   	}
			...
			where ps.created_at>='$start_date 00:00:00' and ps.created_at<='$end_date 23:59:59' $filter_dealer
		...
		public function report_wo_gantung($id_dealer,$start_date,$end_date){
			...
			wo.id_work_order, sa.id_sa_form, DATE_FORMAT(sa.tgl_servis,'%d-%m-%Y') as tgl_servis, 
				...
				(SELECT stats FROM tr_h2_wo_dealer_waktu WHERE id_work_order=wo.id_work_order ORDER BY set_at DESC LIMIT 1) AS last_stats ,
				jasa.deskripsi as nama_jasa, jasa.id_type, wops.id_part, wops.qty, mp.nama_part,DATE_FORMAT(wo.created_at,'%d-%m-%Y') as tgl_wo, md.nama_dealer
				from tr_h2_wo_dealer wo
				...
				JOIN ms_dealer md on md.id_dealer =wo.id_dealer
				where wo.id_work_order IS NOT NULL  and wo.status not in ('Closed','Canceled','Cancel') $filter_dealer
