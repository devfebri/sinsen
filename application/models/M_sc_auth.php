...
class M_sc_auth extends CI_Model
{
  ...
  {
    $where = "WHERE akses_sc='1' AND akses_sc=1 AND kd.active=1";
  ...
  function validasiToken($filter = null)
  {
    ...
    if ($filter != null) {
      ...
      if (isset($filter['token'])) {
        $user_agent = get_user_agent();
        // $where .= " AND token='{$filter['token']}' AND user_agent='$user_agent'

        // hapus cek user_agent karna sering terjadi bug
        $where .= " AND token='{$filter['token']}'
        ";
      } else {
  ...
  function cekToken($filter = null)
  {
    ...
    if ($filter != null) {
      $where = "WHERE 1=1 ";
      if (isset($filter['token'])) {
        $where .= " AND token='{$filter['token']}' AND logout_at IS NULL
        ";
      } else {
        $where .= " AND 0=1 "; //disabled untuk testing monju
      }
    ...
    $result = $this->db->query("SELECT $select
    FROM sc_api_token sc_tk
    LEFT JOIN ms_user usr ON usr.regid=sc_tk.regid
    LEFT JOIN ms_karyawan_dealer kry ON kry.id_karyawan_dealer=usr.id_karyawan_dealer
    LEFT JOIN ms_user usr_r ON usr_r.id_user=sc_tk.id_user_renew
    LEFT JOIN ms_karyawan_dealer kry_r ON kry_r.id_karyawan_dealer=usr_r.id_karyawan_dealer
    $where
  ...
  {
    $validate = $this->cekToken($filter);
    if ($validate) {
      $upd['expired_datetime'] = converted_datetime(waktu_full());
      $upd['expired_at'] = strtotime($upd['expired_datetime']);
      $this->db->update('sc_api_token', $upd, ['token' => $filter['token']]);
      $upd['token'] = $filter['token'];
      return $upd;
    }
  }
