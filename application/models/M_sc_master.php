...
class M_sc_master extends CI_Model
{
  ...
  function getCustomer($filter = NULL)
  {
    ...
    if ($filter != NULL) {
      if (isset($filter['id_dealer'])) {
        $where .= " AND prp.id_dealer='{$filter['id_dealer']}'";
      }
    ...
    $where 
    ORDER BY prp.nama_konsumen ASC
    $limit
    ");
  }
  ...
  {
    $where = "WHERE 1=1 and kec.id_kabupaten >=1500 and kec.id_kabupaten <=1600 and kec.id_kabupaten not in ('1501','1572')";
    $limit = '';
    if ($filter != NULL) {
      if (isset($filter['search'])) {
        if ($filter['search'] != '') {
  ...
  function getMotorType($filter = NULL)
  {
    ...
    if ($filter != NULL) {
      ...
      if (isset($filter['search'])) {
        if ($filter['search'] != '') {
          $where .= " AND (jenis_sebelumnya LIKE '%{$filter['search']}%'
                        )
        ";
        }
      }
  ...
  function getPekerjaan($filter = NULL)
  {
    ...
    if ($filter != NULL) {
      if (isset($filter['id_sub_pekerjaan'])) {
        if ($filter['id_sub_pekerjaan'] != '') {
          $is_api = 0;
          $id_sub_pekerjaan = preg_replace("/[^0-9]/", "", $filter['id_sub_pekerjaan']);
          $where .= " AND id_sub_pekerjaan='$id_sub_pekerjaan'";
        }
      }

      if (isset($filter['search'])) {
        if ($filter['search'] != '') {
          $where .= " AND (id_sub_pekerjaan LIKE '%{$filter['search']}%'
                        OR sub_pekerjaan LIKE '%{$filter['search']}%'
                        )
        ";
        }
      }
    ...
    $result = $this->db->query("SELECT 
      CASE WHEN $is_api=1 THEN
        CASE WHEN id_sub_pekerjaan='101' THEN 'ID_101' ELSE id_sub_pekerjaan END
      ELSE id_sub_pekerjaan END id,
      sub_pekerjaan name,
      id_pekerjaan,
      required_instansi is_address_instansi 
    FROM ms_sub_pekerjaan pkj
    $where 
    ORDER BY sub_pekerjaan ASC
    $limit
    ");
  ...
  function getElearning($filter)
  {
    ...
    if ($filter != NULL) {
      ...
      if (isset($filter['role'])) {
        if ($filter['role'] != '') {
          if ($filter['role'] == 'SP') {
            $filter['role'] = 'sales_person';
          } elseif ($filter['role'] == 'SC') {
            $filter['role'] = 'sales_coordinator';
          } elseif ($filter['role'] == 'BM') {
            $filter['role'] = 'branch_manager';
          }
