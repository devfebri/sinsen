...
class M_h1_dealer_sales_order extends CI_Model
{
  ...
  function getSalesOrderIndividu($filter = NULL)
  {
    ...
    $where = "WHERE so.id_dealer = '$id_dealer' ";
    $status_on_sc = "CASE WHEN so.status_close='closed' THEN 'Complete' ELSE 'Incomplete' END";
    ...
    if (isset($filter['bulan_so'])) {
      if ($filter['bulan_so'] != '') {
        $where .= " AND (LEFT(so.created_at,7)='{$filter['bulan_so']}' OR LEFT(so.tgl_cetak_invoice,7)='{$filter['bulan_so']}')";
      }
    }
    //cek all bulan di tahun saat ini
    if (isset($filter['all_bulan_so'])) {
      if ($filter['all_bulan_so'] != '') {
        if ( $filter['all_bulan_so'] == '2022') {
          $where .= " AND ( ( LEFT(so.tgl_cetak_invoice,7) BETWEEN '2022-03' AND '2022-12' ) )";
        } else {
          $where .= " AND (LEFT(so.created_at,4)='{$filter['all_bulan_so']}' OR LEFT(so.tgl_cetak_invoice,4)='{$filter['all_bulan_so']}')";
        }
        
      }
    }

    if (isset($filter['periode_so'])) {
      $periode = $filter['periode_so'];
      $where .= " AND (
                      (LEFT(so.created_at,10) BETWEEN '{$periode[0]}' AND '{$periode[1]}' )
                      OR 
                      (LEFT(so.tgl_cetak_invoice,10) BETWEEN '{$periode[0]}' AND '{$periode[1]}')
                      )";
    }
    if (isset($filter['bulan_so_only'])) {
      if ($filter['bulan_so_only'] != '') {
        $where .= " AND LEFT(so.created_at,7)='{$filter['bulan_so_only']}'";
      }
    }
  ...
  {
    $where = 'WHERE 1=1 ';
    $select="so_nosin.*,sc.no_rangka,tk.id_tipe_kendaraan,wr.id_warna,CONCAT(tk.id_tipe_kendaraan,' | ',tk.tipe_ahm) AS tipe_kendaraan, CONCAT(wr.id_warna,' | ',wr.warna) AS warna_kendaraan,pd.id_master_plat,pd.driver,status_delivery,delivery_document_id";
    ...
    if (isset($filter['id_dealer'])) {
      if ($filter['id_dealer'] != '') {
        $where .= " AND so_gc.id_dealer='{$filter['id_dealer']}'";
      }
    }
    
    if (isset($filter['bulan_so'])) {
      if ($filter['bulan_so'] != '') {
        $where .= " AND LEFT(so_gc.created_at,7)='{$filter['bulan_so']}'";
      }
    }
    ...
    if (isset($filter['all_bulan_so'])) {
      if ($filter['all_bulan_so'] != '') {
        ...
        if ($filter['all_bulan_so'] == '2022') {
          $where .= " AND LEFT(so_gc.created_at,7) BETWEEN '2022-03' AND '2022-12' ";
        } else {
          $where .= " AND LEFT(so_gc.created_at,4)='{$filter['all_bulan_so']}'";
        }
      }
    }
    ...
    if (isset($filter['tgl_cetak_invoice_not_null'])) {
      if ($filter['tgl_cetak_invoice_not_null'] != '') {
        $where .= " AND IFNULL(so_gc.tgl_cetak_invoice,'') != ''";
      }
    }

    if (isset($filter['select'])) {
      if ($filter['select']=='count') {
        $select = "COUNT(so_nosin.id_sales_order_gc) count";
      }
    }

    $result = $this->db->query("SELECT $select
            FROM tr_sales_order_gc_nosin so_nosin
            JOIN tr_sales_order_gc so_gc ON so_gc.id_sales_order_gc=so_nosin.id_sales_order_gc
  ...
  function getSalesOrderActivity($id_karyawan_dealer, $user, $honda_id)
  {
    ...
    $so_target=$this->m_dms->getH1TargetManagement($filter_target_sales)->row()->sum_sales;
    return [
      'actual' => (int)$actual_sales,
      'target' => (int)$so_target,
      'tunai' => (int)$so_tunai,
      'kredit' => (int)$so_kredit,
    ];
  }

  function getRankSalesPeopleOnTeam($filter, $point_new = false)
  {
    ...
    foreach ($karyawan->result() as $kd) {
      ...
      $sales_gc = $this->getSalesOrderGCNoMesin($filter_actual_sales)->row()->count;

      //hitung point new
      $point = 0;
      $point_tmp = (int)$sales+$sales_gc;

      if ($point_new == true) {
        if ( get_id_jabatan($kd->id_karyawan_dealer) == 'JBT-063' OR 
             get_id_jabatan($kd->id_karyawan_dealer) == 'JBT-064' OR 
             get_id_jabatan($kd->id_karyawan_dealer) == 'JBT-065' 
        ) {
          $point = $point_tmp / 10;
        } else {
          $point = $point_tmp / 3;
  ...
  function getAllSalesOrderTotalForHomeApk($id_karyawan_dealer, $user, $honda_id)
  {
    $this->load->model('m_dms');
    ...
    $actual_sales = $this->getSalesOrderIndividu($filter_actual_sales)->row()->count;
    $sales_gc = $this->getSalesOrderGCNoMesin($filter_actual_sales)->row()->count;
    $actual_sales = $actual_sales + $sales_gc;
    ...
    $so_tunai = $this->getSalesOrderIndividu($f_so)->row()->count;
    $sales_gc = $this->getSalesOrderGCNoMesin($f_so)->row()->count;
    $so_tunai = $so_tunai+$sales_gc;
