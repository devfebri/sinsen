...
function cetakSPK($id, $save_server = NULL)
  {
    ...
    if ($spk->num_rows() > 0) {
      ...
      if ($row->jenis_beli == 'Cash') {
        ...
        $kerja        = $this->db->query("SELECT pekerjaan FROM ms_pekerjaan WHERE id_pekerjaan = '$row->pekerjaan'");
        if ($kerja->num_rows() > 0) {
          $tr = $kerja->row();
        ...
        $jabat        = $this->db->query("SELECT jabatan FROM ms_jabatan WHERE id_jabatan = '$row->jabatan'");
        if ($jabat->num_rows() > 0) {
          $tr = $jabat->row();
          $jabatan = $tr->jabatan;
        } else {
        ...
        $pdf->Cell(2, 5, ':', 0, 0, 'L');
        $pdf->Cell(55, 5, 'Rp. ' . $this->mata_uang($row->dp_stor - $return['diskon']) . ' (UANG MUKA / DP YANG DISETOR KONSUMEN)', 0, 1, 'L');
        $pdf->Cell(33, 5, 'Voucher', 0, 0, 'L');
        $pdf->Cell(2, 5, ':', 0, 0, 'L');
  ...
  function cetakSPK_old($id, $save_server = NULL)
  {
    $spk = $this->db->query("SELECT * FROM tr_spk 
						left join ms_tipe_kendaraan on tr_spk.id_tipe_kendaraan = ms_tipe_kendaraan.id_tipe_kendaraan
						left join ms_warna on tr_spk.id_warna = ms_warna.id_warna
