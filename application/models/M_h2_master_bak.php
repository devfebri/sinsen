...
class M_h2_master extends CI_Model
{
  ...
  public function fetch_jasa_h2_dealer($filter)
  {
  ...
  function get_booking($filter = null)
  {
    ...
    return $this->db->query("SELECT ms_customer_h23.*,tr_h2_manage_booking.*,CONCAT(ms_customer_h23.id_tipe_kendaraan,' | ',tipe_ahm) AS tipe_ahm, CONCAT(ms_customer_h23.id_warna,' | ',warna) AS warna, id_sa_form
  ...
  function wo_detail($filter = null)
  {
    ...
    foreach ($details->result() as $rs) {
      ...
      $rs->parts = $this->db->query("SELECT tr_h2_wo_dealer_parts.*,
      CASE
  ...
  function getPekerjaanWO($id_work_order)
  {
    ...
    $diskon AS diskon_rp,(wdp.harga-$diskon) AS harga_net,subtotal,wdp.pekerjaan_batal
  ...
  function getPartsWO($id_work_order)
  {
    ...
    return $this->db->query("SELECT wps.id_part,qty,wps.harga,nama_part,wps.id_gudang,wps.id_jasa,wps.id_rak,
      ...
      END AS picking,wps.id_promo,wps.diskon_value,wps.tipe_diskon,ms_part.id_part_int 
  ...
  public function get_id_customer($filter = NULL)
  {
    ...
    $get_data  = $this->db->query("SELECT * FROM ms_customer_h23
			WHERE id_dealer='$id_dealer'
  ...
  public function get_id_booking()
  {
    ...
    $ymd     = date('Y-m-d');
    $ymd2     = date('ymd');
    $get_data  = $this->db->query("SELECT * FROM tr_h2_manage_booking
  ...
  public function get_id_kirim_part()
  {
    ...
    $get_data  = $this->db->query("SELECT * FROM tr_h2_kirim_ke_part_counter where id_dealer='$id_dealer'
			-- WHERE LEFT(created_at,10)='$ymd' 
      ORDER BY created_at DESC LIMIT 0,1");
  ...
  public function get_id_satisfaction()
  {
    ...
    $get_data  = $this->db->query("SELECT * FROM tr_h2_service_satisfaction
  ...
  function getWOSatisfaction($filter)
  {
    ...
    if ($filter != null) {
      if (isset($filter['id_work_order'])) {
        $where .= " AND sat.id_work_order='{$filter['id_work_order']}' ";
      }
    }
    return $this->db->query("SELECT *
      FROM tr_h2_service_satisfaction AS sat
      JOIN tr_h2_wo_dealer AS wo ON wo.id_work_order=sat.id_work_order      
      $where
  ...
  public function get_id_pembawa($id_dealer = NULL)
  {
    ...
    $get_data  = $this->db->query("SELECT * FROM ms_h2_pembawa
			-- WHERE LEFT(created_at,10)='$ymd' 
      ORDER BY created_at DESC LIMIT 0,1");

    if ($get_data->num_rows() > 0) {
      $row           = $get_data->row();
      $id_pembawa = substr($row->id_pembawa, -4);
      $new_kode      = $dealer->kode_dealer_md . '/PBW/' . $ym . '/' . sprintf("%'.04d", $id_pembawa + 1);
      $i = 0;
      while ($i < 1) {
        $cek = $this->db->get_where('ms_h2_pembawa', ['id_pembawa' => $new_kode])->num_rows();
        if ($cek > 0) {
          $neww     = substr($new_kode, -3);
          $new_kode = $dealer->kode_dealer_md . '/PBW/' . $ym . '/' . sprintf("%'.04d", $neww + 1);
          $i        = 0;
        } else {
          $i++;
        }
      ...
      $new_kode   = $dealer->kode_dealer_md . '/PBW/' . $ym . '/0001';
    }
    return strtoupper($new_kode);
  }
  public function get_id_surat_jalan()
  {
    ...
    $get_data  = $this->db->query("SELECT * FROM tr_h2_wo_dealer_surat_jalan_keluar
			-- WHERE LEFT(created_at,10)='$ymd' 
      ORDER BY created_at DESC LIMIT 0,1");
    // $wo             = substr($wo, -4);
    if ($get_data->num_rows() > 0) {
  ...
  public function get_id_follow_up()
  {
    ...
    $get_data  = $this->db->query("SELECT * FROM tr_h2_follow_up_after_service
			WHERE LEFT(created_at,7)='$th_bln' 
      ORDER BY created_at DESC LIMIT 0,1");
  ...
  public function get_id_promo($kode, $sumber = NULL)
  {
    ...
    $get_data  = $this->db->query("SELECT * FROM ms_promo_servis
			WHERE LEFT(created_at,7)='$th_bln' 
      $where
      ORDER BY created_at DESC LIMIT 0,1");
  ...
  function get_promo_servis_jasa($filter = null)
  {
    ...
    return $this->db->query("SELECT prsj.*,js.deskripsi,ps.nama_promo,start_date,end_date,ps.id_promo_int,CASE WHEN ($sel_hrg_dealer)=0 THEN js.harga ELSE ($sel_hrg_dealer) END harga
  ...
  function get_promo_servis_dealer($filter = null)
  {
    ...
    return $this->db->query("SELECT prsd.*,dl.kode_dealer_md,nama_dealer
  ...
  public function get_id_vendor($dealer)
  {
    ...
    $my     = date('y/m');
    $where  = '';
    $get_data  = $this->db->query("SELECT * FROM ms_h2_vendor_pekerjaan_luar
  ...
  function cekKPB($params)
  {
    ...
    if ($cek_kpb == 0) {
      ...
      //     }
      //   }
      $get_batas = $this->db->query("SELECT * FROM ms_kpb_detail WHERE id_tipe_kendaraan='$id_tipe_kendaraan' AND kpb_ke='$kpb_ke'");
      if ($get_batas->num_rows() > 0) {
        ...
        if (bandingTgl($date_now, $tgl_maks) == 1) {
          $resp = [
            'status' => 'tgl_lewat',
            'msg' => "Telah melewati batas tanggal KPB $kpb_ke !"
          ];
        }
        //Cek KM
        ...
        $resp = ['status' => 'kosong', 'msg' => "Batas KPB $kpb_ke belum ditentukan untuk tipe kendaraan ini !. Hubungi MD"];
      }
    } else {
      $resp = ['status' => 'kosong', 'msg' => "No. Mesin = {$params['no_mesin']} sudah melakukan KPB Ke-$kpb_ke"];
    }
