...
class H3_md_create_ap_part_sales_campaign_insentif_poin_model extends Honda_Model{
	...
	private function non_rekap($id_sales_campaign){
		return $this->db
			->select('"5.03.5303.03" as nomor_account')
			...
			->select('"perolehan_insentif_poin_sales_campaign" as jenis_transaksi')
	...
	private function rekap($id_sales_campaign){
		...
		->select('"ms_h3_md_sales_campaign" as referensi_table')
		->select('"rekap_perolehan_insentif_poin_sales_campaign" as jenis_transaksi')
		->select("'{$this->tanggal_transaksi->format('Y-m-d')}' as tanggal_transaksi", false)
		->select("'{$this->tanggal_jatuh_tempo->format('Y-m-d')}' as tanggal_jatuh_tempo", false)
		->select('"Lain-lain" as nama_vendor')
	...
	private function check_sales_campaign($id_sales_campaign){
		...
		if ($sales_campaign != null AND $sales_campaign['sudah_proses_insentif'] == 1) {
			throw new Exception(sprintf('Sales campaign %s - %s sudah pernah diproses insentif poin sebelumnya [%s]', $sales_campaign['kode_campaign'], $sales_campaign['nama'], $sales_campaign['id']));
		}
	}
