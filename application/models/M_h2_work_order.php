...
class M_h2_work_order extends CI_Model
{
  ...
  function get_sa_form($filter)
  {
    ...
    if (isset($filter['cek_riwayat_servis'])) {
      $id_dealer = '';
    }
    ...
    if ($filter != null) {
      ...
      if (isset($filter['status_wo_not'])) {
        $where .= " AND wod.status not in('{$filter['status_wo_not']}','cancel')";
      }
      ...
      if (isset($filter['no_polisi'])) {
        ...
        $where .= " AND replace(cus.no_polisi , ' ','')='{$filter['no_polisi']}' ";
      }
      ...
      if (isset($filter['offset'])) {
        ...
        if(!isset($filter['id_work_order_not_null'])){
          $limit = " Limit 7";
        }else if(!isset($filter['cek_riwayat_servis'])){
          // $limit = " Limit 2";
        }
      }    
    ...
    $tot_part_ppn = "
        SELECT CASE 
            WHEN sa_form.pkp=1 THEN ($tot_part+($tot_part*$persen_ppn))
            ELSE $tot_part
          END
    ...
    DATE_FORMAT(sa_form.tgl_servis,'%d/%m/%Y') AS tgl_servis, sa_form.tgl_servis tgl_servis_real,
  ...
  CONCAT(DATE_FORMAT(LEFT(wod.waktu_njb,10),'%d/%m/%Y'),' ',RIGHT(wod.waktu_njb,8)) AS waktu_njb,cetak_njb_ke,id_wo_job_return,
    ...
    mdl.kode_dealer_md,wod.updated_at AS updated_at_wo,ROUND(($tot_pekerjaan)) AS tot_pekerjaan,cus.jangka_waktu_top,wod.tgl_jatuh_tempo,tk.kode_ptm,sa_form.id_antrian_int,cus.id_customer_int,sa_form.id_booking,cus.facebook facebook_cus,cus.twitter twitter_cus,cus.instagram instagram_cus,activity_capacity_id,activity_promotion_id,id_work_order_int,LEFT(wod.created_at,10) tgl_wo,wod.document,wod.saran_mekanik,sa_form.assigned_at,wod.created_by AS id_user_sa,vehicle_offroad,waktu_kedatangan,($km_sebelumnya) AS km_sebelumnya,DATE_FORMAT(LEFT(sa_form.tgl_servis,10),'%d/%m/%Y') AS tgl_servis_indo,DATE_FORMAT(LEFT(cus.tgl_pembelian,10),'%d/%m/%Y') AS tgl_pembelian_indo,$id_antrian_short id_antrian_short,ms_warna.warna only_warna,sa_form.activity_promotion_id,sa_form.activity_capacity_id,wod.id_sa_form_int,sa_form.created_by,cus.is_ev,sa_form.serial_number_battery,sa_form.soc,(SELECT lr.kesediaan_customer_lcr_id from tr_h2_wo_lcr_history lr 
    WHERE lr.status_lcr='LC' and lr.id_work_order=wod.id_work_order) as kesediaan_customer_lcr_id,
    (SELECT lr.record_reason_lcr_id from tr_h2_wo_lcr_history lr 
    WHERE lr.status_lcr='LC' and lr.id_work_order=wod.id_work_order) as record_reason_lcr_id,
    (SELECT lr.hasil_pengecekan_lcr_id from tr_h2_wo_lcr_history lr 
    WHERE lr.status_lcr='LC' and lr.id_work_order=wod.id_work_order) as hasil_pengecekan_lcr_id,
    (SELECT lr.kesediaan_customer_lcr_id from tr_h2_wo_lcr_history lr 
    WHERE lr.status_lcr='LT1' and lr.id_work_order=wod.id_work_order) as kesediaan_customer_lcr_treatment,
    (SELECT lr.record_reason_lcr_id from tr_h2_wo_lcr_history lr 
    WHERE lr.status_lcr='LT1' and lr.id_work_order=wod.id_work_order) as record_reason_lcr_id_treatment";
    ...
    return $this->db->query("SELECT $select
        ...
        LEFT JOIN tr_h2_wo_dealer AS wod ON wod.id_sa_form =sa_form.id_sa_form
  ...
  function getWOPekerjaan($filter = NULL)
  {
    ...
    if ($filter != NULL) {
      ...
      if (isset($filter['id_work_order_int'])) {
        $where .= " AND wo.id_work_order_int='{$filter['id_work_order_int']}'";
      }
      if (isset($filter['id_sa_form'])) {
  ...
  function getWOParts($filter = null)
  {
    ...
    if ($filter != null) {
      ...
      if (isset($filter['qty_lebih_dari'])) {
        if ($filter['qty_lebih_dari'] != '') {
          $where .= " AND IFNULL(wop.qty,0)>{$filter['qty_lebih_dari']}";
        }
      }
    }
    $potongan_nsc = "
    ...
    $harga_beli = "
          (CASE 
            WHEN wo.pkp_njb=1 THEN 
              CASE 
                WHEN wop.tipe_diskon='Percentage' THEN wop.harga - (wop.harga*(diskon_value/100)) * wop.qty
                ELSE wop.harga
              END
            ELSE 
              CASE 
    ...
    $qty = "
      ...
      ";
    $promo_rp = "$potongan_nsc+(wop.harga-$harga_beli)+(CASE WHEN qty<$qty THEN $harga_beli*(qty-$qty) ELSE 0 END)";
    ...
    if (isset($filter['join_ass1'])) {
      if ($filter['join_ass1'] == 'claim_oli_kpb1') {
        $join .= " JOIN ms_h2_jasa hj on hj.id_jasa = wop.id_jasa and hj.id_type ='ASS1'";
      }
    }
  ...
  public function get_id_claim_c2()
  {
    ...
    $get_data  = $this->db->query("SELECT no_claim_c2 FROM tr_h2_sa_form
			WHERE id_dealer='$id_dealer'
			AND IFNULL(no_claim_c2,'')!='' AND LEFT(created_at,4)='$th'
			ORDER BY created_at DESC LIMIT 0,1");
    if ($get_data->num_rows() > 0) {
      $row        = $get_data->row();
  ...
  function cetak_wo($params, $return = true)
  {
    $data['set'] = 'print';
    ...
    if ($get_data->num_rows() > 0) {
      ...
      $get_nama_mekanik ='________________';
          if($row->id_pit !='' and $row->id_work_order !=''){
      $get_nama_mekanik = $this->db->query("select a.id_work_order , b.nama_lengkap from tr_h2_wo_dealer a join ms_karyawan_dealer b on a.id_karyawan_dealer = b.id_karyawan_dealer where id_work_order ='$row->id_work_order'")->row()->nama_lengkap;
      }
      $data['nama_mekanik'] = $get_nama_mekanik;
      ...
      // send_json($data);
      $html = $this->load->view('dealer/work_order_dealer_cetak_new', $data, true);
      $mpdf->WriteHTML($html);
  ...
  function cetak_wo_html($params, $return = true)
  {
    $data['set'] = 'print';
    $data['judul_laporan'] = 'PERINTAH KERJA BENGKEL';
    $get_data = $this->m_wo->get_sa_form($params);
    ...
    if ($get_data->num_rows() > 0) {
      ...
      $data['id_dealer'] = $row->id_dealer;
      // send_json($data);
      return $this->load->view('dealer/work_order_dealer_cetak_new', $data, true);
  ...
  function updateGrandTotalWO($id_work_order)
  {
    ...
    foreach ($wo->result() as $w) {
      $disc_amount = "ROUND(CASE WHEN pr.tipe_diskon='persen' THEN (pkj.harga * (pr.diskon/100)) ELSE pr.diskon END)";

      $subtotal = "CASE WHEN pr.id_promo is null then subtotal
      ELSE pkj.harga-($disc_amount)
      END";

      $desc_jasa = $this->db->query("SELECT ($subtotal) tot, pkj.id_jasa,pr.tipe_diskon,$disc_amount disc_amount,pr.diskon
      FROM tr_h2_wo_dealer_pekerjaan pkj
      LEFT JOIN ms_promo_servis_jasa pr ON pr.id_promo=pkj.id_promo AND pr.id_jasa=pkj.id_jasa
      WHERE pkj.id_work_order='$w->id_work_order' AND pkj.pekerjaan_batal!=1")->result();
      $jasa = 0;
      foreach ($desc_jasa as $js) {
        $jasa += $js->tot;
        if ($js->tipe_diskon == 'persen') {
          $disc_percentage = $js->diskon;
        } else {
          $disc_percentage = 0;
        }
        $upd_wo_pekerjaan = [
          'disc_amount' => $js->disc_amount,
          'diskon_value' => $js->disc_amount ?: 0,
          'disc_percentage' => $disc_percentage ?: 0,
          'subtotal' => $js->tot,
        ];
        $cond = ['id_jasa' => $js->id_jasa, 'id_work_order' => $id_work_order];
        $this->db->update('tr_h2_wo_dealer_pekerjaan', $upd_wo_pekerjaan, $cond);
      }
      // send_json($upd_wo_pekerjaan);
      ...
      if ($grand_total > 0) {
        $upd[] = [
          ...
          'total_tanpa_ppn' => $grand_total,
          'total_ppn'       => $grand_total* getPPN(0.1,false),
          'total_jasa'      => $jasa,
          'total_part'      => $total_part
        ];
      }
      // $cek = [
      //   'upd_wo_pekerjaan' => $upd_wo_pekerjaan,
      //   'upd_subtotal' => $upd_subtotal,
      //   'upd' => $upd
      // ];
      // send_json($cek);
      if (isset($upd)) {
  ...
  function cetak_sa_form($params, $id_dealer = null)
  {
    ...
    if ($get_data->num_rows() > 0) {
      ...
      $doc = $path . '/' . $nama_file . '-' . $next_cetak . '.pdf';
  ...
  function get_sa_form_header($filter = null)
  {
    ...
    $select = "id_antrian,sa.id_sa_form,tgl_servis,jenis_customer,cus.no_polisi,cus.nama_customer,cus.no_mesin,cus.no_rangka,tipe_ahm,warna,cus.tahun_produksi,sa.status_form,sa.id_antrian_int, sa.id_type";
  ...
  public function get_rekap_kpb_cetak_program_oli_gratis($filter){
    // if ($filter != null) {
    //   if (isset($filter['id_rekap_kpb'])) {
    //     $where .= " AND wod.id_rekap_kpb='{$filter['id_rekap_kpb']}'";
    //   }
    //   if (isset($filter['id_type_wo_in'])) {
    //     $where .= " AND id_type IN ({$filter['id_type_wo_in']})";
    //   }
      
    // }

    return $this->db->query("SELECT wo.no_njb, sa.km_terakhir,ROUND((SELECT IFNULL(SUM(wop.harga-IFNULL((wop.harga * (prj.diskon/100)),0)),0) 
    FROM tr_h2_wo_dealer_pekerjaan AS wop 
    JOIN ms_h2_jasa jstp ON jstp.id_jasa=wop.id_jasa 
    LEFT JOIN ms_promo_servis_jasa prj ON prj.id_promo=wop.id_promo AND prj.id_jasa=wop.id_jasa 
    LEFT JOIN ms_promo_servis pr ON pr.id_promo=prj.id_promo WHERE wop.id_work_order=wo.id_work_order AND jstp.id_type 
    IN ('ASS1','ASS2','ASS3','ASS4'))) AS tot_pekerjaan, (CASE WHEN nsc.no_nsc is not null then nsc.no_nsc else '-' end) as no_nsc, cust.id_tipe_kendaraan, 
    cust.no_mesin,cust.no_rangka, DATE_FORMAT(LEFT(cust.tgl_pembelian,10),'%d/%m/%Y') AS tgl_pembelian_indo, 
    DATE_FORMAT(LEFT(sa.tgl_servis,10),'%d/%m/%Y') AS tgl_servis_indo ,cust.no_mesin, 
    -- (case when so.id_dealer is not null then d2.nama_dealer else '-' end) as dealer_beli,
    (case when so.id_dealer  is not null and so.id_dealer = '{$filter['id_dealer']}' then 'Own Dealer' when so.id_dealer  is not null and so.id_dealer != '{$filter['id_dealer']}' then 'Other Dealer' else '-' end) as dealer_beli,
    IFNULL((SELECT RIGHT(id_type,1) FROM tr_h2_wo_dealer_pekerjaan wdp JOIN ms_h2_jasa js ON js.id_jasa=wdp.id_jasa 
    WHERE wdp.id_work_order=wo.id_work_order AND id_type IN ({$filter['id_type_wo_in']}) LIMIT 1),0) AS kpb_ke, nscp.harga_beli, 
    IFNULL(((SELECT ROUND(SUM((( ( CASE WHEN tipe_diskon='Percentage' THEN harga_beli - (harga_beli*(diskon_value/100)) ELSE harga_beli END ) 
    ...
    * (CASE WHEN tipe_diskon='Value' THEN diskon_value ELSE 0 END ) ))))) AS tot FROM tr_h23_nsc_parts nscp JOIN tr_h23_nsc ns ON ns.no_nsc=nscp.no_nsc 
    JOIN ms_part mp ON mp.id_part=nscp.id_part LEFT JOIN tr_h2_wo_dealer wos ON wos.id_work_order=ns.id_referensi 
    LEFT JOIN tr_h2_sa_form sa ON sa.id_sa_form=wos.id_sa_form LEFT JOIN ms_customer_h23 cus_nsc ON cus_nsc.id_customer=sa.id_customer 
    WHERE 1=1 AND ns.id_referensi=wo.id_work_order AND mp.kelompok_part IN ('OIL') GROUP BY nscp.no_nsc )),0) AS jml_oli, 
    IFNULL((SELECT SUM(qty) FROM tr_h2_wo_dealer_parts wdparts JOIN ms_part prt ON prt.id_part=wdparts.id_part
    WHERE wdparts.id_work_order=wo.id_work_order AND prt.kelompok_part = 'OIL'),0) AS tot_qty_oli 
    from tr_h2_sa_form sa 
    left join tr_h2_wo_dealer wo on wo.id_sa_form=sa.id_sa_form left join ms_customer_h23 cust on cust.id_customer=sa.id_customer
    left join tr_sales_order so on so.no_mesin=cust.no_mesin join ms_dealer d on d.id_dealer=sa.id_dealer 
    left join ms_dealer d2 on d2.id_dealer=so.id_dealer left join tr_h23_nsc nsc on nsc.id_referensi=wo.id_work_order 
    left join ms_tipe_kendaraan mtk on mtk.id_tipe_kendaraan=cust.id_tipe_kendaraan left join tr_h23_nsc_parts nscp on nscp.no_nsc=nsc.no_nsc  
    where 1=1 and sa.id_dealer='{$filter['id_dealer']}' and nsc.no_nsc is not null
    and wo.id_rekap_kpb='{$filter['id_rekap_kpb']}'
    AND (SELECT COUNT(id_type) FROM tr_h2_wo_dealer_pekerjaan wdp JOIN ms_h2_jasa js ON js.id_jasa=wdp.id_jasa 
    WHERE wdp.id_work_order=wo.id_work_order AND id_type IN ({$filter['id_type_wo_in']}) )>0 
    group by wo.no_njb 
    ORDER BY sa.created_at DESC");
  }

  public function get_rekap_kpb_v1($filter){
    $id_dealer = '';
    ...
    // if (isset($filter['kategori_kpb'])) {
    //   // if($filter['kategori_kpb'] == 'satu'){
    //   //   $kat_kpb = " = 'ASS1'";
    //   // }elseif($filter['kategori_kpb'] == 'dua'){
    //   //   $kat_kpb = " = 'ASS2'";
    //   // }elseif($filter['kategori_kpb'] == 'tiga'){
    //   //   $kat_kpb = " = 'ASS3'";
    //   // }elseif($filter['kategori_kpb'] == 'empat'){
    //   //   $kat_kpb = " = 'ASS4'";
    //   // }else{
    //   //   $kat_kpb = "IN('ASS1','ASS2','ASS3','ASS4')";
    //   // }
    //   if($filter['kategori_kpb'] == 'dua_empat'){
    //     $kat_kpb = "IN('ASS2','ASS3','ASS4')";
    //   }else{
    //     $kat_kpb = "IN('ASS1','ASS2','ASS3','ASS4')";
    //   }
    // } else {
    ...
    return $this->db->query("SELECT wo.no_njb, wo.id_work_order, 
    (CASE WHEN nsc.no_nsc is not null then nsc.no_nsc else '-' end) as no_nsc, 
    cust.id_tipe_kendaraan, cust.no_mesin,cust.no_rangka, DATE_FORMAT(LEFT(so.tgl_cetak_invoice,10),'%d/%m/%Y') AS tgl_pembelian_indo,
    DATE_FORMAT(LEFT(sa.tgl_servis,10),'%d/%m/%Y') AS tgl_servis_indo ,cust.no_mesin, 
    -- (case when so.id_dealer is not null then d2.nama_dealer else '-' end) as dealer_beli,
    (case when so.id_dealer  is not null and so.id_dealer = '$id_dealer' then 'Own Dealer' when so.id_dealer  is not null and so.id_dealer != '$id_dealer' then 'Other Dealer' else '-' end) as dealer_beli,
    IFNULL((SELECT RIGHT(id_type,1) FROM tr_h2_wo_dealer_pekerjaan wdp JOIN ms_h2_jasa js ON js.id_jasa=wdp.id_jasa 
    WHERE wdp.id_work_order=wo.id_work_order and wdp.pekerjaan_batal=0 AND id_type $kat_kpb LIMIT 1),0) AS kpb_ke, 
    IFNULL((0),0) AS checked, 
    ROUND((SELECT IFNULL(SUM(wop.harga-IFNULL((wop.harga * (prj.diskon/100)),0)),0) 
    FROM tr_h2_wo_dealer_pekerjaan AS wop 
    JOIN ms_h2_jasa jstp ON jstp.id_jasa=wop.id_jasa 
    LEFT JOIN ms_promo_servis_jasa prj ON prj.id_promo=wop.id_promo AND prj.id_jasa=wop.id_jasa 
    LEFT JOIN ms_promo_servis pr ON pr.id_promo=prj.id_promo WHERE wop.id_work_order=wo.id_work_order AND jstp.id_type 
    $kat_kpb)) AS tot_pekerjaan, 
    nscp.harga_beli, sa.km_terakhir,
    (CASE WHEN sa.id_type='ASS1' THEN IFNULL(((SELECT ROUND(SUM((( ( CASE WHEN tipe_diskon='Percentage' THEN harga_beli - (harga_beli*(diskon_value/100)) ELSE harga_beli END ) * 
    (CASE WHEN tipe_diskon='FoC' THEN qty-diskon_value ELSE qty END ) -( (CASE WHEN tipe_diskon='FoC' THEN qty-diskon_value ELSE qty END ) * 
    (CASE WHEN tipe_diskon='Value' THEN diskon_value ELSE 0 END ) ))))) AS tot 
    FROM tr_h23_nsc_parts nscp JOIN tr_h23_nsc ns ON ns.no_nsc=nscp.no_nsc JOIN ms_part mp ON mp.id_part=nscp.id_part 
    LEFT JOIN tr_h2_wo_dealer wos ON wos.id_work_order=ns.id_referensi LEFT JOIN tr_h2_sa_form sa ON sa.id_sa_form=wos.id_sa_form 
    LEFT JOIN ms_customer_h23 cus_nsc ON cus_nsc.id_customer=sa.id_customer WHERE 1=1 AND ns.id_referensi=wo.id_work_order 
    AND mp.kelompok_part IN ('OIL') GROUP BY nscp.no_nsc )),0) ELSE 0 END) AS jml_oli,
    -- IFNULL((SELECT SUM(qty) FROM tr_h2_wo_dealer_parts wdparts JOIN ms_part prt ON prt.id_part=wdparts.id_part 
    -- WHERE wdparts.id_work_order=wo.id_work_order AND kelompok_part IN(SELECT id_kelompok_part FROM ms_h3_md_setting_kelompok_produk 
    -- WHERE produk='Oil') ),0) AS tot_qty_oli
    (CASE WHEN sa.id_type='ASS1' then IFNULL((SELECT SUM(qty) FROM tr_h2_wo_dealer_parts wdparts JOIN ms_part prt ON prt.id_part=wdparts.id_part 
    WHERE wdparts.id_work_order=wo.id_work_order AND prt.kelompok_part = 'OIL'),0) ELSE 0 END) AS tot_qty_oli
    from tr_h2_sa_form sa
    left join tr_h2_wo_dealer wo on wo.id_sa_form=sa.id_sa_form 
    left join ms_customer_h23 cust on cust.id_customer=sa.id_customer 
    left join tr_sales_order so on so.no_mesin=cust.no_mesin 
    join ms_dealer d on d.id_dealer=sa.id_dealer
    left join ms_dealer d2 on d2.id_dealer=so.id_dealer 
    ...
    left join tr_h23_nsc_parts nscp on nscp.no_nsc=nsc.no_nsc 
    -- join ms_part mp on mp.id_part_int=nscp.id_part_int 
    where 1=1 and sa.id_dealer='$id_dealer' and wo.no_njb is not null 
    AND NOT EXISTS(SELECT id_rekap_kpb FROM tr_h2_dealer_rekap_kpb 
    WHERE id_rekap_kpb=wo.id_rekap_kpb) AND sa.tgl_servis BETWEEN '{$filter['start']}' AND '{$filter['end']}' 
    AND 
    (SELECT COUNT(id_type) FROM tr_h2_wo_dealer_pekerjaan wdp JOIN ms_h2_jasa js ON js.id_jasa=wdp.id_jasa 
    WHERE wdp.id_work_order=wo.id_work_order AND wdp.pekerjaan_batal=0 AND id_type $kat_kpb )>0
    -- and mp.kelompok_part = 'OIL'
    group by wo.no_njb 
    ORDER BY sa.created_at DESC");
  }

  function cek_jasa_kpb_by_wo($id_work_order) 
  {
    return $this->db->query("SELECT wdp.id_jasa
    FROM tr_h2_wo_dealer_pekerjaan wdp
    JOIN ms_h2_jasa js ON js.id_jasa=wdp.id_jasa
    WHERE wdp.id_work_order='$id_work_order'
    AND id_type IN('ASS1','ASS2','ASS3','ASS4')
    ");
  }
}
